...
int asCCompiler::CompileDefaultConstructor( asCBuilder* in_builder, asCScriptCode* in_script, asCScriptNode* in_node,
                                            asCScriptFunction* in_outFunc, sClassDeclaration* in_classDecl ) {
    ...
    byteCode.DebugOutput( ( "__" + outFunc->objectType->name + "_" + outFunc->name + "__defconstr.txt" ).AddressOf(),
                          in_outFunc );
#endif
    return 0;
}
...
int asCCompiler::CompileFactory( asCBuilder* in_builder, asCScriptCode* in_script, asCScriptFunction* in_outFunc ) {
    ...
    for ( int a = int( outFunc->parameterTypes.GetLength() ) - 1; a >= 0; a-- ) {
        if ( !outFunc->parameterTypes[a].IsPrimitive() || outFunc->parameterTypes[a].IsReference() ) {
            offset -= AS_PTR_SIZE;
...
void asCCompiler::FinalizeFunction() {
    ...
    for ( n = 0; n < variableAllocations.GetLength(); n++ ) {
        if ( ( variableAllocations[n].IsObject() || variableAllocations[n].IsFuncdef() )
             && !variableAllocations[n].IsReference() ) {
...
int asCCompiler::SetupParametersAndReturnVariable( asCArray<asCString>& parameterNames, asCScriptNode* func ) {
    ...
    if ( returnType != asCDataType::CreatePrimitive( ttVoid, false ) && !returnType.CanBeInstantiated()
         && !returnType.IsReference() && !returnType.IsObjectHandle() ) {
    ...
    for ( n = asUINT( vs.variables.GetLength() ); n-- > 0; )
        variables->DeclareVariable( vs.variables[n]->name.AddressOf(), vs.variables[n]->type,
                                    vs.variables[n]->stackOffset, vs.variables[n]->onHeap );
    ...
    for ( asUINT n = 0; n < outFunc->objectType->properties.GetLength(); n++ ) {
        ...
        if ( declNode ) {
            ...
            if ( prop->type.IsObject() && !prop->type.IsObjectHandle()
                 && ( ( ( prop->type.GetTypeInfo()->flags & asOBJ_REF ) && prop->type.GetBehaviour()->factory == 0 )
...
// Entry
int asCCompiler::CompileFunction( asCBuilder* in_builder, asCScriptCode* in_script,
                                  asCArray<asCString>& in_parameterNames, asCScriptNode* in_func,
                                  asCScriptFunction* in_outFunc, sClassDeclaration* in_classDecl ) {
    ...
    // DEBUG: output byte code
    if ( outFunc->objectType )
        byteCode.DebugOutput( ( "__" + outFunc->objectType->name + "_" + outFunc->name + ".txt" ).AddressOf(),
                              in_outFunc );
    else
...
int asCCompiler::CallCopyConstructor( asCDataType& type, int offset, bool isObjectOnHeap, asCByteCode* bc,
                                      asCExprContext* arg, asCScriptNode* node, bool isGlobalVar, bool derefDest ) {
    if ( !type.IsObject() )
        return 0;
...
int asCCompiler::CallDefaultConstructor( const asCDataType& type, int offset, bool isObjectOnHeap, asCByteCode* bc,
                                         asCScriptNode* node, int isVarGlobOrMem, bool derefDest ) {
        ...
        if ( beh ) {
            ...
            if ( func == 0 ) {
                for ( asUINT n = 0; n < beh->factories.GetLength(); n++ ) {
                    ...
                    if ( f->defaultArgs.GetLength() == f->parameterTypes.GetLength() && f->defaultArgs[0] != 0 ) {
            ...
            if ( isVarGlobOrMem == 0 ) {
                ...
                if ( isVarGlobOrMem == 1 ) {
                    ...
                    ctx.bc.InstrSHORT_DW(
                        asBC_ADDSi, (short) offset,
                        engine->GetTypeIdFromDataType( asCDataType::CreateType( outFunc->objectType, false ) ) );
                }

                if ( type.GetTypeInfo()->flags & asOBJ_SCOPED ) {
                    // For scoped typed we must move the reference from the local
                    // variable rather than copy it as there is no AddRef behaviour
                    ctx.bc.InstrSHORT_DW( asBC_COPY, AS_PTR_SIZE,
                                          asTYPEID_OBJHANDLE | engine->GetTypeIdFromDataType( type ) );
...
void asCCompiler::CompileStatementBlock( asCScriptNode* block, bool ownVariableScope, bool* hasReturn,
                                         asCByteCode* bc ) {
    *hasReturn = false;
...
// Entry
int asCCompiler::CompileGlobalVariable( asCBuilder* in_builder, asCScriptCode* in_script, asCScriptNode* in_node,
                                        sGlobalVariableDescription* in_gvar, asCScriptFunction* in_outFunc ) {
    Reset( in_builder, in_script, in_outFunc );
    m_globalVar = in_gvar;
    ...
    if ( in_gvar->property == 0 ) {
        in_gvar->property
            = builder->module->AllocateGlobalProperty( in_gvar->name.AddressOf(), in_gvar->datatype, in_gvar->ns );
    ...
    if ( CompileInitialization( in_node, &ctx.bc, in_gvar->datatype, in_gvar->declaredAtNode, in_gvar->index,
                                &constantValue, 1, preCompiled ? &compiledCtx : 0 ) ) {
...
void asCCompiler::DetermineSingleFunc( asCExprContext* ctx, asCScriptNode* node ) {
    ...
    if ( !builder->GetFunctionDescription( funcs[0] )->IsShared() && outFunc->IsShared() ) {
        asCString msg;
        msg.Format( TXT_SHARED_CANNOT_CALL_NON_SHARED_FUNC_s,
                    builder->GetFunctionDescription( funcs[0] )->GetDeclaration() );
        Error( msg, node );
    ...
    ctx->type.Set( asCDataType::CreateType(
        engine->FindMatchingFuncdef( builder->GetFunctionDescription( funcs[0] ), builder->module ), false ) );
    ctx->type.dataType.MakeHandle( true );
    ctx->type.isExplicitHandle = true;
    ctx->methodName = "";
}

void asCCompiler::CompileInitAsCopy( asCDataType& dt, int offset, asCByteCode* bc, asCExprContext* arg,
                                     asCScriptNode* node, bool derefDestination ) {
...
int asCCompiler::PrepareArgument( asCDataType* paramType, asCExprContext* ctx, asCScriptNode* node, bool isFunction,
                                  int refType, bool isMakingCopy ) {
    ...
    if ( paramType->GetTokenType() == ttQuestion ) {
        ...
        if ( builder->engine->ep.disallowValueAssignForRefType && ctx->type.dataType.GetTypeInfo()
             && ( ctx->type.dataType.GetTypeInfo()->flags & asOBJ_REF )
             && !( ctx->type.dataType.GetTypeInfo()->flags & asOBJ_SCOPED ) ) {
        ...
        if ( refType == asTM_INREF ) {
            ...
            if ( dt.IsPrimitive() ) {
                ...
                if ( ( !dt.SupportHandles() && !dt.IsObjectHandle() )
                     || ( dt.GetTypeInfo() && ( dt.GetTypeInfo()->GetFlags() & asOBJ_ASHANDLE ) ) ) {
                    asCString str;
                    str.Format( TXT_CANT_IMPLICITLY_CONVERT_s_TO_s,
                                ctx->type.dataType.Format( outFunc->nameSpace ).AddressOf(),
                ...
                if ( !isMakingCopy ) {
                    ...
                    if ( !ctx->type.isTemporary
                         && !( param.IsReadOnly() && ( ctx->type.isVariable || ctx->type.isRefSafe ) ) ) {
                        if ( ctx->type.dataType.IsFuncdef()
                             || ( ( ctx->type.dataType.GetTypeInfo()->flags & asOBJ_REF ) && param.IsReadOnly()
                                  && !( ctx->type.dataType.GetTypeInfo()->flags & asOBJ_SCOPED ) ) ) {
                    ...
                    if ( ( ctx->type.dataType.IsObject() || ctx->type.dataType.IsFuncdef() )
                         && !ctx->type.dataType.IsObjectHandle() && ctx->type.dataType.IsReference() )
                        Dereference( ctx, true );
                }
            }
            ...
            if ( ( ctx->type.dataType.IsObject() || ctx->type.dataType.IsFuncdef() )
                 && ctx->type.dataType.GetTypeInfo() != dt.GetTypeInfo() )
            ...
            // references
            if ( !engine->ep.allowUnsafeReferences && !ctx->type.isVariable
                 && ( ctx->type.dataType.IsObject() || ctx->type.dataType.IsFuncdef() )
                 && !ctx->type.dataType.IsObjectHandle()
                 && ( ( ctx->type.dataType.GetBehaviour()->addref && ctx->type.dataType.GetBehaviour()->release )
            ...
            if ( ( ctx->type.dataType.IsObject() || ctx->type.dataType.IsFuncdef() ) && ctx->type.dataType.IsReference()
                 && !param.IsObjectHandle() )
                Dereference( ctx, true );
...
int asCCompiler::PrepareFunctionCall( int funcId, asCByteCode* bc, asCArray<asCExprContext*>& args ) {
    ...
    if ( descr->parameterTypes.GetLength() == 1
         ...
         && ( ( ( descr->name == "opAssign" || descr->name == "$beh0" ) && descr->objectType
                && descr->objectType == args[0]->type.dataType.GetTypeInfo() )
              || ( descr->objectType == 0 && args[0]->type.dataType.GetTypeInfo()
                   && descr->name == args[0]->type.dataType.GetTypeInfo()->name ) ) )
        makingCopy = true;
...
int asCCompiler::CompileArgumentList( asCScriptNode* node, asCArray<asCExprContext*>& args,
...
int asCCompiler::CompileDefaultAndNamedArgs( asCScriptNode* node, asCArray<asCExprContext*>& args, int funcId,
                                             asCObjectType* objectType, asCArray<asSNamedArgument>* namedArgs ) {
...
asUINT asCCompiler::MatchFunctions( asCArray<int>& funcs, asCArray<asCExprContext*>& args, asCScriptNode* node,
                                    const char* name, asCArray<asSNamedArgument>* namedArgs, asCObjectType* objectType,
                                    bool isConstMethod, bool silent, bool allowObjectConstruct,
...
bool asCCompiler::CompileAutoType( asCDataType& type, asCExprContext& compiledCtx, asCScriptNode* node,
                                   asCScriptNode* errNode ) {
    if ( node && node->nodeType == snAssignment ) {
        ...
        if ( r >= 0 ) {
            ...
            if ( newType.GetTypeInfo() && ( newType.GetTypeInfo()->flags & asOBJ_IMPLICIT_HANDLE ) )
...
void asCCompiler::CompileDeclaration( asCScriptNode* decl, asCByteCode* bc ) {
    // Get the data type
    asCDataType type
        = builder->CreateDataTypeFromNode( decl->firstChild, script, outFunc->nameSpace, false, outFunc->objectType );
    ...
    while ( node ) {
        ...
        if ( !type.CanBeInstantiated() ) {
            ...
            if ( type.IsAbstractClass() )
                str.Format( TXT_ABSTRACT_CLASS_s_CANNOT_BE_INSTANTIATED,
...
bool asCCompiler::CompileInitialization( asCScriptNode* node, asCByteCode* bc, const asCDataType& type,
                                         asCScriptNode* errNode, int offset, asQWORD* constantValue, int isVarGlobOrMem,
                                         asCExprContext* preCompiled ) {
    ...
    if ( node && node->nodeType == snArgList ) {
        ...
        if ( type.GetTypeInfo() == 0 || type.IsObjectHandle() ) {
            ...
            if ( CompileArgumentList( node, args, namedArgs ) >= 0 ) {
                ...
                if ( funcs.GetLength() == 1 ) {
                    // Add the default values for arguments not explicitly supplied
                    int r = CompileDefaultAndNamedArgs( node, args, funcs[0], CastToObjectType( type.GetTypeInfo() ),
                                                        &namedArgs );
                        ...
                        if ( type.GetTypeInfo() && ( type.GetTypeInfo()->flags & asOBJ_REF ) ) {
                            if ( isVarGlobOrMem == 0 )
                                ...
                                if ( isVarGlobOrMem == 1 ) {
                                    ...
                                    ctx.bc.InstrSHORT( asBC_PSF, 0 );
                                    ctx.bc.Instr( asBC_RDSPtr );
                                    ctx.bc.InstrSHORT_DW( asBC_ADDSi, (short) offset,
                                                          engine->GetTypeIdFromDataType(
                                                              asCDataType::CreateType( outFunc->objectType, false ) ) );
                                }
                                if ( type.IsFuncdef() )
                                    ctx.bc.InstrPTR( asBC_REFCPY, &engine->functionBehaviours );
                                else
                            ...
                            if ( isVarGlobOrMem == 0 ) {
                                ...
                                onHeap = !( type.IsObject() || type.IsFuncdef() ) || type.IsReference()
                            ...
                            PerformFunctionCall( funcs[0], &ctx, onHeap, &args,
                                                 CastToObjectType( type.GetTypeInfo() ) );
            ...
            if ( r >= 0 ) {
                if ( type.IsPrimitive() ) {
                    ...
                    if ( isVarGlobOrMem == 0 )
                        ...
                        if ( !lexpr.type.dataType.IsObject() || lexpr.type.dataType.IsFuncdef()
                             || ( lexpr.type.dataType.GetTypeInfo()->flags & asOBJ_REF ) )
                            lexpr.type.dataType.MakeReference( true );
                    }
                    ...
                    if ( ( lexpr.type.dataType.IsObject() || lexpr.type.dataType.IsFuncdef() )
                         && ( !lexpr.type.isExplicitHandle
                              || ( lexpr.type.dataType.GetTypeInfo()
                                   && ( lexpr.type.dataType.GetTypeInfo()->flags & asOBJ_ASHANDLE ) ) ) ) {
                        ...
                        if ( lexpr.type.dataType.IsHandleToAsHandleType() ) {
                            ...
                            if ( !expr->type.isExplicitHandle && !expr->type.IsNullConstant() ) {
                                ...
                                if ( !expr->type.dataType.IsObjectHandle() && expr->type.dataType.GetTypeInfo()
                                     && !( expr->type.dataType.GetTypeInfo()->flags & asOBJ_ASHANDLE ) ) {
                                ...
                                } else if ( expr->type.dataType.GetTypeInfo()
                                            && expr->type.dataType.GetTypeInfo()->flags & asOBJ_ASHANDLE ) {
        ...
        if ( isVarGlobOrMem == 0 )
            ...
            if ( !( type.IsObject() || type.IsFuncdef() ) || type.IsReference()
                 || ( type.GetTypeInfo()->flags & asOBJ_REF ) )
                CallDefaultConstructor( type, offset, true, bc, errNode, isVarGlobOrMem );
            else
...
void asCCompiler::CompileInitList( asCExprValue* var, asCScriptNode* node, asCByteCode* bc, int isVarGlobOrMem ) {
    ...
    if ( var->dataType.GetTypeInfo() == 0 || var->dataType.GetBehaviour()->listFactory == 0 ) {
        asCString str;
    ...
    asSListPatternNode* patternNode
        = engine->scriptFunctions[listPatternType->templateSubTypes[0].GetBehaviour()->listFactory]->listPattern;
    int elementsInSubList = -1;
    int r = CompileInitListElement( patternNode, el,
                                    engine->GetTypeIdFromDataType( asCDataType::CreateType( listPatternType, false ) ),
                                    short( bufferVar ), bufferSize, valueExpr.bc, elementsInSubList );
    ...
    if ( var->isVariable ) {
        ...
        if ( var->dataType.GetTypeInfo()->GetFlags() & asOBJ_REF ) {
            ...
            if ( isVarGlobOrMem == 1 ) {
                ...
                ctx.bc.Instr( asBC_RDSPtr );
                ctx.bc.InstrSHORT_DW(
                    asBC_ADDSi, (short) var->stackOffset,
                    engine->GetTypeIdFromDataType( asCDataType::CreateType( outFunc->objectType, false ) ) );
            }
            ...
            if ( isVarGlobOrMem == 1 )
                ...
                onHeap = !( var->dataType.IsObject() || var->dataType.IsFuncdef() ) || var->dataType.IsReference()
                    || ( var->dataType.GetTypeInfo()->flags & asOBJ_REF );
                if ( onHeap ) {
                    ctx.bc.InstrSHORT( asBC_PSF, 0 );
                    ctx.bc.Instr( asBC_RDSPtr );
                    ctx.bc.InstrSHORT_DW(
...
int asCCompiler::CompileInitListElement( asSListPatternNode*& patternNode, asCScriptNode*& valueNode, int bufferTypeId,
                                         short bufferVar, asUINT& bufferSize, asCByteCode& bcInit,
                                         int& elementsInSubList ) {
    if ( patternNode->type == asLPT_START ) {
        ...
        while ( patternNode->type != asLPT_END ) {
            ...
            int r = CompileInitListElement( patternNode, node, bufferTypeId, bufferVar, bufferSize, bcInit,
                                            elementsInSubList );
            if ( r < 0 )
                return r;
        ...
        if ( repeatType == asLPT_REPEAT_SAME && elementsInSubList != -1
             && asUINT( elementsInSubList ) != countElements ) {
        ...
        if ( valueNode->nodeType == snAssignment || valueNode->nodeType == snInitList ) {
            ...
            if ( valueNode->nodeType == snAssignment ) {
                ...
                if ( dt.GetTokenType() == ttQuestion ) {
                    ...
                    bcInit.InstrSHORT_DW_DW( asBC_SetListType, bufferVar, bufferSize,
                                             engine->GetTypeIdFromDataType( dt ) );
                    bufferSize += 4;
                }
            ...
            if ( dt.IsPrimitive() ) {
                lctx.bc.Instr( asBC_PopRPtr );
                lctx.type.dataType.MakeReference( true );
            } else if ( dt.IsObjectHandle() || dt.GetTypeInfo()->flags & asOBJ_REF ) {
            ...
            if ( lctx.type.dataType.IsNullHandle() ) {
                ...
                asASSERT( reinterpret_cast<asSListPatternDataTypeNode*>( patternNode )->dataType.GetTokenType()
                          == ttQuestion );
            } else {
                asCExprContext ctx( engine );
...
void asCCompiler::CompileIfStatement( asCScriptNode* inode, bool* hasReturn, asCByteCode* bc ) {
    ...
    if ( ( constructorCall1 && !constructorCall2 ) || ( constructorCall2 && !constructorCall1 ) ) {
...
void asCCompiler::CompileForStatement( asCScriptNode* fnode, asCByteCode* bc ) {
    ...
    if ( second->firstChild ) {
        ...
        if ( r >= 0 ) {
            // Allow value types to be converted to bool using 'bool opImplConv()'
            if ( expr.type.dataType.GetTypeInfo() && ( expr.type.dataType.GetTypeInfo()->GetFlags() & asOBJ_VALUE ) )
                ImplicitConversion( &expr, asCDataType::CreatePrimitive( ttBool, false ), second->firstChild,
                                    asIC_IMPLICIT_CONV );
...
void asCCompiler::CompileWhileStatement( asCScriptNode* wnode, asCByteCode* bc ) {
    ...
    if ( r == 0 ) {
        // Allow value types to be converted to bool using 'bool opImplConv()'
        if ( expr.type.dataType.GetTypeInfo() && ( expr.type.dataType.GetTypeInfo()->GetFlags() & asOBJ_VALUE ) )
            ImplicitConversion( &expr, asCDataType::CreatePrimitive( ttBool, false ), wnode->firstChild,
                                asIC_IMPLICIT_CONV );
...
void asCCompiler::CompileDoWhileStatement( asCScriptNode* wnode, asCByteCode* bc ) {
    ...
    // Allow value types to be converted to bool using 'bool opImplConv()'
    if ( expr.type.dataType.GetTypeInfo() && ( expr.type.dataType.GetTypeInfo()->GetFlags() & asOBJ_VALUE ) )
        ImplicitConversion( &expr, asCDataType::CreatePrimitive( ttBool, false ), wnode->lastChild,
                            asIC_IMPLICIT_CONV );
...
void asCCompiler::CompileReturnStatement( asCScriptNode* rnode, asCByteCode* bc ) {
    ...
    if ( rnode->firstChild ) {
        ...
        if ( v->type.IsReference() ) {
            ...
            if ( ( expr.type.isVariable && !( expr.type.stackOffset == 0 && outFunc->objectType ) )
            ...
            if ( !( v->type.IsEqualExceptConst( expr.type.dataType )
                ...
                str.Format( TXT_CANT_IMPLICITLY_CONVERT_s_TO_s,
                            ...
                            v->type.Format( outFunc->nameSpace ).AddressOf() );
                Error( str, rnode );
                return;
            }
        ...
        } else // if( !v->type.IsReference() )
        {
            ...
            if ( v->type.IsPrimitive() ) {
                ...
                if ( expr.type.dataType != v->type ) {
                    asCString str;
                    str.Format( TXT_NO_CONVERSION_s_TO_s, expr.type.dataType.Format( outFunc->nameSpace ).AddressOf(),
                                v->type.Format( outFunc->nameSpace ).AddressOf() );
                    Error( str, rnode );
                    return;
                } else {
...
int asCCompiler::AllocateVariableNotIn( const asCDataType& type, bool isTemporary, bool forceOnHeap,
                                        asCExprContext* ctx ) {
    int l = int( reservedVariables.GetLength() );
...
void asCCompiler::PrepareForAssignment( asCDataType* lvalue, asCExprContext* rctx, asCScriptNode* node,
                                        bool toTemporary, asCExprContext* lvalueExpr ) {
    // Reserve the temporary variables used in the lvalue expression so they won't end up being used by the rvalue too
    int l = int( reservedVariables.GetLength() );
    if ( lvalueExpr )
        lvalueExpr->bc.GetVarsUsed( reservedVariables );
    ...
    if ( lvalue->IsPrimitive() ) {
        ...
        if ( !lvalue->IsObjectHandle() && ( lvalue->GetTypeInfo()->flags & asOBJ_SCRIPT_OBJECT ) )
            to.MakeHandle( true );
...
bool asCCompiler::IsLValue( asCExprValue& type ) {
    ...
    if ( type.dataType.IsReadOnly() )
        return false;
    if ( !type.dataType.IsObject() && !type.isVariable && !type.dataType.IsReference() )
        return false;
    return true;
}
        ...
        *lvalue = ctx.type;
        ...
        if ( beh && beh->copy && beh->copy != engine->scriptTypeBehaviours.beh.copy ) {
            ...
            *lvalue = res.type;
            ...
            bc->InstrSHORT_DW( asBC_COPY, (short) lvalue->dataType.GetSizeInMemoryDWords(),
                               engine->GetTypeIdFromDataType( lvalue->dataType ) );
        }
    } else {
...
bool asCCompiler::CompileRefCast( asCExprContext* ctx, const asCDataType& to, bool isExplicit, asCScriptNode* node,
                                  bool generateCode ) {
    ...
    for ( asUINT n = 0; ot && n < ot->methods.GetLength(); n++ ) {
        ...
        if ( ( isExplicit && func->name == "opCast" ) || func->name == "opImplCast" ) {
    ...
    if ( ops.GetLength() == 1 ) {
        ...
        if ( generateCode ) {
            ...
            if ( ( ctx->type.dataType.GetTypeInfo()->flags & asOBJ_REF )
                 && !( ctx->type.dataType.GetTypeInfo()->flags & asOBJ_NOHANDLE ) ) {
        ...
        for ( asUINT n = 0; ot && n < ot->methods.GetLength(); n++ ) {
            ...
            if ( ( isExplicit && func->name == "opCast" ) || func->name == "opImplCast" ) {
                ...
                if ( func->returnType.GetTokenType() != ttVoid || func->parameterTypes.GetLength() != 1
                     || func->parameterTypes[0].GetTokenType() != ttQuestion || func->inOutFlags[0] != asTM_OUTREF )
                    continue;
...
asUINT asCCompiler::ImplicitConvPrimitiveToPrimitive( asCExprContext* ctx, const asCDataType& toOrig,
                                                      asCScriptNode* node, EImplicitConv convType, bool generateCode ) {
    ...
    if ( ctx->enumValue != "" ) {
        if ( to.IsEnumType() ) {
            ...
            asCDataType out;
            asDWORD value;
            if ( builder->GetEnumValueFromType( CastToEnumType( to.GetTypeInfo() ), ctx->enumValue.AddressOf(), out,
                                                value ) ) {
    ...
    if ( ( to.IsIntegerType() || to.IsUnsignedType() )
        ...
        cost = asCC_INT_FLOAT_CONV;
    else if ( ctx->type.dataType.IsEnumType() && to.IsIntegerType()
              && to.GetSizeInMemoryBytes() == ctx->type.dataType.GetSizeInMemoryBytes() )
    ...
    if ( generateCode ) {
        ...
        if ( ( to.IsIntegerType() && to.GetSizeInMemoryDWords() == 1 && !to.IsEnumType() )
            ...
            if ( ctx->type.dataType.IsIntegerType() || ctx->type.dataType.IsUnsignedType() ) {
        ...
        if ( ( ( to.IsIntegerType() && !to.IsEnumType() ) || to.IsUnsignedType() || to.IsFloatType()
               || to.IsDoubleType() || ( to.IsEnumType() && convType == asIC_EXPLICIT_VAL_CAST ) )
             && ( ctx->type.dataType.IsIntegerType() || ctx->type.dataType.IsUnsignedType()
                  || ctx->type.dataType.IsFloatType() || ctx->type.dataType.IsDoubleType() ) ) {
            ctx->type.dataType.SetTokenType( to.GetTokenType() );
            ctx->type.dataType.SetTypeInfo( to.GetTypeInfo() );
...
asUINT asCCompiler::ImplicitConvLambdaToFunc( asCExprContext* ctx, const asCDataType& to, asCScriptNode* /*node*/,
                                              EImplicitConv /*convType*/, bool generateCode ) {
    ...
    while ( argNode->nodeType != snStatementBlock ) {
        ...
        if ( argNode->nodeType == snDataType ) {
            asCDataType dt
                = builder->CreateDataTypeFromNode( argNode, script, outFunc->nameSpace, false, outFunc->objectType );
            asETypeModifiers inOutFlag;
            dt = builder->ModifyDataTypeFromNode( dt, argNode->next, script, &inOutFlag, 0 );

            if ( count >= funcDef->parameterTypes.GetLength() || funcDef->parameterTypes[count] != dt
    ...
    if ( generateCode ) {
        ...
        asCScriptFunction* func
            = builder->RegisterLambda( ctx->exprNode, script, funcDef, name, outFunc->nameSpace, outFunc->IsShared() );
...
asUINT asCCompiler::ImplicitConversion( asCExprContext* ctx, const asCDataType& to, asCScriptNode* node,
                                        EImplicitConv convType, bool generateCode, bool allowObjectConstruct ) {
    asASSERT( ctx->type.dataType.GetTokenType() != ttUnrecognizedToken || ctx->type.dataType.IsNullHandle()
...
asUINT asCCompiler::ImplicitConvObjectToPrimitive( asCExprContext* ctx, const asCDataType& to, asCScriptNode* node,
                                                   EImplicitConv convType, bool generateCode ) {
    if ( ctx->type.isExplicitHandle ) {
        // An explicit handle cannot be converted to a primitive
        if ( convType != asIC_IMPLICIT_CONV && node ) {
            asCString str;
            str.Format( TXT_CANT_IMPLICITLY_CONVERT_s_TO_s, ctx->type.dataType.Format( outFunc->nameSpace ).AddressOf(),
    ...
    if ( convType == asIC_EXPLICIT_VAL_CAST ) {
        for ( unsigned int n = 0; n < ot->methods.GetLength(); n++ ) {
            ...
            if ( ( mthd->name == "opConv" || mthd->name == "opImplConv" ) && mthd->parameterTypes.GetLength() == 0
        ...
        for ( unsigned int n = 0; n < ot->methods.GetLength(); n++ ) {
            ...
            asCScriptFunction* mthd = engine->scriptFunctions[ot->methods[n]];
            if ( mthd->name == "opImplConv" && mthd->parameterTypes.GetLength() == 0 && mthd->returnType.IsPrimitive() )
                funcs.PushLast( ot->methods[n] );
        }
    }
    ...
    for ( asUINT n = 0; n < ot->methods.GetLength(); n++ ) {
        asCScriptFunction* func = engine->scriptFunctions[ot->methods[n]];
        if ( ( ( convType == asIC_EXPLICIT_VAL_CAST ) && func->name == "opConv" ) || func->name == "opImplConv" ) {
            // Does the operator take the ?&out parameter?
            if ( func->returnType != asCDataType::CreatePrimitive( ttVoid, false )
                 || func->parameterTypes.GetLength() != 1 || func->parameterTypes[0].GetTokenType() != ttQuestion
...
asUINT asCCompiler::ImplicitConvObjectRef( asCExprContext* ctx, const asCDataType& to, asCScriptNode* node,
                                           EImplicitConv convType, bool generateCode ) {
    ...
    if ( to.IsFuncdef() ) {
        ...
        if ( ctx->type.dataType.IsFuncdef() && to.GetTypeInfo() != ctx->type.dataType.GetTypeInfo() ) {
...
asUINT asCCompiler::ImplicitConvObjectValue( asCExprContext* ctx, const asCDataType& to, asCScriptNode* node,
                                             EImplicitConv convType, bool generateCode ) {
    asUINT cost = asCC_NO_CONV;
        ...
        if ( convType == asIC_EXPLICIT_VAL_CAST ) {
            for ( unsigned int n = 0; n < ot->methods.GetLength(); n++ ) {
                ...
                if ( ( func->name == "opConv" || func->name == "opImplConv" )
                     && func->returnType.GetTypeInfo() == to.GetTypeInfo() && func->parameterTypes.GetLength() == 0 )
                    funcs.PushLast( ot->methods[n] );
            }
        } else {
            for ( unsigned int n = 0; n < ot->methods.GetLength(); n++ ) {
                ...
                // accept only implicit cast
                if ( func->name == "opImplConv" && func->returnType.GetTypeInfo() == to.GetTypeInfo()
        ...
        if ( funcs.GetLength() == 1 ) {
            ...
            if ( funcs.GetLength() == 1 ) {
                ...
                for ( asUINT n = 0; n < funcs.GetLength(); n++ ) {
                    ...
                    if ( f == 0 || f->parameterTypes.GetLength() != 1
                         || ( convType != asIC_EXPLICIT_VAL_CAST && f->IsExplicit() ) )
                        funcs.RemoveIndex( n-- );
                }
                ...
                if ( funcs.GetLength() == 1 ) {
                    if ( generateCode ) {
                        ...
                        PerformFunctionCall( funcs[0], &e, onHeap, &args,
                                             CastToObjectType( tempObj.dataType.GetTypeInfo() ) );
...
asUINT asCCompiler::ImplicitConvObjectToObject( asCExprContext* ctx, const asCDataType& to, asCScriptNode* node,
                                                EImplicitConv convType, bool generateCode, bool allowObjectConstruct ) {
    ...
    if ( to.GetTypeInfo() && ( to.GetTypeInfo()->flags & asOBJ_ASHANDLE )
         && to.GetTypeInfo() != ctx->type.dataType.GetTypeInfo() && allowObjectConstruct ) {
        asCArray<int> funcs;
        funcs = CastToObjectType( to.GetTypeInfo() )->beh.constructors;
    ...
    } else // if( !to.IsObjectHandle() )
    {
        ...
        } else // if( to.IsReference() )
        {
            ...
            if ( ctx->type.dataType.IsReference() ) {
                if ( ctx->type.isExplicitHandle && ctx->type.dataType.GetTypeInfo()
                     && ( ctx->type.dataType.GetTypeInfo()->flags & asOBJ_ASHANDLE ) ) {
...
asUINT asCCompiler::ImplicitConvPrimitiveToObject( asCExprContext* ctx, const asCDataType& to, asCScriptNode* node,
                                                   EImplicitConv isExplicit, bool generateCode,
    ...
    if ( objType->flags & asOBJ_VALUE ) {
        ...
        for ( asUINT n = 0; n < objType->beh.constructors.GetLength(); n++ ) {
            ...
            if ( func->parameterTypes.GetLength() == 1 && func->parameterTypes[0].IsPrimitive()
                 && !( func->inOutFlags[0] & asTM_OUTREF )
...
void asCCompiler::ImplicitConversionConstant( asCExprContext* from, const asCDataType& to, asCScriptNode* node,
    ...
    if ( to.IsEqualExceptRefAndConst( from->type.dataType ) )
        return;

    // References cannot be constants
    if ( from->type.dataType.IsReference() )
        return;

    if ( ( to.IsIntegerType() && to.GetSizeInMemoryDWords() == 1 && !to.IsEnumType() )
         || ( to.IsEnumType() && convType == asIC_EXPLICIT_VAL_CAST ) ) {
        if ( from->type.dataType.IsFloatType() || from->type.dataType.IsDoubleType()
             || from->type.dataType.IsUnsignedType() || from->type.dataType.IsIntegerType() ) {
            ...
            // Double constants can be implicitly converted to int
                ...
                if ( from->type.dataType.GetSizeInMemoryBytes() == 4 && int( from->type.GetConstantDW() ) < 0
                     && convType != asIC_EXPLICIT_VAL_CAST && node != 0 )
                ...
                if ( asQWORD( from->type.GetConstantQW() ) >> 31 )
                    if ( convType != asIC_EXPLICIT_VAL_CAST && node )
                        Warning( TXT_VALUE_TOO_LARGE_FOR_TYPE, node );
                ...
                // Convert to 32bit
                from->type.SetConstantDW( targetDt, int( from->type.GetConstantQW() ) );
            } else if ( from->type.dataType.IsIntegerType() && from->type.dataType.GetSizeInMemoryBytes() < 4 ) {
                // Convert to 32bit
                if ( from->type.dataType.GetSizeInMemoryBytes() == 1 )
                ...
                asASSERT( ( from->type.dataType.GetTokenType() == ttInt || from->type.dataType.IsEnumType() )
        ...
        if ( to.IsIntegerType() && from->type.dataType.IsIntegerType()
            ...
            if ( to.GetSizeInMemoryBytes() == 1 ) {
                ...
                from->type.SetConstantB( asCDataType::CreatePrimitive( to.GetTokenType(), true ),
                                         asINT8( from->type.GetConstantDW() ) );
                ...
                from->type.SetConstantW( asCDataType::CreatePrimitive( to.GetTokenType(), true ),
                                         asINT16( from->type.GetConstantDW() ) );
            }
        }
    } else if ( to.IsIntegerType() && to.GetSizeInMemoryDWords() == 2 ) {
        // Float constants can be implicitly converted to int
        ...
        // Double constants can be implicitly converted to int
            ...
            // Convert to 64bit
            if ( from->type.dataType.GetSizeInMemoryBytes() == 1 )
                from->type.SetConstantQW( asCDataType::CreatePrimitive( ttInt64, true ),
                                          (asINT8) from->type.GetConstantB() );
            else if ( from->type.dataType.GetSizeInMemoryBytes() == 2 )
                from->type.SetConstantQW( asCDataType::CreatePrimitive( ttInt64, true ),
                                          (asINT16) from->type.GetConstantW() );
            else if ( from->type.dataType.GetSizeInMemoryBytes() == 4 )
                from->type.SetConstantQW( asCDataType::CreatePrimitive( ttInt64, true ),
                                          (int) from->type.GetConstantDW() );
        }
    } else if ( to.IsUnsignedType() && to.GetSizeInMemoryDWords() == 1 ) {
        if ( from->type.dataType.IsFloatType() ) {
            float fc = from->type.GetConstantF();
            ...
            if ( from->type.dataType.GetSizeInMemoryBytes() > 4 && ( from->type.GetConstantQW() >> 32 ) != 0
                 && ( from->type.GetConstantQW() >> 32 ) != 0xFFFFFFFF ) {
                if ( convType != asIC_EXPLICIT_VAL_CAST && node )
                    Warning( TXT_VALUE_TOO_LARGE_FOR_TYPE, node );
            }
            ...
            // Convert to 32bit
            if ( from->type.dataType.GetSizeInMemoryBytes() == 1 )
                from->type.SetConstantDW( asCDataType::CreatePrimitive( ttUInt, true ),
                                          (asINT8) from->type.GetConstantB() );
            else if ( from->type.dataType.GetSizeInMemoryBytes() == 2 )
                from->type.SetConstantDW( asCDataType::CreatePrimitive( ttUInt, true ),
                                          (asINT16) from->type.GetConstantW() );
                ...
                from->type.SetConstantDW( asCDataType::CreatePrimitive( ttUInt, true ),
                                          (int) (asINT64) from->type.GetConstantQW() );

            // Try once more, in case of a smaller type
            ImplicitConversionConstant( from, to, node, convType );
        } else if ( from->type.dataType.IsUnsignedType() && from->type.dataType.GetSizeInMemoryBytes() < 4 ) {
            // Convert to 32bit
            if ( from->type.dataType.GetSizeInMemoryBytes() == 1 )
            ...
            if ( to.GetSizeInMemoryBytes() == 1 ) {
                if ( ( from->type.dataType.GetSizeInMemoryBytes() == 2
                       && asBYTE( from->type.GetConstantW() ) != from->type.GetConstantW() )
                     || ( from->type.dataType.GetSizeInMemoryBytes() == 4
                          && asBYTE( from->type.GetConstantDW() ) != from->type.GetConstantDW() )
                     || ( from->type.dataType.GetSizeInMemoryBytes() == 8
                          && asBYTE( from->type.GetConstantQW() ) != from->type.GetConstantQW() ) )
                    if ( convType != asIC_EXPLICIT_VAL_CAST && node )
                        Warning( TXT_VALUE_TOO_LARGE_FOR_TYPE, node );

                if ( from->type.dataType.GetSizeInMemoryBytes() == 2 )
                    from->type.SetConstantB( asCDataType::CreatePrimitive( to.GetTokenType(), true ),
                                             asBYTE( from->type.GetConstantW() ) );
                else if ( from->type.dataType.GetSizeInMemoryBytes() == 4 )
                    from->type.SetConstantB( asCDataType::CreatePrimitive( to.GetTokenType(), true ),
                                             asBYTE( from->type.GetConstantDW() ) );
                else if ( from->type.dataType.GetSizeInMemoryBytes() == 8 )
                    from->type.SetConstantB( asCDataType::CreatePrimitive( to.GetTokenType(), true ),
                                             asBYTE( from->type.GetConstantQW() ) );
            } else if ( to.GetSizeInMemoryBytes() == 2 ) {
                if ( ( from->type.dataType.GetSizeInMemoryBytes() == 4
                       && asWORD( from->type.GetConstantDW() ) != from->type.GetConstantDW() )
                     || ( from->type.dataType.GetSizeInMemoryBytes() == 8
                          && asWORD( from->type.GetConstantQW() ) != from->type.GetConstantQW() ) )
                    if ( convType != asIC_EXPLICIT_VAL_CAST && node )
                        Warning( TXT_VALUE_TOO_LARGE_FOR_TYPE, node );

                if ( from->type.dataType.GetSizeInMemoryBytes() == 4 )
                    from->type.SetConstantW( asCDataType::CreatePrimitive( to.GetTokenType(), true ),
                                             asWORD( from->type.GetConstantDW() ) );
                else if ( from->type.dataType.GetSizeInMemoryBytes() == 8 )
                    from->type.SetConstantW( asCDataType::CreatePrimitive( to.GetTokenType(), true ),
                                             asWORD( from->type.GetConstantQW() ) );
                ...
                from->type.SetConstantDW( asCDataType::CreatePrimitive( to.GetTokenType(), true ),
                                          asDWORD( from->type.GetConstantQW() ) );
            }
        }
    } else if ( to.IsUnsignedType() && to.GetSizeInMemoryDWords() == 2 ) {
        if ( from->type.dataType.IsFloatType() ) {
            ...
            // Convert to 64bit
            if ( from->type.dataType.GetSizeInMemoryBytes() == 1 )
                from->type.SetConstantQW( asCDataType::CreatePrimitive( ttUInt64, true ),
                                          (asINT64) (asINT8) from->type.GetConstantB() );
            else if ( from->type.dataType.GetSizeInMemoryBytes() == 2 )
                from->type.SetConstantQW( asCDataType::CreatePrimitive( ttUInt64, true ),
                                          (asINT64) (asINT16) from->type.GetConstantW() );
            else if ( from->type.dataType.GetSizeInMemoryBytes() == 4 )
                from->type.SetConstantQW( asCDataType::CreatePrimitive( ttUInt64, true ),
                                          (asINT64) (int) from->type.GetConstantDW() );
...
int asCCompiler::DoAssignment( asCExprContext* ctx, asCExprContext* lctx, asCExprContext* rctx, asCScriptNode* lexpr,
                               asCScriptNode* rexpr, eTokenType op, asCScriptNode* opNode ) {
        ...
        if ( lctx->type.dataType.GetTypeInfo() && ( lctx->type.dataType.GetTypeInfo()->flags & asOBJ_ASHANDLE ) ) {
            ...
            if ( !rctx->type.isExplicitHandle
                 && !( rctx->type.dataType.GetTypeInfo()
                       && ( rctx->type.dataType.GetTypeInfo()->flags & asOBJ_ASHANDLE ) ) ) {
            ...
            asCString msg;
            msg.Format( TXT_NO_APPROPRIATE_OPHNDLASSIGN_s,
                        lctx->type.dataType.Format( outFunc->nameSpace ).AddressOf() );
    ...
    } else // if( lctx->type.dataType.IsObject() )
    {
        ...
        bool simpleExpr
            = ( lctx->type.dataType.GetTypeInfo()->GetFlags() & asOBJ_VALUE ) && lctx->bc.IsSimpleExpression();
        ...
        if ( !simpleExpr || needConversion ) {
            if ( rctx->type.dataType.IsObjectHandle() && !rctx->type.isExplicitHandle
                 && !lctx->type.dataType.IsObjectHandle()
                 && rctx->type.dataType.GetTypeInfo() == lctx->type.dataType.GetTypeInfo() ) {
            ...
            if ( rctx->type.dataType.IsReference()
                 && ( !( rctx->type.isVariable || rctx->type.isTemporary )
                      || IsVariableOnHeap( rctx->type.stackOffset ) ) )
...
int asCCompiler::CompileCondition( asCScriptNode* expr, asCExprContext* ctx ) {
    ...
    if ( cexpr->next ) {
        ...
        if ( e.type.dataType.IsReference() )
            ConvertToVariable( &e );
        ...
        if ( lr >= 0 && rr >= 0 ) {
            ...
            if ( le.IsAnonymousInitList() && re.type.dataType.GetBehaviour()
                 && re.type.dataType.GetBehaviour()->listFactory ) {
                asCDataType to = re.type.dataType;
                to.MakeReference( false );
                to.MakeReadOnly( false );
                ImplicitConversion( &le, to, cexpr->next, asIC_IMPLICIT_CONV );
            } else if ( re.IsAnonymousInitList() && le.type.dataType.GetBehaviour()
                        && le.type.dataType.GetBehaviour()->listFactory ) {
            ...
            if ( le.type.dataType != re.type.dataType ) {
                ...
                tmp.type.dataType.MakeReference( false );
                asUINT costBtoA
                    = ImplicitConversion( &tmp, le.type.dataType, cexpr->next->next, asIC_IMPLICIT_CONV, false );
            ...
            if ( !le.type.dataType.IsObjectHandle() && re.type.dataType.IsObjectHandle()
                 && le.type.dataType.GetTypeInfo() == re.type.dataType.GetTypeInfo() ) {
                asCDataType dt = le.type.dataType;
                dt.MakeHandle( true );
                ImplicitConversion( &le, dt, cexpr->next, asIC_IMPLICIT_CONV );
            }
            if ( !re.type.dataType.IsObjectHandle() && le.type.dataType.IsObjectHandle()
                 && le.type.dataType.GetTypeInfo() == re.type.dataType.GetTypeInfo() ) {
            ...
            if ( ( le.type.dataType.IsHandleToConst() && !le.type.IsNullConstant() )
                 || ( re.type.dataType.IsHandleToConst() && !re.type.dataType.IsNullHandle() ) ) {
            ...
            if ( le.type.dataType.IsEqualExceptConst( asCDataType::CreatePrimitive( ttVoid, false ) ) ) {
                ...
                if ( le.type.isLValue && re.type.isLValue && le.deferredParams.GetLength() == 0
                     && re.deferredParams.GetLength() == 0 && !le.type.isTemporary && !re.type.isTemporary
                    ...
                    if ( le.type.dataType.IsReference() && le.type.dataType.IsObject()
                         && !le.type.dataType.IsObjectHandle() ) {
                        asASSERT( re.type.dataType.IsReference() && re.type.dataType.IsObject()
                                  && !re.type.dataType.IsObjectHandle() );
                    ...
                    ctx->type.isRefToLocal
                        = le.type.isVariable || le.type.isRefToLocal || re.type.isVariable || re.type.isRefToLocal;
                } else {
...
void asCCompiler::ConvertToPostFix( asCScriptNode* expr, asCArray<asCScriptNode*>& postfix ) {
    ...
    while ( node ) {
        ...
        while ( stackA.GetLength() > 0 && precedence <= GetPrecedence( stackA[stackA.GetLength() - 1] ) )
...
int asCCompiler::CompileAnonymousInitList( asCScriptNode* node, asCExprContext* ctx, const asCDataType& dt ) {
    ...
    if ( outFunc->IsShared() && dt.GetTypeInfo() && !dt.GetTypeInfo()->IsShared() ) {
        asCString msg;
...
//  SL_NOMATCH         = no match
//  SL_ERROR           = error
asCCompiler::SYMBOLTYPE asCCompiler::SymbolLookupMember( const asCString& name, asCObjectType* objType,
                                                         asCExprContext* outResult ) {
    ...
    for ( asUINT n = 0; n < ot->methods.GetLength(); n++ ) {
        ...
        if ( f->name == name && ( builder->module->m_accessMask & f->accessMask ) ) {
            outResult->type.dataType.SetTypeInfo( objType );
...
//  SL_ERROR            = error
asCCompiler::SYMBOLTYPE asCCompiler::SymbolLookup( const asCString& name, const asCString& scope,
                                                   asCObjectType* objType, asCExprContext* outResult ) {
    asASSERT( outResult );
    ...
    while ( !objType && currNamespace ) {
        ...
        if ( ns ) {
            ...
            asQWORD constantValue = 0;
            asCGlobalProperty* prop = builder->GetGlobalProperty( name.AddressOf(), ns, &isCompiled, &isPureConstant,
                                                                  &constantValue, &isAppProp );
            if ( prop ) {
...
int asCCompiler::CompileVariableAccess( const asCString& name, const asCString& scope, asCExprContext* ctx,
                                        asCScriptNode* errNode, bool isOptional, asCObjectType* objType ) {
    ...
    // Is it a class member?
    if ( symbolType == SL_CLASSPROPACCESS || symbolType == SL_CLASSPROP || symbolType == SL_CLASSMETHOD
         || symbolType == SL_THISPTR ) {
        // This is not accessible by default arg expressions
    ...
    if ( symbolType == SL_GLOBALCONST || symbolType == SL_GLOBALPROPACCESS || symbolType == SL_GLOBALVAR
         || symbolType == SL_GLOBALFUNC || symbolType == SL_ENUMVAL ) {
            ...
            if ( isPureConstant )
                ...
                if ( ctx->type.dataType.IsPrimitive() ) {
                    ...
                    if ( ( ctx->type.dataType.GetTypeInfo()->flags & asOBJ_VALUE )
                        ...
                        asASSERT( ( ctx->type.dataType.GetTypeInfo()->flags & asOBJ_REF )
                                  && !ctx->type.dataType.IsObjectHandle() );
        ...
        if ( symbolType == SL_ENUMVAL ) {
            ...
            asDWORD value = 0;
            builder->GetEnumValueFromType( CastToEnumType( lookupResult.type.dataType.GetTypeInfo() ), name.AddressOf(),
                                           dt, value );
...
int asCCompiler::CompileExpressionValue( asCScriptNode* node, asCExprContext* ctx ) {
    ...
    if ( vnode->nodeType == snVariableAccess ) {
        ...
        if ( vnode->tokenType == ttIntConstant ) {
        ...
        } else if ( vnode->tokenType == ttTrue || vnode->tokenType == ttFalse ) {
#if AS_SIZEOF_BOOL == 1
            ctx->type.SetConstantB( asCDataType::CreatePrimitive( ttBool, true ),
                                    vnode->tokenType == ttTrue ? VALUE_OF_BOOLEAN_TRUE : 0 );
#else
            ctx->type.SetConstantDW( asCDataType::CreatePrimitive( ttBool, true ),
                                     vnode->tokenType == ttTrue ? VALUE_OF_BOOLEAN_TRUE : 0 );
#endif
        } else if ( vnode->tokenType == ttStringConstant || vnode->tokenType == ttMultilineStringConstant
            ...
            if ( script->code[snode->tokenPos] == '\'' && engine->ep.useCharacterLiterals ) {
                ...
                if ( engine->stringFactory == 0 ) {
                    ...
                    void* strPtr = const_cast<void*>(
                        engine->stringFactory->GetStringConstant( str.AddressOf(), (asUINT) str.GetLength() ) );
                    if ( strPtr == 0 ) {
...
void asCCompiler::ProcessHeredocStringConstant( asCString& str, asCScriptNode* node ) {
    ...
    for ( start = 0; start < (int) str.GetLength(); start++ ) {
        ...
        if ( str[start] != ' ' && str[start] != '\t' && str[start] != '\r' ) {
...
int asCCompiler::CompileConversion( asCScriptNode* node, asCExprContext* ctx ) {
    ...
    if ( node->nodeType == snConstructCall || node->nodeType == snFunctionCall ) {
        ...
        if ( node->lastChild->firstChild == 0 || node->lastChild->firstChild != node->lastChild->lastChild ) {
            Error( TXT_ONLY_ONE_ARGUMENT_IN_CAST, node->lastChild );
            expr.type.SetDummy();
            anyErrors = true;
        } else if ( node->lastChild->firstChild && node->lastChild->firstChild->nodeType == snNamedArgument ) {
    ...
    if ( outFunc->IsShared() && to.GetTypeInfo() && !to.GetTypeInfo()->IsShared() ) {
        asCString msg;
    ...
    if ( !expr.type.isConstant && expr.type.dataType != asCDataType::CreatePrimitive( ttVoid, false ) ) {
        ...
        if ( to.IsObjectHandle() && expr.type.dataType.IsObjectHandle()
    ...
    for ( ; n >= 0; n-- ) {
        ...
        if ( ( descr->parameterTypes[n].IsReference() && ( descr->inOutFlags[n] & asTM_OUTREF )
             ...
             || ( descr->parameterTypes[n].IsObject() && deferAll
                  && ( descr->parameterTypes[n].IsReference() || descr->parameterTypes[n].IsObjectHandle() ) ) ) {
            asASSERT( !( descr->parameterTypes[n].IsReference() && ( descr->inOutFlags[n] == asTM_OUTREF )
                         && !args[n]->isCleanArg )
                      || args[n]->origExpr );

            // For &inout, only store the argument if it is for a temporary variable
            if ( engine->ep.allowUnsafeReferences || descr->inOutFlags[n] != asTM_INOUTREF
                 || args[n]->type.isTemporary ) {
    ...
    for ( asUINT n = 0; n < ctx->deferredParams.GetLength(); n++ ) {
        ...
        if ( outParam.argInOutFlags < asTM_OUTREF ) // &in, or not reference
        {
            ...
            if ( IsLValue( expr->type ) || expr->property_get || expr->property_set ) {
                ...
                if ( !o.type.dataType.IsPrimitive() )
                    o.bc.Instr( asBC_PopPtr );
                ...
                if ( !expr->IsVoidExpression() && !expr->type.IsNullConstant()
                     && !( expr->type.isConstant && expr->type.dataType.IsPrimitive()
                           && expr->type.GetConstantData() == 0 ) )
    ...
    if ( CompileArgumentList( node->lastChild, args, namedArgs ) >= 0 ) {
        ...
        if ( args.GetLength() == 0 ) {
            ...
            if ( beh && beh->construct == 0 && !( dt.GetTypeInfo()->flags & asOBJ_REF ) ) {
                ...
                CallDefaultConstructor( tempObj.dataType, tempObj.stackOffset, IsVariableOnHeap( tempObj.stackOffset ),
                                        &ctx->bc, node );
        ...
        if ( dt.IsFuncdef() && args.GetLength() == 1 && args[0]->methodName != "" ) {
            ...
            if ( !args[0]->type.dataType.SupportHandles() ) {
                ...
                for ( asUINT n = 0; n < type->methods.GetLength(); n++ ) {
                    ...
                    if ( func->IsSignatureExceptNameAndObjectTypeEqual(
                ...
                if ( bestMethod ) {
                    ...
                    asCString msg;
                    msg.Format( TXT_NO_MATCHING_SIGNATURES_TO_s,
                                CastToFuncdefType( dt.GetTypeInfo() )->funcdef->GetDeclaration() );
        ...
        if ( funcs.GetLength() != 1 ) {
            error = true; // The error was reported by MatchFunctions()
            ctx->type.SetDummy(); // Dummy value
        } else {
            // TODO: Clean up: Merge this with MakeFunctionCall

            // Add the default values for arguments not explicitly supplied
            int r
                = CompileDefaultAndNamedArgs( node, args, funcs[0], CastToObjectType( dt.GetTypeInfo() ), &namedArgs );
...
int asCCompiler::CompileFunctionCall( asCScriptNode* node, asCExprContext* ctx, asCObjectType* objectType,
                                      bool objIsConst, const asCString& scope ) {
    asCExprValue tempObj;
    asCArray<int> funcs;
    int localVar = -1;
    ...
    if ( symbolType == SL_LOCALCONST || symbolType == SL_LOCALVAR || symbolType == SL_THISPTR
         || symbolType == SL_CLASSPROPACCESS || symbolType == SL_CLASSPROP || symbolType == SL_GLOBALPROPACCESS
         || symbolType == SL_GLOBALCONST || symbolType == SL_GLOBALVAR || symbolType == SL_ENUMVAL ) {
        ...
        if ( funcExpr.type.dataType.IsFuncdef() ) {
            ...
            builder->GetObjectMethodDescriptions( "opCall", CastToObjectType( funcExpr.type.dataType.GetTypeInfo() ),
                                                  funcs, objIsConst );
        } else {
            // The variable is not a function or object with opCall
            asCString msg;
            msg.Format( TXT_NOT_A_FUNC_s_IS_TYPE_s, name.AddressOf(),
                        lookupResult.type.dataType.Format( outFunc->nameSpace ).AddressOf() );
            Error( msg, node );
            return -1;
        }
    }
    ...
    if ( symbolType == SL_CLASSMETHOD ) {
        ...
        if ( scope == "" && m_isConstructor && name == SUPER_TOKEN ) {
            ...
            builder->GetObjectMethodDescriptions( name.AddressOf(),
                                                  CastToObjectType( lookupResult.type.dataType.GetTypeInfo() ), funcs,
                                                  objIsConst, scope, node, script );
        }
        ...
        if ( funcs.GetLength() && !objectType && outFunc->objectType ) {
            ...
            if ( !outFunc->objectType->DerivesFrom( lookupResult.type.dataType.GetTypeInfo() ) ) {
                ...
                msg.Format( TXT_METHOD_s_NOT_PART_OF_OBJECT_s, mthd.AddressOf(),
                            outFunc->objectType->name.AddressOf() );
                Error( msg, node );
                return -1;
            }
    ...
    if ( symbolType == SL_CLASSTYPE || symbolType == SL_GLOBALTYPE ) {
        bool isValid = false;
        asCDataType dt = builder->CreateDataTypeFromNode( node->firstChild, script, outFunc->nameSpace, false,
                                                          outFunc->objectType, false, &isValid );
        if ( isValid )
...
int asCCompiler::CompileExpressionPreOp( asCScriptNode* node, asCExprContext* ctx ) {
    ...
    if ( op == ttHandle ) {
        ...
        ctx->type.isExplicitHandle = true;
    } else if ( ( op == ttMinus || op == ttPlus || op == ttBitNot || op == ttInc || op == ttDec )
        ...
        if ( opName ) {
            ...
            for ( asUINT n = 0; n < ot->methods.GetLength(); n++ ) {
                asCScriptFunction* func = engine->scriptFunctions[ot->methods[n]];
                if ( func->name == opName && func->parameterTypes.GetLength() == 0
            ...
            if ( funcs.GetLength() == 1 ) {
                asCArray<asCExprContext*> args;
                return MakeFunctionCall( ctx, funcs[0], CastToObjectType( ctx->type.dataType.GetTypeInfo() ), args,
                                         node );
...
int asCCompiler::FindPropertyAccessor( const asCString& name, asCExprContext* ctx, asCScriptNode* node,
                                       asSNameSpace* ns, bool isThisAccess ) {
...
int asCCompiler::FindPropertyAccessor( const asCString& name, asCExprContext* ctx, asCExprContext* arg,
                                       asCScriptNode* node, asSNameSpace* ns, bool isThisAccess ) {
    ...
    if ( multipleGetFuncs.GetLength() > 0 ) {
        ...
        if ( multipleGetFuncs.GetLength() > 1 ) {
            ...
            getId = multipleGetFuncs[0]; // The id may have changed
    }
    ...
    if ( multipleSetFuncs.GetLength() > 0 ) {
        ...
        if ( multipleSetFuncs.GetLength() > 1 ) {
            ...
            setId = multipleSetFuncs[0]; // The id may have changed
    }
    ...
    if ( outFunc->objectType && isThisAccess ) {
        ...
        if ( getFunc && getFunc->funcType == asFUNC_VIRTUAL && outFunc->objectType->DerivesFrom( getFunc->objectType ) )
        ...
        if ( setFunc && setFunc->funcType == asFUNC_VIRTUAL && outFunc->objectType->DerivesFrom( setFunc->objectType ) )
    ...
    if ( ( isThisAccess || outFunc->objectType == 0 )
         && ( ( realGetId && realGetId == outFunc->id ) || ( realSetId && realSetId == outFunc->id ) ) ) {
        getId = 0;
        setId = 0;
    }
    ...
    return 0; // No accessor was found
}
...
int asCCompiler::ProcessPropertyGetSetAccessor( asCExprContext* ctx, asCExprContext* lctx, asCExprContext* rctx,
                                                eTokenType op, asCScriptNode* errNode ) {
...
int asCCompiler::CompileExpressionPostOp( asCScriptNode* node, asCExprContext* ctx ) {
    ...
    if ( ( op == ttInc || op == ttDec ) && ctx->type.dataType.IsObject() ) {
        ...
        if ( node->firstChild->nodeType == snIdentifier ) {
            ...
            if ( ctx->type.dataType.IsObject() ) {
                ...
                if ( prop ) {
                    ...
                    if ( ( prop->isPrivate || prop->isProtected )
                         && ( !outFunc || outFunc->objectType != ctx->type.dataType.GetTypeInfo() ) ) {
                        asCString msg;
                        if ( prop->isPrivate )
                    ...
                    if ( prop->compositeOffset || prop->isCompositeIndirect )
                        ctx->bc.InstrSHORT_DW( asBC_ADDSi, (short) prop->compositeOffset,
                                               engine->GetTypeIdFromDataType( asCDataType::CreateType(
                                                   ctx->type.dataType.GetTypeInfo(), false ) ) );
                    if ( prop->isCompositeIndirect )
                        ctx->bc.Instr( asBC_RDSPtr );

                    // Put the offset on the stack
                    // This must always be done even if the offset is 0 so the type info is stored
                    ctx->bc.InstrSHORT_DW( asBC_ADDSi, (short) prop->byteOffset,
                                           engine->GetTypeIdFromDataType(
                                               asCDataType::CreateType( ctx->type.dataType.GetTypeInfo(), false ) ) );
                    ...
                    if ( func ) {
                        ...
                        asCString str;
                        str.Format( TXT_s_NOT_MEMBER_OF_s, name.AddressOf(),
                                    ctx->type.dataType.Format( outFunc->nameSpace ).AddressOf() );
                        Error( str, node );
                        return -1;
                    }
            ...
            int r = CompileFunctionCall( node->firstChild, ctx, CastToObjectType( ctx->type.dataType.GetTypeInfo() ),
                                         ctx->type.dataType.IsObjectConst() );
            if ( r < 0 )
                return r;
        }
        ...
        if ( CompileArgumentList( node->firstChild, args, namedArgs ) >= 0 ) {
            ...
            if ( propertyName == "" ) {
                ...
                if ( funcs.GetLength() > 0 ) {
                    ...
                    if ( funcs.GetLength() != 1 ) {
                        ...
                        if ( r < 0 )
                            isOK = false;
                        else if ( MakeFunctionCall( ctx, funcs[0], objectType, args, node, false, 0,
                                                    ctx->type.stackOffset )
                                  < 0 )
                            isOK = false;
                    }
                }
            ...
            if ( lookForProperty && isOK ) {
                if ( args.GetLength() != 1 ) {
                    ...
                    int r = FindPropertyAccessor( propertyName == "" ? "opIndex" : propertyName.AddressOf(), &lctx,
                                                  args[0], node, ns );
                    if ( r == 0 ) {
                        asCString str;
                        str.Format( TXT_OBJECT_DOESNT_SUPPORT_INDEX_OP,
        ...
        if ( !ctx->type.dataType.GetTypeInfo()
             || ( !ctx->type.dataType.IsFuncdef() && !ctx->type.dataType.IsObject() ) ) {
        ...
        if ( CompileArgumentList( node->lastChild, args, namedArgs ) >= 0 ) {
            ...
            if ( ctx->type.dataType.IsFuncdef() ) {
                ...
                builder->GetObjectMethodDescriptions( "opCall", CastToObjectType( ctx->type.dataType.GetTypeInfo() ),
                                                      funcs, isConst );
                MatchFunctions( funcs, args, node, "opCall", &namedArgs,
                                CastToObjectType( ctx->type.dataType.GetTypeInfo() ), isConst );
            }
            ...
            if ( funcs.GetLength() != 1 ) {
                ...
                // Add the default values for arguments not explicitly supplied
                int r = CompileDefaultAndNamedArgs( node, args, funcs[0],
                                                    CastToObjectType( ctx->type.dataType.GetTypeInfo() ), &namedArgs );
                ...
                if ( r == asSUCCESS ) {
                    ...
                    r = MakeFunctionCall(
                        ctx, funcs[0],
                        ctx->type.dataType.IsFuncdef() ? 0 : CastToObjectType( ctx->type.dataType.GetTypeInfo() ), args,
                        node, false, 0, ctx->type.stackOffset );
                    if ( r < 0 )
                        isOK = false;
                } else
                    isOK = false;
...
int asCCompiler::GetPrecedence( asCScriptNode* op ) {
    ...
    if ( tokenType == ttBitShiftLeft || tokenType == ttBitShiftRight || tokenType == ttBitShiftRightArith )
        return -3;
    ...
    if ( tokenType == ttLessThanOrEqual || tokenType == ttLessThan || tokenType == ttGreaterThanOrEqual
         || tokenType == ttGreaterThan )
        return -7;

    if ( tokenType == ttEqual || tokenType == ttNotEqual || tokenType == ttXor || tokenType == ttIs
         || tokenType == ttNotIs )
        return -8;
...
asUINT asCCompiler::MatchArgument( asCArray<int>& funcs, asCArray<asSOverloadCandidate>& matches,
                                   const asCExprContext* argExpr, int paramNum, bool allowObjectConstruct ) {
    matches.SetLength( 0 );
...
int asCCompiler::MatchArgument( asCScriptFunction* desc, const asCExprContext* argExpr, int paramNum,
                                bool allowObjectConstruct ) {
    ...
    if ( desc->parameterTypes[paramNum].IsReference() && desc->inOutFlags[paramNum] == asTM_INOUTREF
        ...
        if ( desc->parameterTypes[paramNum].IsObjectHandle() && !argExpr->type.dataType.IsObjectHandle() ) {
            asASSERT( engine->ep.allowUnsafeReferences );
            return -1;
        }

        // Don't allow a value type to be converted
        if ( ( desc->parameterTypes[paramNum].GetTypeInfo()
               && ( desc->parameterTypes[paramNum].GetTypeInfo()->GetFlags() & asOBJ_VALUE ) )
             && ( desc->parameterTypes[paramNum].GetTypeInfo() != argExpr->type.dataType.GetTypeInfo() ) ) {
            asASSERT( engine->ep.allowUnsafeReferences );
            return -1;
        }
...
int asCCompiler::PrepareArgument2( asCExprContext* ctx, asCExprContext* arg, asCDataType* paramType, bool isFunction,
                                   int refType, bool isMakingCopy ) {
...
bool asCCompiler::CompileOverloadedDualOperator( asCScriptNode* node, asCExprContext* lctx, asCExprContext* rctx,
                                                 bool leftToRight, asCExprContext* ctx, bool isHandle,
                                                 eTokenType token ) {
    ...
    if ( token == ttAnd || token == ttOr || token == ttXor )
        return false;

    // Dual operators can also be implemented as class methods
    if ( token == ttEqual || token == ttNotEqual ) {
        ...
        int r = CompileOverloadedDualOperator2( node, "opEquals", lctx, rctx, leftToRight, ctx, true,
                                                asCDataType::CreatePrimitive( ttBool, false ) );
        if ( r == 0 ) {
            // Try again by switching the order of the operands
            r = CompileOverloadedDualOperator2( node, "opEquals", rctx, lctx, !leftToRight, ctx, true,
                                                asCDataType::CreatePrimitive( ttBool, false ) );
        }
    ...
    if ( token == ttEqual || token == ttNotEqual || token == ttLessThan || token == ttLessThanOrEqual
         || token == ttGreaterThan || token == ttGreaterThanOrEqual ) {
        bool swappedOrder = false;

        // TODO: Should evaluate which of the two have the best match. If both have equal match, the first version should be used
        // Find the matching opCmp method
        int r = CompileOverloadedDualOperator2( node, "opCmp", lctx, rctx, leftToRight, ctx, true,
                                                asCDataType::CreatePrimitive( ttInt, false ) );
        if ( r == 0 ) {
            // Try again by switching the order of the operands
            swappedOrder = true;
            r = CompileOverloadedDualOperator2( node, "opCmp", rctx, lctx, !leftToRight, ctx, true,
                                                asCDataType::CreatePrimitive( ttInt, false ) );
        }
        ...
        if ( r == 1 ) {
            ...
            else if ( ( token == ttLessThan && !swappedOrder ) || ( token == ttGreaterThan && swappedOrder ) )
            ...
            else if ( ( token == ttGreaterThan && !swappedOrder ) || ( token == ttLessThan && swappedOrder ) )
...
// Compiler error, don't continue
    ...
    if ( op && op_r ) {
        ...
        if ( r == 1 )
            return true; // Success, don't continue
        else if ( r < 0 ) {
            // Compiler error, don't continue
            ctx->type.SetDummy();
            return true;
    ...
    if ( op ) {
        if ( builder->engine->ep.disallowValueAssignForRefType && lctx->type.dataType.GetTypeInfo()
             && ( lctx->type.dataType.GetTypeInfo()->flags & asOBJ_REF )
             && !( lctx->type.dataType.GetTypeInfo()->flags & asOBJ_SCOPED ) ) {
            if ( token == ttAssignment )
    ...
    return false; // No suitable operator was found
}

// Returns negative on compile error
//         zero on no matching operator
//         one on matching operator
int asCCompiler::CompileOverloadedDualOperator2( asCScriptNode* node, const char* methodName, asCExprContext* lctx,
                                                 asCExprContext* rctx, bool leftToRight, asCExprContext* ctx,
                                                 bool specificReturn, const asCDataType& returnType ) {
    // Find the matching method
    if ( lctx->type.dataType.IsObject()
         && ( !lctx->type.isExplicitHandle || lctx->type.dataType.GetTypeInfo()->flags & asOBJ_ASHANDLE )
        ...
        for ( n = 0; ot && n < ot->methods.GetLength(); n++ ) {
            asCScriptFunction* func = engine->scriptFunctions[ot->methods[n]];
            asASSERT( func );
            if ( func && func->name == methodName && ( !specificReturn || func->returnType == returnType )
                 && func->parameterTypes.GetLength() == 1 && ( !isConst || func->IsReadOnly() ) ) {
        ...
        if ( ops.GetLength() == 1 ) {
            ...
            if ( leftToRight ) {
                ...
                if ( !lctx->type.isVariable ) {
                    ...
                    if ( engine->ep.allowUnsafeReferences && lctx->type.dataType.IsObject()
                         && ( lctx->type.dataType.GetTypeInfo()->flags & asOBJ_VALUE ) ) {
                ...
                for ( n = 0; n < rctx->deferredParams.GetLength(); n++ ) {
                    if ( rctx->deferredParams[n].argType.isTemporary
                        ...
                        int offset
                            = AllocateVariableNotIn( rctx->deferredParams[n].argType.dataType, true, false, rctx );
    ...
    return 0; // No matching operator
}

int asCCompiler::MakeFunctionCall( asCExprContext* ctx, int funcId, asCObjectType* objectType,
                                   asCArray<asCExprContext*>& args, asCScriptNode* node, bool useVariable,
                                   int stackOffset, int funcPtrVar ) {
    if ( objectType )
        Dereference( ctx, true );
...
int asCCompiler::CompileOperator( asCScriptNode* node, asCExprContext* lctx, asCExprContext* rctx, asCExprContext* ctx,
                                  eTokenType op, bool leftToRight ) {
    ...
    if ( lctx->type.isExplicitHandle || rctx->type.isExplicitHandle || lctx->type.IsNullConstant()
         || rctx->type.IsNullConstant() || op == ttIs || op == ttNotIs ) {
        ...
        if ( lctx->type.dataType.IsObject() && rctx->type.dataType.IsObject() ) {
            asCString str;
            str.Format( TXT_NO_MATCHING_OP_FOUND_FOR_TYPES_s_AND_s,
                        lctx->type.dataType.Format( outFunc->nameSpace ).AddressOf(),
                        rctx->type.dataType.Format( outFunc->nameSpace ).AddressOf() );
            Error( str, node );
            ctx->type.SetDummy();
        ...
        // Math operators
        // + - * / % ** += -= *= /= %= **=
        if ( op == ttPlus || op == ttAddAssign || op == ttMinus || op == ttSubAssign || op == ttStar
             || op == ttMulAssign || op == ttSlash || op == ttDivAssign || op == ttPercent || op == ttModAssign
        ...
        // Bitwise operators
        // << >> >>> & | ^ <<= >>= >>>= &= |= ^=
        if ( op == ttAmp || op == ttAndAssign || op == ttBitOr || op == ttOrAssign || op == ttBitXor
             || op == ttXorAssign || op == ttBitShiftLeft || op == ttShiftLeftAssign || op == ttBitShiftRight
             || op == ttShiftRightLAssign || op == ttBitShiftRightArith || op == ttShiftRightAAssign ) {
        ...
        // == != < > <= >=
        if ( op == ttEqual || op == ttNotEqual || op == ttLessThan || op == ttLessThanOrEqual || op == ttGreaterThan
...
void asCCompiler::ConvertToTempVariableNotIn( asCExprContext* ctx, asCExprContext* exclude ) {
    int l = int( reservedVariables.GetLength() );
    if ( exclude )
        exclude->bc.GetVarsUsed( reservedVariables );
    ConvertToTempVariable( ctx );
    reservedVariables.SetLength( l );
}
...
void asCCompiler::ConvertToVariable( asCExprContext* ctx ) {
    ...
    if ( !ctx->type.isVariable
        ...
        ctx->type.dataType.MakeHandle( true );
        ctx->type.dataType.MakeReference( true );
    } else if ( ( !ctx->type.isVariable || ctx->type.dataType.IsReference() ) && ctx->type.dataType.IsPrimitive() ) {
        if ( ctx->type.isConstant ) {
            offset = AllocateVariable( ctx->type.dataType, true );
            if ( ctx->type.dataType.GetSizeInMemoryBytes() == 1 )
...
void asCCompiler::ImplicitConvObjectToBestMathType( asCExprContext* ctx, asCScriptNode* node ) {
    ...
    if ( ot ) {
        for ( unsigned int n = 0; n < ot->methods.GetLength(); n++ ) {
            ...
            asCScriptFunction* func = engine->scriptFunctions[ot->methods[n]];
            if ( func->name == "opImplConv" && func->returnType.IsPrimitive() && func->parameterTypes.GetLength() == 0 )
                funcs.PushLast( ot->methods[n] );
        }

        // Use the one with the highest precision
        const eTokenType match[10]
            = { ttDouble, ttFloat, ttInt64, ttUInt64, ttInt, ttUInt, ttInt16, ttUInt16, ttInt8, ttUInt8 };
        ...
        // Do the conversion
        if ( funcs.GetLength() )
            ImplicitConvObjectToPrimitive( ctx, builder->GetFunctionDescription( funcs[0] )->returnType, node,
                                           asIC_IMPLICIT_CONV );
    }
}

void asCCompiler::CompileMathOperator( asCScriptNode* node, asCExprContext* lctx, asCExprContext* rctx,
                                       asCExprContext* ctx, eTokenType op ) {
    ...
    if ( ( lctx->type.isConstant && lctx->type.dataType.IsDoubleType() && !rctx->type.isConstant
           && rctx->type.dataType.IsFloatType() )
         || ( rctx->type.isConstant && rctx->type.dataType.IsDoubleType() && !lctx->type.isConstant
    ...
    if ( to.IsPrimitive() ) {
        ...
        // left-hand operand.
        if ( ( op == ttStarStar || op == ttPowAssign ) && lctx->type.dataType.IsDoubleType()
             && ( rctx->type.dataType.IsIntegerType() || rctx->type.dataType.IsUnsignedType() ) ) {
            to.SetTokenType( ttInt );
    ...
    if ( !lctx->type.dataType.IsIntegerType() && !lctx->type.dataType.IsUnsignedType()
         && !lctx->type.dataType.IsFloatType() && !lctx->type.dataType.IsDoubleType() ) {
        asCString str;
    ...
    if ( !rctx->type.dataType.IsIntegerType() && !rctx->type.dataType.IsUnsignedType()
         && !rctx->type.dataType.IsFloatType() && !rctx->type.dataType.IsDoubleType() ) {
        asCString str;
    ...
    if ( rctx->type.isConstant && ( op == ttSlash || op == ttDivAssign || op == ttPercent || op == ttModAssign )
    ...
    if ( !isConstant ) {
        ...
        if ( op == ttAddAssign || op == ttSubAssign || op == ttMulAssign || op == ttDivAssign || op == ttModAssign
             || op == ttPowAssign ) {
            ...
            if ( lctx->type.dataType.GetSizeInMemoryDWords() == 1 ) {
                ...
                if ( op == ttPlus )
                    ...
                    if ( rctx->type.GetConstantDW() == 0
                         || ( int( rctx->type.GetConstantDW() ) == -1 && lctx->type.GetConstantDW() == 0x80000000 ) )
                        v = 0;
                ...
                if ( lctx->type.dataType.GetTokenType() == ttUInt && op == ttMinus
                     && lctx->type.GetConstantDW() < rctx->type.GetConstantDW() )
                    ctx->type.dataType.SetTokenType( ttInt );
            } else {
                asQWORD v = 0;
                if ( op == ttPlus )
                    ...
                    if ( rctx->type.GetConstantQW() == 0
                         || ( rctx->type.GetConstantQW() == asQWORD( -1 )
                              && lctx->type.GetConstantQW() == ( asQWORD( 1 ) << 63 ) ) )
                        v = 0;
                    else if ( lctx->type.dataType.IsIntegerType() )
                        v = asINT64( lctx->type.GetConstantQW() ) / asINT64( rctx->type.GetConstantQW() );
                    else
                    ...
                    bool isOverflow;
                    if ( lctx->type.dataType.IsIntegerType() )
                        v = as_powi64( asINT64( lctx->type.GetConstantQW() ), asINT64( rctx->type.GetConstantQW() ),
                                       isOverflow );
                    else
                ...
                // If the right value is greater than the left value in a minus operation, then we need to convert the type to int
                if ( lctx->type.dataType.GetTokenType() == ttUInt64 && op == ttMinus
                     && lctx->type.GetConstantQW() < rctx->type.GetConstantQW() )
...
void asCCompiler::CompileBitwiseOperator( asCScriptNode* node, asCExprContext* lctx, asCExprContext* rctx,
                                          asCExprContext* ctx, eTokenType op ) {
    // TODO: If a constant is only using 32bits, then a 32bit operation is preferred
    ...
    if ( op == ttAmp || op == ttAndAssign || op == ttBitOr || op == ttOrAssign || op == ttBitXor
        ...
        asCDataType to;
        if ( lctx->type.dataType.GetSizeInMemoryDWords() == 2 || rctx->type.dataType.GetSizeInMemoryDWords() == 2 )
            to.SetTokenType( ttInt64 );
        else
            to.SetTokenType( ttInt );
        ...
        if ( lctx->type.dataType != to ) {
            asCString str;
            str.Format( TXT_NO_CONVERSION_s_TO_s, lctx->type.dataType.Format( outFunc->nameSpace ).AddressOf(),
                        to.Format( outFunc->nameSpace ).AddressOf() );
            Error( str, node );
        }
        ...
        if ( rctx->type.dataType != to ) {
            asCString str;
            str.Format( TXT_NO_CONVERSION_s_TO_s, rctx->type.dataType.Format( outFunc->nameSpace ).AddressOf(),
                        lctx->type.dataType.Format( outFunc->nameSpace ).AddressOf() );
            Error( str, node );
        }
    ...
    } else if ( op == ttBitShiftLeft || op == ttShiftLeftAssign || op == ttBitShiftRight || op == ttShiftRightLAssign
        ...
        // Also do not permit float/double to be implicitly converted to integer in this case
        if ( lctx->type.dataType.IsObject() || lctx->type.dataType.IsFloatType()
        ...
        if ( lctx->type.dataType.IsUnsignedType() && lctx->type.dataType.GetSizeInMemoryBytes() < 4 ) {
            // Upgrade to 32bit
            to = asCDataType::CreatePrimitive( ttUInt, false );
...
void asCCompiler::CompileComparisonOperator( asCScriptNode* node, asCExprContext* lctx, asCExprContext* rctx,
                                             asCExprContext* ctx, eTokenType op ) {
    ...
    for ( int n = 0; !signMismatch && n < 2; n++ ) {
        ...
        if ( opCtx->type.dataType.IsUnsignedType() != to.IsUnsignedType() ) {
            ...
            if ( opCtx->type.isConstant ) {
                if ( opCtx->type.dataType.GetTokenType() == ttUInt64
                     || opCtx->type.dataType.GetTokenType() == ttInt64 ) {
                    if ( !( opCtx->type.GetConstantQW() & ( asQWORD( 1 ) << 63 ) ) )
                        signMismatch = false;
                } else if ( opCtx->type.dataType.GetTokenType() == ttUInt
                            || opCtx->type.dataType.GetTokenType() == ttInt || opCtx->type.dataType.IsEnumType() ) {
                    if ( !( opCtx->type.GetConstantDW() & ( 1 << 31 ) ) )
                        signMismatch = false;
                } else if ( opCtx->type.dataType.GetTokenType() == ttUInt16
                            || opCtx->type.dataType.GetTokenType() == ttInt16 ) {
                    if ( !( opCtx->type.GetConstantW() & ( 1 << 15 ) ) )
                        signMismatch = false;
                } else if ( opCtx->type.dataType.GetTokenType() == ttUInt8
                            || opCtx->type.dataType.GetTokenType() == ttInt8 ) {
    ...
    if ( !isConstant ) {
        ...
        if ( to.IsBooleanType() ) {
            if ( op == ttEqual || op == ttNotEqual ) {
                ...
                // Make sure they are equal if not false
                if ( lv != 0 )
                    lv = VALUE_OF_BOOLEAN_TRUE;
                if ( rv != 0 )
                    rv = VALUE_OF_BOOLEAN_TRUE;
...
#endif
            } else { // TODO: Use TXT_ILLEGAL_OPERATION_ON
                Error( TXT_ILLEGAL_OPERATION, node );
            }
        } else {
            int i = 0;
...
void asCCompiler::CompileBooleanOperator( asCScriptNode* node, asCExprContext* lctx, asCExprContext* rctx,
                                          asCExprContext* ctx, eTokenType op ) {
    ...
    if ( op == ttXor ) {
        if ( !isConstant ) {
            ...
            // Make sure they are equal if not false
#if AS_SIZEOF_BOOL == 1
            if ( lctx->type.GetConstantB() != 0 )
                lctx->type.SetConstantB( VALUE_OF_BOOLEAN_TRUE );
            if ( rctx->type.GetConstantB() != 0 )
                rctx->type.SetConstantB( VALUE_OF_BOOLEAN_TRUE );

            asBYTE v = 0;
            v = lctx->type.GetConstantB() - rctx->type.GetConstantB();
            if ( v != 0 )
                v = VALUE_OF_BOOLEAN_TRUE;
            else
                v = 0;
...
#else
            if ( lctx->type.GetConstantDW() != 0 )
                lctx->type.SetConstantDW( VALUE_OF_BOOLEAN_TRUE );
            if ( rctx->type.GetConstantDW() != 0 )
                rctx->type.SetConstantDW( VALUE_OF_BOOLEAN_TRUE );
...
void asCCompiler::CompileOperatorOnHandles( asCScriptNode* node, asCExprContext* lctx, asCExprContext* rctx,
                                            asCExprContext* ctx, eTokenType opToken ) {
    ...
    if ( ( opToken == ttEqual || opToken == ttNotEqual )
         && ( ( !( lctx->type.isExplicitHandle || lctx->type.IsNullConstant() )
                && !( lctx->type.dataType.GetTypeInfo()
                      && ( lctx->type.dataType.GetTypeInfo()->flags & asOBJ_IMPLICIT_HANDLE ) ) )
              || ( !( rctx->type.isExplicitHandle || rctx->type.IsNullConstant() )
                   && !( rctx->type.dataType.GetTypeInfo()
                         && ( rctx->type.dataType.GetTypeInfo()->flags & asOBJ_IMPLICIT_HANDLE ) ) ) ) ) {
    ...
    if ( ( ( lctx->type.dataType.GetTypeInfo() && ( lctx->type.dataType.GetTypeInfo()->flags & asOBJ_ASHANDLE ) )
         ...
         && ( opToken == ttEqual || opToken == ttIs || opToken == ttNotEqual || opToken == ttNotIs ) ) {
        // TODO: Should evaluate which of the two have the best match. If both have equal match, the first version should be used
        // Find the matching opEquals method
        int r = CompileOverloadedDualOperator2( node, "opEquals", lctx, rctx, true, ctx, true,
                                                asCDataType::CreatePrimitive( ttBool, false ) );
        if ( r == 0 ) {
            // Try again by switching the order of the operands
            r = CompileOverloadedDualOperator2( node, "opEquals", rctx, lctx, false, ctx, true,
                                                asCDataType::CreatePrimitive( ttBool, false ) );
        }
...
void asCCompiler::PerformFunctionCall( int funcId, asCExprContext* ctx, bool isConstructor,
                                       asCArray<asCExprContext*>* args, asCObjectType* objType, bool useVariable,
                                       int varOffset, int funcPtrVar ) {
    asCScriptFunction* descr = builder->GetFunctionDescription( funcId );
    ...
    if ( !ctx->type.isRefSafe && descr->objectType
         && ( ctx->type.dataType.IsObjectHandle() || ctx->type.dataType.SupportHandles() )
         && ( descr->returnType.IsReference()
              || ( ctx->type.dataType.GetTypeInfo()->GetFlags() & asOBJ_SCRIPT_OBJECT ) )
         && !( ctx->type.isVariable || ctx->type.isTemporary )
    ...
    if ( isConstructor ) {
        ...
        // TODO: Maybe we need two different byte codes
            ...
            //    type &obj::func(int)
            //    type &obj::func(uint)
            if ( descr->GetObjectType() && descr->returnType.IsReference() && descr->parameterTypes.GetLength() == 1
                 && ( descr->parameterTypes[0].IsIntegerType() || descr->parameterTypes[0].IsUnsignedType() )
                 && descr->parameterTypes[0].GetSizeInMemoryBytes() == 4 && !descr->parameterTypes[0].IsReference() )
...
bool asCExprValue::IsUndefinedFuncHandle() const {
    ...
    if ( isLValue )
        return false;
    if ( dataType.GetTypeInfo() == 0 )
        return false;
    if ( dataType.GetTypeInfo()->name != "$func" )
        return false;
    if ( dataType.IsFuncdef() )
        return false;
...
bool asCExprContext::IsClassMethod() const {
    if ( type.dataType.GetTypeInfo() == 0 )
        return false;
    if ( methodName == "" )
        return false;
    if ( type.dataType.GetTypeInfo() == &type.dataType.GetTypeInfo()->engine->functionBehaviours )
        return false;
    if ( isAnonymousInitList )
        return false;
    return true;
}
