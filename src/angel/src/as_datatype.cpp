...
bool asCDataType::IsValid() const {
    if ( tokenType == ttUnrecognizedToken && !isObjectHandle )
        return false;
...
bool asCDataType::IsNullHandle() const {
    if ( tokenType == ttUnrecognizedToken && typeInfo == 0 && isObjectHandle )
        return true;
...
bool asCDataType::SupportHandles() const {
    if ( typeInfo && ( typeInfo->flags & ( asOBJ_REF | asOBJ_ASHANDLE | asOBJ_FUNCDEF ) )
         && !( typeInfo->flags & asOBJ_NOHANDLE ) && !isObjectHandle )
        return true;
...
bool asCDataType::CanBeCopied() const {
    if ( IsPrimitive() )
        return true; // All primitives can be copied
    if ( typeInfo->flags & asOBJ_POD )
        return true; // Plain-old-data structures can always be copied
    if ( !CanBeInstantiated() )
        return false; // It must be possible to instantiate the type

    // It must have a default constructor or factory and the opAssign
    // Alternatively it must have the copy constructor
    asCObjectType* ot = CastToObjectType( typeInfo );
    if ( ot
         && ( ( ( ot->beh.construct != 0 || ot->beh.factory != 0 ) && ot->beh.copy != 0 )
...
bool asCDataType::operator==( const asCDataType& dt ) const {
    if ( !IsEqualExceptRefAndConst( dt ) )
        return false;
    if ( isReference != dt.isReference )
        return false;
    if ( isReadOnly != dt.isReadOnly )
        return false;
    if ( isConstHandle != dt.isConstHandle )
        return false;
...
bool asCDataType::IsEqualExceptRefAndConst( const asCDataType& dt ) const {
    // Check base type
    if ( tokenType != dt.tokenType )
        return false;
    if ( typeInfo != dt.typeInfo )
        return false;
    if ( isObjectHandle != dt.isObjectHandle )
        return false;
    if ( isObjectHandle )
        if ( isReadOnly != dt.isReadOnly )
            return false;
...
bool asCDataType::IsPrimitive() const {
    if ( IsEnumType() )
        return true; // Enumerations are primitives
    if ( typeInfo )
        return false; // A registered object is never a primitive neither is a pointer nor an array
    if ( tokenType == ttUnrecognizedToken )
        return false; // Null handle doesn't have a typeInfo, but it is not a primitive
    return true;
}
...
bool asCDataType::IsUnsignedType() const {
    if ( tokenType == ttUInt || tokenType == ttUInt8 || tokenType == ttUInt16 || tokenType == ttUInt64 )
        return true;
...
int asCDataType::GetSizeInMemoryBytes() const {
    ...
    if ( tokenType == ttDouble || tokenType == ttInt64 || tokenType == ttUInt64 )
        return 8;
...
int asCDataType::GetSizeInMemoryDWords() const {
    ...
    if ( s & 0x3 )
        s += 4 - ( s & 0x3 ); // Pad the size to 4 bytes
