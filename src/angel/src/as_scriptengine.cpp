...
asCScriptEngine::asCScriptEngine() {
    ...
    // Engine properties
    {
        ...
        ep.propertyAccessorMode
            = 3; // 0 = disable, 1 = app registered only, 2 = app and script created, 3 = flag with 'property'
        ...
        ep.alterSyntaxNamedArgs
            = 0; // 0 = no alternate syntax, 1 = accept alternate syntax but warn, 2 = accept without warning
        ...
        ep.genericCallMode
            = 1; // 0 = old (pre 2.33.0) behavior where generic ignored auto handles, 1 = treat handles like in native call
...
asCScriptEngine::~asCScriptEngine() {
    ...
    if ( !shuttingDown ) // If ShutDown hasn't been called yet do it now
    {
        AddRef();
        ShutDownAndRelease();
    }

    if ( defaultArrayObjectType ) // Unravel the registered interface
    {
...
// interface
int asCScriptEngine::SetContextCallbacks( asREQUESTCONTEXTFUNC_t requestCtx, asRETURNCONTEXTFUNC_t returnCtx,
                                          void* param ) {
...
asIScriptContext* asCScriptEngine::RequestContext() {
    if ( requestCtxFunc ) {
        asASSERT( returnCtxFunc ); // The return callback must also exist

        asIScriptContext* ctx = requestCtxFunc( this, ctxCallbackParam );
        return ctx;
    }
    return CreateContext(); // As fallback we create a new context
}
...
asCModule* asCScriptEngine::FindNewOwnerForSharedFunc( asCScriptFunction* in_func, asCModule* in_mod ) {
    ...
    if ( ( in_func->objectType && in_func->objectType->module && in_func->objectType->module != in_func->module )
         || ( in_func->IsFactory() && in_func->returnType.GetTypeInfo()->module
              && in_func->returnType.GetTypeInfo()->module != in_func->module ) ) {
...
void asCScriptEngine::ReturnContext( asIScriptContext* ctx ) {
    ...
    if ( ctx )
        ctx->Release(); // As fallback we just release the context
}
...
{
    if( nameSpace == 0 ) return ConfigError(asINVALID_ARG, "SetDefaultNamespace", nameSpace, 0);
    ...
    if( ns != "" )
    {
        ...
        for( ; pos < ns.GetLength(); pos += len)
        {
            ...
            if (expectIdentifier) AddNameSpace(ns.SubString(0, pos + len).AddressOf());

            expectIdentifier = !expectIdentifier;
        }
        // If the namespace ends with :: then strip it off
        if( t == ttScope ) ns.SetLength(ns.GetLength()-2);
    }
...
int asCScriptEngine::SetMessageCallback( const asSFuncPtr& callback, void* obj, asDWORD callConv ) {
    ...
    msgCallbackObj = obj;
    bool isObj = false;
    if ( (unsigned) callConv == asCALL_GENERIC || (unsigned) callConv == asCALL_THISCALL_OBJFIRST
         || (unsigned) callConv == asCALL_THISCALL_OBJLAST ) {
        msgCallback = false;
...
int asCScriptEngine::CreateContext( asIScriptContext** context, bool isInternal ) {
    *context = asNEW( asCContext )( this, !isInternal );
    ...
    PrepareEngine(); // We need to make sure the engine has been prepared before any context is executed
    return 0;
}
...
int asCScriptEngine::RegisterObjectProperty( const char* obj, const char* declaration, int byteOffset,
    ...
    if ( dt.GetTypeInfo() && ( dt.GetTypeInfo()->flags & asOBJ_TEMPLATE )
         && generatedTemplateTypes.Exists( CastToObjectType( dt.GetTypeInfo() ) ) )
    ...
    return idx; // Return the index of the property to signal success
}
...
/*int asCScriptEngine::RegisterInterface(const char *name)
{
    ...
    if( r < 0 ) return ConfigError(asNAME_TAKEN, "RegisterInterface", name, 0);
    ...
    if( st == 0 ) return ConfigError(asOUT_OF_MEMORY, "RegisterInterface", name, 0);
...
/*int asCScriptEngine::RegisterInterfaceMethod( const char *intf, const char *declaration )
{
    ...
    if( r < 0 ) return ConfigError(r, "RegisterInterfaceMethod", intf, declaration);

    asCScriptFunction *func = asNEW(asCScriptFunction)(this, 0, asFUNC_INTERFACE);
    if( func == 0 ) return ConfigError(asOUT_OF_MEMORY, "RegisterInterfaceMethod", intf, declaration);
    ...
    return func->id; // Return function id as success
}*/
...
int asCScriptEngine::RegisterObjectType( const char* name, int byteSize, asDWORD flags ) {
    ...
    if ( flags & asOBJ_REF ) {
        ...
        if ( flags
             & ~( asOBJ_REF | asOBJ_GC | asOBJ_NOHANDLE | asOBJ_SCOPED | asOBJ_TEMPLATE | asOBJ_NOCOUNT
        ...
        if ( ( flags & asOBJ_NOHANDLE )
             && ( flags & ( asOBJ_GC | asOBJ_SCOPED | asOBJ_NOCOUNT | asOBJ_IMPLICIT_HANDLE ) ) )
            return ConfigError( asINVALID_ARG, "RegisterObjectType", name, 0 );
        if ( ( flags & asOBJ_SCOPED )
             && ( flags & ( asOBJ_GC | asOBJ_NOHANDLE | asOBJ_NOCOUNT | asOBJ_IMPLICIT_HANDLE ) ) )
            return ConfigError( asINVALID_ARG, "RegisterObjectType", name, 0 );
        ...
        if ( flags & asOBJ_APP_CLASS ) {
            ...
            if ( flags
                 & ( asOBJ_APP_CLASS_CONSTRUCTOR | asOBJ_APP_CLASS_DESTRUCTOR | asOBJ_APP_CLASS_ASSIGNMENT
                     | asOBJ_APP_CLASS_COPY_CONSTRUCTOR | asOBJ_APP_CLASS_ALLINTS | asOBJ_APP_CLASS_ALLFLOATS ) ) {
                return ConfigError( asINVALID_ARG, "RegisterObjectType", name, 0 );
            }
        }

        if ( flags & asOBJ_APP_PRIMITIVE ) {
            if ( flags & ( asOBJ_APP_CLASS | asOBJ_APP_FLOAT | asOBJ_APP_ARRAY ) )
                return ConfigError( asINVALID_ARG, "RegisterObjectType", name, 0 );
        } else if ( flags & asOBJ_APP_FLOAT ) {
            if ( flags & ( asOBJ_APP_CLASS | asOBJ_APP_PRIMITIVE | asOBJ_APP_ARRAY ) )
                return ConfigError( asINVALID_ARG, "RegisterObjectType", name, 0 );
        } else if ( flags & asOBJ_APP_ARRAY ) {
            if ( flags & ( asOBJ_APP_CLASS | asOBJ_APP_PRIMITIVE | asOBJ_APP_FLOAT ) )
                return ConfigError( asINVALID_ARG, "RegisterObjectType", name, 0 );
        }
    } else
        return ConfigError( asINVALID_ARG, "RegisterObjectType", name, 0 );
    ...
    if ( flags & asOBJ_TEMPLATE ) {
        ...
        if ( r < 0 || dt.GetTypeInfo()->nameSpace != defaultNamespace ) {
            ...
            if ( dt.IsReadOnly() || dt.IsReference() )
...
// interface
int asCScriptEngine::RegisterObjectBehaviour( const char* datatype, asEBehaviours behaviour, const char* decl,
                                              const asSFuncPtr& funcPointer, asDWORD callConv, void* auxiliary,
                                              int compositeOffset, bool isCompositeIndirect ) {
    if ( datatype == 0 )
        return ConfigError( asINVALID_ARG, "RegisterObjectBehaviour", datatype, decl );
    ...
    if ( type.GetTypeInfo() == 0
         || ( type.IsObjectHandle() && !( type.GetTypeInfo()->GetFlags() & asOBJ_IMPLICIT_HANDLE ) ) )
        return ConfigError( asINVALID_TYPE, "RegisterObjectBehaviour", datatype, decl );

    // Don't allow application to modify built-in types
    if ( type.GetTypeInfo() == &functionBehaviours || type.GetTypeInfo() == &scriptTypeBehaviours )
        return ConfigError( asINVALID_TYPE, "RegisterObjectBehaviour", datatype, decl );
    ...
    if ( type.GetTypeInfo() && ( type.GetTypeInfo()->flags & asOBJ_TEMPLATE )
         && generatedTemplateTypes.Exists( CastToObjectType( type.GetTypeInfo() ) ) )
        return ConfigError( asINVALID_TYPE, "RegisterObjectBehaviour", datatype, decl );

    return RegisterBehaviourToObjectType( CastToObjectType( type.GetTypeInfo() ), behaviour, decl, funcPointer,
                                          callConv, auxiliary, compositeOffset, isCompositeIndirect );
}

// internal
int asCScriptEngine::RegisterBehaviourToObjectType( asCObjectType* objectType, asEBehaviours behaviour,
                                                    const char* decl, const asSFuncPtr& funcPointer, asDWORD callConv,
                                                    void* auxiliary, int compositeOffset, bool isCompositeIndirect ) {
    asSSystemFunctionInterface internal;
    bool isMethod = !( behaviour == asBEHAVE_FACTORY || behaviour == asBEHAVE_LIST_FACTORY
    ...
    if ( objectType->flags & asOBJ_TEMPLATE ) {
        for ( asUINT n = 0; n < generatedTemplateTypes.GetLength(); n++ ) {
            ...
            if ( tmpl->name == objectType->name && tmpl->nameSpace == objectType->nameSpace
                 && !( tmpl->templateSubTypes[0].GetTypeInfo()
                       && ( tmpl->templateSubTypes[0].GetTypeInfo()->flags & asOBJ_TEMPLATE_SUBTYPE ) ) ) {
                asCString msg;
                msg.Format( TXT_TEMPLATE_s_ALREADY_GENERATED_CANT_REGISTER,
                            asCDataType::CreateType( tmpl, false ).Format( tmpl->nameSpace ).AddressOf() );
    ...
    asCBuilder bld( this, 0 );
    r = bld.ParseFunctionDeclaration( objectType, decl, &func, true, &internal.paramAutoHandles,
                                      &internal.returnAutoHandle, 0, expectListPattern ? &listPattern : 0 );
    if ( r < 0 ) {
        if ( listPattern )
    ...
    if ( behaviour == asBEHAVE_CONSTRUCT ) {
        ...
        if ( objectType->flags & asOBJ_SCRIPT_OBJECT ) {
            ...
            if ( !( func.objectType->flags & asOBJ_VALUE ) ) {
                ...
                return ConfigError( asILLEGAL_BEHAVIOUR_FOR_TYPE, "RegisterObjectBehaviour",
                                    objectType->name.AddressOf(), decl );
            }

            // The templates take a hidden parameter with the object type
            if ( ( objectType->flags & asOBJ_TEMPLATE )
                 && ( func.parameterTypes.GetLength() == 0 || !func.parameterTypes[0].IsReference() ) ) {
                WriteMessage( "", 0, 0, asMSGTYPE_ERROR, TXT_FIRST_PARAM_MUST_BE_REF_FOR_TEMPLATE_FACTORY );
                return ConfigError( asINVALID_DECLARATION, "RegisterObjectBehaviour", objectType->name.AddressOf(),
                                    decl );
            }
        ...
        if ( !( func.objectType->flags & asOBJ_VALUE ) ) {
            WriteMessage( "", 0, 0, asMSGTYPE_ERROR, TXT_ILLEGAL_BEHAVIOUR_FOR_TYPE );
            return ConfigError( asILLEGAL_BEHAVIOUR_FOR_TYPE, "RegisterObjectBehaviour", objectType->name.AddressOf(),
                                decl );
        }
        ...
        // The templates take a hidden parameter with the object type
        if ( ( !( objectType->flags & asOBJ_TEMPLATE )
               && ( func.parameterTypes.GetLength() != 1 || !func.parameterTypes[0].IsReference() ) )
             || ( ( objectType->flags & asOBJ_TEMPLATE )
                  && ( func.parameterTypes.GetLength() != 2 || !func.parameterTypes[0].IsReference()
                       || !func.parameterTypes[1].IsReference() ) ) ) {
            if ( listPattern )
                listPattern->Destroy( this );
        ...
        if ( ( func.parameterTypes.GetLength() == 0 )
            ...
            if ( behaviour == asBEHAVE_LIST_FACTORY ) {
                ...
                // and same type as this class, then this is a copy constructor.
                if ( paramType.IsObject() && paramType.IsReference() && paramType.IsReadOnly()
                     && func.inOutFlags[func.parameterTypes.GetLength() - 1] == asTM_INREF
                     && paramType.GetTypeInfo() == objectType )
        ...
        if ( !( func.objectType->flags & asOBJ_REF ) || ( func.objectType->flags & asOBJ_NOHANDLE )
             || ( func.objectType->flags & asOBJ_SCOPED ) || ( func.objectType->flags & asOBJ_NOCOUNT ) ) {
            WriteMessage( "", 0, 0, asMSGTYPE_ERROR, TXT_ILLEGAL_BEHAVIOUR_FOR_TYPE );
            return ConfigError( asILLEGAL_BEHAVIOUR_FOR_TYPE, "RegisterObjectBehaviour", objectType->name.AddressOf(),
                                decl );
        }
        ...
        if ( func.inOutFlags[0] != asTM_INREF || func.inOutFlags[1] != asTM_OUTREF
             || !func.parameterTypes[1].IsEqualExceptRef( asCDataType::CreatePrimitive( ttBool, false ) ) )
            return ConfigError( asINVALID_DECLARATION, "RegisterObjectBehaviour", objectType->name.AddressOf(), decl );

        func.id = beh->templateCallback = AddBehaviourFunction( func, internal );
    } else if ( behaviour >= asBEHAVE_FIRST_GC && behaviour <= asBEHAVE_LAST_GC ) {
        ...
        if ( ( behaviour == asBEHAVE_GETREFCOUNT || behaviour == asBEHAVE_SETGCFLAG || behaviour == asBEHAVE_GETGCFLAG )
             && func.parameterTypes.GetLength() != 0 )
            return ConfigError( asINVALID_DECLARATION, "RegisterObjectBehaviour", objectType->name.AddressOf(), decl );

        if ( ( behaviour == asBEHAVE_ENUMREFS || behaviour == asBEHAVE_RELEASEREFS )
             && func.parameterTypes.GetLength() != 1 )
            return ConfigError( asINVALID_DECLARATION, "RegisterObjectBehaviour", objectType->name.AddressOf(), decl );

        // Verify return type
        if ( behaviour == asBEHAVE_GETREFCOUNT && func.returnType != asCDataType::CreatePrimitive( ttInt, false ) )
            return ConfigError( asINVALID_DECLARATION, "RegisterObjectBehaviour", objectType->name.AddressOf(), decl );

        if ( behaviour == asBEHAVE_GETGCFLAG && func.returnType != asCDataType::CreatePrimitive( ttBool, false ) )
            return ConfigError( asINVALID_DECLARATION, "RegisterObjectBehaviour", objectType->name.AddressOf(), decl );

        if ( ( behaviour == asBEHAVE_SETGCFLAG || behaviour == asBEHAVE_ENUMREFS || behaviour == asBEHAVE_RELEASEREFS )
...
int asCScriptEngine::SetTemplateRestrictions( asCObjectType* templateType, asCScriptFunction* func, const char* caller,
                                              const char* decl ) {
        ...
        for ( asUINT n = 0; n < func->parameterTypes.GetLength(); n++ ) {
            if ( func->parameterTypes[n].GetTypeInfo() == templateType->templateSubTypes[subTypeIdx].GetTypeInfo() ) {
                if ( func->parameterTypes[n].IsObjectHandle()
                     || ( !ep.allowUnsafeReferences && func->parameterTypes[n].IsReference()
                          && func->inOutFlags[n] == asTM_INOUTREF ) )
                    templateType->acceptValueSubType = false;
...
int asCScriptEngine::RegisterGlobalProperty( const char* declaration, void* pointer ) {
    ...
    return int( idx ); // Return the index of the property to signal success
}
...
int asCScriptEngine::GetGlobalPropertyByIndex( asUINT index, const char** name, const char** nameSpace, int* typeId,
                                               bool* isConst, const char** configGroup, void** pointer,
                                               asDWORD* accessMask ) const {
    ...
    if ( typeId )
        *typeId = GetTypeIdFromDataType( prop->type );
    if ( isConst )
        *isConst = prop->type.IsReadOnly();
    if ( pointer )
        *pointer = prop->GetRegisteredAddress();
    if ( accessMask )
...
int asCScriptEngine::GetGlobalPropertyIndexByName( const char* in_name ) const {
    ...
    while ( ns ) // Find the global var id
    {
        int id = registeredGlobalProps.GetFirstIndex( ns, name );
        if ( id >= 0 )
            return id;

        ns = GetParentNameSpace( ns ); // Recursively search parent namespace
    }
...
// interface
int asCScriptEngine::RegisterObjectMethod( const char* obj, const char* declaration, const asSFuncPtr& funcPointer,
    ...
    if ( dt.GetTypeInfo() == &functionBehaviours || dt.GetTypeInfo() == &scriptTypeBehaviours
    ...
    return RegisterMethodToObjectType( CastToObjectType( dt.GetTypeInfo() ), declaration, funcPointer, callConv,
                                       auxiliary, compositeOffset, isCompositeIndirect );
}

// internal
int asCScriptEngine::RegisterMethodToObjectType( asCObjectType* objectType, const char* declaration,
                                                 const asSFuncPtr& funcPointer, asDWORD callConv, void* auxiliary,
                                                 int compositeOffset, bool isCompositeIndirect ) {
    asSSystemFunctionInterface internal;
    ...
    asCBuilder bld( this, 0 );
    r = bld.ParseFunctionDeclaration( func->objectType, declaration, func, true, &newInterface->paramAutoHandles,
                                      &newInterface->returnAutoHandle );
    if ( r < 0 ) {
    ...
    if ( func->IsProperty() && ( r = bld.ValidateVirtualProperty( func ) ) < 0 ) {
        func->funcType = asFUNC_DUMMY; // Set as dummy function before deleting
        asDELETE( func, asCScriptFunction );
        if ( r == -5 )
            return ConfigError( asNAME_TAKEN, "RegisterObjectMethod", objectType->name.AddressOf(), declaration );
        else
            return ConfigError( asINVALID_DECLARATION, "RegisterObjectMethod", objectType->name.AddressOf(),
                                declaration );
    }

    // Check against duplicate methods
    if ( func->name == "opConv" || func->name == "opImplConv" || func->name == "opCast"
        ...
        for ( asUINT n = 0; n < func->objectType->methods.GetLength(); n++ ) {
            ...
            if ( f->name == func->name && f->IsSignatureExceptNameEqual( func ) ) {
                func->funcType = asFUNC_DUMMY;
                asDELETE( func, asCScriptFunction );
        ...
        for ( asUINT n = 0; n < func->objectType->methods.GetLength(); n++ ) {
            asCScriptFunction* f = scriptFunctions[func->objectType->methods[n]];
            if ( f->name == func->name && f->IsSignatureExceptNameAndReturnTypeEqual( func ) ) {
                func->funcType = asFUNC_DUMMY;
                asDELETE( func, asCScriptFunction );
                return ConfigError( asALREADY_REGISTERED, "RegisterObjectMethod", objectType->name.AddressOf(),
                                    declaration );
    ...
    if ( func->name == "opAssign" && func->parameterTypes.GetLength() == 1 && !func->IsReadOnly()
         && ( ( objectType->flags & asOBJ_SCRIPT_OBJECT )
              || func->parameterTypes[0].IsEqualExceptRefAndConst(
                  asCDataType::CreateType( func->objectType, false ) ) ) ) {
    ...
    return func->id; // Return the function id as success
}
...
int asCScriptEngine::RegisterGlobalFunction( const char* declaration, const asSFuncPtr& funcPointer, asDWORD callConv,
    ...
    asCBuilder bld( this, 0 );
    r = bld.ParseFunctionDeclaration( 0, declaration, func, true, &newInterface->paramAutoHandles,
                                      &newInterface->returnAutoHandle, defaultNamespace );
    if ( r < 0 ) {
        // Set as dummy function before deleting
        func->funcType = asFUNC_DUMMY;
        asDELETE( func, asCScriptFunction );
...
asUINT asCScriptEngine::GetGlobalFunctionCount() const {
    return asUINT( registeredGlobalFuncs.GetSize() - 1 ); // Don't count the builtin delegate factory
}

// interface
asIScriptFunction* asCScriptEngine::GetGlobalFunctionByIndex( asUINT index ) const {
    index++; // Don't count the builtin delegate factory
...
asIScriptFunction* asCScriptEngine::GetGlobalFunctionByDecl( const char* decl ) const {
    ...
    while ( ns ) {
        ...
        for ( unsigned int n = 0; n < idxs.GetLength(); n++ ) {
            ...
            if ( funcPtr->objectType == 0 && func.returnType == funcPtr->returnType
        ...
        ns = GetParentNameSpace( ns ); // Recursively search parent namespaces
    }
    return 0;
}
        ...
        if ( type && !( type->flags & asOBJ_SCRIPT_OBJECT ) ) {
            ...
            if ( type->flags & asOBJ_GC ) {
                if ( type->flags & asOBJ_REF ) {
                    if ( type->beh.addref == 0 || type->beh.release == 0 || type->beh.gcGetRefCount == 0
                         || type->beh.gcSetFlag == 0 || type->beh.gcGetFlag == 0 || type->beh.gcEnumReferences == 0
            ...
            if ( ( type->flags & asOBJ_REF ) && !( type->flags & asOBJ_SCOPED ) && !( type->flags & asOBJ_NOHANDLE )
                 && !( type->flags & asOBJ_NOCOUNT ) ) {
                if ( type->beh.addref == 0 || type->beh.release == 0 ) {
                    infoMsg = TXT_REF_REQUIRE_ADD_REL_BEHAVIOUR;
                    missingBehaviour = true;
                }
            }
            // Verify that non-pod value types have the constructor and destructor registered
            if ( ( type->flags & asOBJ_VALUE ) && !( type->flags & asOBJ_POD ) ) {
                if ( type->beh.constructors.GetLength() == 0 || type->beh.destruct == 0 ) {
...
int asCScriptEngine::RegisterDefaultArrayType( const char* type ) {
    ...
    if ( dt.GetTypeInfo() == 0 || !( dt.GetTypeInfo()->GetFlags() & asOBJ_TEMPLATE ) )
...
int asCScriptEngine::RegisterStringFactory( const char* datatype, asIStringFactory* factory ) {
    ...
    dt.MakeReadOnly( true ); // All string literals will be treated as const
...
asCModule* asCScriptEngine::GetModule( const char* name, bool create ) {
    if ( name == 0 )
        name = ""; // Accept null as well as zero-length string
...
asCModule* asCScriptEngine::GetModuleFromFuncId( int id ) {
    if ( id < 0 )
        return 0;
    if ( id >= (int) scriptFunctions.GetLength() )
        return 0;
...
void asCScriptEngine::BuildCompleted() {
    memoryMgr.FreeUnusedMemory(); // Always free up pooled memory after a completed build
    isBuilding = false;
}
...
asCObjectType* asCScriptEngine::GetTemplateInstanceType( asCObjectType* templateType, asCArray<asCDataType>& subTypes,
    ...
    for ( n = 0; n < templateInstanceTypes.GetLength(); n++ ) {
        ...
        if ( type && type->name == templateType->name && type->nameSpace == templateType->nameSpace
    ...
    for ( n = 0; n < subTypes.GetLength(); n++ ) {
        if ( !templateType->acceptValueSubType
             && ( subTypes[n].IsPrimitive() || ( subTypes[n].GetTypeInfo()->flags & asOBJ_VALUE ) ) )
            return 0;

        if ( !templateType->acceptRefSubType
             && ( subTypes[n].IsObject() && ( subTypes[n].GetTypeInfo()->flags & asOBJ_REF ) ) )
            return 0;
    }
    ...
    if ( templateType->beh.listFactory ) {
        ...
        func->name = "$list"; // Rename the function to easily identify it in LoadByteCode
        ot->beh.listFactory = func->id;
    }
    ot->beh.addref = templateType->beh.addref;
    if ( scriptFunctions[ot->beh.addref] )
        scriptFunctions[ot->beh.addref]->AddRefInternal();
    ot->beh.release = templateType->beh.release;
    if ( scriptFunctions[ot->beh.release] )
        scriptFunctions[ot->beh.release]->AddRefInternal();
    ot->beh.destruct = templateType->beh.destruct;
    if ( scriptFunctions[ot->beh.destruct] )
        scriptFunctions[ot->beh.destruct]->AddRefInternal();
    ot->beh.copy = templateType->beh.copy;
    if ( scriptFunctions[ot->beh.copy] )
        scriptFunctions[ot->beh.copy]->AddRefInternal();
    ot->beh.gcGetRefCount = templateType->beh.gcGetRefCount;
    if ( scriptFunctions[ot->beh.gcGetRefCount] )
        scriptFunctions[ot->beh.gcGetRefCount]->AddRefInternal();
    ot->beh.gcSetFlag = templateType->beh.gcSetFlag;
    if ( scriptFunctions[ot->beh.gcSetFlag] )
        scriptFunctions[ot->beh.gcSetFlag]->AddRefInternal();
    ot->beh.gcGetFlag = templateType->beh.gcGetFlag;
    if ( scriptFunctions[ot->beh.gcGetFlag] )
        scriptFunctions[ot->beh.gcGetFlag]->AddRefInternal();
    ot->beh.gcEnumReferences = templateType->beh.gcEnumReferences;
    if ( scriptFunctions[ot->beh.gcEnumReferences] )
        scriptFunctions[ot->beh.gcEnumReferences]->AddRefInternal();
    ot->beh.gcReleaseAllReferences = templateType->beh.gcReleaseAllReferences;
    if ( scriptFunctions[ot->beh.gcReleaseAllReferences] )
        scriptFunctions[ot->beh.gcReleaseAllReferences]->AddRefInternal();
    ot->beh.getWeakRefFlag = templateType->beh.getWeakRefFlag;
    if ( scriptFunctions[ot->beh.getWeakRefFlag] )
        scriptFunctions[ot->beh.getWeakRefFlag]->AddRefInternal();
...
asCDataType asCScriptEngine::DetermineTypeForTemplate( const asCDataType& orig, asCObjectType* tmpl,
                                                       asCObjectType* ot ) {
    asCDataType dt;
    if ( orig.GetTypeInfo() && ( orig.GetTypeInfo()->flags & asOBJ_TEMPLATE_SUBTYPE ) ) {
        ...
        dt.MakeReadOnly( orig.IsReadOnly() );
                ...
                && CastToFuncdefType( orig.GetTypeInfo() )->parentClass == tmpl ) {
...
// internal
asCScriptFunction* asCScriptEngine::GenerateTemplateFactoryStub( asCObjectType* templateType, asCObjectType* ot,
                                                                 int factoryId ) {
    ...
    asUINT bcLength = asBCTypeSize[asBCInfo[asBC_OBJTYPE].type] + asBCTypeSize[asBCInfo[asBC_CALLSYS].type]
...
bool asCScriptEngine::RequireTypeReplacement( asCDataType& type, asCObjectType* templateType ) {
    if ( type.GetTypeInfo() == templateType )
        return true;
    if ( type.GetTypeInfo() && ( type.GetTypeInfo()->flags & asOBJ_TEMPLATE_SUBTYPE ) )
        return true;
    if ( type.GetTypeInfo() && ( type.GetTypeInfo()->flags & asOBJ_TEMPLATE ) ) {
    ...
    if ( type.GetTypeInfo() && ( type.GetTypeInfo()->flags & asOBJ_FUNCDEF )
         && CastToFuncdefType( type.GetTypeInfo() )->parentClass == templateType )
        return true;

    return false;
}

bool asCScriptEngine::GenerateNewTemplateFunction( asCObjectType* templateType, asCObjectType* ot,
                                                   asCScriptFunction* func, asCScriptFunction** newFunc ) {
    ...
    if ( func2->sysFuncIntf->callConv == ICC_GENERIC_FUNC || func2->sysFuncIntf->callConv == ICC_GENERIC_METHOD )
    ...
    *newFunc = func2; // Return the new function
    return true;
}

asCFuncdefType* asCScriptEngine::GenerateNewTemplateFuncdef( asCObjectType* templateType, asCObjectType* ot,
                                                             asCFuncdefType* func ) {
...
void asCScriptEngine::CallObjectMethod( void* obj, void* param, asSSystemFunctionInterface* i,
                                        asCScriptFunction* s ) const {
...
void asCScriptEngine::CallGlobalFunction( void* param1, void* param2, asSSystemFunctionInterface* i,
                                          asCScriptFunction* s ) const {
    if ( i->callConv == ICC_CDECL ) {
        void ( *f )( void*, void* ) = ( void ( * )( void*, void* ) )( i->func );
        f( param1, param2 );
...
bool asCScriptEngine::CallGlobalFunctionRetBool( void* param1, void* param2, asSSystemFunctionInterface* i,
                                                 asCScriptFunction* s ) const {
    if ( i->callConv == ICC_CDECL ) {
...
void asCScriptEngine::GetGCStatistics( asUINT* currentSize, asUINT* totalDestroyed, asUINT* totalDetected,
                                       asUINT* newObjects, asUINT* totalNewDestroyed ) const {
...
int asCScriptEngine::GetTypeIdFromDataType( const asCDataType& dtIn ) const {
    ...
    if ( typeId == -1 ) {
        ...
        if ( ot->typeId == -1 ) {
            ...
            else if ( ot->flags & asOBJ_TEMPLATE )
                typeId |= asTYPEID_TEMPLATE;
            else if ( ot->flags & asOBJ_ENUM ) {
            } // TODO: Should we have a specific bit for this?
            else
...
asCDataType asCScriptEngine::GetDataTypeFromTypeId( int typeId ) const {
    ...
    if ( typeId <= asTYPEID_DOUBLE ) {
        eTokenType type[] = { ttVoid,  ttBool,   ttInt8, ttInt16,  ttInt,   ttInt64,
                              ttUInt8, ttUInt16, ttUInt, ttUInt64, ttFloat, ttDouble };
...
// interface
int asCScriptEngine::RefCastObject( void* obj, asITypeInfo* fromType, asITypeInfo* toType, void** newPtr,
    ...
    *newPtr = 0;
    ...
    for ( asUINT n = 0; n < from->methods.GetLength(); n++ ) {
        ...
        if ( func->name == "opImplCast" || ( !useOnlyImplicitCast && func->name == "opCast" ) ) {
            if ( func->returnType.GetTypeInfo() == toType ) {
                *newPtr = CallObjectMethodRetPtr( obj, func->id );
                ...
                if ( func->sysFuncIntf->returnAutoHandle )
                    AddRefScriptObject( *newPtr, toType );
                return asSUCCESS;
            } else if ( func->returnType.GetTokenType() == ttVoid && func->parameterTypes.GetLength() == 1
    ...
    if ( ( fromType->GetFlags() & asOBJ_SCRIPT_OBJECT ) && ( toType->GetFlags() & asOBJ_SCRIPT_OBJECT ) ) {
        ...
        if ( fromType->DerivesFrom( toType ) || fromType->Implements( toType ) ) {
            *newPtr = obj;
            reinterpret_cast<asCScriptObject*>( *newPtr )->AddRef();
            return asSUCCESS;
        }
        ...
        if ( !useOnlyImplicitCast ) {
            ...
            if ( trueType->DerivesFrom( toType ) || trueType->Implements( toType ) ) {
                *newPtr = obj;
                reinterpret_cast<asCScriptObject*>( *newPtr )->AddRef();
                return asSUCCESS;
            }
    ...
    return asSUCCESS; // The cast is not available, but it is still a success
}
...
void* asCScriptEngine::CreateScriptObject( const asITypeInfo* type ) {
    ...
    asCObjectType* objType
        = CastToObjectType( const_cast<asCTypeInfo*>( reinterpret_cast<const asCTypeInfo*>( type ) ) );
    if ( objType == 0 )
        return 0;
    void* ptr = 0;
        ...
        if ( funcIndex ) {
            if ( objType->flags & asOBJ_TEMPLATE )
                CallScriptObjectMethod(
                    ptr, funcIndex ); // Templates of value types create script functions as the constructors
            else
...
int asCScriptEngine::CallScriptObjectMethod( void* obj, int funcId ) {
    ...
    if ( ctx == 0 ) {
        ctx = RequestContext(); // Request a context from the engine
        if ( ctx == 0 )
            return asERROR; // TODO: How to best report this failure?
    }
    ...
    ctx->SetObject( obj ); // Set the object
    ...
    if ( r != asEXECUTION_FINISHED ) {
        ...
        return asERROR; // TODO: How to best report the error?
    }
...
void asCScriptEngine::RemoveScriptFunction( asCScriptFunction* func ) {
    ...
    if ( func->funcType == asFUNC_IMPORTED ) {
        if ( id >= (int) importedFunctions.GetLength() )
            return;
...
int asCScriptEngine::RegisterFuncdef( const char* decl ) {
    if ( decl == 0 )
        return ConfigError( asINVALID_ARG, "RegisterFuncdef", decl, 0 );
    ...
    allRegisteredTypes.Insert( asSNameSpaceNamePair( fdt->nameSpace, fdt->name ),
                               fdt ); // constructor already set the ref count to 1
...
// internal
int asCScriptEngine::DetermineNameAndNamespace( const char* in_name, asSNameSpace* implicitNs, asCString& out_name,
                                                asSNameSpace*& out_ns ) const {
    if ( in_name == 0 )
        return asINVALID_ARG;
            ...
            while ( count-- ) {
                ...
                if ( ti && ( ti->flags & asOBJ_ENUM ) == 0 ) {
                    ...
                    if ( ti->flags & asOBJ_VALUE ) {
                        ...
                        buffer += size; // Advance the pointer in the buffer
                    } else {
                        // Align the offset to 4 bytes boundary
                        if ( asPWORD( buffer ) & 0x3 )
                            buffer += 4 - ( asPWORD( buffer ) & 0x3 );
                    ...
                    buffer += size; // Advance the buffer
                }
            }
