        ...
        if ( f ) {
            ...
            for ( n = 0; n < 256; n++ ) {
                if ( asBCInfo[n].name ) {
                    for ( int m = 0; m < 256; m++ ) {
                        if ( instrCount2[n][m] )
                            fprintf( f, "%-10.10s, %-10.10s : %.0f\n", asBCInfo[n].name, asBCInfo[m].name,
                                     instrCount2[n][m] );
                    }
                }
            }
...
void asCContext::DetachEngine() {
    ...
    do {
        asCContext::Abort(); // Abort any execution
        asCContext::Unprepare(); // Free all resources
    } while ( asCContext::IsNested() );
...
int asCContext::Unprepare() {
    ...
    if ( m_status != asEXECUTION_UNINITIALIZED && m_status != asEXECUTION_FINISHED )
        CleanStack();
    ...
    if ( m_initialFunction && m_initialFunction->objectType
         && ( m_initialFunction->objectType->flags & asOBJ_SCRIPT_OBJECT ) ) {
    ...
    if ( m_initialFunction ) {
        ...
        asASSERT( /*IsNested() ||*/ m_stackIndex > 0
...
int asCContext::checkArgumentType( asUINT arg, int size ) {
    ...
    if ( dt->IsObject() || dt->IsFuncdef() || dt->IsReference() || dt->GetSizeInMemoryBytes() != size ) {
        m_status = asEXECUTION_ERROR;
...
int asCContext::SetArgAddress( asUINT arg, void* value ) {
    ...
    *(asPWORD*) ( &m_regs.stackFramePointer[offset] ) = (asPWORD) value; // Set the value
    return 0;
}
...
void asCContext::prepareJit0( asIScriptFunction* func ) {
    ...
    m_argumentsSize
        = 0; //m_currentFunction->GetSpaceNeededForArguments() + (m_currentFunction->objectType ? AS_PTR_SIZE : 0);
    m_returnValueSize = 0;
...
int asCContext::Prepare( asIScriptFunction* func ) {
    ...
    if ( m_status == asEXECUTION_ACTIVE || m_status == asEXECUTION_SUSPENDED ) {
        asCString str;
        str.Format( TXT_FAILED_IN_FUNC_s_WITH_s_s_d, "Prepare", func->GetDeclaration( true, true ),
                    errorNames[-asCONTEXT_ACTIVE], asCONTEXT_ACTIVE );
        m_engine->WriteMessage( "", 0, 0, asMSGTYPE_ERROR, str.AddressOf() );
        return asCONTEXT_ACTIVE;
    }
    ...
    if ( m_initialFunction
         && m_initialFunction == func ) // If the same function is executed again, we can skip a lot of the setup
    {
        m_currentFunction = m_initialFunction;
        ...
        m_argumentsSize
            = m_currentFunction->GetSpaceNeededForArguments() + ( m_currentFunction->objectType ? AS_PTR_SIZE : 0 );
...
int asCContext::executeJit0( asIScriptFunction* func ) {
    ...
    //    ExecuteNextJit();
    asPWORD jitArg = asBC_PTRARG( m_regs.programPointer );
...
int asCContext::Execute() {
    ...
    if ( m_regs.programPointer == 0 ) {
        ...
        if ( m_currentFunction->funcType == asFUNC_VIRTUAL || m_currentFunction->funcType == asFUNC_INTERFACE ) {
        ...
        } else if ( m_currentFunction->funcType
                    == asFUNC_SYSTEM ) // The current function is an application registered function
        {
...
int asCContext::PushState() {
    ...
    if ( m_status != asEXECUTION_ACTIVE ) {
        return asERROR; // TODO: Write message. Wrong usage
    }
    ...
    if ( m_callStack.GetLength() >= m_callStack.GetCapacity() - 2 * CALLSTACK_FRAME_SIZE ) {
        if ( m_engine->ep.maxCallStackSize > 0
             && m_callStack.GetLength() >= m_engine->ep.maxCallStackSize * CALLSTACK_FRAME_SIZE ) {
...
bool asCContext::ReserveStackSpace( asUINT size ) {
    if ( m_stackBlocks.GetLength() == 0 ) // Make sure the first stack block is allocated
    {
    ...
    while ( m_regs.stackPointer - ( size + RESERVE_STACK ) < m_stackBlocks[m_stackIndex] ) {
        ...
        m_regs.stackPointer = m_stackBlocks[m_stackIndex] + ( m_stackBlockSize << m_stackIndex )
            - m_currentFunction->GetSpaceNeededForArguments() - ( m_currentFunction->objectType ? AS_PTR_SIZE : 0 )
...
void asCContext::ExecuteNext() {
    ...
    for ( ;; ) {
...
#else
            *( l_fp - asBC_SWORDARG0( l_bc ) )
                = ( *( l_fp - asBC_SWORDARG0( l_bc ) ) == 0 ? VALUE_OF_BOOLEAN_TRUE : 0 );
#endif
            l_bc++;
            break;
        ...
        case asBC_RET: {
            ...
            if ( m_callStack.GetLength() == 0 || m_callStack[m_callStack.GetLength() - CALLSTACK_FRAME_SIZE] == 0 ) {
                m_status = asEXECUTION_FINISHED;
                return;
            }
        ...
        case asBC_BAND:
            *( l_fp - asBC_SWORDARG0( l_bc ) )
                = *( l_fp - asBC_SWORDARG1( l_bc ) ) & *( l_fp - asBC_SWORDARG2( l_bc ) );
            l_bc += 2;
            break;
        ...
        case asBC_BSRA:
            *( l_fp - asBC_SWORDARG0( l_bc ) )
                = int( *( l_fp - asBC_SWORDARG1( l_bc ) ) ) >> *( l_fp - asBC_SWORDARG2( l_bc ) );
            l_bc += 2;
            break;
        ...
        case asBC_ADDi:
            *(int*) ( l_fp - asBC_SWORDARG0( l_bc ) )
                = *(int*) ( l_fp - asBC_SWORDARG1( l_bc ) ) + *(int*) ( l_fp - asBC_SWORDARG2( l_bc ) );
            l_bc += 2;
            break;
        ...
        case asBC_ADDf:
            *(float*) ( l_fp - asBC_SWORDARG0( l_bc ) )
                = *(float*) ( l_fp - asBC_SWORDARG1( l_bc ) ) + *(float*) ( l_fp - asBC_SWORDARG2( l_bc ) );
            l_bc += 2;
            break;
        ...
        case asBC_ADDd:
            *(double*) ( l_fp - asBC_SWORDARG0( l_bc ) )
                = *(double*) ( l_fp - asBC_SWORDARG1( l_bc ) ) + *(double*) ( l_fp - asBC_SWORDARG2( l_bc ) );
            l_bc += 2;
            break;
        ...
        case asBC_MODd: {
            ...
            *(double*) ( l_fp - asBC_SWORDARG0( l_bc ) )
                = fmod( *(double*) ( l_fp - asBC_SWORDARG1( l_bc ) ), divider );
            l_bc += 2;
        } break;
        ...
        case asBC_ADDIi:
            *(int*) ( l_fp - asBC_SWORDARG0( l_bc ) )
                = *(int*) ( l_fp - asBC_SWORDARG1( l_bc ) ) + asBC_INTARG( l_bc + 1 );
            l_bc += 3;
            break;
        ...
        case asBC_ADDIf:
            *(float*) ( l_fp - asBC_SWORDARG0( l_bc ) )
                = *(float*) ( l_fp - asBC_SWORDARG1( l_bc ) ) + asBC_FLOATARG( l_bc + 1 );
            l_bc += 3;
            break;
        ...
        case asBC_fTOu64:
            *(asQWORD*) ( l_fp - asBC_SWORDARG0( l_bc ) )
                = asQWORD( asINT64( *(float*) ( l_fp - asBC_SWORDARG1( l_bc ) ) ) );
            l_bc += 2;
            break;

        case asBC_dTOu64:
            *(asQWORD*) ( l_fp - asBC_SWORDARG0( l_bc ) )
                = asQWORD( asINT64( *(double*) ( l_fp - asBC_SWORDARG0( l_bc ) ) ) );
            l_bc++;
            break;
        ...
        case asBC_ADDi64:
            *(asQWORD*) ( l_fp - asBC_SWORDARG0( l_bc ) )
                = *(asQWORD*) ( l_fp - asBC_SWORDARG1( l_bc ) ) + *(asQWORD*) ( l_fp - asBC_SWORDARG2( l_bc ) );
            l_bc += 2;
            break;
        ...
        case asBC_BSRL64:
            *(asQWORD*) ( l_fp - asBC_SWORDARG0( l_bc ) )
                = *(asQWORD*) ( l_fp - asBC_SWORDARG1( l_bc ) ) >> *( l_fp - asBC_SWORDARG2( l_bc ) );
            l_bc += 2;
            break;

        case asBC_BSRA64:
            *(asINT64*) ( l_fp - asBC_SWORDARG0( l_bc ) )
                = *(asINT64*) ( l_fp - asBC_SWORDARG1( l_bc ) ) >> *( l_fp - asBC_SWORDARG2( l_bc ) );
            l_bc += 2;
            break;
        ...
        case asBC_POWi: {
            bool isOverflow;
            *(int*) ( l_fp - asBC_SWORDARG0( l_bc ) ) = as_powi(
                *(int*) ( l_fp - asBC_SWORDARG1( l_bc ) ), *(int*) ( l_fp - asBC_SWORDARG2( l_bc ) ), isOverflow );
            if ( isOverflow ) {
                // Need to move the values back to the context
                m_regs.programPointer = l_bc;
                m_regs.stackPointer = l_sp;
        ...
        case asBC_POWu: {
            bool isOverflow;
            *(asDWORD*) ( l_fp - asBC_SWORDARG0( l_bc ) )
                = as_powu( *(asDWORD*) ( l_fp - asBC_SWORDARG1( l_bc ) ), *(asDWORD*) ( l_fp - asBC_SWORDARG2( l_bc ) ),
                           isOverflow );
            if ( isOverflow ) {
                // Need to move the values back to the context
                m_regs.programPointer = l_bc;
        ...
        case asBC_POWd: {
            double r
                = pow( *(double*) ( l_fp - asBC_SWORDARG1( l_bc ) ), *(double*) ( l_fp - asBC_SWORDARG2( l_bc ) ) );
        ...
        case asBC_POWi64: {
            bool isOverflow;
            *(asINT64*) ( l_fp - asBC_SWORDARG0( l_bc ) )
                = as_powi64( *(asINT64*) ( l_fp - asBC_SWORDARG1( l_bc ) ),
                             *(asINT64*) ( l_fp - asBC_SWORDARG2( l_bc ) ), isOverflow );
            if ( isOverflow ) {
                // Need to move the values back to the context
                m_regs.programPointer = l_bc;
                m_regs.stackPointer = l_sp;
        ...
        case asBC_POWu64: {
            bool isOverflow;
            *(asQWORD*) ( l_fp - asBC_SWORDARG0( l_bc ) )
                = as_powu64( *(asQWORD*) ( l_fp - asBC_SWORDARG1( l_bc ) ),
                             *(asQWORD*) ( l_fp - asBC_SWORDARG2( l_bc ) ), isOverflow );
            if ( isOverflow ) {
                // Need to move the values back to the context
                m_regs.programPointer = l_bc;
                m_regs.stackPointer = l_sp;
        ...
        if ( instr != asBC_JMP && instr != asBC_JMPP && ( instr < asBC_JZ || instr > asBC_JNP ) && instr != asBC_JLowZ
             && instr != asBC_JLowNZ && instr != asBC_CALL && instr != asBC_CALLBND && instr != asBC_CALLINTF
             && instr != asBC_RET && instr != asBC_ALLOC && instr != asBC_CallPtr && instr != asBC_JitEntry ) {
...
void asCContext::SetInternalException( const char* descr, bool allowCatch ) {
    ...
    if ( m_currentFunction->scriptData ) {
        m_exceptionLine = m_currentFunction->GetLineNumber(
            int( m_regs.programPointer - m_currentFunction->scriptData->byteCode.AddressOf() ),
            &m_exceptionSectionIdx );
...
void asCContext::CleanReturnObject() {
    if ( m_initialFunction && m_initialFunction->DoesReturnOnStack() && m_status == asEXECUTION_FINISHED ) {
        ...
        if ( CastToObjectType( m_initialFunction->returnType.GetTypeInfo() )->beh.destruct )
            m_engine->CallObjectMethod( GetReturnObject(),
                                        CastToObjectType( m_initialFunction->returnType.GetTypeInfo() )->beh.destruct );
    ...
    /// asASSERT( m_regs.objectType != 0 );  // Redundant, just below we check
...
bool asCContext::IsVarInScope( asUINT varIndex, asUINT stackLevel ) {
    ...
    if ( func->scriptData->variables.GetLength() <= varIndex )
        return false;
    if ( func->scriptData->variables[varIndex]->declaredAtProgramPos > pos )
        return false;
    ...
    for ( int n = 0; n < (int) func->scriptData->objVariableInfo.GetLength(); n++ ) {
        if ( func->scriptData->objVariableInfo[n].programPos >= declaredAt ) {
            ...
            for ( ; n < (int) func->scriptData->objVariableInfo.GetLength(); n++ ) {
                ...
                if ( func->scriptData->objVariableInfo[n].option == asBLOCK_BEGIN )
                    level++;
    ...
    return true; // Variable is visible
}
...
void asCContext::DetermineLiveObjects( asCArray<int>& liveObjects, asUINT stackLevel ) {
    ...
    for ( int n = 0; n < (int) func->scriptData->objVariableInfo.GetLength(); n++ ) {
        ...
        if ( func->scriptData->objVariableInfo[n].programPos > pos ) {
            ...
            for ( --n; n >= 0; n-- ) {
                ...
                case asOBJ_UNINIT: // Object was destroyed
                {
                    ...
                    for ( asUINT v = 0; v < func->scriptData->objVariablePos.GetLength(); v++ )
                        if ( func->scriptData->objVariablePos[v]
                             == func->scriptData->objVariableInfo[n].variableOffset ) {
                            var = v;
                            break;
                        }
                    liveObjects[var] -= 1;
            ...
            break; // We're done with the investigation
        }
    }
}
...
void asCContext::CleanArgsOnStack() {
    ...
    for ( asUINT n = 0; n < func->parameterTypes.GetLength(); n++ ) {
        if ( ( func->parameterTypes[n].IsObject() || func->parameterTypes[n].IsFuncdef() )
...
bool asCContext::CleanStackFrame( bool catchException ) {
                ...
                if ( *(asPWORD*) &m_regs.stackFramePointer[-pos] ) {
                    ...
                    if ( m_currentFunction->scriptData->objVariableTypes[n]->flags & asOBJ_FUNCDEF ) {
                        ...
                        asSTypeBehaviour* beh
                            = &CastToObjectType( m_currentFunction->scriptData->objVariableTypes[n] )->beh;
                        asASSERT( ( m_currentFunction->scriptData->objVariableTypes[n]->flags & asOBJ_NOCOUNT )
                                  || beh->release );
                        if ( beh->release )
                            m_engine->CallObjectMethod( (void*) *(asPWORD*) &m_regs.stackFramePointer[-pos],
                                                        beh->release );
                    } else {
                        asSTypeBehaviour* beh
                            = &CastToObjectType( m_currentFunction->scriptData->objVariableTypes[n] )->beh;
                        if ( beh->destruct )
                            ...
                            m_engine->DestroyList(
                                (asBYTE*) *(asPWORD*) &m_regs.stackFramePointer[-pos],
                                CastToObjectType( m_currentFunction->scriptData->objVariableTypes[n] ) );
    ...
    for ( asUINT n = 0; n < m_currentFunction->parameterTypes.GetLength(); n++ ) {
        if ( ( m_currentFunction->parameterTypes[n].IsObject() || m_currentFunction->parameterTypes[n].IsFuncdef() )
             && !m_currentFunction->parameterTypes[n].IsReference() ) {
            if ( *(asPWORD*) &m_regs.stackFramePointer[offset] ) {
                ...
                if ( m_currentFunction->parameterTypes[n].GetTypeInfo()->flags & asOBJ_FUNCDEF ) {
                    ...
                    asASSERT( ( m_currentFunction->parameterTypes[n].GetTypeInfo()->flags & asOBJ_NOCOUNT )
                              || beh->release );

                    if ( beh->release )
                        m_engine->CallObjectMethod( (void*) *(asPWORD*) &m_regs.stackFramePointer[offset],
                                                    beh->release );
                } else {
                    if ( beh->destruct )
                        m_engine->CallObjectMethod( (void*) *(asPWORD*) &m_regs.stackFramePointer[offset],
...
int asCContext::CallGeneric( asCScriptFunction* descr ) {
    ...
    if ( sysFunc->callConv == ICC_GENERIC_METHOD ) {
        ...
        if ( currentObject == 0 ) // Check for null pointer
        {
            SetInternalException( TXT_NULL_POINTER_ACCESS );
            return 0;
        }
        ...
        args += AS_PTR_SIZE; // Skip object pointer
    }
    ...
    if ( sysFunc->returnAutoHandle && m_engine->ep.genericCallMode == 1 && m_regs.objectRegister ) {
        ...
        m_engine->CallObjectMethod( m_regs.objectRegister,
                                    CastToObjectType( descr->returnType.GetTypeInfo() )->beh.addref );
    }
    ...
    return popSize; // Return how much should be popped from the stack
}
...
void* asCContext::GetAddressOfVar( asUINT varIndex, asUINT stackLevel ) {
    ...
    if ( ( func->scriptData->variables[varIndex]->type.IsObject()
        ...
        if ( !onHeap && pos <= 0 ) {
            ...
            for ( asUINT n = 0; n < func->parameterTypes.GetLength(); n++ ) {
                if ( stackPos == pos ) // The right argument was found. Is this a reference parameter?
                {
...
int as_powi( int base, int exponent, bool& isOverflow ) {
    if ( exponent < 0 ) {
        if ( base == 0 )
            isOverflow = true; // Divide by zero
        else
            isOverflow = false; // Result is less than 1, so it truncates to 0

        return 0;
    } else if ( exponent == 0 && base == 0 ) // Domain error
    {
        isOverflow = true;
        return 0;
    } else if ( exponent >= 31 ) {
