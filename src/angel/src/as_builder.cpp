...
template <>
void asCSymbolTable<sGlobalVariableDescription>::GetKey( const sGlobalVariableDescription* entry,
                                                         asSNameSpaceNamePair& key ) const {
...
#ifndef AS_NO_COMPILER
int asCBuilder::AddCode( const char* name, const char* code, int codeLength, int lineOffset, int sectionIdx,
                         bool makeCopy ) {
...
asCScriptCode* asCBuilder::FindOrAddCode( const char* name, const char* code, size_t length ) {
    for ( asUINT n = 0; n < scripts.GetLength(); n++ )
        if ( scripts[n]->name == name && scripts[n]->codeLength == length
             && memcmp( scripts[n]->code, code, length ) == 0 )
            return scripts[n];
...
void asCBuilder::EvaluateTemplateInstances( asUINT startIdx, bool keepSilent ) {
    ...
    for ( asUINT n = startIdx; n < engine->templateInstanceTypes.GetLength(); n++ ) {
        ...
        if ( callback
             && !engine->CallGlobalFunctionRetBool( tmpl, &dontGarbageCollect, callback->sysFuncIntf, callback ) ) {
            ...
            WriteError( tmpl->scriptSectionIdx >= 0 ? engine->scriptSectionNames[tmpl->scriptSectionIdx]->AddressOf()
                                                    : "",
                        str, tmpl->declaredAt & 0xFFFFF, ( tmpl->declaredAt >> 20 ) & 0xFFF );
        } else {
    ...
    if ( keepSilent ) // Restore message callback
    {
...
int asCBuilder::CompileGlobalVar( const char* sectionName, const char* code, int lineOffset ) {
    ...
    if ( node == 0 || node->firstChild == 0 || node->firstChild != node->lastChild
    ...
    for ( asUINT n = 0; n < functions.GetLength(); n++ ) {
        ...
        int r
            = compiler.CompileFunction( this, functions[n]->script, func->parameterNames, functions[n]->node, func, 0 );
        if ( r < 0 )
            break;
    }
...
int asCBuilder::CheckForConflictsDueToDefaultArgs( asCScriptCode* script, asCScriptNode* node, asCScriptFunction* func,
                                                   asCObjectType* objType ) {
    // TODO: Implement for global functions too
    if ( func->objectType == 0 || objType == 0 )
        return 0;
    ...
    for ( asUINT n = 0; n < funcs.GetLength(); n++ ) {
        ...
        for ( ; p < func->parameterTypes.GetLength() && p < func2->parameterTypes.GetLength(); p++ ) {
            ...
            if ( func->parameterTypes[p] != func2->parameterTypes[p] || func->inOutFlags[p] != func2->inOutFlags[p] ) {
                match = false;
                break;
            }
        }
        ...
        if ( match ) {
            if ( !( ( p >= func->parameterTypes.GetLength() && p < func2->defaultArgs.GetLength()
                      && func2->defaultArgs[p] )
                    || ( p >= func2->parameterTypes.GetLength() && p < func->defaultArgs.GetLength()
                         && func->defaultArgs[p] ) ) ) {
...
int asCBuilder::CompileFunction( const char* sectionName, const char* code, int lineOffset, asDWORD compileFlags,
    ...
    asCScriptFunction* func
        = asNEW( asCScriptFunction )( engine, compileFlags & asCOMP_ADD_TO_MODULE ? module : 0, asFUNC_SCRIPT );
    if ( func == 0 )
        return asOUT_OF_MEMORY;

    GetParsedFunctionDetails( node, scripts[0], 0, func->name, func->returnType, func->parameterNames,
                              func->parameterTypes, func->inOutFlags, func->defaultArgs, funcTraits,
                              module->m_defaultNamespace );
...
void asCBuilder::ParseScripts() {
    ...
    asUINT n = 0;
    for ( n = 0; n < scripts.GetLength(); n++ ) // Parse all the files as if they were one
    {
        asCParser* parser = asNEW( asCParser )( this );
        if ( parser != 0 ) {
            parsers.PushLast( parser );
            parser->ParseScript( scripts[n] ); // Parse the script file
        }
    }
    ...
    if ( numErrors == 0 ) {
        ...
        for ( n = 0; n < scripts.GetLength(); n++ ) // Find other global nodes
        {
            asCScriptNode* node = parsers[n]->GetScriptNode();
        ...
        for ( n = 0; n < interfaceDeclarations.GetLength(); n++ ) {
            ...
            while ( node ) {
                ...
                if ( node->nodeType == snFunction ) {
                    node->DisconnectParent();
                    RegisterScriptFunctionFromNode( node, decl->script, CastToObjectType( decl->typeInfo ), true, false,
                                                    0, decl->isExistingShared );
                } else if ( node->nodeType == snVirtualProperty ) {
                    node->DisconnectParent();
                    RegisterVirtualProperty( node, decl->script, CastToObjectType( decl->typeInfo ), true, false, 0,
                                             decl->isExistingShared );
                }
                node = next;
            }
        }

        for ( n = 0; n < classDeclarations.GetLength(); n++ ) // Register script methods found in the classes
        {
            ...
            while ( node ) {
                ...
                if ( node->nodeType == snFunction ) {
                    node->DisconnectParent();
                    RegisterScriptFunctionFromNode( node, decl->script, CastToObjectType( decl->typeInfo ), false,
                                                    false, 0, decl->isExistingShared );
                } else if ( node->nodeType == snVirtualProperty ) {
                    node->DisconnectParent();
                    RegisterVirtualProperty( node, decl->script, CastToObjectType( decl->typeInfo ), false, false, 0,
                                             decl->isExistingShared );
                }
                node = next;
            }
...
int asCBuilder::ParseDataType( const char* datatype, asCDataType* result, asSNameSpace* implicitNamespace,
                               bool isReturnType ) {
    Reset();
...
bool asCBuilder::DoesGlobalPropertyExist( const char* prop, asSNameSpace* ns, asCGlobalProperty** outProp,
                                          sGlobalVariableDescription** outDesc, bool* isAppProp ) {
    if ( outProp )
        *outProp = 0;
    if ( outDesc )
        *outDesc = 0;
...
asCGlobalProperty* asCBuilder::GetGlobalProperty( const char* prop, asSNameSpace* ns, bool* isCompiled,
                                                  bool* isPureConstant, asQWORD* constantValue, bool* isAppProp ) {
    if ( isCompiled )
        *isCompiled = true;
    if ( isPureConstant )
    ...
    if ( DoesGlobalPropertyExist( prop, ns, &globProp, &globDesc, isAppProp ) ) {
        ...
        if ( globDesc ) {
            ...
            if ( isCompiled )
                *isCompiled = globDesc->isCompiled;
            if ( isPureConstant )
                *isPureConstant = globDesc->isPureConstant;
            if ( constantValue )
                *constantValue = globDesc->constantValue;
        } else
#endif
            if ( isAppProp ) {
...
int asCBuilder::ParseFunctionDeclaration( asCObjectType* objType, const char* decl, asCScriptFunction* func,
                                          bool isSystemFunction, asCArray<bool>* paramAutoHandles,
                                          bool* returnAutoHandle, asSNameSpace* ns, asCScriptNode** listPattern,
                                          asCObjectType** outParentClass ) {
    asASSERT( objType || ns );
    ...
    func->returnType = CreateDataTypeFromNode( node->firstChild, &source, objType ? objType->nameSpace : ns, true,
                                               parentClass ? parentClass : objType );
    ...
    if ( autoHandle && ( !func->returnType.IsObjectHandle() || func->returnType.IsReference() ) )
        return asINVALID_DECLARATION;
    if ( returnAutoHandle )
        *returnAutoHandle = autoHandle;

    // Reference types cannot be returned by value from system functions
    if ( isSystemFunction && ( func->returnType.GetTypeInfo() && ( func->returnType.GetTypeInfo()->flags & asOBJ_REF ) )
         && !( func->returnType.IsReference() || func->returnType.IsObjectHandle() ) )
        return asINVALID_DECLARATION;
    ...
    if ( paramAutoHandles )
        paramAutoHandles->Allocate( paramCount, false );

    n = paramList->firstChild;
    asUINT index = 0;
    while ( n ) {
        asETypeModifiers inOutFlags;
        asCDataType type = CreateDataTypeFromNode( n, &source, objType ? objType->nameSpace : ns, false,
                                                   parentClass ? parentClass : objType );
        ...
        if ( isSystemFunction && ( type.GetTypeInfo() && ( type.GetTypeInfo()->flags & asOBJ_REF ) )
             && !( type.IsReference() || type.IsObjectHandle() ) )
            return asINVALID_DECLARATION;
        ...
        if ( paramAutoHandles )
            paramAutoHandles->PushLast( autoHandle );

        // Make sure that var type parameters are references
        if ( type.GetTokenType() == ttQuestion && !type.IsReference() )
            return asINVALID_DECLARATION;
...
int asCBuilder::ParseVariableDeclaration( const char* decl, asSNameSpace* implicitNamespace, asCString& outName,
                                          asSNameSpace*& outNamespace, asCDataType& outDt ) {
    Reset();
...
int asCBuilder::CheckNameConflictMember( asCTypeInfo* t, const char* name, asCScriptNode* node, asCScriptCode* code,
                                         bool isProperty, bool isVirtualProperty ) {
...
int asCBuilder::CheckNameConflict( const char* name, asCScriptNode* node, asCScriptCode* code, asSNameSpace* ns,
    ...
    if ( !isProperty || !isVirtualProperty ) {
        for ( asUINT n = 0; n < engine->registeredGlobalFuncs.GetSize(); n++ ) {
            ...
            if ( func->IsProperty() && func->nameSpace == ns && func->name.SubString( 4 ) == name ) {
                if ( code ) {
                    asCString str;
                    if ( ns->name != "" )
                        str = ns->name + "::" + name;
    ...
    asUINT n;
    for ( n = 0; n < interfaceDeclarations.GetLength(); n++ ) {
        if ( interfaceDeclarations[n]->name == name && interfaceDeclarations[n]->typeInfo->nameSpace == ns ) {
            if ( code ) {
                asCString str;
                if ( ns->name != "" )
                    str = ns->name + "::" + name;
    ...
    for ( n = 0; n < classDeclarations.GetLength(); n++ ) {
        if ( classDeclarations[n]->name == name && classDeclarations[n]->typeInfo->nameSpace == ns ) {
            if ( code ) {
                asCString str;
                if ( ns->name != "" )
                    str = ns->name + "::" + name;
    ...
    for ( n = 0; n < namedTypeDeclarations.GetLength(); n++ ) {
        if ( namedTypeDeclarations[n]->name == name && namedTypeDeclarations[n]->typeInfo->nameSpace == ns ) {
            if ( code ) {
                asCString str;
                if ( ns->name != "" )
                    str = ns->name + "::" + name;
    ...
    for ( n = 0; n < funcDefs.GetLength(); n++ ) {
        if ( funcDefs[n]->name == name && module->m_funcDefs[funcDefs[n]->idx]->nameSpace == ns ) {
            if ( code ) {
                asCString str;
                if ( ns->name != "" )
                    str = ns->name + "::" + name;
    ...
    if ( !isProperty && !isVirtualProperty ) {
        for ( n = 0; n < functions.GetLength(); n++ ) {
            ...
            if ( func && func->IsProperty() && func->objectType == 0 && func->nameSpace == ns
    ...
    if ( isProperty ) {
        for ( n = 0; n < functions.GetLength(); n++ ) {
            if ( functions[n] && functions[n]->objType == 0 && functions[n]->name == name
...
int asCBuilder::ValidateVirtualProperty( asCScriptFunction* func ) {
    ...
    if ( prefix == "set_"
         && ( func->returnType != asCDataType::CreatePrimitive( ttVoid, false ) || func->parameterTypes.GetLength() < 1
    ...
    if ( prefix == "get_" ) {
        ...
        for ( asUINT n = 0; n < engine->scriptFunctions.GetLength(); n++ ) {
            ...
            if ( func->module != setFunc->module || func->nameSpace != setFunc->nameSpace
                 || func->objectType != setFunc->objectType )
                continue;
        ...
        for ( asUINT n = 0; n < engine->scriptFunctions.GetLength(); n++ ) {
            ...
            // Is it the same scope?
            if ( func->module != getFunc->module || func->nameSpace != getFunc->nameSpace
                 || func->objectType != getFunc->objectType )
                continue;
...
sMixinClass* asCBuilder::GetMixinClass( const char* name, asSNameSpace* ns ) {
    for ( asUINT n = 0; n < mixinClasses.GetLength(); n++ )
        if ( mixinClasses[n]->name == name && mixinClasses[n]->ns == ns )
...
void asCBuilder::CompleteFuncDef( sFuncDef* funcDef ) {
    ...
    GetParsedFunctionDetails( funcDef->node, funcDef->script, fdt->parentClass, funcDef->name, func->returnType,
                              func->parameterNames, func->parameterTypes, func->inOutFlags, defaultArgs, funcTraits,
                              implicitNs );
    ...
    for ( asUINT n = 0; funcTraits.GetTrait( asTRAIT_SHARED ) && n < func->parameterTypes.GetLength(); n++ )
        if ( func->parameterTypes[n].GetTypeInfo() && !func->parameterTypes[n].GetTypeInfo()->IsShared() ) {
            if ( declaredShared ) {
                asCString s;
                s.Format( TXT_SHARED_CANNOT_USE_NON_SHARED_TYPE_s,
                          func->parameterTypes[n].GetTypeInfo()->name.AddressOf() );
                WriteError( s.AddressOf(), funcDef->script, funcDef->node );
            }
            funcTraits.SetTrait( asTRAIT_SHARED, false );
        }
    ...
    if ( func->IsShared() ) {
        for ( asUINT n = 0; n < engine->funcDefs.GetLength(); n++ ) {
            ...
            if ( fdt2->name == fdt->name && fdt2->nameSpace == fdt->nameSpace
...
int asCBuilder::RegisterGlobalVar( asCScriptNode* node, asCScriptCode* file, asSNameSpace* ns ) {
    ...
    while ( n ) {
        ...
        if ( n && ( n->nodeType == snAssignment || n->nodeType == snArgList || n->nodeType == snInitList ) ) {
...
int asCBuilder::RegisterMixinClass( asCScriptNode* node, asCScriptCode* file, asSNameSpace* ns ) {
    ...
    while ( n->tokenType == ttIdentifier
        ...
        asCString msg;
        msg.Format( TXT_MIXIN_CANNOT_BE_DECLARED_AS_s,
                    asCString( &file->code[n->tokenPos], n->tokenLength ).AddressOf() );
...
int asCBuilder::RegisterClass( asCScriptNode* node, asCScriptCode* file, asSNameSpace* ns ) {
    ...
    while ( n->tokenType == ttIdentifier ) {
        if ( file->TokenEquals( n->tokenPos, n->tokenLength, FINAL_TOKEN ) ) {
            if ( isAbstract )
                ...
                if ( isFinal ) {
                    asCString msg;
                    msg.Format( TXT_ATTR_s_INFORMED_MULTIPLE_TIMES,
                                asCString( &file->code[n->tokenPos], n->tokenLength ).AddressOf() );
            ...
            break; // This is the name of the class
    ...
    if ( isShared ) {
        for ( asUINT i = 0; i < engine->sharedScriptTypes.GetLength(); i++ ) {
            ...
            if ( st && st->IsShared() && st->name == name && st->nameSpace == ns && !st->IsInterface() ) {
        ...
        for ( ; it; it++ ) {
            ...
            if ( gvar->isCompiled ) {
                ...
                if ( initFunc && initFunc->scriptData->byteCode.GetLength() > 2 ) {
                    ...
                    initFunc->scriptData->scriptSectionIdx
                        = engine->GetScriptSectionNameIndex( gvar->script->name.AddressOf() );
                    if ( gvar->declaredAtNode ) {
                        int row, col;
                        gvar->script->ConvertPosToRowCol( gvar->declaredAtNode->tokenPos, &row, &col );
...
int asCBuilder::GetNamespaceAndNameFromNode( asCScriptNode* n, asCScriptCode* script, asSNameSpace* implicitNs,
                                             asSNameSpace*& outNs, asCString& outName ) {
...
void asCBuilder::CompileInterfaces() {
    ...
    for ( n = 0; n < interfaceDeclarations.GetLength(); n++ ) {
        ...
        if ( intfType->interfaces.GetLength() == 0 )
            continue;
...
void asCBuilder::DetermineTypeRelations() {
    ...
    for ( asUINT n = 0; n < classDeclarations.GetLength(); n++ ) {
        ...
        while ( node && node->nodeType == snIdentifier ) {
            ...
            } else if ( !( objType->flags & asOBJ_SCRIPT_OBJECT ) || ( objType->flags & asOBJ_NOINHERIT ) ) {
...
void asCBuilder::CompileClasses( asUINT numTempl ) {
    ...
    for ( n = 0; n < classDeclarations.GetLength(); n++ ) {
        ...
        if ( !decl->isExistingShared && ot->derivedFrom ) {
            ...
            for ( asUINT p = 0; p < baseType->properties.GetLength(); p++ ) {
                asCObjectProperty* prop = AddPropertyToClass(
                    decl, baseType->properties[p]->name, baseType->properties[p]->type,
                    baseType->properties[p]->isPrivate, baseType->properties[p]->isProtected, true );
            ...
            for ( asUINT m = 0; m < baseType->methods.GetLength(); m++ ) {
                ...
                if ( !found ) {
                    ...
                    CheckForConflictsDueToDefaultArgs( decl->script, decl->node, baseType->virtualFunctionTable[m],
                                                       ot );
                }
        ...
        if ( !decl->isExistingShared ) {
            ...
            for ( asUINT m = 0; m < ot->methods.GetLength(); m++ ) {
                ...
                if ( func->funcType != asFUNC_VIRTUAL ) {
                    ...
                    ot->methods.PushLast(
                        CreateVirtualFunction( func, (int) ot->virtualFunctionTable.GetLength() - 1 ) );
                    m--;
                }
            }
            ...
            while ( nd ) {
                ...
                if ( !decl->isExistingShared ) {
                    ...
                    for ( asUINT p = 0; p < ot->properties.GetLength(); p++ ) {
                        ...
                        if ( prop->isPrivate == isPrivate && prop->isProtected == isProtected && prop->name == name
    ...
    for ( n = 0; n < classDeclarations.GetLength(); n++ ) {
        ...
        for ( asUINT k = 0; k < ot->methods.GetLength(); k++ )
            overrideValidations.PushLast(
                !static_cast<asCScriptFunction*>( ot->GetMethodByIndex( k, false ) )->IsOverride() );
    ...
    while ( typesToValidate.GetLength() ) {
        ...
        numReevaluations = 0; // Reset the counter
    }
}
        ...
        if ( mixin ) {
            ...
            while ( n ) {
                if ( n->nodeType == snFunction ) {
                    ...
                    RegisterScriptFunctionFromNode( copy, mixin->script, CastToObjectType( decl->typeInfo ), false,
                                                    false, mixin->ns, false, true );
                } else if ( n->nodeType == snVirtualProperty ) {
                    // TODO: mixin: Support virtual properties too
                    WriteError( "The virtual property syntax is currently not supported for mixin classes",
                                mixin->script, n );
                    ...
                    while ( n2 ) {
                        ...
                        if ( !exists ) {
                            if ( !decl->isExistingShared ) {
                                ...
                                for ( asUINT p = 0; p < ot->properties.GetLength(); p++ ) {
                                    asCObjectProperty* prop = ot->properties[p];
                                    if ( prop->isPrivate == isPrivate && prop->isProtected == isProtected
                                         && prop->name == name && prop->type == dt ) {
                                        found = true;
                                        break;
                                    }
                                }
...
asCObjectProperty* asCBuilder::AddPropertyToClass( sClassDeclaration* decl, const asCString& name,
                                                   const asCDataType& dt, bool isPrivate, bool isProtected,
                                                   bool isInherited, asCScriptCode* file, asCScriptNode* node ) {
    if ( node ) {
...
bool asCBuilder::DoesMethodExist( asCObjectType* objType, int methodId, asUINT* methodIndex ) {
    ...
    for ( asUINT n = 0; n < objType->methods.GetLength(); n++ ) {
        ...
        if ( m->returnType != method->returnType )
            continue;
        if ( m->IsReadOnly() != method->IsReadOnly() )
            continue;
        if ( m->parameterTypes != method->parameterTypes )
            continue;
        if ( m->inOutFlags != method->inOutFlags )
            continue;
...
int asCBuilder::RegisterEnum( asCScriptNode* node, asCScriptCode* file, asSNameSpace* ns ) {
    ...
    if ( isShared ) {
        ...
        for ( asUINT n = 0; n < engine->sharedScriptTypes.GetLength(); n++ ) {
            ...
            if ( o && o->IsShared() && ( o->flags & asOBJ_ENUM ) && o->name == name && o->nameSpace == ns ) {
...
int asCBuilder::RegisterTypedef( asCScriptNode* node, asCScriptCode* file, asSNameSpace* ns ) {
    ...
    asCTypedefType* st = 0;
    if ( asSUCCESS == r ) // Create the new type
    {
...
void asCBuilder::GetParsedFunctionDetails( asCScriptNode* node, asCScriptCode* file, asCObjectType* objType,
                                           asCString& name, asCDataType& returnType,
                                           asCArray<asCString>& parameterNames, asCArray<asCDataType>& parameterTypes,
                                           asCArray<asETypeModifiers>& inOutFlags, asCArray<asCString*>& defaultArgs,
                                           asSFunctionTraits& funcTraits, asSNameSpace* implicitNamespace ) {
    node = node->firstChild;
    ...
    if ( !funcTraits.GetTrait( asTRAIT_CONSTRUCTOR ) && !funcTraits.GetTrait( asTRAIT_DESTRUCTOR ) ) {
        ...
        if ( engine->ep.disallowValueAssignForRefType && returnType.GetTypeInfo()
             && ( returnType.GetTypeInfo()->flags & asOBJ_REF ) && !( returnType.GetTypeInfo()->flags & asOBJ_SCOPED )
    ...
    while ( n ) {
        ...
        if ( engine->ep.disallowValueAssignForRefType && type.GetTypeInfo() && ( type.GetTypeInfo()->flags & asOBJ_REF )
             && !( type.GetTypeInfo()->flags & asOBJ_SCOPED ) && !type.IsReference() && !type.IsObjectHandle() ) {
        ...
        if ( n && n->nodeType == snIdentifier ) {
            ...
            n = n->next;
        } else // No name was given for the parameter
        {
...
#ifndef AS_NO_COMPILER
int asCBuilder::RegisterScriptFunctionFromNode( asCScriptNode* node, asCScriptCode* file, asCObjectType* objType,
                                                bool isInterface, bool isGlobalFunction, asSNameSpace* ns,
                                                bool isExistingShared, bool isMixin ) {
    asCString name;
    ...
    GetParsedFunctionDetails( node, file, objType, name, returnType, parameterNames, parameterTypes, inOutFlags,
                              defaultArgs, funcTraits, ns );

    return RegisterScriptFunction( node, file, objType, isInterface, isGlobalFunction, ns, isExistingShared, isMixin,
                                   name, returnType, parameterNames, parameterTypes, inOutFlags, defaultArgs,
                                   funcTraits );
}

asCScriptFunction* asCBuilder::RegisterLambda( asCScriptNode* node, asCScriptCode* file, asCScriptFunction* funcDef,
                                               const asCString& name, asSNameSpace* ns, bool isShared ) {
    ...
    int r = RegisterScriptFunction( args, file, 0, 0, true, ns, false, false, funcName, funcDef->returnType,
                                    parameterNames, funcDef->parameterTypes, funcDef->inOutFlags, defaultArgs, traits );
    if ( r < 0 )
        return 0;
...
int asCBuilder::RegisterScriptFunction( asCScriptNode* node, asCScriptCode* file, asCObjectType* objType,
                                        bool isInterface, bool isGlobalFunction, asSNameSpace* ns,
                                        bool isExistingShared, bool isMixin, asCString& name, asCDataType& returnType,
                                        asCArray<asCString>& parameterNames, asCArray<asCDataType>& parameterTypes,
                                        asCArray<asETypeModifiers>& inOutFlags, asCArray<asCString*>& defaultArgs,
                                        asSFunctionTraits funcTraits ) {
    ...
    if ( isExistingShared ) {
        ...
        if ( funcTraits.GetTrait( asTRAIT_CONSTRUCTOR ) || funcTraits.GetTrait( asTRAIT_DESTRUCTOR ) ) {
            ...
            for ( asUINT n = 0; n < objType->methods.GetLength(); n++ ) {
                ...
                if ( func->name == name
                     && func->IsSignatureExceptNameEqual( returnType, parameterTypes, inOutFlags, objType,
                                                          funcTraits.GetTrait( asTRAIT_CONST ) ) ) {
    ...
    if ( !isInterface ) {
        ...
        if ( funcTraits.GetTrait( asTRAIT_SHARED ) ) {
            ...
            for ( asUINT n = 0; n < engine->scriptFunctions.GetLength(); n++ ) {
                ...
                if ( f && f->IsShared() && f->name == name && f->nameSpace == ns && f->objectType == objType
        ...
        if ( funcTraits.GetTrait( asTRAIT_EXTERNAL )
             && !( node->tokenType == ttEndStatement || node->lastChild->tokenType == ttEndStatement ) ) {
            asCString str;
            str.Format( TXT_EXTERNAL_SHARED_s_CANNOT_REDEF, name.AddressOf() );
            WriteError( str, file, node );
        } else if ( !funcTraits.GetTrait( asTRAIT_EXTERNAL )
                    && !( node->nodeType == snStatementBlock || node->lastChild->nodeType == snStatementBlock ) ) {
            asCString str;
            str.Format( TXT_MISSING_DEFINITION_OF_s, name.AddressOf() );
            WriteError( str, file, node );
        }
    ...
    if ( objType && ( name == "opConv" || name == "opImplConv" || name == "opCast" || name == "opImplCast" )
        ...
        for ( asUINT n = 0; n < funcs.GetLength(); ++n ) {
            ...
            if ( func->IsSignatureExceptNameEqual( returnType, parameterTypes, inOutFlags, objType,
                                                   funcTraits.GetTrait( asTRAIT_CONST ) ) ) {
        ...
        for ( asUINT n = 0; n < funcs.GetLength(); ++n ) {
            asCScriptFunction* func = GetFunctionDescription( funcs[n] );
            if ( func->IsSignatureExceptNameAndReturnTypeEqual( parameterTypes, inOutFlags, objType,
                                                                funcTraits.GetTrait( asTRAIT_CONST ) ) ) {
                if ( isMixin ) {
                    // Clean up the memory, as the function will not be registered
                    if ( node )
    ...
    if ( isExistingShared ) {
        ...
        module->AddScriptFunction( file->idx, ( row & 0xFFFFF ) | ( ( col & 0xFFF ) << 20 ), funcId, name, returnType,
                                   parameterTypes, parameterNames, inOutFlags, defaultArgs, isInterface, objType,
                                   isGlobalFunction, funcTraits, ns );
    }
    ...
    if ( objType ) {
        ...
        if ( funcTraits.GetTrait( asTRAIT_CONSTRUCTOR ) ) {
            ...
            module->AddScriptFunction( file->idx, engine->scriptFunctions[funcId]->scriptData->declaredAt, factoryId,
                                       name, dt, parameterTypes, parameterNames, inOutFlags, defaultArgs, false, 0,
                                       false, funcTraits );
            ...
            if ( f->name == "opAssign" && f->parameterTypes.GetLength() == 1
                 && f->parameterTypes[0].GetTypeInfo() == f->objectType && ( f->inOutFlags[0] & asTM_INREF ) ) {
...
int asCBuilder::RegisterVirtualProperty( asCScriptNode* node, asCScriptCode* file, asCObjectType* objType,
    ...
    while ( node ) {
        ...
        if ( success ) {
            if ( !isExistingShared )
                RegisterScriptFunction( funcNode, file, objType, isInterface, isGlobalFunction, ns, false, false, name,
                                        returnType, paramNames, paramTypes, paramModifiers, defaultArgs, funcTraits );
            else {
                // Free the funcNode as it won't be used
                if ( funcNode )
                    funcNode->Destroy( engine );
                ...
                for ( asUINT n = 0; n < objType->methods.GetLength(); n++ ) {
                    asCScriptFunction* func = engine->scriptFunctions[objType->methods[n]];
                    if ( func->name == name
                         && func->IsSignatureExceptNameEqual( returnType, paramTypes, paramModifiers, objType,
                                                              funcTraits.GetTrait( asTRAIT_CONST ) ) ) {
                        found = true;
                        break;
                    }
...
int asCBuilder::RegisterImportedFunction( int importID, asCScriptNode* node, asCScriptCode* file, asSNameSpace* ns ) {
    ...
    GetParsedFunctionDetails( node->firstChild, file, 0, name, returnType, parameterNames, parameterTypes, inOutFlags,
                              defaultArgs, funcTraits, ns );
    CheckNameConflict( name.AddressOf(), node, file, ns, false, false );
    ...
    // Register the function
    module->AddImportedFunction( importID, name, returnType, parameterTypes, inOutFlags, defaultArgs, funcTraits, ns,
                                 moduleName );
...
void asCBuilder::GetObjectMethodDescriptions( const char* name, asCObjectType* objectType, asCArray<int>& methods,
                                              bool objIsConst, const asCString& scope, asCScriptNode* errNode,
    ...
    for ( asUINT n = 0; n < objectType->methods.GetLength(); n++ ) {
        ...
        if ( func->name == name && ( !objIsConst || func->IsReadOnly() )
...
asSNameSpace* asCBuilder::GetNameSpaceFromNode( asCScriptNode* node, asCScriptCode* script, asSNameSpace* implicitNs,
                                                asCScriptNode** next, asCObjectType** objType ) {
    if ( objType )
        *objType = 0;
    ...
    while ( sn ) {
        if ( sn->next->tokenType == ttScope ) {
            ...
            return 0; // Return no namespace, since this is an object type
        }
    }
    asCTypeInfo* ti = 0;
    asSNameSpace* ns
        = GetNameSpaceByString( scope, implicitNs ? implicitNs : engine->nameSpaces[0], node, script, &ti );
    if ( ti && objType )
        *objType = CastToObjectType( ti );
    return ns;
}

asSNameSpace* asCBuilder::GetNameSpaceByString( const asCString& nsName, asSNameSpace* implicitNs,
                                                asCScriptNode* errNode, asCScriptCode* script, asCTypeInfo** scopeType,
                                                bool isRequired ) {
    if ( scopeType )
        *scopeType = 0;
    ...
    if ( nsName == "::" )
        ...
        if ( ns == 0 && scopeType ) {
            ...
            asSNameSpace* nsTmp
                = searchNs == "::" ? engine->nameSpaces[0] : engine->FindNameSpace( searchNs.AddressOf() );
            asCTypeInfo* ti = 0;
            while ( !ti && nsTmp ) {
...
asCDataType asCBuilder::CreateDataTypeFromNode( asCScriptNode* node, asCScriptCode* file,
                                                asSNameSpace* implicitNamespace, bool acceptHandleForScope,
                                                asCObjectType* currentType, bool reportError, bool* isValid ) {
        ...
        while ( ( ns || parentType ) && !found ) {
            ...
            if ( currentType ) {
                ...
                if ( currentType->flags & asOBJ_TEMPLATE ) {
                    for ( asUINT subtypeIndex = 0; subtypeIndex < currentType->templateSubTypes.GetLength();
                          subtypeIndex++ ) {
                ...
                if ( !module || ( module->m_accessMask & ti->accessMask ) ) {
                    if ( asOBJ_TYPEDEF == ( ti->flags & asOBJ_TYPEDEF ) ) {
                        ...
                        if ( ti->flags & asOBJ_TEMPLATE ) {
                            ti = GetTemplateInstanceFromNode( n, file, CastToObjectType( ti ), implicitNamespace,
                                                              currentType, &n );
                            if ( ti == 0 ) {
                                if ( isValid )
                                    *isValid = false;
        ...
        if ( !found ) {
            if ( reportError ) {
                ...
                if ( origNs && origNs->name == "" )
                    ...
                    msg.Format( TXT_IDENTIFIER_s_NOT_DATA_TYPE_IN_NS_s, str.AddressOf(),
                                pt.Format( origParentType->nameSpace, false ).AddressOf() );
                }
                WriteError( msg, file, n );
            }
...
asCObjectType* asCBuilder::GetTemplateInstanceFromNode( asCScriptNode* node, asCScriptCode* file,
                                                        asCObjectType* templateType, asSNameSpace* implicitNamespace,
                                                        asCObjectType* currentType, asCScriptNode** next ) {
    ...
    while ( n && n->next && n->next->nodeType == snDataType ) {
        ...
        asCDataType subType = CreateDataTypeFromNode( n, file, implicitNamespace, false,
                                                      module ? 0 : ( currentType ? currentType : templateType ) );
    ...
    if ( subTypes.GetLength() != templateType->templateSubTypes.GetLength() ) {
        asCString msg;
        msg.Format( TXT_TMPL_s_EXPECTS_d_SUBTYPES, templateType->name.AddressOf(),
                    int( templateType->templateSubTypes.GetLength() ) );
        WriteError( msg, file, node );
...
asCDataType asCBuilder::ModifyDataTypeFromNode( const asCDataType& type, asCScriptNode* node, asCScriptCode* file,
                                                asETypeModifiers* inOutFlags, bool* autoHandle ) {
    asCDataType dt = type;

    if ( inOutFlags )
        *inOutFlags = asTM_NONE;
    ...
    if ( n && n->tokenType == ttAmp ) {
        ...
        if ( !engine->ep.allowUnsafeReferences && inOutFlags && *inOutFlags == asTM_INOUTREF
             && !( dt.GetTypeInfo() && ( dt.GetTypeInfo()->flags & asOBJ_TEMPLATE_SUBTYPE ) ) ) {
            // Verify that the base type support &inout parameter types
            if ( !dt.IsObject() || dt.IsObjectHandle()
                 || !( ( dt.GetTypeInfo()->flags & asOBJ_NOCOUNT )
                       || ( CastToObjectType( dt.GetTypeInfo() )->beh.addref
                            && CastToObjectType( dt.GetTypeInfo() )->beh.release ) ) )
...
bool asCBuilder::DoesTypeExist( const asCString& type ) {
    ...
    return knownTypes.MoveTo( 0, type ); // Check if the type is known
}
#endif
...
asCTypeInfo* asCBuilder::GetTypeFromTypesKnownByObject( const char* type, asCObjectType* currentType ) {
    ...
    for ( n = 0; found == 0 && n < currentType->methods.GetLength(); n++ ) {
        ...
        if ( func->returnType.GetTypeInfo() && func->returnType.GetTypeInfo()->name == type )
            found = func->returnType.GetTypeInfo();

        for ( asUINT f = 0; found == 0 && f < func->parameterTypes.GetLength(); f++ )
            if ( func->parameterTypes[f].GetTypeInfo() && func->parameterTypes[f].GetTypeInfo()->name == type )
...
int asCBuilder::GetEnumValue( const char* name, asCDataType& outDt, asDWORD& outValue, asSNameSpace* ns ) {
    ...
    for ( t = 0; t < engine->registeredEnums.GetLength(); t++ ) {
        ...
        if ( GetEnumValueFromType( et, name, outDt, outValue ) ) {
            if ( !found )
                ...
                return 2; // Found more than one value in different enum types
        }
    }
    ...
    return 0; // Didn't find any value
}
