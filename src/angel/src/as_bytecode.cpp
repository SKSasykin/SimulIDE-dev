...
void asCByteCode::Finalize( const asCArray<int>& tempVariableOffsets ) {
    ...
    PostProcess(); // verify the bytecode
    Optimize(); // Optimize the code
    ResolveJumpAddresses(); // Resolve jumps
    ExtractLineNumbers(); // Build line numbers buffer
}
...
void asCByteCode::AddPath( asCArray<asCByteInstruction*>& paths, asCByteInstruction* instr, int stackSize ) {
    if ( instr->marked )
        asASSERT( instr->stackSize == stackSize ); // Verify the size of the stack
    else {
...
bool asCByteCode::CanBeSwapped( asCByteInstruction* curr ) {
    ...
    if ( a->op != asBC_PshNull && a->op != asBC_PshVPtr && a->op != asBC_PSF )
        return false;

    if ( b->op != asBC_PshNull && b->op != asBC_PshVPtr && b->op != asBC_PSF )
        return false;
...
bool asCByteCode::PostponeInitOfTemp( asCByteInstruction* curr, asCByteInstruction** next ) {
    ...
    if ( ( curr->op != asBC_SetV4 && curr->op != asBC_SetV8 ) || !IsTemporary( curr->wArg[0] ) )
        return false;
...
bool asCByteCode::RemoveUnusedValue( asCByteInstruction* curr, asCByteInstruction** next ) {
    ...
    if ( curr->op != asBC_FREE && // Can't remove the FREE instruction
         ( asBCInfo[curr->op].type == asBCTYPE_wW_rW_rW_ARG || asBCInfo[curr->op].type == asBCTYPE_wW_rW_ARG
    ...
    if ( curr->op == asBC_SetV4 && curr->next ) {
        ...
        if ( ( curr->next->op == asBC_CMPi || curr->next->op == asBC_CMPf || curr->next->op == asBC_CMPu )
             && curr->wArg[0] == curr->next->wArg[1] && IsTemporary( curr->wArg[0] )
             && // The variable is temporary and never used again
        ...
        // The value is immediately used and then never again
        if ( ( curr->next->op == asBC_ADDi || curr->next->op == asBC_SUBi || curr->next->op == asBC_MULi
        ...
        if ( ( curr->next->op == asBC_ADDi || curr->next->op == asBC_MULi || curr->next->op == asBC_ADDf
        ...
        if ( curr->next->op == asBC_CpyVtoV4 && curr->wArg[0] == curr->next->wArg[1] && IsTemporary( curr->wArg[0] )
             && !IsTempVarRead( curr->next, curr->wArg[0] ) ) {
        ...
        if ( curr->next->op == asBC_PshV4 && curr->wArg[0] == curr->next->wArg[0] && IsTemporary( curr->wArg[0] )
             && !IsTempVarRead( curr->next, curr->wArg[0] ) ) {
            curr->op = asBC_PshC4;
        ...
        if ( curr->next->op == asBC_CpyVtoG4 && curr->wArg[0] == curr->next->wArg[0] && IsTemporary( curr->wArg[0] )
             && !IsTempVarRead( curr->next, curr->wArg[0] ) ) {
            curr->op = asBC_SetG4;
    ...
    if ( curr->op == asBC_CpyRtoV4 && curr->next && curr->next->op == asBC_CpyVtoR4
         && curr->wArg[0] == curr->next->wArg[0] && IsTemporary( curr->wArg[0] )
         && !IsTempVarRead( curr->next, curr->wArg[0] ) ) {
    ...
    if ( curr->op == asBC_SetV8 && curr->next && curr->next->op == asBC_PshV8 && curr->wArg[0] == curr->next->wArg[0]
         && IsTemporary( curr->wArg[0] ) && !IsTempVarRead( curr->next, curr->wArg[0] ) ) {
        curr->op = asBC_PshC8;
...
void asCByteCode::OptimizeLocally( const asCArray<int>& tempVariableOffsets ) {
    ...
    while ( instr ) {
        ...
        if ( currOp == asBC_SwapPtr ) {
            ...
            // T??, ClrHi -> T??
            if ( instr
                 && ( instr->op == asBC_TZ || instr->op == asBC_TNZ || instr->op == asBC_TS || instr->op == asBC_TNS
                      || instr->op == asBC_TP || instr->op == asBC_TNP ) ) {
            ...
            // ClrHi, JZ -> JLowZ
            if ( curr->next && curr->next->op == asBC_JZ ) {
            ...
            // ClrHi, JNZ -> JLowNZ
            if ( curr->next && curr->next->op == asBC_JNZ ) {
            ...
            if ( curr->next && ( curr->next->op == asBC_ADDSi || curr->next->op == asBC_RDSPtr ) ) {
            ...
            // PGA, CHKREF -> PGA
            // PSF, CHKREF -> PSF
            else if ( instr && ( instr->op == asBC_ADDSi || instr->op == asBC_PGA || instr->op == asBC_PSF ) ) {
            ...
            else if ( instr && instr->op == asBC_ChkRefS && instr->prev && instr->prev->op == asBC_PGA ) {
            ...
            else if ( instr && instr->op == asBC_RefCpyV && instr->prev && instr->prev->op == asBC_PshNull ) {
                DeleteInstruction( curr );
            ...
            // PshRPtr  , PopPtr -> nothing
                      ...
                      && ( instr->op == asBC_PshRPtr || instr->op == asBC_PSF || instr->op == asBC_VAR
                           || instr->op == asBC_PshVPtr || instr->op == asBC_PshNull ) ) {
            ...
            else if ( instr && instr->op == asBC_ChkRefS && instr->prev && instr->prev->op == asBC_PSF ) {
                instr = instr->prev;
                instr->op = asBC_ChkNullV;
                instr->stackInc = 0;

                DeleteInstruction( curr ); // Delete the PopPtr instruction

                DeleteInstruction( instr->next ); // Delete the ChkRefS instruction
                instr = GoForward( instr );
            }
            // PshVPtr, CHKREF, PopPtr -> ChkNullV
            else if ( instr && instr->op == asBC_CHKREF && instr->prev && instr->prev->op == asBC_PshVPtr ) {
                instr = instr->prev;
                instr->op = asBC_ChkNullV;
                instr->stackInc = 0;
            ...
            if ( instr && instr->op == asBC_ADDSi && instr->prev && instr->prev->op == asBC_PshVPtr
                 && instr->prev->wArg[0] == 0 ) {
                DeleteInstruction( instr->prev );
            ...
            else if ( instr && instr->op == asBC_ADDSi && instr->prev && instr->prev->op == asBC_PshVPtr
                      && instr->prev->wArg[0] != 0 ) {
                instr = instr->prev;
            ...
            else if ( instr && instr->op == asBC_ADDSi && instr->prev && instr->prev->op == asBC_PSF ) {
                instr = instr->prev;
        ...
        } else if ( ( ( currOp >= asBC_JZ && currOp <= asBC_JNP ) || currOp == asBC_JLowZ || currOp == asBC_JLowNZ )
                    && instr ) {
            // T**; J** +x -> J** +x
            ...
            if ( curr->next && curr->next->op == asBC_PSF && curr->next->next && curr->next->next->op == asBC_GETOBJREF
            ...
            if ( curr->next && curr->next->op == asBC_PSF && curr->next->next && curr->next->next->op == asBC_GETREF
                 && curr->next->next->wArg[0] == AS_PTR_SIZE ) {
                curr->op = asBC_PSF;
                DeleteInstruction( curr->next->next );
                instr = GoForward( curr );
    ...
    if ( last && last->op == asBC_LOADOBJ && IsTemporary( last->wArg[0] ) ) {
        ...
        if ( instr && instr->op == asBC_Block ) {
            ...
            instr = instr->prev;
            if ( instr && instr->op == asBC_PopPtr )
                instr = instr->prev;
            if ( instr && instr->op == asBC_RefCpyV && instr->wArg[0] == tempVar )
                instr = instr->prev;
    ...
    while ( instr ) {
        ...
        if ( instr ) {
            ...
            if ( currOp == asBC_PopPtr && instrOp == asBC_RET ) {
                ...
                instr = GoBack( DeleteInstruction( curr ) ); // Delete PopPtr
                ...
                // SUSPEND, SUSPEND -> SUSPEND
                    ...
                    instr = GoBack( DeleteInstruction( curr ) ); // Delete the first instruction
                }
...
bool asCByteCode::IsTempVarReadByInstr( asCByteInstruction* curr, int offset ) {
    ...
    if ( asBCInfo[curr->op].type == asBCTYPE_wW_rW_rW_ARG
        ...
        return true;
                ...
                || asBCInfo[curr->op].type == asBCTYPE_rW_QW_ARG || asBCInfo[curr->op].type == asBCTYPE_rW_W_DW_ARG
                || asBCInfo[curr->op].type == asBCTYPE_rW_DW_DW_ARG || curr->op == asBC_FREE )
              && // FREE both read and write to the variable
...
bool asCByteCode::IsInstrJmpOrLabel( asCByteInstruction* curr ) {
    if ( curr->op == asBC_JS || curr->op == asBC_JNS || curr->op == asBC_JP || curr->op == asBC_JNP
         ...
         || curr->op == asBC_JLowZ || curr->op == asBC_JLowNZ || curr->op == asBC_LABEL )
        return true;
    ...
    while ( openPaths.GetLength() ) {
        ...
        while ( curr ) {
            ...
            if ( curr->op == asBC_JMP ) {
                ...
                if ( r >= 0 && !closedPaths.Exists( curr ) && !openPaths.Exists( curr ) )
                    openPaths.PushLast( curr );

                break;
            } else if ( curr->op == asBC_JZ || curr->op == asBC_JNZ || curr->op == asBC_JS || curr->op == asBC_JNS
                        || curr->op == asBC_JP || curr->op == asBC_JNP || curr->op == asBC_JLowZ
                        || curr->op == asBC_JLowNZ ) {
                // Find the destination. If it cannot be found it is because we're doing a localized
                // optimization and the label hasn't been added to the final bytecode yet
                ...
                if ( r >= 0 && !closedPaths.Exists( dest ) && !openPaths.Exists( dest ) )
...
bool asCByteCode::IsTempRegUsed( asCByteInstruction* curr ) {
    ...
    while ( curr->next ) {
        ...
        if ( curr->op == asBC_INCi || curr->op == asBC_INCi16 || curr->op == asBC_INCi8 || curr->op == asBC_INCf
             ...
             || curr->op == asBC_JNZ || curr->op == asBC_JLowZ || curr->op == asBC_JLowNZ || curr->op == asBC_JS
             || curr->op == asBC_JNS || curr->op == asBC_JP || curr->op == asBC_JNP )
            return true;
        ...
        if ( curr->op == asBC_CALL || curr->op == asBC_PopRPtr || curr->op == asBC_CALLSYS || curr->op == asBC_CALLBND
             || curr->op == asBC_Thiscall1 || curr->op == asBC_SUSPEND || curr->op == asBC_ALLOC
             || curr->op == asBC_CpyVtoR4 || curr->op == asBC_LdGRdR4 || curr->op == asBC_LDG || curr->op == asBC_LDV
             || curr->op == asBC_TZ || curr->op == asBC_TNZ || curr->op == asBC_TS || curr->op == asBC_TNS
             || curr->op == asBC_TP || curr->op == asBC_TNP || curr->op == asBC_JS || curr->op == asBC_JNS
             || curr->op == asBC_JP || curr->op == asBC_JNP || curr->op == asBC_JMPP || curr->op == asBC_JMP
             || curr->op == asBC_JZ || curr->op == asBC_JNZ || curr->op == asBC_JLowZ || curr->op == asBC_JLowNZ
             || curr->op == asBC_CMPi || curr->op == asBC_CMPu || curr->op == asBC_CMPf || curr->op == asBC_CMPd
             || curr->op == asBC_CMPIi || curr->op == asBC_CMPIu || curr->op == asBC_CMPIf || curr->op == asBC_LABEL
             || curr->op == asBC_LoadThisR || curr->op == asBC_LoadRObjR || curr->op == asBC_LoadVObjR )
            return false;
    }
    return false;
}
...
bool asCByteCode::IsSimpleExpression() {
    ...
    asCByteInstruction* instr = first;
    while ( instr ) {
        if ( instr->op == asBC_ALLOC || instr->op == asBC_CALL || instr->op == asBC_CALLSYS || instr->op == asBC_SUSPEND
             || instr->op == asBC_LINE || instr->op == asBC_FREE || instr->op == asBC_CallPtr
             || instr->op == asBC_CALLINTF || instr->op == asBC_CALLBND || instr->op == asBC_Thiscall1 )
            return false;
    ...
    while ( instr ) {
        ...
        if ( curr->op == asBC_LINE ) {
            ...
            if ( !engine->ep.buildWithoutLineCues ) {
                ...
                DeleteInstruction( curr ); // Delete the instruction
        } else
            pos += curr->size;
    }
}
    ...
    while ( instr ) {
        if ( instr->op == asBC_Block ) {
            ...
            if ( info.option == asBLOCK_BEGIN ) {
                ...
                if ( outFunc->scriptData->objVariableInfo[outFunc->scriptData->objVariableInfo.GetLength() - 1].option
                         == asBLOCK_BEGIN
                     && outFunc->scriptData->objVariableInfo[outFunc->scriptData->objVariableInfo.GetLength() - 1]
                             .programPos
                         == pos )
            ...
            if ( outFunc->scriptData->tryCatchInfo.GetLength()
                 && outFunc->scriptData->variables[instr->wArg[0]]->type.GetTypeInfo() ) {
                asSObjectVariableInfo info;
                info.programPos = pos;
...
void asCByteCode::Call( asEBCInstr instr, int funcID, int pop ) {
    ...
    last->stackInc
        = -pop; // BC_CALL and BC_CALLBND doesn't pop the argument but when the callee returns the arguments are already popped
...
int asCByteCode::ResolveJumpAddresses() {
    ...
    asCByteInstruction* instr = first;
    while ( instr ) {
        if ( instr->op == asBC_JMP || instr->op == asBC_JZ || instr->op == asBC_JNZ || instr->op == asBC_JLowZ
             || instr->op == asBC_JLowNZ || instr->op == asBC_JS || instr->op == asBC_JNS || instr->op == asBC_JP
             || instr->op == asBC_JNP ) {
    ...
    for ( asUINT p = 0; p < paths.GetLength(); ++p ) {
        ...
        while ( instr ) {
            ...
            if ( instr->op == asBC_JMP ) {
                ...
                break;
            } else if ( instr->op == asBC_JZ || instr->op == asBC_JNZ || instr->op == asBC_JLowZ
                        || instr->op == asBC_JLowNZ || instr->op == asBC_JS || instr->op == asBC_JNS
                        || instr->op == asBC_JP || instr->op == asBC_JNP || instr->op == asBC_TryBlock ) {
