    ...
    if ( internal->func ) {
        if ( ptr.flag == 1 && callConv != asCALL_GENERIC )
            return asWRONG_CALLING_CONV;
                  ...
                  && ( callConv == asCALL_GENERIC || callConv == asCALL_THISCALL || callConv == asCALL_THISCALL_ASGLOBAL
                       || callConv == asCALL_THISCALL_OBJFIRST || callConv == asCALL_THISCALL_OBJLAST ) )
            return asWRONG_CALLING_CONV;
        else if ( ptr.flag == 3
                  && !( callConv == asCALL_THISCALL || callConv == asCALL_THISCALL_ASGLOBAL
                        || callConv == asCALL_THISCALL_OBJFIRST || callConv == asCALL_THISCALL_OBJLAST ) )
            return asWRONG_CALLING_CONV;
    }
...
#if ( defined( AS_ARM64 ) || defined( AS_ARM ) || defined( AS_MIPS ) )                                                 \
    && ( defined( __GNUC__ ) || defined( AS_PSVITA ) )
    ...
    for ( asUINT n = 0; n < func->parameterTypes.GetLength(); n++ ) {
        ...
        if ( ( dt.IsObject() || dt.IsFuncdef() ) && !dt.IsReference() ) {
            if ( dt.IsFuncdef() ) {
                ...
                if ( engine->ep.genericCallMode == 0
                ...
                // If the generic call mode is set to old behaviour then always release handles
                // else only release the handle if the function is declared with auto handles
                if ( !dt.IsObjectHandle() || engine->ep.genericCallMode == 0
                     || ( internal->paramAutoHandles.GetLength() > n && internal->paramAutoHandles[n] ) ) {
...
int PrepareSystemFunction( asCScriptFunction* func, asSSystemFunctionInterface* internal, asCScriptEngine* engine ) {
        ...
        if ( !( objType & ( asOBJ_APP_CLASS | asOBJ_APP_PRIMITIVE | asOBJ_APP_FLOAT | asOBJ_APP_ARRAY ) ) ) {
            ...
            if ( objType & COMPLEX_RETURN_MASK ) {
                ...
                if ( ( internal->callConv == ICC_THISCALL ||
                     ...
                     &&
#else
                       internal->callConv == ICC_VIRTUAL_THISCALL || internal->callConv == ICC_THISCALL_OBJFIRST
            ...
            if ( !internal->hostReturnInMemory
                 && !( func->returnType.GetTypeInfo()->flags
                       & ( asOBJ_APP_CLASS_ALLINTS | asOBJ_APP_CLASS_ALLFLOATS ) ) ) {
                engine->WriteMessage( "", 0, 0, asMSGTYPE_INFORMATION, func->GetDeclarationStr().AddressOf() );

                asCString str;
                str.Format( TXT_DONT_SUPPORT_RET_TYPE_s_BY_VAL,
                            func->returnType.Format( func->nameSpace ).AddressOf() );
                engine->WriteMessage( "", 0, 0, asMSGTYPE_ERROR, str.AddressOf() );
                engine->ConfigError( asINVALID_CONFIGURATION, 0, 0, 0 );
            }
#endif
    ...
    // Primitive types can easily be determined
        ...
        internal->hostReturnFloat
            = func->returnType.IsEqualExceptConst( asCDataType::CreatePrimitive( ttDouble, true ) );
    } else if ( func->returnType.GetSizeInMemoryDWords() == 1 ) {
        internal->hostReturnInMemory = false;
        internal->hostReturnSize = 1;
        internal->hostReturnFloat
            = func->returnType.IsEqualExceptConst( asCDataType::CreatePrimitive( ttFloat, true ) );
    } else {
        internal->hostReturnInMemory = false;
    ...
    for ( n = 0; n < func->parameterTypes.GetLength(); n++ ) {
        if ( func->parameterTypes[n].IsObject() && !func->parameterTypes[n].IsObjectHandle()
            ...
            if ( !( func->parameterTypes[n].GetTypeInfo()->flags
                    & ( asOBJ_APP_CLASS | asOBJ_APP_PRIMITIVE | asOBJ_APP_FLOAT | asOBJ_APP_ARRAY ) ) ) {
                engine->WriteMessage( "", 0, 0, asMSGTYPE_INFORMATION, func->GetDeclarationStr().AddressOf() );
...
#endif
                !( func->parameterTypes[n].GetTypeInfo()->flags
                   & ( asOBJ_APP_PRIMITIVE | asOBJ_APP_FLOAT | asOBJ_APP_CLASS_ALLINTS
    ...
    for ( n = 0; n < func->parameterTypes.GetLength(); n++ ) {
        ...
        bool needFree = false;
#ifdef COMPLEX_OBJS_PASSED_BY_REF
        if ( dt.GetTypeInfo() && dt.GetTypeInfo()->flags & COMPLEX_MASK )
            needFree = true;
#endif
        ...
        if ( dt.GetSizeInMemoryDWords() >= AS_LARGE_OBJ_MIN_SIZE )
            needFree = true;
#endif
        if ( needFree && dt.IsObject() && !dt.IsObjectHandle() && !dt.IsReference() ) {
            asSSystemFunctionInterface::SClean clean;
            clean.op = 1; // call free
            clean.ot = CastToObjectType( dt.GetTypeInfo() );
            clean.off = short( offset );
...
asQWORD CallSystemFunctionNative( asCContext* context, asCScriptFunction* descr, void* obj, asDWORD* args,
                                  void* retPointer, asQWORD& retQW2, void* secondObj );
...
int CallSystemFunction( int id, asCContext* context ) {
    ...
    if ( callConv >= ICC_THISCALL ) {
        ...
        if ( obj ) // Add the base offset for multiple inheritance
        {
            obj = (void*) ( asPWORD( obj ) + sysFunc->baseOffset );
        }
        ...
        if ( continueCheck ) {
            ...
            args += AS_PTR_SIZE; // Skip the object pointer
    ...
    if ( ( descr->returnType.IsObject() || descr->returnType.IsFuncdef() ) && !descr->returnType.IsReference() ) {
        if ( descr->returnType.IsObjectHandle() ) {
            ...
            if ( sysFunc->returnAutoHandle && context->m_regs.objectRegister ) {
                ...
                engine->CallObjectMethod( context->m_regs.objectRegister,
            ...
            if ( context->m_status == asEXECUTION_EXCEPTION && !cppException ) {
                ...
                if ( CastToObjectType( descr->returnType.GetTypeInfo() )->beh.destruct )
                    engine->CallObjectMethod( retPointer,
                                              CastToObjectType( descr->returnType.GetTypeInfo() )->beh.destruct );
            }
        }
    } else {
