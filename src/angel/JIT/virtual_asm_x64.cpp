...
enum ModBits : byte { ADR = 0, ADR8 = 1, ADR32 = 2, REG = 3 };

enum IndexScale : byte { Scale1 = 0, Scale2 = 1, Scale4 = 2, Scale8 = 3 };
...
template <> MemAddress as<float>( MemAddress addr ) {
...
template <> MemAddress as<double>( MemAddress addr ) {
...
template <> Processor& Processor::operator<<( MemAddress addr ) {
    if ( addr.absolute_address != 0 ) {
        if ( (size_t) addr.absolute_address <= INT_MAX ) {
            //Normal 32 bit address needs a none sib byte
            return *this << mod_rm( addr.other % 8, ADR, SIB ) << sib( 0x4, 0, ADDR )
                         << (int) (size_t) addr.absolute_address;
        } else {
    ...
    if ( addr.scaleFactor != 0 ) {
        ...
        if ( addr.offset == 0 && addr.code % 8 != EBP )
            ...
            return *this << mod_rm( addr.other % 8, ADR8, SIB ) << sib( addr.scaleReg % 8, scale, addr.code % 8 )
                         << (char) addr.offset;
        else
            return *this << mod_rm( addr.other % 8, ADR32, SIB ) << sib( addr.scaleReg % 8, scale, addr.code % 8 )
                         << addr.offset;
    }
...
template <> Processor& Processor::operator<<( AddrPrefix pr ) {
...
template <> Processor& Processor::operator<<( RegPrefix pr ) {
    Register& reg = pr.reg;
...
unsigned char shortJumpCodes[JumpTypeCount]
    = { 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7A, 0x7B, 0x7C, 0x7D, 0x7E, 0x7F, 0xEB };
unsigned char longJumpCodes[JumpTypeCount]
    = { 0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8A, 0x8B, 0x8C, 0x8D, 0x8E, 0x8F, 0xE9 };
...
void Processor::jump( JumpType type, volatile byte* dest ) {
    ...
    if ( offset >= CHAR_MIN && offset <= CHAR_MAX )
        ...
        *this << longJumpCodes[Jump]
              << (int) ( offset - 3 ); //Long jump is 3 bytes larger, jump is from the end of the full opcode
    else
        *this << '\x0F' << longJumpCodes[type]
              << (int) ( offset
                         - 4 ); //Conditional long jump is 4 bytes larger, jump is from the end of the full opcode
}
...
MemAddress::MemAddress( Processor& CPU, void* address )
    : cpu( CPU ), code( ESP ), absolute_address( address ), other( NONE ), offset( 0 ), bitMode( cpu.bitMode ),
      Float( false ), Signed( false ), scaleFactor( 0 ), scaleReg( NONE ) { }

MemAddress::MemAddress( Processor& CPU, RegCode Code )
    : cpu( CPU ), code( Code ), absolute_address( 0 ), other( NONE ), offset( 0 ), bitMode( cpu.bitMode ),
      Float( false ), Signed( false ), scaleFactor( 0 ), scaleReg( NONE ) { }

MemAddress::MemAddress( Processor& CPU, RegCode Code, int Offset )
    : cpu( CPU ), code( Code ), absolute_address( 0 ), other( NONE ), offset( Offset ), bitMode( cpu.bitMode ),
      Float( false ), Signed( false ), scaleFactor( 0 ), scaleReg( NONE ) { }
...
void MemAddress::operator=( unsigned int value ) {
    switch ( bitMode ) {
    case 8:
        cpu << prefix() << '\xC6' << *this << (byte) value;
        break;
    case 16:
        cpu << prefix( '\x66' ) << '\xC7' << *this << (unsigned short) value;
        break;
    case 32:
    default:
        cpu << prefix() << '\xC7' << *this << value;
        break;
    }
}
...
void MemAddress::operator|=( unsigned int value ) {
    ...
    switch ( bitMode ) {
    case 8:
        cpu << prefix() << '\x80' << *this << (byte) value;
        break;
    case 16:
        cpu << prefix( '\x66' ) << '\x81' << *this << (unsigned short) value;
        break;
    case 32:
    case 64:
        cpu << prefix() << '\x81' << *this << value;
...
void Register::swap( MemAddress address ) {
    ...
    case 8:
        cpu << address.prefix() << '\x86' << address;
        break;
    case 16:
        cpu << address.prefix( '\x66' ) << '\x87' << address;
        break;
    case 32:
    case 64:
        cpu << address.prefix() << '\x87' << address;
        break;
    }
}
...
void Register::operator--() {
    switch ( getBitMode() ) {
    case 8:
        cpu << prefix( EX_1 ) << '\xFE' << modrm( EX_1 );
        break;
    case 16:
        cpu << '\x66' << prefix( EX_1 ) << byte( '\x48' + ( code % 8 ) );
        break;
    default:
        cpu << prefix( EX_1 ) << '\xFF' << modrm( EX_1 );
        break;
    }
}
...
void Register::operator++() {
    switch ( getBitMode() ) {
    case 8:
        cpu << prefix() << '\xFE' << modrm( EX_0 );
        break;
    case 16:
        cpu << '\x66' << prefix() << byte( '\x40' + ( code % 8 ) );
        break;
    default:
        cpu << prefix() << '\xFF' << modrm( EX_0 );
        break;
    }
}
...
void Register::operator&=( unsigned long long mask ) {
    switch ( getBitMode() ) {
    case 8:
        cpu << prefix() << '\x80' << modrm( EX_4 ) << (byte) mask;
        break;
    case 16:
        cpu << prefix( '\x66' ) << '\x81' << modrm( EX_4 ) << (unsigned short) mask;
        break;
    case 32:
    case 64:
        if ( code == EAX )
...
void Register::operator&=( Register other ) {
    switch ( getBitMode() ) {
    case 8:
        cpu << prefix( other ) << '\x20' << modrm( other.code );
        break;
    case 16:
        cpu << '\x66' << prefix( other ) << '\x21' << modrm( other.code );
        break;
    case 32:
    case 64:
        cpu << prefix( other ) << '\x21' << modrm( other.code );
        break;
    }
}
...
void Register::operator|=( unsigned long long mask ) {
    switch ( getBitMode() ) {
    case 8:
        cpu << prefix() << '\x80' << modrm( EX_1 ) << (byte) mask;
        break;
    case 16:
        cpu << '\x66' << prefix() << '\x81' << modrm( EX_1 ) << (unsigned short) mask;
        break;
    case 32:
    case 64:
        if ( code == EAX )
...
void Register::operator=( unsigned long long value ) {
    switch ( getBitMode() ) {
    case 8:
        cpu << prefix() << (byte) ( '\xB0' + ( code % 8 ) ) << (byte) value;
        break;
    case 16:
        cpu << '\x66' << prefix() << (byte) ( '\xB8' + ( code % 8 ) ) << (unsigned short) value;
        break;
    case 32:
        cpu << prefix() << (byte) ( '\xB8' + ( code % 8 ) ) << (unsigned int) value;
        break;
    case 64:
        cpu << prefix() << (byte) ( '\xB8' + ( code % 8 ) ) << value;
        break;
    }
}
...
void Register::copy_expanding( MemAddress address ) {
    ...
    switch ( getBitMode( address ) ) {
    case 8:
        cpu << address.prefix() << '\x0F' << '\xBE' << address;
        break;
    case 16:
        cpu << address.prefix() << '\x0F' << '\xBF' << address;
        break;
    case 32:
        cpu << address.prefix() << '\x63' << address;
        break;
    case 64:
        *this = address;
        break;
...
void Register::copy_zeroing( Register& other ) {
    ...
    case 32:
    case 64:
        cpu << other.prefix( *this ) << '\x0F' << '\xB6' << other.modrm( code );
        break;
    }
}
...
void Register::operator=( MemAddress addr ) {
    ...
    if ( xmm() ) {
        ...
        case 8:
            cpu << addr.prefix() << '\x8A' << addr;
            break;
        case 16:
            cpu << addr.prefix( '\x66' ) << '\x8B' << addr;
            break;
        default:
            cpu << addr.prefix() << '\x8B' << addr;
            break;
        }
    }
}
...
void Register::operator==( Register other ) {
    switch ( getBitMode() ) {
    case 8:
        cpu << other.prefix( *this ) << '\x3A' << other.modrm( code );
        break;
    case 16:
        cpu << '\x66' << other.prefix( *this ) << '\x3B' << other.modrm( code );
        break;
    case 32:
    case 64:
        cpu << other.prefix( *this ) << '\x3B' << other.modrm( code );
        break;
    }
}
...
void Register::operator==( MemAddress addr ) {
    ...
    switch ( getBitMode( addr ) ) {
    case 8:
        cpu << addr.prefix() << '\x3A' << addr;
        break;
    case 16:
        cpu << addr.prefix( '\x66' ) << '\x3B' << addr;
        break;
    case 32:
    case 64:
        cpu << addr.prefix() << '\x3B' << addr;
        break;
    }
}
...
void Register::operator==( unsigned int test ) {
    switch ( getBitMode() ) {
    case 8:
        cpu << prefix( EX_7 ) << '\x80' << modrm( EX_7 ) << (byte) test;
        break;
    case 16:
        cpu << '\x66' << prefix( EX_7 ) << '\x80' << modrm( EX_7 ) << (unsigned short) test;
        break;
    case 32:
    case 64:
        cpu << prefix( EX_7 ) << '\x81' << modrm( EX_7 ) << test;
        break;
    }
}
