...
MCUMonitor::MCUMonitor( QWidget* parent, eMcu* mcu ) : QDialog( parent ), m_status( 1, 8 ), m_pc( 1, 2 ) {
    setupUi( this );
    hide();
    ...
    if ( m_watcher ) {
        if ( mcu->ramSize() ) {
            ...
            tabWidget->addTab( m_watcher, tr( "Watch" ) );
    }
    if ( mcu->ramSize() ) {
...
void MCUMonitor::updateStep() {
    ...
    if ( m_statusReg ) {
        ...
        for ( int i = 0; i < 8; i++ ) {
            int bit = status & 1;
            if ( bit )
                m_status.item( 0, 7 - i )->setBackground( QColor( 255, 150, 00 ) );
            else
                m_status.item( 0, 7 - i )->setBackground( QColor( 120, 230, 255 ) );
            status >>= 1;
        }

        if ( byteButton->isChecked() )
            pc *= m_processor->wordSize();
    ...
    if ( m_flashMonitor && m_flashMonitor->isVisible() ) {
        ...
        if ( Simulator::self()->simState() == SIM_RUNNING || Simulator::self()->simState() == SIM_PAUSED )
