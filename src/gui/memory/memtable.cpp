...
MemTable::MemTable( QWidget* parent, int dataSize, int wordBytes ) : QWidget( parent ) {
    setupUi( this );
...
void MemTable::setValue( int address, int val ) {
    ...
    if ( m_byteRatio > 1 ) {
        ...
        for ( int i = 0; i < m_byteRatio; ++i )
            setCellValue( address + i, ( val >> ( i * 8 ) ) & mask );
    } else
...
void MemTable::setData( QVector<int>* data, int wordBytes ) {
    ...
    if ( data->size() != m_dataSize || wordBytes != m_wordBytes ) {
        m_wordBytes = wordBytes;
        m_cellBytes = wordBytes;
        m_byteRatio = 1;
...
void MemTable::resizeTable( int dataSize ) {
    ...
    table->setHorizontalHeaderLabels( QStringList() << "00" << "01" << "02" << "03" << "04" << "05" << "06" << "07"
...
void MemTable::on_table_itemChanged( QTableWidgetItem* item ) {
    ...
    if ( ok ) {
        if ( m_data )
            m_data->replace( dataAddr, val );
        emit dataChanged( dataAddr, val );
    } else if ( m_data )
        setValue( dataAddr, m_data->at( dataAddr ) );
