...
RamTable::RamTable( QWidget* parent, eMcu* processor, bool cpuMonitor ) : QWidget( parent ) {
    setupUi( this );
    ...
    for ( int row = 0; row < m_numRegs; row++ ) {
        ...
        for ( int col = 0; col < 4; col++ ) {
            ...
            if ( col != 1 ) {
                ...
                it->setText( "---" );
                if ( col == 3 )
                    it->setForeground( QColor( 0x904020 ) );
                else
    ...
    table->setHorizontalHeaderLabels( QStringList()
                                      << tr( "Address" ) << tr( "Name" ) << tr( "Value" ) << tr( "Type" ) );
    ...
    connect( variables, SIGNAL( activated( QModelIndex ) ), this, SLOT( VarDoubleClick( QModelIndex ) ) );

    connect( registers, SIGNAL( activated( QModelIndex ) ), this, SLOT( RegDoubleClick( QModelIndex ) ) );

    connect( this, SIGNAL( customContextMenuRequested( const QPoint& ) ), this,
             SLOT( slotContextMenu( const QPoint& ) ), Qt::UniqueConnection );

    connect( table, SIGNAL( itemChanged( QTableWidgetItem* ) ), this, SLOT( addToWatch( QTableWidgetItem* ) ),
             Qt::UniqueConnection );
}
...
void RamTable::loadVarSet() {
    ...
    QString fileName
        = QFileDialog::getOpenFileName( this, tr( "Load VarSet" ), dir, tr( "VarSets (*.vst);;All files (*.*)" ) );
...
QStringList RamTable::getVarSet() {
    ...
    for ( int row = m_numRegs - 1; row >= 0; --row ) {
        ...
        if ( !name.isEmpty() )
            empty = false; // Find first non empty
        if ( empty )
            continue;
        varset.prepend( name );
    }
...
void RamTable::setVariables( QStringList vars ) {
    ...
    for ( QString var : vars )
        m_variableModel->appendRow( new QStandardItem( var ) );
}
...
void RamTable::setAddress( int r, QString a ) {
    table->item( r, 0 )->setText( a );
}
void RamTable::setName( int r, QString n ) {
    table->item( r, 1 )->setText( n );
}
void RamTable::setValue( int r, QString v ) {
    table->item( r, 2 )->setText( " " + v );
}
void RamTable::setType( int r, QString t ) {
    table->item( r, 3 )->setText( t );
}

void RamTable::addToWatch( QTableWidgetItem* it ) {
    if ( table->column( it ) != 1 )
        return;
    ...
    if ( name.isEmpty() ) {
        ...
        int addr = -1;
        if ( m_varsTable.contains( name ) )
            addr = m_varsTable.value( name );
        else
            addr = m_processor->getRegAddress( name );

        if ( addr == 65535 ) {
            bool ok;
            addr = name.toInt( &ok, 10 );
            if ( !ok )
                addr = name.toInt( &ok, 16 );
            if ( !ok )
                addr = -1;
        }
        if ( addr >= 0 ) {
        ...
        if ( !m_loadingVars && varType.contains( "array" ) ) {
            ...
            for ( int i = 1; i < size; i++ ) {
                ...
                if ( varName.contains( name ) )
                    table->item( _row + i, 1 )->setText( varName );
            }
        }
    }
}
...
void RamTable::updateValues() {
    ...
    for ( int _row : watchList.keys() ) {
        ...
        if ( m_cpuMonitor ) {
            if ( type == "string" )
                strVal = m_processor->cpu()->getStrReg( name );
            else {
            ...
            if ( !ok && name.startsWith( "0x" ) )
                addr = name.toInt( &ok, 16 );

            if ( ok )
                value = m_processor->getRamValue( addr ); // Address
            else // Var or Reg name
            {
                ...
                int address = -1;
                if ( m_varsTable.contains( name ) )
                    address = m_varsTable.value( name );
                else
                    address = m_processor->getRegAddress( name );
                if ( address < 0 )
                    return;
