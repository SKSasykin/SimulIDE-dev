...
void MemData::showTable( int dataSize, int wordBytes ) {
    if ( !m_memTable ) {
        ...
        m_memTable->setWindowFlags( Qt::Window | Qt::WindowTitleHint | Qt::Tool | Qt::WindowSystemMenuHint
...
bool MemData::loadData( QVector<int>* toData, bool resize, int bits ) {
    ...
    QString fileName = QFileDialog::getOpenFileName(
        nullptr, "MemData::loadData", dir, simulideTr( "MemData", "All files (*.*);;.data (*.data);;.bin (*.bin)" ) );

    if ( fileName.isEmpty() )
        return false; // User cancels loading
...
bool MemData::loadFile( QVector<int>* toData, QString file, bool resize, int bits, eMcu* eMcu ) {
    m_eMcu = eMcu;
    if ( eMcu )
        return loadHex( toData, file, resize, bits ); // MCUs file must be hex format
    ...
    bool ok = false;
    if ( ext == ".data" )
        ok = loadDat( toData, file, resize );
    else if ( ext == ".hex" || ext == ".ihx" )
        ok = loadHex( toData, file, resize, bits ); // Intel Hex Format
    else
        ok = loadBin( toData, file, resize, bits ); // Binary Format
    ...
    for ( QString line : lineList ) {
        ...
        for ( i = 8; i < 8 + nBytes * 2; i += 2 * WordSize ) {
            ...
            if ( addr > dataEnd ) {
                bool ok = false;
                if ( m_eMcu )
                    ok = m_eMcu->setCfgWord( addr, data );
                if ( !ok ) {
                    qDebug() << "    Warning: PGM End reached at Line" << QString::number( nLine )
                             << "\n    Address:" << "0x" + QString::number( addr, 16 )
                             << "> PMG End:" << "0x" + QString::number( dataEnd, 16 ).toUpper()
                             << "\n    TODO: Config word ??" << "\n";
                    return true;
                }
            } else
...
void MemData::saveData( QVector<int>* data, int bits ) {
    ...
    QString fileName
        = QFileDialog::getSaveFileName( MainWindow::self(), simulideTr( "MemData", "Save Data" ), dir,
