...
Watcher::Watcher( QWidget* parent, Watched* cpu, bool proxy ) : QWidget( parent ) {
    setupUi( this );
...
void Watcher::updateValues() {
    if ( !m_core )
        return;
    for ( ValueWidget* vw : m_values )
        vw->updateValue();
    for ( ValLabelWidget* vw : m_valLabels )
        vw->updateValue();
}
...
void Watcher::addRegister( QString name, QString type, QString unit ) {
    if ( m_typeTable.keys().contains( name ) )
        return;
...
QStringList Watcher::getVarSet() {
    ...
    for ( QString name : m_values.keys() ) {
        ...
        for ( i = 0; i < varset.size(); i++ ) {
            ...
            if ( index < m_valuesLayout->indexOf( m_values[n] ) )
                break;
        }
...
void Watcher::insertValue( QString name ) {
    if ( m_values.keys().contains( name ) )
        return;
    if ( !m_proxy && !m_header )
        addHeader();
...
void Watcher::mousePressEvent( QMouseEvent* event ) {
    if ( m_proxy && event->button() == Qt::LeftButton ) {
        m_mousePos = CircuitView::self()->mapToScene( CircuitView::self()->mapFromGlobal( event->globalPos() ) );
...
void Watcher::mouseMoveEvent( QMouseEvent* event ) {
    ...
    QPointF pos = CircuitView::self()->mapToScene( CircuitView::self()->mapFromGlobal( event->globalPos() ) );
