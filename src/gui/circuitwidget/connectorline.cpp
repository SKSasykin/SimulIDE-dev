...
ConnectorLine::ConnectorLine( int x1, int y1, int x2, int y2, Connector* connector ) : QGraphicsItem() {
...
QRectF ConnectorLine::boundingRect() const {
    ...
    if ( dx != 0 && dy != 0 )
        return QRect( 0, 0, dx, dy );
    else if ( dx > 0 )
        return QRect( -1, -2, dx + d, 4 );
    else if ( dx < 0 )
        return QRect( dx + p, -2, -dx + d, 4 );
    else if ( dy > 0 )
        return QRect( -2, -1, 4, dy + d );
    else if ( dy < 0 )
        return QRect( -2, dy + p, 4, -dy + d );
    else
...
void ConnectorLine::moveSimple( QPointF delta ) {
    ...
    if ( m_prevLine && !( m_prevLine->isSelected() ) ) {
        ...
        if ( ( isHoriz && deltaV ) || ( !isHoriz && deltaH ) )
...
bool ConnectorLine::isDiagonal() {
    return ( fabs( m_p2X - m_p1X ) > 0 && fabs( m_p2Y - m_p1Y ) > 0 );
}
...
bool ConnectorLine::connectToWire( QPoint point1 ) {
    ...
    if ( !Circuit::self()->is_constarted() )
        Circuit::self()->beginUndoStep(); // A new Connector started here
...
void ConnectorLine::mousePressEvent( QGraphicsSceneMouseEvent* event ) {
    if ( event->button() == Qt::MiddleButton ) // Move Line
    {
        ...
        if ( dy() == 0 )
            CircuitView::self()->viewport()->setCursor( Qt::SplitVCursor );
        else if ( dx() == 0 )
            CircuitView::self()->viewport()->setCursor( Qt::SplitHCursor );
        else
            CircuitView::self()->viewport()->setCursor( Qt::SizeAllCursor );
    } else if ( event->button() == Qt::LeftButton ) {
        if ( event->modifiers() == Qt::ControlModifier )
            setSelected( !isSelected() ); // Select - Deselect
        ...
        else if ( event->modifiers() & Qt::ShiftModifier ) // Move Corner
        {
            ...
            if ( Circuit::self()->is_constarted() ) // Wire started at Pin is connecting here
            {
                ...
                if ( con == this->connector() )
                    return;
                if ( con->isBus() != m_isBus ) {
                    event->ignore();
                    return;
                } // Avoid connect Bus with no-Bus
            }
            QPoint point1 = toGrid( event->scenePos() ).toPoint();

            if ( connectToWire( point1 ) )
                event->accept();
            else
                event->ignore();
        }
...
QPainterPath ConnectorLine::shape() const {
    ...
    if ( fabs( m_p2X - m_p1X ) > fabs( m_p2Y - m_p1Y ) ) {
        points << mapFromScene( QPointF( m_p1X, m_p1Y - 2 ) ) << mapFromScene( QPointF( m_p1X, m_p1Y + 2 ) )
               << mapFromScene( QPointF( m_p2X + q, m_p2Y + 2 ) ) << mapFromScene( QPointF( m_p2X + q, m_p2Y - 2 ) );
    } else {
        points << mapFromScene( QPointF( m_p1X - 2, m_p1Y ) ) << mapFromScene( QPointF( m_p1X + 2, m_p1Y ) )
               << mapFromScene( QPointF( m_p2X + 2, m_p2Y + i ) ) << mapFromScene( QPointF( m_p2X - 2, m_p2Y + i ) );
    }
...
void ConnectorLine::paint( QPainter* p, const QStyleOptionGraphicsItem*, QWidget* ) {
    ...
    if ( isSelected() )
        ...
        if ( m_pConnector->getVoltage() > 2.5 )
    ...
    if ( m_mousePos.x() < 1e6 ) {
        ...
        QToolTip::showText( m_mousePos,
                            "current = " + QString::number( current ) + " A\n"
