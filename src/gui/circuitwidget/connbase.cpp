...
ConnBase::ConnBase( QString type, QString id ) : Component( type, id ), eElement( id ) {
    m_graphical = true;
    //this->setZValue(-1 );
    ...
    addPropGroup( { tr( "Main" ),
                    { new IntProp<ConnBase>( "Size", tr( "Size" ), "_Pins", this, &ConnBase::size, &ConnBase::setSize,
                                             propNoCopy, "uint" ) },
                    groupNoCopy } );
}
ConnBase::~ConnBase() {
...
void ConnBase::registerEnode( eNode* enode, int n ) {
    if ( n < m_size ) {
        if ( m_sockPins[n]->conPin() )
            m_sockPins[n]->registerPinsW( enode, n );
    } else {
        int nS = n - m_size;
        if ( m_pin[nS]->conPin() )
            m_pin[nS]->registerPinsW( enode, nS );
    }
}
...
void ConnBase::createPins( int c ) {
    ...
    for ( int i = start; i < m_size; i++ ) {
        ...
        m_sockPins[i] = new Pin( 90, QPoint( 0, -32 + 8 + i * 8 ), m_id + "-pin" + QString::number( i + m_size ),
                                 i + m_size, this );
...
void ConnBase::setSize( int size ) {
    ...
    if ( size < m_size )
        deletePins( m_size - size );
    else if ( size > m_size )
        createPins( size - m_size );
...
void ConnBase::setHidden( bool hid, bool hidArea, bool hidLabel ) {
    ...
    for ( int i = 0; i < m_size; i++ ) {
        ...
        if ( hid )
            parentComp->addSignalPin( m_sockPins[i] );
        else
            parentComp->remSignalPin( m_sockPins[i] );
    }
}
