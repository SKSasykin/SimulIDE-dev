...
IoPin::IoPin( int angle, const QPoint pos, QString id, int index, Component* parent, pinMode_t mode )
    : Pin( angle, pos, id, index, parent ), eElement( id ) {
    m_outState = false;
    m_stateZ = false;
    m_skipStamp = false;
...
void IoPin::updateStep() {
    ...
    if ( m_stateZ )
        ...
        case undef_mode:
            ...
            break;
        case input:
            m_pinState = state ? input_high : input_low;
            break;
        case openCo: {
        ...
        case output:
        case source:
            m_pinState = state ? out_high : out_low;
            break;
        }
    }
    Pin::updateStep();
    ...
    if ( m_step == m_steps ) {
        ...
        if ( m_pinMode == openCo ) {
            ...
            if ( m_step == 0 )
                delta = 1e-5;
            if ( nextState )
                stampVolt( m_outLowV + delta * ( m_outHighV - m_outLowV ) / m_steps ); // L to H
            else
                stampVolt( m_outHighV - delta * ( m_outHighV - m_outLowV ) / m_steps ); // H to L
        }
...
double IoPin::getCurrent() {
    ...
    case output:
    case openCo:
        m_current = ( m_outVolt - volt ) * m_admit;
        break;
    default:
        break;
    }
...
void IoPin::contextMenuEvent( QGraphicsSceneContextMenuEvent* event ) {
    ...
    QObject::connect( editAction, &QAction::triggered, [=]() { setUserInverted( !m_inverted ); } );
...
QStringList IoPin::registerScript( asIScriptEngine* engine ) {
    ...
    engine->RegisterObjectMethod( "IoPin", "void setPinMode(uint m)", asMETHODPR( IoPin, setPinMode, ( uint ), void ),
                                  asCALL_THISCALL );

    memberList << "getInpState()";
    engine->RegisterObjectMethod( "IoPin", "bool getInpState()", asMETHODPR( IoPin, getInpState, (), bool ),
                                  asCALL_THISCALL );

    memberList << "void setOutState( bool state )";
    engine->RegisterObjectMethod( "IoPin", "void setOutState(bool s)", asMETHODPR( IoPin, setOutState, (bool), void ),
                                  asCALL_THISCALL );

    memberList << "setStateZ( bool state )";
    engine->RegisterObjectMethod( "IoPin", "void setStateZ( bool z )", asMETHODPR( IoPin, setStateZ, (bool), void ),
                                  asCALL_THISCALL );

    memberList << "setOutStatFast( bool state )";
    engine->RegisterObjectMethod( "IoPin", "void setOutStatFast(bool s)",
                                  asMETHODPR( IoPin, setOutStatFast, (bool), void ), asCALL_THISCALL );
    ...
    engine->RegisterObjectMethod( "IoPin", "void scheduleState( bool state, uint64 time )",
                                  asMETHODPR( IoPin, scheduleState, ( bool, uint64_t ), void ), asCALL_THISCALL );

    memberList << "getVoltage()";
    engine->RegisterObjectMethod( "IoPin", "double getVoltage()", asMETHODPR( IoPin, getVoltage, (), double ),
                                  asCALL_THISCALL );

    memberList << "setVoltage( double voltage )";
    engine->RegisterObjectMethod( "IoPin", "void setVoltage(double v)", asMETHODPR( IoPin, setVoltage, (double), void ),
                                  asCALL_THISCALL );

    memberList << "setOutHighV(double v)";
    engine->RegisterObjectMethod( "IoPin", "void setOutHighV(double v)",
                                  asMETHODPR( IoPin, setOutHighV, (double), void ), asCALL_THISCALL );

    memberList << "setImpedance( double impedance )";
    engine->RegisterObjectMethod( "IoPin", "void setImpedance( double imp )",
                                  asMETHODPR( IoPin, setImpedance, (double), void ), asCALL_THISCALL );

    memberList << "changeCallBack( eElement@ e, bool call )";
    engine->RegisterObjectMethod( "IoPin", "void changeCallBack(eElement@ p, bool s)",
                                  asMETHODPR( IoPin, changeCallBack, (eElement*, bool), void ), asCALL_THISCALL );

    memberList << "setX( double x )";
    engine->RegisterObjectMethod( "IoPin", "void setX( double X )", asMETHODPR( IoPin, setX, (double), void ),
                                  asCALL_THISCALL );

    memberList << "setY( double y )";
    engine->RegisterObjectMethod( "IoPin", "void setY( double y )", asMETHODPR( IoPin, setY, (double), void ),
                                  asCALL_THISCALL );
