...
Pin::Pin( int angle, const QPoint pos, QString id, int index, Component* parent, int length )
    : QGraphicsItem( parent ), ePin( id, index ), m_label( parent ) {
...
void Pin::connectorRemoved() {
    setConnector( NULL );
    if ( !Circuit::self()->undoRedo() )
        m_component->pinMessage( 1 ); // Used by node to remove
}
...
void Pin::registerPinsW( eNode* enode, int n ) // Called by component, calls conPin
{
    ...
    if ( !m_isBus )
        ePin::setEnode( enode );
    if ( m_conPin )
        m_conPin->registerEnode( enode, n ); // Call pin at other side of Connector
...
void Pin::registerEnode( eNode* enode, int n ) // Called by m_conPin
{
    ...
    if ( m_dataCannel )
        m_dataCannel->registerEnode( enode, n );
...
void Pin::removeConnector() {
    if ( my_connector )
        Circuit::self()->removeConnector( my_connector );
    setConnector( NULL );
}
...
Pin* Pin::connectPin( bool connect ) // Auto-Connect
{
    ...
    while ( !list.isEmpty() ) {
        ...
        if ( it->type() == 65536 + 3 ) // Pin found
        {
            ...
            if ( pin->parentItem() == this->parentItem() )
                continue;
            if ( fabs( scenePos().x() - pin->scenePos().x() ) > 3 )
                continue;
            if ( fabs( scenePos().y() - pin->scenePos().y() ) > 3 )
                continue;
            if ( m_isBus != pin->isBus() )
                continue; // Only connect Bus to Bus
            if ( pin->connector() )
                continue;
            if ( pin->unused() )
                continue;
            if ( !connect && pin->pinType() < pinSocket && pin->isVisible() && !pin->isObscuredBy( m_component ) )
                _pin = pin;
            if ( connect ) {
        ...
        } else if ( connect && ( it->type() == UserType + 2 ) ) // ConnectorLine
        {
            ...
            if ( m_isBus != line->connector()->isBus() )
                continue;
...
void Pin::isMoved() {
    if ( my_connector )
        my_connector->updateConRoute( this, scenePos() );
    else if ( this->isVisible() && !my_connector ) { // Auto-Connect
        if ( !Circuit::self()->isBusy() && ( QApplication::queryKeyboardModifiers() == Qt::ShiftModifier ) )
            connectPin( true );
    }
    setLabelPos();
}
...
void Pin::mousePressEvent( QGraphicsSceneMouseEvent* event ) {
    ...
    if ( event->button() == Qt::LeftButton ) // Start/Close Connector
    {
        if ( my_connector )
            ...
            event->accept();
            if ( Circuit::self()->is_constarted() )
                Circuit::self()->closeconnector( this, true );
            else
                Circuit::self()->newconnector( this, true );
        }
    }
...
void Pin::setLabelPos() {
    ...
    if ( m_angle == 0 ) // Right side
    {
        ...
        if ( m_Hflip == -1 )
            ...
            xlabelpos -= m_labelWidth + offset;
        if ( m_Vflip == -1 )
            ylabelpos += m_labelheight;
        else
            ylabelpos -= m_labelheight;
    } else if ( m_angle == 90 ) // Top
    {
        ...
        if ( m_Hflip == -1 )
            xlabelpos -= m_labelheight;
        else
            xlabelpos += m_labelheight;
        if ( m_Vflip == -1 )
            ylabelpos += m_labelWidth + offset;
        else
            ylabelpos += offset;
...
void Pin::paint( QPainter* p, const QStyleOptionGraphicsItem*, QWidget* ) {
    ...
    if ( m_unused )
        pen.setColor( QColor( 75, 120, 170 ) );
    else if ( m_isBus )
        pen.setColor( Qt::darkGreen );
    else if ( m_animate )
        pen.setColor( m_color[m_pinState] );

    p->setPen( pen );
    if ( m_length > 1 )
        p->drawLine( QPointF( 0, 0 ), QPointF( m_length - 0.7, 0 ) );
    else
