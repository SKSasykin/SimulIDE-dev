...
Label::Label() : QGraphicsTextItem() {
    ...
    connect( document(), &QTextDocument::contentsChange, this, &Label::updateGeometry, Qt::UniqueConnection );
}
Label::~Label() { }
...
void Label::contextMenuEvent( QGraphicsSceneContextMenuEvent* event ) {
    if ( !acceptedMouseButtons() )
        ...
        connect( rotateCWAction, &QAction::triggered, this, &Label::rotateCW, Qt::UniqueConnection );

        QAction* rotateCCWAction = menu.addAction( QIcon( ":/rotateccw.svg" ), "Rotate CCW" );
        connect( rotateCCWAction, &QAction::triggered, this, &Label::rotateCCW, Qt::UniqueConnection );

        QAction* rotate180Action = menu.addAction( QIcon( ":/rotate180.svg" ), "Rotate 180º" );
        connect( rotate180Action, &QAction::triggered, this, &Label::rotate180, Qt::UniqueConnection );
...
int Label::hFlip() {
    int val = 1;
    if ( m_parentComp != nullptr )
        val = ( m_parentComp->hflip() > 0 ) ? val : -val;
    return val;
}
...
int Label::vFlip() {
    int val = 1;
    if ( m_parentComp != nullptr )
        val = ( m_parentComp->vflip() > 0 ) ? val : -val;
    return val;
}
