...
Dialed::Dialed( QString type, QString id ) : Component( type, id ) {
...
QList<ComProperty*> Dialed::dialProps() {
    return { new BoolProp<Dialed>( "Slider", tr( "Slider" ), "", this, &Dialed::slider, &Dialed::setSlider ),
...
void Dialed::dialChanged( int val ) // Called when dial is rotated
{
    ...
    if ( m_step > 0 )
        m_value = round( m_value / m_step ) * m_step;
