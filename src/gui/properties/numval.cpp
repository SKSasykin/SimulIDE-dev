...
NumVal::NumVal( PropDialog* parent, CompBase* comp, ComProperty* prop ) : PropVal( parent, comp, prop ) {
    setupUi( this );
    m_type = prop->type();
    if ( m_type == "double" )
        return;
...
void NumVal::setup( bool isComp ) {
    ...
    if ( unit.isEmpty() || unit.startsWith( "_" ) ) {
        ...
        if ( unit.isEmpty() ) {
            ...
            unitBox->addItem( unit.remove( "_" ) ); // No multiplier
    } else {
        m_useMult = true;
        QString un = unit;
        ...
        if ( match.hasMatch() && match.capturedStart() == 0 )
            un.remove( 0, 1 ); // Remove multiplier

        if ( m_type == "double" )
            addDividers( unitBox, un );
...
void NumVal::on_showVal_toggled( bool checked ) {
    ...
    if ( checked ) {
        ...
        m_component->setPropStr( "ShowProp", "" );
...
void NumVal::on_valueBox_valueChanged( double val ) {
    ...
    if ( m_useMult )
        ...
        m_property->setValStr( QString::number( valueBox->value() ) );
    m_blocked = false;
    m_propDialog->updtValues();
    saveChanges();
}
...
void NumVal::updtValues() {
    ...
    double multiplier = 1;
    if ( m_useMult )
        multiplier = getMultiplier( unitBox->currentText() );
