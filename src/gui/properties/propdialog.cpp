...
PropDialog::PropDialog( QWidget* parent, QString help ) : QDialog( parent ) {
    setupUi( this );
...
void PropDialog::setComponent( CompBase* comp, bool isComp ) {
    ...
    for ( propGroup group : *groups ) {
        if ( group.flags & groupHidden )
            continue;

        bool groupEnabled = true;
        bool isMaincomp
            = comp->isHidden() && ( comp->getPropStr( "mainComp" ) == "true" ); // main Compoenent & is in subcircuit
        if ( isMaincomp && ( group.flags & groupNoCopy ) )
            groupEnabled = false;
        ...
        if ( !propList.isEmpty() ) {
            ...
            for ( ComProperty* prop : propList ) {
                ...
                if ( type == "double" )
                    mp = new NumVal( this, comp, prop );
                else if ( type == "color" )
                    mp = new ColorVal( this, comp, prop );
                else if ( type == "uint" )
                    mp = new NumVal( this, comp, prop );
                else if ( type == "int" )
                    mp = new NumVal( this, comp, prop );
                else if ( type == "string" )
                    mp = new StrVal( this, comp, prop );
                else if ( type == "path" )
                    mp = new PathVal( this, comp, prop, false );
                else if ( type == "file" )
                    mp = new PathVal( this, comp, prop, true );
                else if ( type == "textEdit" )
                    mp = new TextVal( this, comp, prop );
                else if ( type == "enum" )
                    mp = new EnumVal( this, comp, prop );
                else if ( type == "bool" )
                    mp = new BoolVal( this, comp, prop );
                ...
                if ( isMaincomp && ( prop->flags() & propNoCopy ) )
                    propEnabled = false;
