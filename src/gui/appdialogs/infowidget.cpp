...
void InfoWidget::setRate( double rate, double simLoad, double guiLoad, int fps ) {
    ...
    if ( rate < 0 ) {
        if ( rate == -1 )
            realSpeed->setText( "  " + tr( "Speed: Debugger" ) );
        else
            realSpeed->setText( "  " + tr( "Circuit ERROR!!!" ) );
    } else {
...
void InfoWidget::setCircTime( uint64_t tStep ) {
    ...
    while ( strUS.length() < 3 )
        strUS = "0" + strUS;
    QString strNS = QString::number( nSecs );
    while ( strNS.length() < 3 )
        strNS = "0" + strNS;
    QString strPS = QString::number( pSecs );
    while ( strPS.length() < 3 )
        strPS = "0" + strPS;

    simTime->setText( strH + ":" + strM + ":" + strS + " s  " + strMS + " ms  " + strUS + " µs  " + strNS + " ns  "
                      + strPS + " ps " );
}
