...
FileBrowser::FileBrowser( QWidget* parent ) : QTreeView( parent ) {
    m_pSelf = this;
    m_showHidden = false;
...
void FileBrowser::open() {
    ...
    if ( m_fileSystemModel->isDir( currentIndex() ) )
        ...
        if ( path.endsWith( ".sim2" ) || path.endsWith( ".sim1" ) )
            CircuitWidget::self()->loadCirc( path );
        else
            EditorWindow::self()->loadFile( path );
    }
}
...
void FileBrowser::setPath( QString path ) {
    ...
    if ( m_showHidden )
        m_fileSystemModel->setFilter( QDir::AllEntries | QDir::NoDotAndDotDot | QDir::AllDirs | QDir::Hidden );
    else
...
void FileBrowser::contextMenuEvent( QContextMenuEvent* event ) {
    ...
    if ( !event->isAccepted() ) {
        ...
        if ( m_fileSystemModel->isDir( currentIndex() ) ) {
            ...
            connect( addBookMarkAction, SIGNAL( triggered() ), this, SLOT( addBookMark() ), Qt::UniqueConnection );

            menu.addSeparator();
        } else {
            QAction* openWithEditor = menu.addAction( QIcon( ":/open.png" ), tr( "Open in editor" ) );
            connect( openWithEditor, SIGNAL( triggered() ), this, SLOT( openInEditor() ), Qt::UniqueConnection );

            menu.addSeparator();
        }
        QAction* openExternally = menu.addAction( QIcon( ":/open.png" ), tr( "Open externally" ) );
        connect( openExternally, SIGNAL( triggered() ), this, SLOT( openExternally() ), Qt::UniqueConnection );
        QAction* openParentDirExternally = menu.addAction( QIcon( ":/open.png" ), tr( "Open Parent Dir externally" ) );
        connect( openParentDirExternally, SIGNAL( triggered() ), this, SLOT( openParentDirExternally() ),
                 Qt::UniqueConnection );
        menu.addSeparator();
        QAction* showHidden = menu.addAction( tr( "Show Hidden" ) );
        showHidden->setCheckable( true );
        showHidden->setChecked( m_showHidden );
        connect( showHidden, SIGNAL( triggered() ), this, SLOT( showHidden() ), Qt::UniqueConnection );
        menu.exec( eventPos );
    }
}
