...
FileWidget::FileWidget( QWidget* parent ) : QSplitter( parent ) {
    m_pSelf = this;
    m_blocked = false;
    ...
    connect( m_bookmarks, SIGNAL( itemClicked( QListWidgetItem* ) ), this, SLOT( itemClicked( QListWidgetItem* ) ),
             Qt::UniqueConnection );

    //connect( m_searchFiles, SIGNAL( editingFinished() ),
    //         this,          SLOT( searchChanged() ), Qt::UniqueConnection);

    connect( m_cdUpButton, SIGNAL( released() ), m_fileBrowser, SLOT( cdUp() ), Qt::UniqueConnection );

    connect( m_path, SIGNAL( editingFinished() ), this, SLOT( pathChanged() ), Qt::UniqueConnection );
...
void FileWidget::writeSettings() {
    ...
    while ( m_bookmarks->count() )
        delete m_bookmarks->takeItem( 0 );
}

void FileWidget::addEntry( QString name, QString path, bool force ) {
    if ( !force && !QDir( path ).exists() )
        return;
...
void FileWidget::pathChanged() {
    if ( m_blocked )
        return; // Avoid circular call with setPath()
...
void FileWidget::contextMenuEvent( QContextMenuEvent* event ) {
    ...
    if ( m_bookmarkList.contains( item->data( 4 ).toString() ) ) {
        ...
        connect( remBookMarkAction, SIGNAL( triggered() ), this, SLOT( remBookMark() ) );
