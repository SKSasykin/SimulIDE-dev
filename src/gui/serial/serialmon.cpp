...
SerialMonitor::SerialMonitor( QWidget* parent, TransModule* module, bool send )
    : QDialog( parent ), Updatable(), m_uartInPanel( this ), m_uartOutPanel( this ) {
    setupUi( this );
...
void SerialMonitor::on_text_returnPressed() {
    ...
    if ( m_addCR )
        m_outBuffer.append( 13 ); //    m_usart->uartIn( 13 );
}
...
QString SerialMonitor::valToString( int val ) {
    ...
    case 0:
        ...
        break; // ASCII
    case 1:
        text = decToBase( byte, 16, 2 ) + " ";
        break; // HEX
    case 2:
        text = decToBase( byte, 10, 3 ) + " ";
        break; // DEC
    case 3:
        text = decToBase( byte, 8, 3 ) + " ";
        break; // OCT
    case 4:
        text = decToBase( byte, 2, 8 ) + " "; // BIN
    }
    return text;
}
