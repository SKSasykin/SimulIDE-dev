...
void CustomSlider::paintEvent( QPaintEvent* ) {
    ...
    QPointF points[3] = { QPointF( pos - 2, 1.5 ), QPointF( pos, 4 ), QPointF( pos + 2, 1.5 ) };
    painter.drawPolygon( points, 3 );

    QPointF points2[3] = { QPointF( pos - 2, h - 1.5 ), QPointF( pos, h - 4 ), QPointF( pos + 2, h - 1.5 ) };
    ...
    if ( this->singleStep() > 1 ) {
        ...
        painter.drawLine( QPointF( 3 * ( w - 12 ) / 4 + 6, h / 2 - 1.5 ),
                          QPointF( 3 * ( w - 12 ) / 4 + 6, h / 2 + 1.5 ) );
