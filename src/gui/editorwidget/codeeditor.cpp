...
QStringList CodeEditor::m_picInstr
    = QString(
          "addlw addwf andlw andwf banksel bcf bov bsf btfsc btg btfss clrf clrw clrwdt comf decf decfsz goto incf "
          "incfsz iorlw iorwf movf movlw movwf reset retfie retlw return rlf rrf sublw subwf swapf xorlw xorwf" )
          .split( " " );
QStringList CodeEditor::m_avrInstr
    = QString( "nop add adc adiw call inc sleep sub subi sbc sbci sbiw and andi or ori eor elpm fmul fmuls fmulsu mul "
               "muls smp com neg sbr cbr dec tst clr ser mul rjmp ijmp jmp rcall icall ret reti cpse cp cpc cpi sbrc "
               "sbrs sbic sbis brbs brbc breq brne brcs break brcc brsh brlo brmi brpl brge brlt brhs brhc brts brtc "
               "brvs brvc brie brid mov movw ldi lds ld ldd sts st std lpm in out push pop lsl lsr rol ror asr swap "
               "bset bclr sbi cbi bst bld sec clc sen cln sez clz sei cli ses cls sev clv set clt seh clh wdr des "
               "eicall eijmp lac las lat spm xch" )
          .split( " " );
QStringList CodeEditor::m_i51Instr
    = QString( "nop ajmp ljmp rr inc jbc acall lcall rrc dec jb ajmp ret rl add jnb reti rlc addc jc jnz orl jmp jnc "
               "anl jz xrl mov sjmp ajmp movc subb mul cpl cjne push clr swap xch pop setb da djnz xchd movx" )
          .split( " " );

QList<CodeEditor*> CodeEditor::m_documents;

CodeEditor::CodeEditor( QWidget* parent, OutPanelText* outPane ) : QPlainTextEdit( parent ), CompBase( "File", "" ) {
    ...
    connect( this, &CodeEditor::blockCountChanged, this, &CodeEditor::updateLineNumberAreaWidth, Qt::UniqueConnection );

    connect( this, &CodeEditor::updateRequest, this, &CodeEditor::updateLineNumberArea, Qt::UniqueConnection );

    connect( this, &CodeEditor::cursorPositionChanged, this, &CodeEditor::highlightCurrentLine, Qt::UniqueConnection );
    ...
    QString enums
        = EditorWindow::self()->compilers().join( "," ) + "," + EditorWindow::self()->assemblers().join( "," );

    enums = "None," + enums + ";" + tr( "None" ) + "," + enums;

    addPropGroup( { tr( "File Settings" ),
                    {
                        new StrProp<CodeEditor>( "Compiler", tr( "Compiler" ), enums, this, &CodeEditor::compName,
                                                 &CodeEditor::setCompName, 0, "enum" ),
                        ...
                        new BoolProp<CodeEditor>( "LoadCompiler", tr( "Load Compiler" ), "", this,
                                                  &CodeEditor::loadCompiler, &CodeEditor::setLoadCompiler ),

                        new BoolProp<CodeEditor>( "LoadBreakp", tr( "Load Breakpoints" ), "", this,
                                                  &CodeEditor::loadBreakp, &CodeEditor::setLoadBreakp ),

                        new BoolProp<CodeEditor>( "OpenFiles", tr( "Restore files" ), "", this, &CodeEditor::openFiles,
                                                  &CodeEditor::setOpenFiles ),
                    },
                    0 } );
    ...
    addPropGroup( { "Hidden",
                    {
                        ...
                        new StrProp<CodeEditor>( "Circuit", "Circuit", "", this, &CodeEditor::circuit,
                                                 &CodeEditor::setCircuit, 0 ),

                        new StrProp<CodeEditor>( "FileList", "FileList", "", this, &CodeEditor::fileList,
                                                 &CodeEditor::setFileList, 0 ),

                        new StrProp<CodeEditor>( "Breakpoints", "Breakpoints", "", this, &CodeEditor::breakpoints,
                                                 &CodeEditor::setBreakpoints, 0 ),
                    },
                    groupHidden } );
}
...
void CodeEditor::fileProps() {
    if ( !m_propDialog ) {
        if ( m_help == "" )
            m_help = MainWindow::self()->getHelp( "codeeditor" );
...
void CodeEditor::setBreakpoints( QString bp ) {
    ...
    list.removeAll( "" );
    for ( QString brk : list )
        addBreakPoint( brk.toInt() );
}
...
void CodeEditor::setFunctions( QStringList words ) {
    ...
    QStringList functs;
    for ( QString func : words )
        functs.append( func.split( "(" ).first() );
...
void CodeEditor::setFile( QString filePath ) {
    ...
    if ( extension == ".gcb" ) {
    ...
    } else if ( extension == ".cpp" || extension == ".c" || extension == ".ino" || extension == ".h"
                || extension == ".as" ) {
        ...
        if ( extension == ".ino" ) {
            ...
            if ( !m_compiler )
                m_compiler = EditorWindow::self()->createDebugger( "AScript", this );
    ...
    } else if ( extension == ".asm" ) // We should identify if pic, avr or i51 asm
    {
        ...
        } else if ( type == 3 ) // Is 8051
        {
            ...
            m_outPane->appendLine( "Unknown asm\n" );
    } else if ( extension == ".xml" || extension == ".html" || extension == ".package" || extension == ".mcu"
                || extension == ".sim2" || extension == ".sim1" || extension == ".simu" ) {
        setSyntaxFile( "xml.syntax" );
    } else if ( getFileName( m_file ).toLower() == "makefile" ) {
        setSyntaxFile( "makef.syntax" );
    } else if ( extension == ".hex" || extension == ".ihx" ) {
    ...
    if ( !m_compiler )
        m_compiler = EditorWindow::self()->createDebugger( "None", this, code );
    m_outPane->appendLine( "-------------------------------------------------------" );
}
...
int CodeEditor::getSyntaxCoincidences() {
    ...
    if ( pic < 3 && pic < avr && pic < i51 )
        result = 1;
    else if ( avr < 3 && avr < pic && avr < i51 )
        result = 2;
    else if ( i51 < 3 && i51 < pic && i51 < avr )
        result = 3;
...
void CodeEditor::addBreakPoint( int line ) {
    if ( EditorWindow::self()->debugState() == DBG_RUNNING )
        return;
...
void CodeEditor::startDebug() {
    ...
    for ( int line : m_brkPoints ) {
        line = EditorWindow::self()->debugger()->getValidLine( { m_file, line } );
        if ( line > 0 && !brkPoints.contains( line ) )
            brkPoints.append( line );
    }
...
void CodeEditor::contextMenuEvent( QContextMenuEvent* event ) {
    ...
    connect( undoAction, &QAction::triggered, this, &CodeEditor::undo, Qt::UniqueConnection );

    QAction* redoAction = menu.addAction( QIcon( ":/redo.svg" ), tr( "Redo" ) + "\tCtrl+Y" );
    connect( redoAction, &QAction::triggered, this, &CodeEditor::redo, Qt::UniqueConnection );

    menu.addSeparator();

    QAction* cutAction = menu.addAction( QIcon( ":/cut.svg" ), tr( "Cut" ) + "\tCtrl+X" );
    connect( cutAction, &QAction::triggered, this, &CodeEditor::cut, Qt::UniqueConnection );

    QAction* copyAction = menu.addAction( QIcon( ":/copy.svg" ), tr( "Copy" ) + "\tCtrl+C" );
    connect( copyAction, &QAction::triggered, this, &CodeEditor::copy, Qt::UniqueConnection );

    QAction* pasteAction = menu.addAction( QIcon( ":/paste.svg" ), tr( "Paste" ) + "\tCtrl+V" );
    connect( pasteAction, &QAction::triggered, this, &CodeEditor::paste, Qt::UniqueConnection );

    QAction* removeAction = menu.addAction( QIcon( ":/remove.svg" ), tr( "Remove" ) );
    connect( removeAction, &QAction::triggered, this, &CodeEditor::deleteSelected, Qt::UniqueConnection );
    ...
    connect( reloadAction, &QAction::triggered, EditorWindow::self(), &EditorWindow::reload, Qt::UniqueConnection );
...
void CodeEditor::loadConfig() {
    ...
    if ( !file.exists() ) // No .cfg file, Still support old brk files and file info
    {
        ...
        if ( !compiler.isEmpty() ) {
            ...
            if ( m_compiler != nullptr ) {
                if ( m_compiler->compName() == compiler )
                    return;
                delete m_compiler;
            }
    ...
    for ( QString line : docLines ) {
        ...
        if ( type == m_type )
            loadProperties( properties ); // CodeEditor
        else if ( type == "Compiler" && m_compiler )
            m_compiler->loadProperties( properties ); // Compiler
    }
}
...
void CodeEditor::updateLineNumberArea( const QRect& rect, int dy ) {
    if ( dy )
        m_lNumArea->scroll( 0, dy );
    else
        m_lNumArea->update( 0, rect.y(), m_lNumArea->width(), rect.height() );
...
void CodeEditor::highlightCurrentLine() {
    ...
    if ( m_found.size() )
        extraSelections.append( m_found );
...
void CodeEditor::lineNumberAreaPaintEvent( QPaintEvent* event ) {
    ...
    if ( m_numLines != numLines ) // Line added or removed: move or remove points
    {
        ...
        while ( block.isValid() ) {
            ...
            if ( oldLine > numLines ) {
                if ( brkPoints.contains( oldLine ) )
                    brkPoints.removeOne( oldLine );
                if ( errors.contains( oldLine ) )
                    errors.removeOne( oldLine );
                if ( warnings.contains( oldLine ) )
                    warnings.removeOne( oldLine );
            }
            if ( block == cBlock ) {
                if ( delta > 0 ) // Line removed, check if point at cursor line and remove it
                {
                    if ( m_brkPoints.contains( newLine ) )
                        brkPoints.removeOne( newLine );
                    if ( m_errors.contains( newLine ) )
                        errors.removeOne( newLine );
                    if ( m_warnings.contains( newLine ) )
                        warnings.removeOne( newLine );
                }
                found = true;
            }
            if ( found ) // Replace lines
            {
                if ( m_brkPoints.contains( oldLine ) )
                    brkPoints.replace( m_brkPoints.indexOf( oldLine ), newLine ); // Replace breakpoint line
                if ( m_errors.contains( oldLine ) )
                    errors.replace( m_errors.indexOf( oldLine ), newLine ); // Replace error line
                if ( m_warnings.contains( oldLine ) )
                    warnings.replace( m_warnings.indexOf( oldLine ), newLine ); // Replace warning line
            }
            block = block.next();
        }
        ...
        if ( block.isVisible() && bottom >= event->rect().top() ) {
            ...
            if ( m_compiler ) // Breakpoints an error indicator
            {
                ...
                if ( pos > top && pos < bottom ) // Check if there is a new breakpoint request from context menu
                {
                    ...
                    else if ( m_brkAction == 2 )
                        remBreakPoint( lineNumber );
                ...
                if ( m_warnings.contains( lineNumber ) ) // Draw warning
                {
                    ...
                    painter.setPen( Qt::NoPen );
                    QPolygon polygon;
                    polygon << QPoint( 1, bottom - 2 ) << QPoint( fontSize / 2, top + 1 )
                            << QPoint( fontSize - 1, bottom - 2 );
...
void LineNumberArea::contextMenuEvent( QContextMenuEvent* event ) {
    ...
    connect( addBrkAction, &QAction::triggered, m_codeEditor, &CodeEditor::slotAddBreak, Qt::UniqueConnection );

    QAction* remBrkAction = menu.addAction( QIcon( ":/nobreakpoint.png" ), tr( "Remove BreakPoint" ) );
    connect( remBrkAction, &QAction::triggered, m_codeEditor, &CodeEditor::slotRemBreak, Qt::UniqueConnection );

    menu.addSeparator();

    QAction* clrBrkAction = menu.addAction( QIcon( ":/remove.svg" ), tr( "Clear All BreakPoints" ) );
    connect( clrBrkAction, &QAction::triggered, m_codeEditor, &CodeEditor::slotClearBreak, Qt::UniqueConnection );
