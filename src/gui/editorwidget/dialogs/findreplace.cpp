...
FindReplace::FindReplace( QWidget* parent ) : QDialog( parent ) {
    setupUi( this );
    m_editor = nullptr;
...
bool FindReplace::find( bool next ) {
    ...
    if ( regexp->isChecked() ) {
        QRegularExpression reg(
            toSearch,
            ( caseS->isChecked() ? QRegularExpression::NoPatternOption : QRegularExpression::CaseInsensitiveOption ) );

        m_textCursor = m_editor->document()->find( reg, m_textCursor, flags );
        m_editor->setTextCursor( m_textCursor );
        result = !m_textCursor.isNull();
    } else
        result = m_editor->find( toSearch, flags );
