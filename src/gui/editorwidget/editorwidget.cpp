...
EditorWidget::EditorWidget( QWidget* parent )
    : QWidget( parent ), CompBase( "Editor", "" ), m_outPane( this ), m_settingsMenu( this ), m_fileMenu( this ) {
    setAcceptDrops( true );
    ...
    addPropGroup(
        { tr( "Editor Settings" ),
          {
              new IntProp<EditorWidget>( "FontSize", tr( "Font Size" ), "_px", this, &EditorWidget::fontSize,
                                         &EditorWidget::setFontSize, 0 ),

              new IntProp<EditorWidget>( "TabSize", tr( "Tab Size" ), "", this, &EditorWidget::tabSize,
                                         &EditorWidget::setTabSize, 0 ),

              new BoolProp<EditorWidget>( "SpaceTabs", tr( "Tabs as spaces" ), "", this, &EditorWidget::spaceTabs,
                                          &EditorWidget::setSpaceTabs ),
              ...
              new BoolProp<EditorWidget>( "CloseParenthesis", tr( "Auto close Parenthesis ( )" ), "", this,
                                          &EditorWidget::closeParenthesis, &EditorWidget::setCloseParenthesis ),

              new BoolProp<EditorWidget>( "CloseBraces", tr( "Auto close Braces { }" ), "", this,
                                          &EditorWidget::closeBraces, &EditorWidget::setCloseBraces ),

              new BoolProp<EditorWidget>( "CloseBrackets", tr( "Auto close Brackets [ ]" ), "", this,
                                          &EditorWidget::closeBrackets, &EditorWidget::setCloseBrackets ),

              new BoolProp<EditorWidget>( "CloseQuotes", tr( "Auto close Quotes \" \"" ), "", this,
                                          &EditorWidget::closeQuotes, &EditorWidget::setCloseQuotes ),

              new BoolProp<EditorWidget>( "CloseSquotes", tr( "Auto close Single Quotes ' '" ), "", this,
                                          &EditorWidget::closeSquotes, &EditorWidget::setCloseSquotes ),
          },
          0 } );
}
...
void EditorWidget::setFontSize( int size ) {
    ...
    for ( CodeEditor* ce : getCodeEditors() )
        ce->setFont( m_font );
...
void EditorWidget::addDocument( QString file, bool main ) {
    ...
    connect( ce->document(), &QTextDocument::contentsChanged, this, &EditorWidget::documentWasModified,
             Qt::UniqueConnection );
    ...
    if ( main ) {
        ...
        if ( file.isEmpty() )
            ce->document()->setModified( true ); // New
        documentWasModified();
        enableFileActs( true );
...
void EditorWidget::open() {
    ...
    QString fileName = QFileDialog::getOpenFileName(
        this, tr( "Load File" ), dir, tr( "All files" ) + " (*);;Arduino (*.ino);;Asm (*.asm);;GcBasic (*.gcb)" );
    ...
    if ( action ) {
        ...
        if ( pfile.exists() )
            ...
            QMessageBox::StandardButton ret = QMessageBox::warning(
                this, "EditorWidget::openRecentFile",
...
bool EditorWidget::saveAs() {
    ...
    QString extensions = "";
    if ( ext == "" )
        extensions = tr( "All files" ) + " (*);;Arduino (*.ino);;Asm (*.asm);;GcBasic (*.gcb)";
    else
        extensions = "." + ext + "(*." + ext + ");;" + tr( "All files" ) + " (*.*)";
...
void EditorWidget::closeTab( int index ) {
    if ( m_debuggerToolBar->isVisible() )
        stop();
...
void EditorWidget::confEditor() {
    if ( !m_propDialog ) {
        if ( m_help == "" )
            m_help = MainWindow::self()->getHelp( "editor" );
...
void EditorWidget::findReplaceDialog() {
    ...
    if ( text != "" )
        m_findRepDialog->setTextToFind( text );
...
void EditorWidget::createWidgets() {
    ...
    connect( m_docWidget, SIGNAL( tabCloseRequested( int ) ), this, SLOT( closeTab( int ) ), Qt::UniqueConnection );
...
void EditorWidget::createActions() {
    ...
    for ( int i = 0; i < MaxRecentFiles; i++ ) {
        ...
        recentFileActs[i]->setVisible( false );
        connect( recentFileActs[i], SIGNAL( triggered() ), this, SLOT( openRecentFile() ), Qt::UniqueConnection );
    }
