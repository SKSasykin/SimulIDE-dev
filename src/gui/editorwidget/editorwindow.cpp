...
EditorWindow::EditorWindow( QWidget* parent ) : EditorWidget( parent ) {
    m_pSelf = this;
...
void EditorWindow::updateStep() {
    ...
    while ( lineStr.length() < 6 )
        lineStr.append( " " );

    QString cycleStr = QString::number( cycle - m_lastCycle );
    while ( cycleStr.length() < 15 )
        cycleStr.append( " " );

    m_outPane.appendLine( tr( "Line " ) + lineStr + tr( "Clock Cycles: " ) + cycleStr + tr( "Time us: " )
                          + QString::number( time - m_lastTime ) );
    m_lastCycle = cycle;
    m_lastTime = time;
...
void EditorWindow::lineReached( codeLine_t line ) // Processor reached PC related to source line
{
    if ( m_state == DBG_RUNNING ) // Check if there is a breakpoint
    {
        if ( m_fileList.contains( line.file ) ) {
            ...
            if ( ce && !ce->getBreakPoints()->contains( line.lineNumber ) )
                return;
        } else
            return;
    }
...
BaseDebugger* EditorWindow::createDebugger( QString name, CodeEditor* ce, QString code ) {
    ...
    if ( type == "arduino" )
        debugger = new InoDebugger( ce, &m_outPane );
    else if ( type == "avrgcc" )
        debugger = new AvrGccDebugger( ce, &m_outPane );
    else if ( type == "xc8" )
        debugger = new Xc8Debugger( ce, &m_outPane );
    else if ( type == "sdcc" )
        debugger = new SdccDebugger( ce, &m_outPane );
    else if ( type == "gcbasic" )
        debugger = new GcbDebugger( ce, &m_outPane );
    else if ( type == "ascript" )
        debugger = new asDebugger( ce, &m_outPane );
    else {
...
void EditorWindow::loadCompilerSet( QString compilsPath, QMap<QString, compilData_t>* compList ) {
    if ( compilsPath.isEmpty() )
        return;

    QDir compilsDir = QDir( compilsPath );
    if ( !compilsDir.exists() )
        return;
    ...
    for ( QString compilFile : xmlList ) {
        ...
        if ( !compilFilePath.isEmpty() ) {
            ...
            if ( ( el.tagName() == "compiler" ) && el.hasAttribute( "name" ) && el.hasAttribute( "type" ) ) {
                QString compiler = el.attribute( "name" );
                if ( compList->contains( compiler ) )
                    continue;
                ...
                qDebug() << tr( "Error Loading Compiler at:" ) << "\n" << compilFilePath << "\n";
        }
    }
}
