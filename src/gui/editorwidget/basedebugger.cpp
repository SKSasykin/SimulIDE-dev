...
BaseDebugger::BaseDebugger( CodeEditor* parent, OutPanelText* outPane ) : Compiler( parent, outPane ) {
    m_compName = "None";
    m_langLevel = 0;
    m_lstType = 0;
...
BaseDebugger::~BaseDebugger() {
    if ( eMcu::self() )
        eMcu::self()->getRamTable()->remDebugger( this );
}
...
bool BaseDebugger::postProcess() {
    ...
    if ( m_langLevel ) // High level language
    {
        ...
        {
            if ( isNoValid( srcLine ) )
                continue;
            srcLine = srcLine.split( ";" ).first();
            if ( !srcLine.toUpper().contains( "CALL" ) )
                continue;
            ...
            funcName = l.last();
            if ( !funcName.isEmpty() )
                m_functions[funcName.toUpper()] = -1;
        }
...
bool BaseDebugger::isNoValid( QString line ) {
    return ( line.isEmpty() || line.startsWith( "void" ) || line.startsWith( "{" ) || line.startsWith( "}" )
...
int BaseDebugger::getValidLine( codeLine_t line ) {
    ...
    for ( codeLine_t cd : m_flashToSource.values() ) {
        if ( line.file == cd.file ) {
            if ( cd.lineNumber > lastLine )
                lastLine = cd.lineNumber;
            lineList.append( cd.lineNumber );
        }
    }
    while ( !lineList.contains( lineNumber ) && lineNumber <= lastLine )
        lineNumber++;
    if ( lineNumber == lastLine && !lineList.contains( lineNumber ) )
