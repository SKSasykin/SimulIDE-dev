...
Compiler::Compiler( CodeEditor* editor, OutPanelText* outPane )
    : QObject( editor ), CompBase( "Compiler", "" ), m_compProcess( this ) {
    m_editor = editor;
    m_outPane = outPane;
    ...
    addPropGroup( { tr( "Compiler Settings" ),
                    {
                        ...
                        new StrProp<Compiler>( "ToolPath", tr( "Tool Path" ), "", this, &Compiler::toolPath,
                                               &Compiler::setToolPath, 0, "path" ),
                    },
                    0 } );
}
Compiler::~Compiler() { }
...
void Compiler::loadCompiler( QString file ) {
    ...
    if ( compiler.hasAttribute( "syntax" ) )
        m_editor->setSyntaxFile( compiler.attribute( "syntax" ) + ".syntax" );
    ...
    if ( compiler.hasAttribute( "inclPath" ) )
        inclPath = compiler.attribute( "inclPath" );
    if ( !inclPath.isEmpty() )
        m_inclPath = inclPath;
    ...
    while ( !node.isNull() ) {
        ...
        if ( step.tagName() == "step" ) {
            ...
            if ( argsDebug.isEmpty() )
                args = argsDebug = arguments;
            else
                args = arguments + argsDebug;

            if ( args.contains( "$inclPath" ) ) {
                addProperty( tr( "Compiler Settings" ),
                             new StrProp<Compiler>( "InclPath", tr( "Include Path" ), "", this, &Compiler::includePath,
                                                    &Compiler::setIncludePath, 0 ) );
            }
            ...
            if ( args.contains( "$device" ) ) {
                addFilePropHead();
                addProperty( tr( "Compiler Settings" ),
                             new StrProp<Compiler>( "Device", tr( "Device" ), "", this, &Compiler::device,
                                                    &Compiler::setDevice, 0 ) );
            }
            if ( args.contains( "$family" ) ) {
                addFilePropHead();
                addProperty( tr( "Compiler Settings" ),
                             new StrProp<Compiler>( "Family", tr( "Family" ), "", this, &Compiler::family,
                                                    &Compiler::setFamily, 0 ) );
            }
...
void Compiler::addFilePropHead() {
    ...
    addProperty( tr( "Compiler Settings" ), new ComProperty( "", "separator", "", "", 0 ) );

    addProperty( tr( "Compiler Settings" ), new ComProperty( "", tr( "For this file:" ), "", "", 0 ) );
}
...
int Compiler::compile( bool debug ) {
    if ( m_compName == "None" )
        ...
        m_outPane->appendLine( tr( "     No command Defined" ) );
    ...
    for ( int i = 0; i < m_command.size(); ++i ) {
        ...
        if ( arguments.contains( "$family" ) ) {
            if ( m_family.isEmpty() ) {
                ...
                arguments = arguments.replace( "$family", m_family );
        }
        if ( arguments.contains( "$device" ) ) {
            if ( m_device.isEmpty() ) {
                ...
                arguments = arguments.replace( "$device", m_device );
        }
    ...
    if ( error == 0 )
        compiled( m_buildPath + m_fileName + ".hex" );
...
int Compiler::getErrors() {
    ...
    if ( !p_stdout.isEmpty() )
        error = getErrorLine( p_stdout );
    if ( error )
        return error;

    QString p_stderr = m_compProcess.readAllStandardError();
    if ( !p_stderr.isEmpty() )
        error = getErrorLine( p_stderr );
    return error;
}
...
void Compiler::readSettings() {
    ...
    if ( settings->contains( prop ) )
        setToolPath( settings->value( prop ).toString() );
    prop = m_compName + "_inclPath";
    if ( settings->contains( prop ) )
        m_inclPath = settings->value( prop ).toString();
}
...
void Compiler::compilerProps() {
    ...
    if ( !m_propDialog ) {
        if ( m_help == "" )
            m_help = MainWindow::self()->getHelp( m_compName );
...
bool Compiler::checkCommand( QString executable ) {
    ...
    if ( started && !check.waitForFinished( 1000 ) )
        check.kill();
