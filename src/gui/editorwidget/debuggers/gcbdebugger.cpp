...
GcbDebugger::GcbDebugger( CodeEditor* parent, OutPanelText* outPane ) : BaseDebugger( parent, outPane ) {
    m_stepOver = true;
...
bool GcbDebugger::mapFlashToSource() // Map asm_source_line <=> gcb_source_line
{
    ...
    for ( QString gcbLine : gcbLines ) // Get Declared Variables
    {
        ...
        if ( !line.contains( "DIM" ) )
            continue; // Search lines containing "Dim"
    ...
    while ( !asmLines.isEmpty() && !asmLine.startsWith( "BASPROGRAMSTART" ) ) {
        ...
        if ( asmLine.isEmpty() )
            continue;
        else if ( asmLine.contains( "locations for variables" ) )
            haveVariable = true;
        ...
        else if ( haveVariable ) // Find Variable Addresses
        {
            ...
            QString type = "uint8";
            if ( m_varTypes.contains( name ) )
                type = m_varTypes.value( name );
