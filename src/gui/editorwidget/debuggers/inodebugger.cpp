...
InoDebugger::InoDebugger( CodeEditor* parent, OutPanelText* outPane ) : AvrGccDebugger( parent, outPane ) {
    m_version = 0;
    m_board = "Uno";
    ...
    mb.insert( "Serial",
               { "available()", "availableForWrite()", "begin( speed, config=SERIAL_8N1 )", "end()",
                 "find( target, length )", "findUntil( target, terminal )", "flush()",
                 "parseFloat( lookahead, ignore )", "parseInt( lookahead, ignore )", "peek()", "print( val, format )",
                 "println( val, format )", "read()", "readBytes( buffer, length )",
                 "readBytesUntil( character, buffer, length )", "readString()", "readStringUntil( terminator )",
                 "setTimeout( time )", "write( buf, len )" } );
    m_editor->setMemberWords( mb );

    QStringList functions;
    functions << "setup()" << "loop()" << "digitalRead( pin )" << "digitalWrite( pin, value )" << "pinMode( pin, mode )"
              << "analogRead( pin )" << "analogReadResolution( bits )" << "analogReference( type )"
              << "analogWrite( pin, value )" << "analogWriteResolution( bits ) " << "noTone( pin )"
              << "pulseIn( pin, value, timeout=1s )" << "pulseInLong( pin, value, timeout=1s )"
              << "shiftIn( dataPin, clockPin, bitOrder )" << "shiftOut( dataPin, clockPin, bitOrder, value )"
              << "tone( pin, frequency, duration ) " << "delay( ms )" << "delayMicroseconds( us )" << "micros()"
              << "millis() " << "abs( x )" << "constrain( x, a, b )" << "map( value, fromLow, fromHigh, toLow, toHigh )"
              << "max( x, y )" << "min( x, y )" << "pow( base, exponent )" << "sq( x )" << "sqrt( x )" << "cos( rad )"
              << "sin( rad )" << "tan( rad ) " << "isAlpha( thisChar )" << "isAlphaNumeric( thisChar )"
              << "isAscii( thisChar )" << "isControl( thisChar )" << "isDigit( thisChar )" << "isGraph( thisChar )"
              << "isHexadecimalDigit( thisChar )" << "isLowerCase( thisChar )" << "isPrintable( thisChar )"
              << "isPunct( thisChar )" << "isSpace( thisChar )" << "isUpperCase( thisChar )"
              << "isWhitespace( thisChar )" << "random( min, max )" << "randomSeed( seed )" << "bit( n )"
              << "bitClear( x, n )" << "bitRead( x, n )" << "bitSet( x, n )" << "bitWrite( x, n, b )" << "highByte( x )"
              << "lowByte( x )" << "attachInterrupt( interrupt, ISR, mode )" << "detachInterrupt( interrupt )"
              << "digitalPinToInterrupt( pin )" << "interrupts()" << "noInterrupts()";
    ...
    addProperty( tr( "Compiler Settings" ),
                 new StrProp<Compiler>( "InclPath", tr( "Custom Library Path" ), "", this, &Compiler::includePath,
                                        &Compiler::setIncludePath, 0, "path" ) );

    addFilePropHead();

    addProperty( tr( "Compiler Settings" ),
                 new StrProp<InoDebugger>( "Board", tr( "Board" ),
                                           m_defaultBoards + ",Custom;" + m_defaultBoards + "," + tr( "Custom" ), this,
                                           &InoDebugger::getBoard, &InoDebugger::setBoard, 0, "enum" ) );

    addProperty( tr( "Compiler Settings" ),
                 new StrProp<InoDebugger>( "CustomBoard", tr( "Custom Board" ), "", this, &InoDebugger::customBoard,
                                           &InoDebugger::setCustomBoard, 0 ) );
}
...
int InoDebugger::compile( bool debug ) {
    ...
    if ( m_version == 1 ) {
        ...
        if ( !m_sketchBook.isEmpty() ) {
            if ( m_inclPath.isEmpty() )
                userLibrar = addQuotes( m_sketchBook + "/libraries" );
            else
                userLibrar = addQuotes( m_inclPath );
    ...
    int error = getErrors();
    if ( error == 0 )
        compiled( m_buildPath + "/build/" + m_fileName + ".ino.hex" );
...
void InoDebugger::compilerProps() {
    ...
    QString enums
        = m_defaultBoards + ",Custom," + userBoards + ";" + m_defaultBoards + "," + tr( "Custom" ) + "," + userBoards;
