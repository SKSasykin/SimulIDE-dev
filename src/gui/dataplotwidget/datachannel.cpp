...
DataChannel::DataChannel( PlotBase* plotBase, QString id ) : eElement( id ), Updatable() {
    m_plotBase = plotBase;
    m_ePin.resize( 2 );
    m_pin = nullptr;
    m_ePin[1] = nullptr;
...
bool DataChannel::doTest( bool test ) {
    ...
    for ( int n = 0; n < m_bufferCounter; ++n ) {
        ...
        if ( qFabs( value ) < 1e-12 )
            value = 0;

        if ( compare ) {
            uint64_t cTime = m_timeTest.at( n );
            double cValue = m_bufferTest.at( n );
            if ( qFabs( cValue ) < 1e-12 )
                cValue = 0;
            if ( qFabs( value - cValue ) > qFabs( value / 1000 ) )
                ok = false;
            if ( time != cTime )
                ok = false;
        } else {
