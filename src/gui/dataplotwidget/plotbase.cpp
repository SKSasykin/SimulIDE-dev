...
PlotBase::PlotBase( QString type, QString id ) : Component( type, id ), ScriptBase( id ) {
    m_graphical = true;
    m_bufferSize = 600000;
    ...
    addPropGroup( { tr( "Main" ),
                    { new IntProp<PlotBase>( "Basic_X", tr( "Screen Width" ), "_px", this, &PlotBase::baSizeX,
                                             &PlotBase::setBaSizeX, 0, "uint" ),

                      new IntProp<PlotBase>( "Basic_Y", tr( "Screen Height" ), "_px", this, &PlotBase::baSizeY,
                                             &PlotBase::setBaSizeY, 0, "uint" ),

                      new IntProp<PlotBase>( "BufferSize", tr( "Buffer Size" ), "", this, &PlotBase::bufferSize,
                                             &PlotBase::setBufferSize, 0, "uint" ),

                      new BoolProp<PlotBase>( "connectGnd", tr( "Connect to ground" ), "", this, &PlotBase::connectGnd,
                                              &PlotBase::setConnectGnd, 0 ),

                      new DoubProp<PlotBase>( "InputImped", tr( "Impedance" ), "MΩ", this, &PlotBase::inputImped,
                                              &PlotBase::setInputImped ) },
                    groupNoCopy } );

    addPropGroup(
        { tr( "Test" ),
          {
              new DoubProp<PlotBase>( "TestTime", tr( "Test Time" ), "ns", this, &PlotBase::testTime,
                                      &PlotBase::setTestTime, 0 ),

              new BoolProp<PlotBase>( "DoTest", tr( "Do Test" ), "", this, &PlotBase::doTest, &PlotBase::setDoTest, 0 ),
          },
          0 } );
    ...
    addPropGroup( { "Hidden",
                    {
                        ...
                        new StrProp<PlotBase>( "TestData", "", "", this, &PlotBase::testData, &PlotBase::setTestData ),
                    },
                    groupHidden } );
}
PlotBase::~PlotBase() {
...
bool PlotBase::propNotFound( QString prop, QString val ) {
    if ( prop == "hTick" )
        setTimeDiv( val.toLongLong() * 1e3 ); // Old: TODELETE
    else if ( prop == "vTick" )
        setVolDiv( val );
...
void PlotBase::runEvent() // Test time reached, make comparison
{
    ...
    if ( m_doTest && testOk )
        qDebug() << idLabel() << "Test passed";

    if ( m_doTest && BatchTest::isRunning() )
...
QString PlotBase::tunnels() {
    QString list;
    for ( int i = 0; i < m_numChannels; ++i )
        list.append( m_channel[i]->m_chTunnel ).append( "," );
    return list;
}
...
void PlotBase::updateConds( QString conds ) {
    ...
    if ( r < 0 ) {
        qDebug() << "PlotBase::updateConds Failed to compile expression:" << conds;
        return;
    }
