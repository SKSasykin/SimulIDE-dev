...
PlotDisplay::PlotDisplay( QWidget* parent ) : QWidget( parent ) {
    m_channels = 4;
    for ( int i = 0; i < 8; ++i ) {
        m_scaleY[i] = 1;
        m_hPos[i] = 0;
...
void PlotDisplay::setTimeZero( double t ) {
    ...
    m_timeZero = tPos.x();
    if ( qFabs( m_timeZero - m_hCenter ) < 5 )
        m_timeZero = m_hCenter;
...
void PlotDisplay::wheelEvent( QWheelEvent* event ) {
    ...
    int cursorX = cPos.x();
    int cursorY = cPos.y();
    if ( ( cursorX > m_ceroX ) && ( cursorX < m_endX ) && ( cursorY > 0 ) && ( cursorY < height() ) ) {
                ...
                if ( m_channel[i]->isBus() ) {
                    ...
                    if ( w > 20 && val != 0 ) {
                        if ( m_expand )
                            ...
                            p.setFont( m_fontXS );
                        p.drawText( x1, y1, w, m_scaleY[i], Qt::AlignCenter,
                                    "0x" + QString::number( val, 16 ).toUpper() );
                    }
                } else {
                    if ( p1Volt > m_vMaxVal[i] )
                        m_vMaxVal[i] = p1Volt; // Maximum Voltage
                    if ( p1Volt < m_vMinVal[i] )
                        m_vMinVal[i] = p1Volt; // MiniMum Voltage
