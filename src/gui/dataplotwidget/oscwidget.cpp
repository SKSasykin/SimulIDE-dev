...
OscWidget::OscWidget( QWidget* parent, Oscope* oscope ) : QDialog( parent ) {
    setupUi( this );
    ...
    for ( int i = 0; i < 5; ++i ) {
        ...
        channelGroup->button( i )->setStyleSheet(
            "QPushButton:checked{background-color:" + color
            + "; border: 3px solid gray; border-radius: 2px; border-style: inset;}\
...
void OscWidget::on_voltDivBox_valueChanged( double voltDiv ) {
    ...
    if ( ch == 4 ) {
        ch = 0;
        for ( int i = 1; i < 4; ++i )
            m_oscope->setVoltDiv( i, voltDiv / 1e12 );
    }
...
void OscWidget::on_voltPosBox_valueChanged( double voltPos ) {
    ...
    if ( ch == 4 ) {
        ch = 0;
        for ( int i = 1; i < 4; ++i )
            m_oscope->setVoltPos( i, voltPos / 1e12 );
    }
...
void OscWidget::hideChannel( int ch, bool hide ) {
    ...
    if ( ch < 4 && hide )
        hideGroup->button( 4 )->setChecked( false );
}
