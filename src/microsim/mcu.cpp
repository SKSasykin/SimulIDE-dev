...
LibraryItem* Mcu::libraryItem() {
    return new LibraryItem( "NEW_MCU", "", "ic2.png", "MCU", Mcu::construct );
}
...
Component* Mcu::construct( QString type, QString id ) {
    ...
    if ( !m_error )
        m_error = McuCreator::createMcu( mcu, id );
...
Mcu::Mcu( QString type, QString id, QString device ) : Chip( type, id, device ), m_eMcu( this, id ) {
    ...
    m_device = m_name;
    if ( m_device.contains( "@" ) )
        m_device = m_device.split( "@" ).last(); // MCU in Subcircuit

    if ( m_device.startsWith( "p" ) ) // PICs TODELETE
    {
        if ( m_device.endsWith( "a" ) )
            m_device.remove( m_device.size() - 1, 1 );
...
void Mcu::setupMcu() {
    ...
    if ( m_packageList.size() > 1 )
        addProperty( tr( "Main" ),
                     new StrProp<Mcu>( "Package", tr( "Package" ), m_packageList.keys().join( "," ), this,
                                       &Mcu::package, &Mcu::setPackage, propNoCopy, "enum" ) );

    if ( m_eMcu.m_intOsc ) {
        addProperty( tr( "Main" ),
                     new DoubProp<Mcu>( "Frequency", tr( "Frequency" ), "MHz", this, &Mcu::uiFreq, &Mcu::setUiFreq ) );
        addProperty( tr( "Main" ),
                     new BoolProp<Mcu>( "ForceFreq", tr( "Force this frequency" ), "", this, &Mcu::forceFreq,
                                        &Mcu::setForceFreq ) );
    }
    ...
    if ( m_eMcu.flashSize() ) {
        addProperty(
            tr( "Main" ),
            new BoolProp<Mcu>( "savePGM", tr( "PGM persistent" ), "", this, &Mcu::savePGM, &Mcu::setSavePGM ) );

        addProperty( tr( "Main" ),
                     new StrProp<Mcu>( "Program", tr( "Firmware" ), "", this, &Mcu::program, &Mcu::setProgram ) );

        addProperty( tr( "Main" ),
                     new BoolProp<Mcu>( "Auto_Load", tr( "Reload hex at Simulation Start" ), "", this, &Mcu::autoLoad,
                                        &Mcu::setAutoLoad ) );
    }
    if ( m_eMcu.romSize() )
        addProperty(
            tr( "Main" ),
            new BoolProp<Mcu>( "saveEepr", tr( "EEPROM persistent" ), "", this, &Mcu::saveEepr, &Mcu::setSaveEepr ) );
    ...
    if ( m_portRstPin )
        cg.propList.append( new BoolProp<Mcu>( "Rst_enabled", tr( "Enable Reset Pin" ), "", this, &Mcu::rstPinEnabled,
                                               &Mcu::enableRstPin ) );

    if ( m_eMcu.m_intOsc && m_eMcu.m_intOsc->hasClockPins() )
        cg.propList.append( new BoolProp<Mcu>( "Ext_Osc", tr( "External Oscillator" ), "", this, &Mcu::extOscEnabled,
                                               &Mcu::enableExtOsc ) );
...
bool Mcu::propNotFound( QString prop, QString val ) {
    if ( prop == "program" )
        setProgram( val ); //  Old: TODELETE
    else if ( prop == "Mhz" )
        setUiFreq( val.toDouble() * 1e6 );
    else
        return Chip::propNotFound( prop, val );
    return true;
}
...
void Mcu::stamp() {
    ...
    if ( m_resetPin ) {
        ...
        if ( resetWarning )
            qDebug() << "    Warning!!" << idLabel() << "Reset pin not connected:\n";
    } else
        m_eMcu.start();

    if ( m_autoLoad ) {
        if ( !m_eMcu.m_firmware.isEmpty() )
...
void Mcu::updateStep() {
    if ( m_backData )
        update(); // used by ScriptDisplay

    if ( m_crashed ) {
        Simulator::self()->setWarning( /*m_warning*/ 0 );
        update();
    }
    if ( m_mcuMonitor && m_mcuMonitor->isVisible() )
        m_mcuMonitor->updateStep();
...
QString Mcu::varList() {
    ...
    if ( ramTable )
        return ramTable->getVarSet().join( "," );
    return "";
}
...
void Mcu::setVarList( QString vl ) {
    ...
    for ( QString str : vars ) // Format: {"var0/aadr0/type0", "var1/addr1/type1",...}
    {
        ...
        bool ok = false;
        if ( words.size() > 1 )
            addr = words.at( 1 ).toInt( &ok, 0 );
        if ( words.size() > 2 )
            type = words.at( 2 );
        if ( ok )
            m_eMcu.getRamTable()->addVariable( name, addr, type );
        varSet.append( name );
    }
...
QString Mcu::getPGM() {
    if ( m_savePGM ) {
        QString pgmStr;
        QVector<int> pgm;
        for ( uint16_t val : m_eMcu.m_progMem )
            pgmStr += QString::number( val ) + ",";
...
void Mcu::setPGM( QString pgm ) {
    ...
    for ( QString valStr : valList ) {
        if ( !valStr.isEmpty() )
            m_eMcu.setFlashValue( i, valStr.toInt() );
        if ( ++i >= size )
            break;
    }
}
...
void Mcu::slotReload() {
    if ( !m_eMcu.m_firmware.isEmpty() )
        ...
        QMessageBox::warning( 0, "Mcu::slotReload", tr( "No File to reload " ) );
}
...
int Mcu::serialMon() {
    if ( m_serialMon < 0 )
        return -1;
    if ( m_eMcu.m_transModules.at( m_serialMon )->monitorOpen() )
        return m_serialMon;
    return -1;
}
...
void Mcu::setLinkedValue( double v, int i ) {
    if ( m_scriptLink )
        m_scriptLink->setLinkedVal( v, i );
}

void Mcu::setLinkedString( QString str, int i ) {
    if ( m_scriptLink )
        m_scriptLink->setLinkedStr( str, i );
}

Pin* Mcu::addPin( QString id, QString type, QString label, int pos, int xpos, int ypos, int angle, int length,
                  int space ) {
    IoPin* pin = nullptr;
    if ( type.contains( "rst" ) ) {
        ...
        pin = m_eMcu.getIoPin( id ); // I/O Port
...
void Mcu::setUiFreq( double freq ) {
    m_uiFreq = freq;
    if ( m_forceFreq )
        m_eMcu.forceFreq( freq );
    else if ( !m_eMcu.intOsc()->freqChanged() )
        m_eMcu.forceFreq( freq ); // McuIntOsc can reconfigure frequency, if not then set directly
}
...
bool Mcu::extOscEnabled() {
    if ( m_eMcu.m_intOsc )
        return m_eMcu.m_intOsc->extClock();
    return false;
}

void Mcu::enableExtOsc( bool en ) {
    if ( m_eMcu.m_intOsc )
        m_eMcu.m_intOsc->enableExtOsc( en );
}
...
void Mcu::enableWdt( bool en ) {
    if ( m_eMcu.m_wdt )
        m_eMcu.m_wdt->enable( en );
}

bool Mcu::clockOut() {
    if ( m_eMcu.m_intOsc )
        return m_eMcu.m_intOsc->clockOut();
    return false;
}

void Mcu::setClockOut( bool clkOut ) {
    if ( m_eMcu.m_intOsc )
        m_eMcu.m_intOsc->setClockOut( clkOut );
}
