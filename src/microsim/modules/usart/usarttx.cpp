...
UartTx::UartTx( UsartModule* usart, eMcu* mcu, QString name ) : UartTR( usart, mcu, name ) {
    m_period = 0;
}
UartTx::~UartTx() { }
...
void UartTx::enable( uint8_t en ) {
    ...
    if ( enabled ) {
        m_state = usartIDLE;
        m_usart->driveTx( true );
    } else
        m_state = usartSTOPPED;
}

void UartTx::runEvent() {
    if ( m_state == usartSTOPPED )
        return;
...
void UartTx::processData( uint8_t data ) {
    m_buffer = data;
    if ( m_enabled && m_state == usartIDLE )
        startTransmission();
}
...
void UartTx::sendBit() {
    m_usart->driveTx( m_frame & 1 );
    m_frame >>= 1;

    if ( ++m_currentBit == m_framesize )
        m_state = usartTXEND; // Data transmission finished
