...
UsartModule::UsartModule( eMcu* mcu, QString name ) : TransModule( name ) {
...
void UsartModule::setSynchronous( bool s ) {
    ...
    if ( !s )
        return;
    if ( !m_uartSync )
        m_uartSync = new UartSync( this, "uartSync" );
...
UartSync::UartSync( UsartModule* usart, /*eMcu* mcu,*/ QString name ) : eElement( name ) {
    m_usart = usart;
...
void UartSync::sendSyncData( uint8_t data ) {
    if ( !m_syncTxPin || !m_syncClkPin || !m_syncPeriod )
        return;
...
UartTR::UartTR( UsartModule* usart, eMcu* mcu, QString name ) : McuModule( mcu, name ), eElement( name ) {
    m_usart = usart;
...
void UartTR::configureA( uint8_t val ) // Select Pin
{
    ...
    if ( pinNum < m_pinList.size() )
        m_ioPin = m_pinList.at( pinNum );
}
