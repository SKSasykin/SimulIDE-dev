...
UartRx::UartRx( UsartModule* usart, eMcu* mcu, QString name ) : UartTR( usart, mcu, name ) {
    m_period = 0;
    m_fifoSize = 2;
    m_ignoreData = false;
}
...
void UartRx::byteReceived( uint16_t frame ) {
    if ( mDATABITS == 9 && m_ignoreData && ( frame & 1 << 8 ) == 0 )
        return; // Multi-proccesor data frame
    ...
    if ( mPARITY > parNONE ) // Check Parity bit
    {
        ...
        if ( parity != parityBit )
            frame |= parityError; // Parity error
    }
    m_fifoP++;
    m_fifo[m_fifoP] = frame;
    if ( m_fifoP == 0 )
        setRxFlags(); // First byte received
...
uint8_t UartRx::getData() {
    ...
    if ( m_fifoSize > 1 && m_fifoP == 0 ) // Advance fifo
    {
        ...
        m_interrupt->clearFlag(); // Fifo empty
...
void UartRx::setRxFlags() {
    uint16_t frame = m_fifo[0];
    if ( mDATABITS == 9 )
        m_usart->setBit9Rx( ( frame & ( 1 << 8 ) ) ? 1 : 0 );
