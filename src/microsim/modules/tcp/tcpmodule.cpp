...
TcpModule::TcpModule( QString name ) : eElement( name ), TransModule( name ) {
    m_debug = false;
    ...
    for ( tcpConnection_t* conn : m_tcpConnections ) {
        ...
        case tcpConnect: {
            ...
            if ( tcpSocket->state() == QAbstractSocket::UnconnectedState ) {
                if ( m_debug )
                    qDebug() << "TcpModule - Connecting Socket" << conn->number << "to" << conn->host << conn->port;
                tcpSocket->connectToHost( conn->host, conn->port );
            } else if ( m_debug )
                qDebug() << "TcpModule - Error Connecting Socket" << conn->number << "to" << conn->host << conn->port
        ...
        case tcpClose: {
            ...
            if ( tcpSocket->state() == QAbstractSocket::ConnectedState ) {
                if ( m_debug )
                    qDebug() << "TcpModule - Disconnecting Socket" << conn->number << "from" << conn->host
                             << conn->port;
                tcpSocket->disconnectFromHost();
            } else if ( m_debug )
                qDebug() << "TcpModule - Error Disconnecting Socket" << conn->number << "from" << conn->host
                         << conn->port << "\n              State = " << tcpSocket->state();
        } break;
        }
        conn->action = tcpNone;
...
void TcpModule::sendMsg( QString msg, int link ) {
    ...
    if ( tcpSocket->state() == QAbstractSocket::ConnectedState ) {
        ...
        if ( m_debug )
            qDebug() << "TcpModule - Sending data to Socket:" << conn->number << "\n" << toSend;
        int bytes = tcpSocket->write( toSend + "\n\0" );
        if ( m_debug )
            qDebug() << "TcpModule -" << bytes << "Bytes Written to Socket" << conn->number;
    } else if ( m_debug )
        qDebug() << "TcpModule - Error Sending data: Socket" << conn->number << "not connected\n";
}
...
void TcpModule::tcpReadyRead( int link ) {
    ...
    if ( !m_debug )
        return;
    qDebug() << "TcpModule - Socket" << link << "Received from Host:" << conn->host << "Port:" << conn->port << "\n"
             << msg;
}
