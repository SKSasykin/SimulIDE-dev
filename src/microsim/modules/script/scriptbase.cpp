...
void ScriptBase::MessageCallback( const asSMessageInfo* msg ) {
    ...
    QString deb = QString( msg->section ) + " line: " + QString::number( msg->row ) + " " + QString::number( msg->col )
        + type + QString( msg->message );

    if ( m_debugger ) {
        m_debugger->outPane()->appendLine( deb.remove( "\n" ) );
        if ( type == " ERROR " )
            m_debugger->scriptError( msg->row );
        else if ( type == " WARNING " )
            m_debugger->scriptWarning( msg->row );
    } else
        CircuitWidget::self()->simDebugMessage( deb.remove( "\n" ) );
...
ScriptBase::ScriptBase( QString name ) : eElement( name ) {
    m_aEngine = nullptr;
    m_asModule = nullptr;
    m_context = nullptr;
    m_debugger = nullptr;
    ...
    if ( m_aEngine == 0 ) {
        qDebug() << "Failed to create script engine.";
        return;
    }

    m_context = m_aEngine->CreateContext();
    if ( m_context == 0 ) {
        qDebug() << "Failed to create the context.";
        return;
    }
...
int ScriptBase::compileScript() {
    ...
    r = m_asModule->Build();
    if ( r < 0 ) {
        qDebug() << Qt::endl << m_elmId + " ScriptBase::compileScript Error" << Qt::endl;
        return -1;
    }
...
void ScriptBase::execute() {
    ...
    if ( m_status != asEXECUTION_FINISHED )
        printError( m_context );
}

void ScriptBase::printError(
    asIScriptContext* context ) // The execution didn't finish as we had planned. Determine why.
{
    if ( m_status == asEXECUTION_ABORTED )
        ...
        qDebug() << "The script ended for some unforeseen reason:" << m_status;
}
