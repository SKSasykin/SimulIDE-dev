...
TwiModule::TwiModule( QString name ) : eClockedDevice( name ), TransModule( name ) {
    m_sda = nullptr;
    m_scl = nullptr;
    m_addrBits = 7;
    m_address = m_cCode = 0;
...
void TwiModule::runEvent() {
    ...
    {
        if ( m_sdaState && clkLow )
            setSDA( false ); // Step 1: Lower SDA
        else if ( !m_sdaState && clkLow )
            setSCL( true ); // Step 2: Raise Clock
        else if ( !m_sdaState && !clkLow )
            setSDA( true ); // Step 3: Raise SDA
    ...
    {
        if ( clkLow )
            setSCL( true ); // Step 1: Clock Low, Raise Clock
        else if ( m_sdaState )
            setSDA( false ); // Step 2: SDA is High, Lower it
    ...
    case I2C_WRITE: // We are Writting data
    {
        ...
        {
            writeBit();
            if ( m_i2cState == I2C_READACK )
                bufferEmpty(); // Data sent (before ACK)
        }
        m_toggleScl = true;
    } break;
    ...
    case I2C_READACK: // Read ACK
    {
        if ( clkLow ) {
            ...
            {
                if ( m_write )
                    m_nextState = m_sdaState ? TWI_MTX_ADR_NACK : TWI_MTX_ADR_ACK; // Transmition started
                else
                    m_nextState = m_sdaState ? TWI_MRX_ADR_NACK : TWI_MRX_ADR_ACK; // Reception started
        ...
        if ( m_i2cState == I2C_START ) // Get Transaction Info
        {
            ...
            if ( m_bitPtr == 8 ) {
                if ( m_addrMatch )
                    ...
                    m_nextState = m_sendACK ? TWI_SRX_GEN_DATA_ACK : TWI_SRX_GEN_DATA_NACK;
                printIn( m_rxReg );
                readByte();
            }
...
void TwiModule::setSdaPin( IoPin* pin ) {
    m_sda = pin;
}
