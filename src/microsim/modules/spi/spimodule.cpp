...
SpiModule::SpiModule( QString name ) : eClockedDevice( name ), TransModule( name ) {
    m_MOSI = nullptr;
    m_MISO = nullptr;
    m_SS = nullptr;
...
void SpiModule::voltChanged() // Called in Slave mode on SCK or SS changes
{
    ...
    if ( enabled != m_enabled ) // Enabling or Disabling
    {
        ...
        if ( m_useSS && m_SS )
            ssChanged( enabled ); // Used by AVR
        m_enabled = enabled;
    }
    if ( !enabled )
        return;
    if ( m_clkState == Clock_High || m_clkState == Clock_Low )
        return; // Not an Edge
