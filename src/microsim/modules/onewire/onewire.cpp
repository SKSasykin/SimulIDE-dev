...
OneWire::OneWire( QString id ) : eElement( id ), TransModule( id ) {
    m_state = W1_IDLE;
    m_inpin = nullptr;
}
OneWire::~OneWire() { }
...
void OneWire::voltChanged() // Called when Input Pin changes
{
    ...
    if ( m_lastIn && !inState )
        m_lastTime = circTime; // Falling edge
    else if ( !m_lastIn && inState ) // Rising edge
    {
        ...
        } else if ( m_state > W1_IDLE ) // Active
        {
            ...
            {
                if ( !m_write )
                    readBit( 0 ); // If writting ignore long pulses
            } else if ( time < 15 * 1e6 ) // < 15 us : 1 received Or Read pulse
            {
                if ( m_state == W1_BUSY ) {
                    if ( m_busyTime > circTime )
                        pulse( 1, 50 );
                } // Still busy: Send a 50 us pulse after 1 us
                else {
                    if ( m_write )
                        writeBit();
                    else
...
void OneWire::writeBit() {
    if ( ( m_txReg & 1 << m_bitIndex ) == 0 )
        pulse( 1, 25 ); // Send a 25 us pulse after 1 us
...
void OneWire::dataReceived() // Complete data has been received (it's in m_rxReg)
{
    switch ( m_state ) {
    case W1_IDLE:
        break; // Error, this shoild not happen
    case W1_ROM_CMD:
        romCommand( m_rxReg );
        break; // ROM Command received
    case W1_FUN_CMD:
        funCommand( m_rxReg );
        break; // Function Command received
    case W1_DATA: {
        readByte();
    } break;
    ...
    case W1_SEARCH: {
        ...
        } else if ( ( m_rxReg > 0 ) == m_bitROM )
            sendSearchBit(); // Bit Match,  keep sending
        else {
    ...
    default:
        qDebug() << m_elmId << "Ds18b20::dataReceived  :  ERROR";
    }
}
...
void OneWire::dataSent() // Last data has been sent
{
    ...
    else if ( !m_txBuff.empty() ) // Send next byte in Tx Buffer
    {
        ...
        {
            if ( m_state == W1_ROM_CMD )
                m_state = W1_FUN_CMD; // Wait for Function command
            else if ( m_state == W1_FUN_CMD )
                m_state = W1_IDLE; // Wait for Reset pulse
        } else
            sendData( m_txBuff.back() ); // Send last byte in list, if list is not empty
    }
}
...
void OneWire::setROM( QString ROMstr ) {
    ...
    lstROM.removeAll( "" );
    for ( int i = 0; i < 8; ++i )
        m_ROM[i] = lstROM.at( i ).toInt( &ok, 16 );
...
QString OneWire::arrayToHex( uint8_t* data, uint len ) // Static
{
    ...
    for ( uint i = 0; i < len; i++ )
        hexValue.append( val2hex( data[i] ) ).append( " " );
    return hexValue;
}
