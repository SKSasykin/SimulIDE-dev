...
template <class Obj> class CallBack : public CallBackBase {
    friend class McuSignal;

public:
    CallBack( Obj* object, void ( Obj::*func )( uint8_t ) ) : CallBackBase() {
        m_object = object;
        m_func = func;
    ...
    virtual void call( uint8_t val ) override { ( m_object->*m_func )( val ); }
...
public:
    ...
    template <class Obj> void connect( Obj* obj, void ( Obj::*func )( uint8_t ), uint8_t mask = 0xFF ) {
    ...
    template <class Obj> void disconnect( Obj* obj, void ( Obj::*func )( uint8_t ) ) {
        ...
        while ( posSlot ) {
            ...
            if ( ( cb->m_object == obj ) && ( cb->m_func == func ) ) {
                if ( preSlot )
                    preSlot->nextCallBack = posSlot->nextCallBack;
                else
                    m_slot = posSlot->nextCallBack;
                delete posSlot;
                break;
            }
            preSlot = posSlot;
