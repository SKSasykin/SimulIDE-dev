...
void DataSpace::initialize() {
    ...
    for ( uint i = 0; i < m_dataMem.size(); i++ )
        writeReg( i, 0, false );
...
uint8_t DataSpace::readReg( uint16_t addr ) {
    ...
    if ( regSignal ) {
        ...
        if ( m_regOverride >= 0 )
            v = (uint8_t) m_regOverride; // Value overriden in callback
        else
            v = m_dataMem[addr]; // Timers update their counters in callback
    }
    return v;
}
...
void DataSpace::writeReg( uint16_t addr, uint8_t v, bool masked ) {
    ...
    {
        if ( addr < m_regMask.size() )
            mask = m_regMask[addr];
        if ( mask != 0xFF && mask != 0x00 )
            v = ( m_dataMem[addr] & ~mask ) | ( v & mask );
    }
...
uint16_t DataSpace::getRegAddress( QString reg ) // Get Reg address by name
{
    ...
    if ( m_regInfo.contains( reg ) )
        addr = m_regInfo.value( reg ).address;
    else if ( m_regInfo.contains( reg.toUpper() ) )
        addr = m_regInfo.value( reg ).address;
