...
McuPort::McuPort( eMcu* mcu, QString name ) : McuModule( mcu, name ) {
    m_numPins = 0;
...
void McuPort::pinChanged( uint8_t pinMask, uint8_t val ) // Pin number in pinMask
{
    ...
    if ( pinState == m_pinState )
        return;
    m_pinState = pinState;

    if ( ( m_intMask & pinMask ) && m_interrupt )
        m_interrupt->raise(); // Pin change interrupt

    if ( m_sleeping )
        return;
    if ( m_inAddr )
        *m_inReg = m_pinState; //m_mcu->writeReg( m_inAddr, m_pinState, false ); // Write to RAM
}
...
void McuPort::outChanged( uint8_t val ) {
    ...
    *m_outReg = val;
    ...
    for ( int i = 0; i < m_numPins; ++i ) {
        if ( changed & ( 1 << i ) )
            m_pins[i]->setPortState( val & ( 1 << i ) ); // Pin changed
    }
}
...
void McuPort::dirChanged( uint8_t val ) {
    ...
    if ( m_dirInv )
        val = ~val; // defaul: 1 for outputs, inverted: 0 for outputs (PICs)

    for ( int i = 0; i < m_numPins; ++i ) {
        if ( changed & 1 << i )
            m_pins[i]->setDirection( val & ( 1 << i ) ); // Pin changed
    }
}
...
void McuPort::intChanged( uint8_t val ) {
    if ( m_intBits.reg )
        m_intMask = getRegBitsVal( val, m_intBits );
    else
        m_intMask = val;
}
// Direct Control---------------------------------------------------
...
uint McuPort::getInpState() {
    uint data = 0;
    for ( int i = 0; i < m_numPins; ++i )
        if ( m_pins[i] && m_pins[i]->getInpState() )
            data |= ( 1 << i );
    return data;
}
...
void McuPort::createPins( Mcu* mcuComp, QString pins, uint32_t pinMask ) {
    ...
    if ( m_numPins ) {
        ...
        for ( int i = 0; i < m_numPins; ++i ) {
            if ( pinMask & 1 << i )
                m_pins[i] = createPin( i, mcuComp->getUid() + "-" + m_name + QString::number( i ),
                                       mcuComp ); //new McuPin( this, i, m_name+QString::number(i), mcuComp );
        }
    } else {
        QStringList pinList = pins.split( "," );
        pinList.removeAll( "" );
        for ( QString pinName : pinList ) {
            McuPin* pin = createPin( m_numPins, mcuComp->getUid() + "-" + m_name + pinName,
                                     mcuComp ); //new McuPin( this, i, m_name+pinName, mcuComp );
...
QStringList McuPort::registerScript( asIScriptEngine* engine ) {
    ...
    engine->RegisterObjectMethod( "McuPort", "void controlPort( bool o, bool d )",
                                  asMETHODPR( McuPort, controlPort, (bool, bool), void ), asCALL_THISCALL );

    memberList << "setDirection( uint direction )";
    engine->RegisterObjectMethod( "McuPort", "void setDirection( uint d )",
                                  asMETHODPR( McuPort, setDirection, ( uint ), void ), asCALL_THISCALL );

    memberList << "getInpState()";
    engine->RegisterObjectMethod( "McuPort", "uint getInpState()", asMETHODPR( McuPort, getInpState, (), uint ),
                                  asCALL_THISCALL );

    memberList << "setOutState( uint state )";
    engine->RegisterObjectMethod( "McuPort", "void setOutState(uint s)",
                                  asMETHODPR( McuPort, setOutState, ( uint ), void ), asCALL_THISCALL );
