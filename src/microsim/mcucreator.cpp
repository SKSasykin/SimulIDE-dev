...
int McuCreator::processFile( QString fileName ) {
    ...
    if ( tagName != "mcu" && tagName != "iou" && tagName != "mpu" && tagName != "parts" ) {
        ...
        while ( !node.isNull() ) {
            ...
            if ( tagName == "mcu" || tagName == "mpu" || tagName == "iou" || tagName == "parts" )
                break;
    ...
    if ( root.hasAttribute( "core" ) ) {
        ...
        if ( root.hasAttribute( "data" ) )
            createDataMem( root.attribute( "data" ).toUInt( 0, 0 ) );
        if ( root.hasAttribute( "prog" ) )
            createProgMem( root.attribute( "prog" ).toUInt( 0, 0 ) );
        if ( root.hasAttribute( "progword" ) )
            mcu->m_wordSize = root.attribute( "progword" ).toUInt( 0, 0 );
        if ( root.hasAttribute( "progpage" ) )
            mcu->m_pgmPage = root.attribute( "progpage" ).toUInt( 0, 0 );
        if ( root.hasAttribute( "eeprom" ) )
            createRomMem( root.attribute( "eeprom" ).toUInt( 0, 0 ) );
        if ( root.hasAttribute( "inst_cycle" ) )
            mcu->setInstCycle( root.attribute( "inst_cycle" ).toDouble() );
        if ( root.hasAttribute( "cpu_cycle" ) )
            mcu->m_cPerTick = root.attribute( "cpu_cycle" ).toDouble();
        if ( root.hasAttribute( "freq" ) )
            m_mcuComp->m_uiFreq = root.attribute( "freq" ).toDouble();
    }
    ...
    while ( !node.isNull() ) {
        ...
        if ( part == "regblock" )
            createRegisters( &el );
        else if ( part == "datablock" )
            createDataBlock( &el );
        else if ( part == "progblock" )
            createProgBlock( &el );
        else if ( part == "stack" ) {
            m_stackEl = el;
            m_newStack = true;
        } else if ( part == "interrupts" )
            createInterrupts( &el );
        else if ( part == "port" )
            createMcuPort( &el );
        else if ( part == "mcuport" )
            createMcuPort( &el );
        else if ( part == "ioport" )
            createIoPort( &el );
        else if ( part == "timer" )
            ...
            createOcm( &el );
        else if ( part == "ccpunit" )
            createCcpUnit( &el );
        else if ( part == "msspunit" )
            createMsspUnit( &el );
        else if ( part == "usart" )
            createUsart( &el );
        else if ( part == "twi" )
    ...
    if ( root.hasAttribute( "core" ) ) {
        if ( m_core == "AVR" )
            mcu->m_cpu = new AvrCore( mcu );
        else if ( m_core == "Pic12" )
            mcu->m_cpu = new Pic12Core( mcu );
        else if ( m_core == "Pic14" )
            mcu->m_cpu = new Pic14Core( mcu );
        else if ( m_core == "Pic14e" )
            mcu->m_cpu = new Pic14eCore( mcu );
        else if ( m_core == "8051" )
            mcu->m_cpu = new I51Core( mcu );
        else if ( m_core == "6502" )
            mcu->m_cpu = new Mcs65Cpu( mcu );
        else if ( m_core == "Z80" )
            mcu->m_cpu = new Z80Core( mcu );
        else if ( m_core == "Z80ULA" )
            mcu->m_cpu = new ULA_ZX48k( mcu );
        else if ( m_core == "scripted" ) {
            ScriptCpu* cpu = new ScriptCpu( mcu );
            mcu->m_cpu = cpu;
            m_mcuComp->m_scripted = true;

            if ( root.attribute( "linkable" ) == "true" || root.attribute( "linker" ) == "true" )
            ...
            while ( !node.isNull() ) {
                ...
                if ( part == "propertygroup" ) // Properties in .mcu file
                {
                    ...
                    if ( pg ) {
                        for ( ComProperty* p : mcuProps )
                            m_mcuComp->addProperty( group, p );
                    } else
                        m_mcuComp->addPropGroup( { group, mcuProps, 0 } );

                    if ( subC ) {
                        pg = subC->getPropGroup( group );
                        if ( pg ) {
                            for ( ComProperty* p : subProps )
                                subC->addProperty(
                                    group, p /*, false*/ ); // If false properties are saved but not set at circuit open
                        } else
                            subC->addPropGroup( { group, subProps, 0 } /*, false*/ );
                    }
                }
                node = node.nextSibling();
            }
        ...
        if ( m_console ) {
            mcu->m_cpu->createWatcher();
            mcu->m_cpu->getWatcher()->addConsole();
        }
        if ( m_newStack )
            createStack( &m_stackEl );
    }
...
void McuCreator::createCfgWord( QDomElement* e ) {
    ...
    if ( m_core.startsWith( "Pic14" ) )
        cfgWord = PicConfigWord::createCfgWord( mcu, name, type );
    if ( !cfgWord )
        return;
...
void McuCreator::createProgMem( uint32_t size ) {
    ...
    if ( m_core == "Pic12" )
        mcu->m_progMem.resize( size, 0x0FFF );
    else if ( m_core == "Pic14" )
        mcu->m_progMem.resize( size, 0x3FFF );
    else if ( m_core == "Pic14e" )
        mcu->m_progMem.resize( size, 0x3FFF );
    else
...
void McuCreator::createEeprom( QDomElement* e ) {
    ...
    if ( m_core == "AVR" )
        eeprom = new AvrEeprom( mcu, eepromName );
    else if ( m_core == "Pic14" )
        eeprom = new PicEeprom( mcu, eepromName );
    else if ( m_core == "Pic14e" )
        eeprom = new PicEeprom( mcu, eepromName );
    else
        return;
...
void McuCreator::createDataBlock( QDomElement* d ) {
    ...
    if ( d->hasAttribute( "mapto" ) )
        mapTo = d->attribute( "mapto" ).toUInt( 0, 0 );

    for ( int i = datStart; i <= datEnd; ++i ) {
        if ( mcu->m_addrMap.at( i ) == 0xFFFF )
            mcu->m_addrMap.at( i ) = mapTo; // Don't overwrite previous values
        mapTo++;
    }
    getRegisters( d );
}
...
void McuCreator::createRegisters( QDomElement* e ) {
    ...
    if ( regStart < mcu->m_regStart )
        mcu->m_regStart = regStart;
    ...
    for ( int i = regStart; i <= regEnd; ++i )
        if ( mcu->m_addrMap.at( i ) == 0xFFFF )
            mcu->m_addrMap.at( i ) = i; // Don't overwrite previous values
...
void McuCreator::getRegisters( QDomElement* e, uint16_t offset ) {
    QString stReg;
    if ( e->hasAttribute( "streg" ) )
        stReg = e->attribute( "streg" );
            ...
            if ( regAddr >= mcu->m_ramSize ) {
                ...
                if ( !wMask.isEmpty() ) {
                    if ( regAddr >= mcu->m_regMask.size() ) {
                        ...
                        mcu->m_regMask.at( regAddr ) = wMask.toUInt( 0, 2 );
                }
...
void McuCreator::createIntOsc( QDomElement* p ) {
    ...
    if ( m_core.startsWith( "Pic14" ) )
        intOsc = PicIntOsc::createIntOsc( mcu, name, type );
    else if ( m_core == "AVR" )
        intOsc = new AvrIntOsc( mcu, "intOsc" );
    else
...
void McuCreator::createMcuPort( QDomElement* p ) {
    ...
    McuPort* port;
    if ( m_core == "AVR" )
        port = new AvrPort( mcu, name );
    else if ( m_core == "Pic12" )
        port = new PicPort( mcu, name );
    else if ( m_core == "Pic14" )
        port = new PicPort( mcu, name );
    else if ( m_core == "Pic14e" )
        port = new PicPort( mcu, name );
    else if ( m_core == "8051" )
        port = new I51Port( mcu, name );
    else {
    ...
    if ( p->hasAttribute( "inreg" ) ) // Connect to PORT In Register
    {
        ...
        if ( m_core == "AVR" )
            watchRegister( inAddr, R_WRITE, (AvrPort*) port, &AvrPort::pinRegChanged, mcu );
    } else if ( !Oreg.isEmpty() )
        watchRegNames( Oreg, R_READ, port, &McuPort::readPort, mcu ); // No Input register, read pin states
        ...
        if ( ok ) {
            ...
            if ( mcu->regExist( pullups ) )
                ...
                if ( pullups.startsWith( "!" ) ) {
                    ...
                    watchBitNames( pullups, R_WRITE, port, &McuPort::setAllPullups, mcu );
            }
        }
    }
    ...
    while ( !node.isNull() ) {
        ...
        if ( el.tagName() == "interrupt" ) {
            ...
            if ( el.hasAttribute( "mask" ) ) {
                ...
                port->rstIntMask( !ok );
                if ( ok )
                    port->setIntMask( bits ); // Hardcode Interrupt to Pin
...
void McuCreator::createTimer( QDomElement* t ) {
    ...
    if ( m_core == "8051" )
        timer = new I51Timer( mcu, timerName );
    else if ( m_core == "AVR" )
        timer = AvrTimer::createTimer( mcu, timerName, type );
    else if ( m_core == "Pic12" )
        timer = PicTimer::createTimer( mcu, timerName, type );
    else if ( m_core == "Pic14" )
        timer = PicTimer::createTimer( mcu, timerName, type );
    else if ( m_core == "Pic14e" )
        timer = PicTimer::createTimer( mcu, timerName, type );
    ...
    if ( t->hasAttribute( "counter" ) ) {
        ...
        if ( !highByte.isEmpty() ) {
            ...
            watchRegNames( highByte, R_WRITE, timer, &McuTimer::countWriteH,
                           mcu ); // Low byte triggers read/write operations
    ...
    if ( t->hasAttribute( "topreg0" ) ) /// Still done in AvrTimer16bit
    {
        ...
        if ( list.size() > 1 )
            timer->m_topReg0H = mcu->getReg( list.value( 1 ) );
    }
    setInterrupt( t->attribute( "interrupt" ), timer );
    setPrescalers( t->attribute( "prescalers" ), timer );
    if ( t->hasAttribute( "prselect" ) )
        timer->m_prSelBits = getRegBits( t->attribute( "prselect" ), mcu );
    ...
    while ( !node.isNull() ) {
        ...
        if ( el.tagName() == "ocunit" ) {
            ...
            if ( m_core == "AVR" )
                ocUnit = new AvrOcUnit( mcu, ocName );
            ...
            if ( m_core == "AVR" )
                icUnit = new AvrIcUnit( mcu, icName );
            if ( !icUnit ) {
                node = node.nextSibling();
                continue;
            }
            ...
            if ( !regs.isEmpty() )
                icUnit->m_icRegL = mcu->getReg( regs.value( 0 ) );
            if ( !regs.isEmpty() )
                icUnit->m_icRegH = mcu->getReg( regs.value( 1 ) );
...
void McuCreator::createCcpUnit( QDomElement* c ) {
    ...
    if ( m_core == "Pic14" )
        ccpUnit = new PicCcpUnit( mcu, c->attribute( "name" ), c->attribute( "type" ).toInt() );
    else if ( m_core == "Pic14e" )
        ccpUnit = new PicCcpUnit( mcu, c->attribute( "name" ), c->attribute( "type" ).toInt() );
    if ( !ccpUnit )
        return;
...
void McuCreator::createUsart( QDomElement* u ) {
    ...
    QString core = m_core;
    if ( u->hasAttribute( "core" ) )
        core = u->attribute( "core" );

    McuUsart* usartM;
    if ( core == "8051" )
        usartM = new I51Usart( mcu, name, number );
    else if ( core == "AVR" )
        usartM = new AvrUsart( mcu, name, number );
    else if ( core == "Pic14" )
        usartM = new PicUsart( mcu, name, number );
    else if ( core == "Pic14e" )
        usartM = new PicUsart( mcu, name, number );
    ...
    while ( !node.isNull() ) {
        ...
        if ( el.tagName() == "trunit" ) {
            ...
            if ( type == "tx" ) {
                ...
                if ( regName.isEmpty() )
                    regName = m_txRegName; // Tx and rx using the same register
            ...
            if ( trUnit ) {
                ...
                for ( QString pinName : pinNames )
                    pinList.append( mcu->getIoPin( pinName ) );
...
void McuCreator::createAdc( QDomElement* e ) {
    ...
    if ( m_core == "AVR" )
        adc = AvrAdc::createAdc( mcu, name, type );
    else if ( m_core == "Pic14" )
        adc = PicAdc::createAdc( mcu, name, type );
    else if ( m_core == "Pic14e" )
        adc = PicAdc::createAdc( mcu, name, type );
    if ( !adc )
        return;
    ...
    if ( e->hasAttribute( "dataregs" ) ) {
        ...
        if ( !lowByte.isEmpty() )
            adc->m_ADCL = mcu->getReg( lowByte );
        if ( !highByte.isEmpty() )
            adc->m_ADCH = mcu->getReg( highByte );
    }
...
void McuCreator::createAcomp( QDomElement* e ) {
    ...
    if ( m_core == "AVR" )
        comp = new AvrComp( mcu, name );
    else if ( m_core == "Pic12" )
        comp = PicComp::createComparator( mcu, name, type );
    else if ( m_core == "Pic14" )
        comp = PicComp::createComparator( mcu, name, type );
    else if ( m_core == "Pic14e" )
        comp = PicComp::createComparator( mcu, name, type );
...
void McuCreator::createVref( QDomElement* e ) {
    ...
    if ( m_core == "Pic14" )
        vref = new PicVref( mcu, name );
    else if ( m_core == "Pic14e" )
        vref = new PicVrefE( mcu, name );
    if ( !vref )
        return;
...
void McuCreator::createTwi( QDomElement* e ) {
    ...
    if ( m_core == "AVR" )
        m_twi = new AvrTwi( mcu, name );
    else if ( m_core == "Pic14" )
        m_twi = new PicTwi( mcu, name );
    else if ( m_core == "scripted" ) {
...
void McuCreator::createSpi( QDomElement* e ) {
    ...
    if ( m_core == "AVR" )
        m_spi = new AvrSpi( mcu, name );
    else if ( m_core == "Pic14" )
        m_spi = new PicSpi( mcu, name );
    else if ( m_core == "scripted" ) {
...
void McuCreator::createWdt( QDomElement* e ) {
    ...
    McuWdt* wdt = nullptr;
    if ( m_core == "AVR" )
        wdt = AvrWdt::createWdt( mcu, name, type );
    else if ( m_core == "Pic12" )
        wdt = PicWdt::createWdt( mcu, name, type );
    else if ( m_core == "Pic14" )
        wdt = PicWdt::createWdt( mcu, name, type );
    else if ( m_core == "Pic14e" )
        wdt = PicWdt::createWdt( mcu, name, type );
...
void McuCreator::createSleep( QDomElement* e ) {
    ...
    McuSleep* sleep;
    if ( m_core == "AVR" )
        sleep = new AvrSleep( mcu, name );
    else if ( m_core == "Pic14" )
        sleep = new PicSleep( mcu, name );
    else
        return;
...
void McuCreator::createDisplay( QDomElement* e ) {
    ...
    if ( m_core == "scripted" ) {
        ...
        display = new DisplayArea( width, height, name, nullptr );
    ...
    if ( e->hasAttribute( "margins" ) )
        m_mcuComp->setMargins( e->attribute( "margins" ) );

    bool embed = e->attribute( "embeed" ) == "true";
    display->setEmbed( embed );
    if ( embed )
        m_mcuComp->setBackData( display->getBackData() ); // Display in Package
    else
        m_displays.append( display ); // Display in Mcu Monitor
}
...
void McuCreator::createInterrupt( QDomElement* el ) {
    ...
    if ( m_core == "Pic14" )
        iv = new PicInterrupt( intName, intVector, mcu );
    else if ( m_core == "Pic14e" )
        iv = new Pic14eInterrupt( intName, intVector, mcu );
    else
        iv = new Interrupt( intName, intVector, mcu );
    if ( !iv )
        return;
    ...
    QString enable = el->attribute( "enable" );
    if ( !enable.isEmpty() )
        watchBitNames( enable, R_WRITE, iv, &Interrupt::enableFlag, mcu );
    ...
    if ( ok )
        ...
        watchBitNames( intPrio, R_WRITE, iv, &Interrupt::setPriority, mcu );
    ...
    if ( el->hasAttribute( "clear" ) ) // If clear="1" Clear flag by writting 1 to it
    {
        ...
        if ( val )
            watchRegister( iv->m_flagReg, R_WRITE, iv, &Interrupt::writeFlag, mcu );
    }
...
void McuCreator::setInterrupt( QString intName, McuModule* module ) {
    ...
    Interrupt* inte = mcu->m_interrupts.m_intList.value( intName );
    if ( !inte )
        qDebug() << "    Error: Interrupt not found: " << intName << Qt::endl;
...
void McuCreator::setPrescalers( QString pr, McuPrescaled* module ) {
    ...
    for ( int i = 0; i < prescalers.size(); ++i ) {
        ...
        uint prVal;
        if ( prStr == "EXT_F" )
            prVal = 0x8000; // Ext clock, falling edge
        else if ( prStr == "EXT_R" )
            prVal = 0x8001; // Ext clock, rising edge
        else
            prVal = prStr.toUInt();
