...
ScriptCpu::ScriptCpu( eMcu* mcu ) : ScriptBase( mcu->getId() + "-" + "ScriptCpu" ), Mcu8bits( mcu ) {
    m_watcher = nullptr;
    ...
    m_aEngine->RegisterObjectMethod( "ScriptCpu", "void setPackageSize( int width, int height )",
                                     asMETHODPR( ScriptCpu, setPackageSize, (int, int), void ), asCALL_THISCALL );
    ...
    m_aEngine->RegisterObjectMethod( "ScriptCpu", "void setMargins( int top, int bottom, int right, int left )",
                                     asMETHODPR( ScriptCpu, setMargins, (int, int, int, int), void ), asCALL_THISCALL );

    m_typeWords.insert( "ScriptCpu", memberList );
}
ScriptCpu::~ScriptCpu() {
    if ( m_vChangedCtx )
        m_vChangedCtx->Release();
    if ( m_runEventCtx )
        m_runEventCtx->Release();
    if ( m_extClockCtx )
        m_extClockCtx->Release();
    if ( m_runStepCtx )
        m_runStepCtx->Release();
}
...
void ScriptCpu::initialize() {
    if ( m_initialize )
        callFunction( m_initialize );
}

void ScriptCpu::updateStep() {
    if ( m_updateStep )
        callFunction( m_updateStep );
...
void ScriptCpu::extClock( bool clkState ) {
    ...
    if ( clkState )
        m_mcu->interrupts()->runInterrupts();
}
...
void ScriptCpu::toConsole( string r ) {
    ...
    if ( c )
        c->appendText( QString::fromStdString( r ) );
}
...
ComProperty* ScriptCpu::addProperty( QString name, QString label, QString type, QString unit ) {
    ComProperty* p
        = new ScriptProp<ScriptCpu>( name, label, unit, this, &ScriptCpu::getProp, &ScriptCpu::setProp, type );
...
void ScriptCpu::setProp( ComProperty* p, QString val ) {
    ...
    if ( !asFunc )
        return; // Repeated in command(), create a function for this
    prepare( asFunc );
    if ( type == "string" ) {
        std::string str = val.toStdString();
        m_context->SetArgObject( 0, &str );
        ...
        m_context->SetArgByte( 0, val == "true" );
    else {
        double multiplier = 1;
        ...
        if ( type == "int" )
            m_context->SetArgDWord( 0, val.toInt() * multiplier );
        else if ( type == "uint" )
            m_context->SetArgDWord( 0, val.toUInt() * multiplier );
        else if ( type == "double" )
            m_context->SetArgDouble( 0, val.toDouble() * multiplier );
    }
    execute();
}

void ScriptCpu::addEvent( uint64_t time ) {
    Simulator::self()->addEvent( time, this );
}
void ScriptCpu::cancelEvents() {
    Simulator::self()->cancelEvents( this );
}
uint64_t ScriptCpu::circTime() {
    return Simulator::self()->circTime();
}

int ScriptCpu::readPGM( uint addr ) {
    if ( addr < m_progSize )
        return m_progMem[addr] & m_progWordMask;
    return -1;
}
void ScriptCpu::writePGM( uint addr, int v ) {
    if ( addr < m_progSize )
        m_progMem[addr] = v & m_progWordMask;
}
int ScriptCpu::readRAM( uint addr ) {
    if ( addr <= m_dataMemEnd )
        return m_dataMem[addr];
    return -1;
}
void ScriptCpu::writeRAM( uint addr, int v ) {
    SET_RAM( addr, v );
}
int ScriptCpu::readROM( uint addr ) {
    if ( addr < m_mcu->romSize() )
        return m_mcu->getRomValue( addr );
    return -1;
}
void ScriptCpu::writeROM( uint addr, int v ) {
    if ( addr < m_mcu->romSize() )
        m_mcu->setRomValue( addr, v );
}
...
IoPort* ScriptCpu::getPort( const string portName ) {
    ...
    if ( !port )
        qDebug() << "Error: ScriptCpu::getPort Port" << name << "Doesn't exist";
    return port;
}
...
IoPin* ScriptCpu::getPin( const string pinName ) {
    ...
    if ( !pin )
        qDebug() << "Error: ScriptCpu::getPin Pin" << name << "Doesn't exist";
    return pin;
}
...
McuPort* ScriptCpu::getMcuPort( const string portName ) {
    ...
    if ( !port )
        qDebug() << "Error: ScriptCpu::getMcuPort Port" << name << "Doesn't exist";
    return port;
}
...
McuPin* ScriptCpu::getMcuPin( const string pinName ) {
    ...
    if ( !pin )
        qDebug() << "Error: ScriptCpu::getMcuPin Pin" << name << "Doesn't exist";
    return pin;
}
