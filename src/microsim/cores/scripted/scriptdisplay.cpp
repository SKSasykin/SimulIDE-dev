...
ScriptDisplay::ScriptDisplay( int w, int h, QString name, QWidget* parent )
    : DisplayArea( w, h, name, parent ), ScriptPerif( name ) {
    m_clear = nullptr;

    m_type = "Display";

    m_methods << "setWidth( int pixels )" << "setHeight( int pixels )" << "setBackground( int RGB )"
              << "setPixel( uint x, uint y, int RGB )" << "setData( array<array<int>> &data )"
              << "void fillData( int data )" << "setPalette( array<int> &data )" << "setBGR( bool b )"
              << "drawLine( int x0, int y0, int x1, int y1, int color )" << "clear()";
}
...
void ScriptDisplay::registerScriptMetods(
    asIScriptEngine* engine ) // Static: register Object type and methods only once
{
    engine->RegisterObjectType( "Display", 0, asOBJ_REF | asOBJ_NOCOUNT );

    engine->RegisterObjectMethod( "Display", "void setWidth(int w)",
                                  asMETHODPR( ScriptDisplay, setWidth, ( uint ), void ), asCALL_THISCALL );

    engine->RegisterObjectMethod( "Display", "void setHeight(int h)",
                                  asMETHODPR( ScriptDisplay, setHeight, ( uint ), void ), asCALL_THISCALL );

    engine->RegisterObjectMethod( "Display", "void setBackground(int b)",
                                  asMETHODPR( ScriptDisplay, setBackground, (int), void ), asCALL_THISCALL );

    engine->RegisterObjectMethod( "Display", "void setPixel(uint x, uint y, int color)",
                                  asMETHODPR( ScriptDisplay, setPixel, (uint, uint, int), void ), asCALL_THISCALL );
    ...
    engine->RegisterObjectMethod( "Display", "void setData( array<array<int>> &data )",
                                  asMETHODPR( ScriptDisplay, setData, (CScriptArray*), void ), asCALL_THISCALL );

    engine->RegisterObjectMethod( "Display", "void fillData( int data )",
                                  asMETHODPR( ScriptDisplay, fillData, (int), void ), asCALL_THISCALL );

    engine->RegisterObjectMethod( "Display", "void setPalette( array<int> &data )",
                                  asMETHODPR( ScriptDisplay, setPalette, (CScriptArray*), void ), asCALL_THISCALL );

    engine->RegisterObjectMethod( "Display", "void clear()", asMETHODPR( ScriptDisplay, clear, (), void ),
                                  asCALL_THISCALL );

    engine->RegisterObjectMethod( "Display", "void setBGR(bool b)", asMETHODPR( ScriptDisplay, setBGR, (bool), void ),
                                  asCALL_THISCALL );
}
...
void ScriptDisplay::initialize() {
    if ( m_clear )
        m_scriptCpu->callFunction( m_clear );
}
