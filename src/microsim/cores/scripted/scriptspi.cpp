...
ScriptSpi::ScriptSpi( eMcu* mcu, QString name ) : McuSpi( mcu, name ), ScriptPerif( name ) {
    m_byteReceived = nullptr;

    m_type = "SPI";

    m_methods << "setMode( int mode )" << "sendByte( uint8 byte )";
}
ScriptSpi::~ScriptSpi() { }
...
QStringList ScriptSpi::registerScript( ScriptCpu* cpu ) {
    ...
    engine->RegisterObjectMethod( "SPI", "void setMode(int t)", asMETHODPR( ScriptSpi, setMode, ( spiMode_t ), void ),
                                  asCALL_THISCALL );

    engine->RegisterObjectMethod( "SPI", "void sendByte(uint8 b)", asMETHODPR( ScriptSpi, sendByte, ( uint8_t ), void ),
                                  asCALL_THISCALL );
