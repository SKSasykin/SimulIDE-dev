...
ScriptTwi::ScriptTwi( eMcu* mcu, QString name ) : McuTwi( mcu, name ), ScriptPerif( name ) {
    m_byteReceived = nullptr;
    m_writeByte = nullptr;

    m_type = "TWI";

    m_methods << "setMode( int mode )" << "sendByte( uint8 byte )" << "setAddress( uint8 address )";
}
ScriptTwi::~ScriptTwi() { }
...
QStringList ScriptTwi::registerScript( ScriptCpu* cpu ) {
    ...
    engine->RegisterObjectMethod( "TWI", "void setMode(int t)", asMETHODPR( ScriptTwi, setMode, ( twiMode_t ), void ),
                                  asCALL_THISCALL );

    engine->RegisterObjectMethod( "TWI", "void sendByte(uint8 b)", asMETHODPR( ScriptTwi, sendByte, ( uint8_t ), void ),
                                  asCALL_THISCALL );

    engine->RegisterObjectMethod( "TWI", "void setAddress(uint8 a)",
                                  asMETHODPR( ScriptTwi, setAddress, ( uint8_t ), void ), asCALL_THISCALL );
...
void ScriptTwi::sendByte( uint8_t data ) {
    ...
    if ( isAddr )
        write = ( data & 1 ) == 0; // Sending address for Read or Write?
