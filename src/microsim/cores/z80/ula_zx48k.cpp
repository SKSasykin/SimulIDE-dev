...
int ULA_ZX48k::m_colours[16] = { 0x000000, 0x00007f, 0x7f0000, 0x7f007f, 0x007f00, 0x007f7f, 0x7f7f00, 0x7f7f7f,
                                 0x000000, 0x0000ff, 0xff0000, 0xff00ff, 0x00ff00, 0x00ffff, 0xffff00, 0xffffff };

ULA_ZX48k::ULA_ZX48k( eMcu* mcu ) : Cpu8bits( mcu ), eElement( mcu->getId() + "-Z80Core" ) {
    m_type = ula6c001e7; // default type
    setParameters( m_type );

    m_types = QStringList() // initialization list of types
        << "ULA 5C102E" << "ULA 5C112E" << "ULA 6C001E6" << "ULA 6C001E7" << "ULA 6C011E";
    ...
    mcu->component()->addProperty( QObject::tr( "Main" ),
                                   new StrProp<ULA_ZX48k>( "Type", QObject::tr( "Type" ), m_types.join( "," ), this,
                                                           &ULA_ZX48k::type, &ULA_ZX48k::setType, 0, "enum" ) );

    mcu->component()->addProperty( QObject::tr( "Main" ),
                                   new BoolProp<ULA_ZX48k>( "Screen", QObject::tr( "Screen" ), "", this,
                                                            &ULA_ZX48k::isScreen, &ULA_ZX48k::setScreen, 0 ) );
}
ULA_ZX48k::~ULA_ZX48k() { }
...
{
    int value = -1;
    if ( reg == "Horizontal Counter" )
        value = m_C;
    else if ( reg == "Vertical Counter" )
        value = m_V;
    else if ( reg == "Border Colour" )
        value = m_borderColour;
    else if ( reg == "Data Latch" )
        value = m_dataLatch;
    else if ( reg == "Atr. Data Latch" )
        value = m_attrDataLatch;
    else if ( reg == "Shift Register" )
        value = m_shiftReg;
    return value;
}
...
void ULA_ZX48k::setType( QString type ) // Setter for type
{
    ...
    if ( m_type == ula6c011e )
        m_mcu->setFreq( 14110000 ); // ULA with NTSC video standard - clock 14.11 MHz
    else
        m_mcu->setFreq( 14000000 ); // ULA with PAL  video standard - clock 14 MHz
    setParameters( m_type );
}
...
void ULA_ZX48k::setScreen( bool s ) {
    ...
    if ( s )
        m_display->setSize( 320, 240 ); // setOffset( 32+32, 60+2 );
    else
        m_display->setSize( 448, 312 ); // setOffset( 0, 0 );
}
...
void ULA_ZX48k::updateStep() {
    ...
    if ( m_isSrceen ) { // 320x240 screen, no beam
        ...
        if ( m_C > 1 && m_V > 1 )
            m_display->setPixel( m_C - 1, m_V - 1, Qt::black );
        if ( m_C > 1 )
            m_display->setPixel( m_C - 1, m_V, Qt::black );
        if ( m_C > 1 && m_V < 311 )
            m_display->setPixel( m_C - 1, m_V + 1, Qt::black );
        if ( m_V > 1 )
            m_display->setPixel( m_C, m_V - 1, Qt::black );
        if ( m_V < 311 )
            m_display->setPixel( m_C, m_V + 1, Qt::black );
        if ( m_C < 447 && m_V > 1 )
            m_display->setPixel( m_C + 1, m_V - 1, Qt::black );
...
void ULA_ZX48k::voltChanged() {
    ...
    if ( tclka != m_tclka ) {
        m_tclka = tclka;
        if ( !m_tclka && ( m_C & 0x020 ) == 0x020 )
            m_C += 63; // Upper Counter Stage Test Clock (Figure 10-3 and Figure 23-1)
        clk7FallingEdge();
    }
    bool tclkb = tclk || !m_rdn || m_wrn; // Test signal TCLKB (Figure 23-1)
    if ( tclkb != m_tclkb ) {
        m_tclkb = tclkb;
        if ( !m_tclkb && ( m_V & 0x100 ) == 0x100 )
            m_flashClock++; // Upper Counter Stage Test Clock (Figure 10-3 and Figure 23-1)
...
void ULA_ZX48k::clk7RisingEdge() {
    ...
    if ( vidCas != m_vidCas ) {
        ...
        if ( m_type == ula6c001e7 || m_type == ula6c011e )
            ...
            delay = 30000; // Delay of video CAS for ULA6C001e-6
        m_casPin->scheduleState( !m_cpuCas && !m_vidCas, delay );
    }
}
...
void ULA_ZX48k::portIO() {
    ...
    if ( portRd != m_portRd ) {
        ...
        if ( portRd )
            ...
            if ( m_micTapePin->getInpState() )
                state |= 1 << 6; // Copy MIC/TAPE input  to data bus (Figure 19-5)
    ...
    if ( portWr != m_portWr ) {
        ...
        if ( !portWr ) {
            ...
            if ( m_type == ula5c102e || m_type == ula5c112e )
                ...
                m_micTapePin->setVoltage( m_micSp6CTable[index] ); // Set MIC/TAPE output (Figure 19-6)
...
void ULA_ZX48k::updateVideo() {
    if ( ( m_C & 0x007 )
         == 0x005 ) { // Load attribute output latch and shift register when video is enabled (Figure 14-2)
        ...
        if ( videoEn ) { // Border or attribute multiplexer (Figure 12-6 and Figure 12-8)
            m_attrOutLatchInk = m_attrDataLatch
                & 0x07; // Load attribute output latch from attribute data latch (Figure 12-6 and Figure 12-10)
            ...
            if ( m_attrOutLatchFlBr & 0x10
                 && m_flashClock & 0x10 ) // Invert pixel stream when flash is enabled (Figure 12-9)
                m_shiftReg
                    ^= 0xff; // The whole pixel stream is inverted instead of inverting pixel by pixel (Figure 12-9)
        } else {
    ...
    if ( m_C == m_hSyncFirst )
        m_evenScanLine = m_V & 0x001; // Even scan line (Figure 16-12)
    bool burst = m_C >= 384 && m_C <= 399; // Colour burst (Table 16-4);
    bool vSync = m_V >= m_vSyncFirst
        && m_V <= m_vSyncLast; // Vertical sync NTSC (Figure 11-6 and Table 11-3) and PAL (Figure 11-5 and Table 11-2)

    m_border = m_C >= 256 || m_V >= 192; // Border (Figure 11-5, Table 11-1 and 11-2)
    uint8_t FlBrGRB;
    if ( hBlank || vSync )
        FlBrGRB = 0; // Blanking video signals (Figure 12-10)
    else
        FlBrGRB = ( m_shiftReg & 0x80 ) ? m_attrOutLatchInk
                                        : m_attrOutLatchPaper; // Colour Ink or Paper (Figure 12-6 and Figure 12-10)
    ...
    if ( m_type == ula6c011e ) {
        ...
        if ( m_evenScanLine )
            ...
            m_vPin->setVoltage(
                burst ? 3.112 : m_vInvTable[FlBrGRB & 0x07] ); // Set PAL inverted chrominance output V (Table 16-3)
    }
}
...
void ULA_ZX48k::readVideoData() {
    ...
    if ( ae ) {
        ...
        case 0x008:
            dma = ( m_C & 0x0f8 ) >> 3 | ( m_V & 0x018 ) << 2;
            m_dmaPort->scheduleState( dma, 20000 );
            break; // Row address (Figure 15-6 and Figure 15-7)
        case 0x009:
            dma = ( m_V & 0x0c0 ) >> 2 | ( m_V & 0x007 ) << 1 | ( m_V & 0x020 ) >> 5;
            m_dmaPort->scheduleState( dma, 20000 );
            break; // Display column address (Figure 15-6 and Figure 15-7)
        case 0x00b:
            dma = 0x30 | ( m_V & 0x0e0 ) >> 5;
            m_dmaPort->scheduleState( dma, 20000 );
            break; // Attribute column address (Figure 15-6 and Figure 15-7)
        }
        ...
        if ( vidSignal != m_vidCas ) {
            ...
            if ( ( m_type == ula6c001e7 ) || ( m_type == ula6c011e ) )
                delay = ( m_C & 0x002 )
                    ? 58000
                    : 78000; // Delay of first video CAS activation is 78 ns and delay of the rest of edges is 53 ns for ULA6C001e-7
            else
                delay = ( m_C & 0x002 )
                    ? 30000
                    : 50000; // Delay of first video CAS activation is 50 ns and delay of the rest of edges is 30 ns for ULA6C001e-6
            m_casPin->scheduleState( !m_cpuCas && !m_vidCas, delay );
        }
...
*/
            ...
            if ( m_C & 0x001 ) {
                if ( m_C & 0x002 )
                    m_attrDataLatch
                        = m_dPort
                              ->getInpState(); // Load attribute data latch (Figure 12-6, Figure 12-8 and Figure 14-2)
                else
                    m_dataLatch = m_dPort->getInpState(); // Load data latch (Figure 12-2, Figure 12-7 and Figure 14-2)
            }
        }
    }
}
...
void ULA_ZX48k::generatePhicpu() {
    ...
    if ( !m_border && ( m_C & 0x00c ) != 0x000 ) {
        ...
        case ula5c102e:
            memContention
                = m_a14 && !m_a15 && !m_mreqT23 && !m_iorqTW3 && m_cpuClk; // Memory contention signal (Figure 18-7)
            ioContention = !m_cpuClk && !m_iorqTW3 && ( m_C & 0x00e ) != 0x008
                && !m_iorqn; // I/O contention signal (Figure 18-7)
            break;
        case ula5c112e:
            memContention = ( !m_iorqn || ( m_a14 && !m_a15 ) ) && !m_mreqT23 && !m_iorqTW3
                && m_cpuClk; // Memory contention signal (Figure 18-12)
            ioContention = !m_cpuClk && !m_iorqTW3 && ( m_C & 0x00e ) != 0x008
                && !m_iorqn; // I/O contention signal (Figure 18-12)
            break;
        default:
            memContention = ( !m_iorqn || ( m_a14 && !m_a15 ) ) && !m_mreqT23 && !m_iorqTW3
                && m_cpuClk; // Memory contention signal (Figure 18-15)
