...
Z80Core::Z80Core( eMcu* mcu ) : Cpu8bits( mcu ), eElement( mcu->getId() + "-Z80Core" ) {
    ...
    m_producers = QStringList() << "Zilog" << "NEC" << "ST";

    mcu->component()->addPropGroup(
        { QObject::tr( "Cpu" ),
          {
              ...
              new StrProp<Z80Core>( "Producer", QObject::tr( "Producer" ), m_producers.join( "," ), this,
                                    &Z80Core::producer, &Z80Core::setProducer, 0, "enum" ),

              new BoolProp<Z80Core>( "CMOS", QObject::tr( "CMOS" ), "", this, &Z80Core::cmos, &Z80Core::setCmos ),

              new BoolProp<Z80Core>( "Single cycle I/O", QObject::tr( "Single cycle I/O" ), "", this, &Z80Core::ioWait,
                                     &Z80Core::setIoWait ),

              new BoolProp<Z80Core>( "Int_Vector", QObject::tr( "Interrupt Vector 0xFF" ), "", this,
                                     &Z80Core::intVector, &Z80Core::setIntVector ),
          },
          0 } );
}
...
int Z80Core::getIntReg( QString reg ) // Called by Mcu Monitor to get Integer values
{
    ...
    if ( reg == "IY" )
        value = regPairs[rIY];
    if ( reg == "AF'" )
        value = regAAlt << 8 | regFAlt;
    if ( reg == "BC'" )
        value = regsAlt[rB] << 8 | regsAlt[rC];
    if ( reg == "DE'" )
        value = regsAlt[rD] << 8 | regsAlt[rE];
    if ( reg == "HL'" )
        value = regsAlt[rH] << 8 | regsAlt[rL];
    if ( reg == "IR" )
        value = regI << 8 | regR;
    if ( reg == "SP" )
        value = regSP;
...
QString Z80Core::getStrReg( QString reg ) // Called by Mcu Monitor to get String values
{
    QString value = "";
    if ( reg == "IFF1" )
        value = IFF1 ? "enabled" : "disabled";
    if ( reg == "IFF2" )
        value = IFF2 ? "enabled" : "disabled";
    if ( reg == "TState" )
        value = QString::number( sm_TState ) + " / " + QString::number( mc_TStates );
    if ( reg == "MCycle" )
        value = QString::number( sm_MCycle ) + " / " + QString::number( mc_MCycles );
    if ( reg == "M1_Type" )
    ...
    if ( reg == "Bus_HighZ" )
        value = highImpedanceBus ? "high Z" : "low Z";
...
QString Z80Core::getStrInst() {
    ...
    if ( m_iSet == noPrefix ) {
        ...
        case 0x00:
            ...
            if ( m_iReg == 0x08 )
                strInst = "EX AF,AF'";
            if ( m_iReg == 0x10 )
                strInst = "DJNZ " + getStrReg8();
            if ( m_iReg == 0x18 )
                strInst = "JR " + getStrReg8();
            if ( ( m_iReg & 0x27 ) == 0x20 )
                strInst = "JR " + getStrFlag( ( m_iReg >> 3 ) & 0x03 ) + "," + getStrReg8();
            if ( ( m_iReg & 0x0f ) == 0x01 )
                strInst = "LD " + getStrRegPair( ( m_iReg >> 4 ) & 0x03 ) + "," + getStrReg16( ( m_iReg >> 4 ) & 0x03 );
            if ( ( m_iReg & 0x0f ) == 0x09 )
                strInst
                    = "ADD " + getStrRegPair( ( m_iReg >> 4 ) & 0x03 ) + "," + getStrRegPair( ( m_iReg >> 4 ) & 0x03 );
            if ( ( m_iReg & 0x2f ) == 0x02 )
                strInst = "LD (" + getStrRegPair( ( m_iReg >> 4 ) & 0x01 ) + "),A";
            if ( ( m_iReg & 0x2f ) == 0x0a )
                strInst = "LD A,(" + getStrRegPair( ( m_iReg >> 4 ) & 0x01 ) + ")";
            if ( m_iReg == 0x22 )
                strInst = "LD (" + getStrReg16() + ")," + getStrRegPair( ( m_iReg >> 4 ) & 0x01 );
            if ( m_iReg == 0x2a )
                strInst = "LD " + getStrRegPair( ( m_iReg >> 4 ) & 0x01 ) + ",(" + getStrReg16() + ")";
            if ( m_iReg == 0x32 )
                strInst = "LD (" + getStrReg16() + "),A";
            if ( m_iReg == 0x3a )
                strInst = "LD A,(" + getStrReg16() + ")";
            if ( ( m_iReg & 0x0f ) == 0x03 )
                strInst = "INC " + getStrRegPair( ( m_iReg >> 4 ) & 0x03 );
            if ( ( m_iReg & 0x0f ) == 0x0b )
                strInst = "DEC " + getStrRegPair( ( m_iReg >> 4 ) & 0x03 );
            if ( ( m_iReg & 0x07 ) == 0x04 )
                strInst = "INC " + getStrReg( ( m_iReg >> 3 ) & 0x07 );
            if ( ( m_iReg & 0x07 ) == 0x05 )
                strInst = "DEC " + getStrReg( ( m_iReg >> 3 ) & 0x07 );
            if ( ( m_iReg & 0x07 ) == 0x06 )
                strInst = "LD " + getStrReg( ( m_iReg >> 3 ) & 0x07 ) + "," + getStrReg8();
            if ( m_iReg == 0x07 )
                strInst = "RLCA";
            if ( m_iReg == 0x0f )
                strInst = "RRCA";
        ...
        case 0x40:
            if ( m_iReg != 0x76 )
                strInst = "LD " + getStrReg( ( m_iReg >> 3 ) & 0x07 ) + "," + getStrReg( m_iReg & 0x07 );
            else
                strInst = "HALT";
            break;
        case 0x80:
            mathOp = ( m_iReg >> 3 ) & 0x07;
            strInst = getStrMathOp( mathOp ) + " ";
            if ( mathOp == 0 || mathOp == 1 || mathOp == 3 )
                strInst += "A,";
            strInst += getStrReg( m_iReg & 0x07 );
            break;
        case 0xc0:
            if ( ( m_iReg & 0x07 ) == 0x00 )
                strInst = "RET " + getStrFlag( ( m_iReg >> 3 ) & 0x07 );
            if ( ( m_iReg & 0x0f ) == 0x01 )
                strInst = "POP " + getStrRegPair( ( m_iReg >> 4 ) & 0x03, true );
            if ( m_iReg == 0xc9 )
                strInst = "RET";
            if ( m_iReg == 0xd9 )
                strInst = "EXX";
            if ( m_iReg == 0xe9 )
                strInst = "JP (" + getStrRegPair( 2 ) + ")";
            if ( m_iReg == 0xf9 )
                strInst = "LD SP," + getStrRegPair( 2 );
            if ( ( m_iReg & 0x07 ) == 0x02 )
                strInst = "JP " + getStrFlag( ( m_iReg >> 3 ) & 0x07 ) + "," + getStrReg16();
            if ( m_iReg == 0xc3 )
                strInst = "JP " + getStrReg16();
            if ( m_iReg == 0xcb )
                strInst = "PREFIX CB";
            if ( m_iReg == 0xd3 )
                strInst = "OUT (" + getStrReg8() + "),A";
            if ( m_iReg == 0xdb )
                strInst = "IN A,(" + getStrReg8() + ")";
            if ( m_iReg == 0xe3 )
                strInst = "EX (SP)," + getStrRegPair( 2 );
            if ( m_iReg == 0xeb )
                strInst = "EX DE,HL";
            if ( m_iReg == 0xf3 )
                strInst = "DI";
            if ( m_iReg == 0xfb )
                strInst = "EI";
            if ( ( m_iReg & 0x07 ) == 0x04 )
                strInst = "CALL " + getStrFlag( ( m_iReg >> 3 ) & 0x07 ) + "," + getStrReg16();
            if ( ( m_iReg & 0x0f ) == 0x05 )
                strInst = "PUSH " + getStrRegPair( ( m_iReg >> 4 ) & 0x03, true );
            if ( m_iReg == 0xcd )
                strInst = "CALL " + getStrReg16();
            if ( m_iReg == 0xdd )
                strInst = "PREFIX IX";
            if ( m_iReg == 0xed )
                strInst = "PREFIX ED";
            if ( m_iReg == 0xfd )
                strInst = "PREFIX IY";
            if ( ( m_iReg & 0x07 ) == 0x06 )
                strInst = getStrMathOp( ( m_iReg >> 3 ) & 0x07 ) + " " + getStrReg8();
            if ( ( m_iReg & 0x07 ) == 0x07 )
                strInst = "RST " + QString::number( m_iReg & 0x38 );
            break;
        }
    }
    ...
    if ( m_iSet == prefixCB ) {
        ...
        case 0x00:
            ...
            if ( XYState == rHL )
                ...
                strInst = "LD " + getStrReg( m_iReg & 0x07 ) + "," + strInst + getStrReg( 6 );
            break;
        case 0x40:
            strInst = "BIT " + QString::number( ( m_iReg >> 3 ) & 0x07 ) + "," + getStrReg( m_iReg & 0x07 );
            break;
        case 0x80:
            if ( XYState == rHL )
                strInst = "RES " + QString::number( ( m_iReg >> 3 ) & 0x07 ) + "," + getStrReg( m_iReg & 0x07 );
            else
                strInst = "LD " + getStrReg( m_iReg & 0x07 ) + ",RES " + QString::number( ( m_iReg >> 3 ) & 0x07 )
                    + getStrReg( 6 );
            break;
        case 0xc0:
            if ( XYState == rHL )
                strInst = "SET " + QString::number( ( m_iReg >> 3 ) & 0x07 ) + "," + getStrReg( m_iReg & 0x07 );
            else
                strInst = "LD " + getStrReg( m_iReg & 0x07 ) + ",SET " + QString::number( ( m_iReg >> 3 ) & 0x07 )
                    + getStrReg( 6 );
            break;
        }
    }
    ...
    if ( m_iSet == prefixED ) {
        ...
        case 0x40:
            ...
            if ( ( m_iReg & 0x07 ) == 0x00 )
                strInst = "IN " + getStrReg( ( m_iReg >> 3 ) & 0x07 ) + "(C)";
            if ( m_iReg == 0x71 ) {
                strInst = ( m_cmos == false ) ? "OUT (C),0" : "OUT (C),FF";
                break;
            }
            if ( ( m_iReg & 0x07 ) == 0x01 )
                strInst = "OUT (C)," + getStrReg( ( m_iReg >> 3 ) & 0x07 );
            if ( ( m_iReg & 0x0f ) == 0x02 )
                strInst = "SBC HL," + getStrRegPair( ( m_iReg >> 4 ) & 0x03 );
            if ( ( m_iReg & 0x0f ) == 0x0a )
                strInst = "ADC HL," + getStrRegPair( ( m_iReg >> 4 ) & 0x03 );
            if ( ( m_iReg & 0x0f ) == 0x03 )
                strInst = "LD (" + getStrReg16( 5 ) + ")," + getStrRegPair( ( m_iReg >> 4 ) & 0x03 );
            if ( ( m_iReg & 0x0f ) == 0x0b )
                strInst = "LD " + getStrRegPair( ( m_iReg >> 4 ) & 0x03 ) + ",(" + getStrReg16( 5 ) + ")";
            if ( ( m_iReg & 0x07 ) == 0x04 )
                strInst = "NEG";
            if ( m_iReg == 0x4d ) {
                strInst = "RETI";
        ...
        case 0x00:
        case 0xc0:
            strInst = "Illegal prefix ED instruction";
            break;
        }
    }
...
QString Z80Core::getStrReg( uint8_t reg ) {
    ...
    case 3:
        ...
        break;
    case 4:
        strReg = XYState == rIX ? "XH" : ( XYState == rIY ? "YH" : "H" );
        break;
    case 5:
        strReg = XYState == rIX ? "XL" : ( XYState == rIY ? "YL" : "L" );
        break;
    case 6:
        if ( XYState == rHL )
            strReg = "(HL)";
        break; //?? tady se musí doplnit ještě IX+d a IY+d
        if ( XYState == rIX )
            strReg = "(IX";
        if ( XYState == rIY )
            strReg = "(IY";
        ...
        default:
            offset = static_cast<int>( regWZ ) - static_cast<int>( regPairs[XYState] );
            strReg += ( offset >= 0 ) ? "+" + QString::number( offset, 16 ).toUpper()
...
QString Z80Core::getStrRegPair( uint8_t reg, bool set ) {
    ...
    case 1:
        ...
        break;
    case 2:
        strRegPair = XYState == rIX ? "IX" : ( XYState == rIY ? "IY" : "HL" );
        break;
    case 3:
        if ( set )
            strRegPair = "AF";
...
QString Z80Core::getStrReg8() {
    ...
    default:
        strReg8 = QString::number( sDI, 16 ).toUpper();
        break;
    case 2:
        strReg8 = "??";
    }
...
QString Z80Core::getStrReg16( uint8_t reg ) {
    ...
    case 1:
        ...
        break;
    case 2:
        value = XYState == rIX ? regPairs[rIX] : ( XYState == rIY ? regPairs[rIY] : regPairs[rHL] );
        break;
    case 3:
        value = regSP;
        break;
    ...
    default:
        strReg16 = QString::number( value, 16 ).toUpper();
        break;
    case 2:
        strReg16 = "????";
        break;
    case 3:
        strReg16 = "??" + QString::number( value & 0xff, 16 ).toUpper();
        break;
    }
...
void Z80Core::runEvent() {
    if ( m_nextClock )
        fallingEdgeDelayed(); // Falling edge
    else
        risingEdgeDelayed(); // Rising edge
}
...
void Z80Core::clkRisingEdge() // Execution of instruction and sampling bus signal at clock rising edge
{
    ...
    if (
        !m_resetPin
             ->getInpState() ) { // if reset input is active, then count TStates until it reachs value 2, then set flag normalReset
        if ( !normalReset ) { // the flag normalReset is not set yet
            if ( rstCount == 2 )
                normalReset = true; // set flag normalReset
            else
                rstCount++; // increase TStates counter for reset
        }
        ...
        if ( normalReset ) { // the flag normalReset is still set
            if ( rstCount > 0 )
                rstCount--; // decrease TStates counter for reset
            else {
                reset();
                normalReset = false;
            } // when TStates counter for reset reach 0 reset CPU
        } else
            rstCount = 0; // restart TStates counter for reset when the reset pulse is shorter than 3 TStates
    }
    // Increase counter TStates after interrupt (it is limited to Z80CORE_MAX_T_INT)
    if ( sm_TStatesAfterInt < Z80CORE_MAX_T_INT )
        sm_TStatesAfterInt++;
    ...
    if ( !normalReset && ( !sBusReq || !sBusAck ) ) {
        ...
        if ( sm_lastTState
             && !sBusReq ) // is executed at last TState, but after bus request and release when it is inserted
        ...
        if ( sm_TState == 3 && sm_MCycle == 1 )
            opCodeFetch(); // Op Code Fetch at rising edge of TState 3
...
void Z80Core::nextTState() {
    if ( sm_TState != 2 || sWait == false ) // Hold in TState 2 when Wait signal is actived
    {
        if ( sm_lastTState ) // If the TState is the last state then it is necessary to increase machine cycle
        {
            ...
            if ( sBusReq )
                sBusAck = true; // The bus is release when it is required by signal BusReq
            else {
                sm_TState = 1; // reset TState to first state

                if (
                    mc_StateMachine
                    == sXYOffsetFetch ) // set machine cycle to IX/IY Address offset fetch cycle (Machine cycle number 6)
                {
                    ...
                    mc_StateMachine = sNone; // reset flag
                            ...
                            || mc_StateMachine
                                == sXYFetchFinish ) // last IX/IY fetch cycle is 7, but it can be finish earlier by flag
                {
                    sm_MCycle = sm_PreXYMCycle
                        + 1; // restore machine cycle to cycle before IX/IY Address offset fetch cycle plus one
                    mc_StateMachine = sNone; // reset flag
                } else if ( sm_MCycle == mc_MCycles ) // Last Machine cycle of instruction
                {
                    ...
                    if ( sm_M1CycleType != tHalt )
                        sm_M1CycleType
                            = tOpCodeFetch; // machine cycle Op Code Fetch only when the previous Machine cycle is not halt machine cycle

                    if (
                        specialReset ) { // if the special reset is required then the next machine cycle is special reset cycle
                    ...
                    if (
                        NMIFF
                        && mc_prefix
                            == noPrefix ) // Non maskable interrupt only when instruction is completed with all prefixes
                    {
                        NMIFF = false;
                        ...
                        sInt && IFF1
                        && mc_prefix
                            == noPrefix ) // Interrupt only when instruction is completed with all prefixes and interrupt is enabled
                    {
            ...
            if ( sm_waitTState == false ) {
                ...
                if ( m_ioWait && sm_TState == 2
                     && ( mc_busOp == oIORead || mc_busOp == oIOWrite || mc_busOp == oIntAck ) )
                    sm_autoWait = 1; // set number of inserted wait states to 1
                if (
                    sm_TState == 2
                    && !m_resetPin
                            ->getInpState() ) //  when the RESET signal is set at rising edge of TState 2 the special reset will be executed
...
void Z80Core::risingEdgeDelayed() {
    ...
    {
        if ( highImpedanceBus )
            releaseBus( false ); // If bus is in high impedance then set bus to low impedance

        switch (
            sm_TState ) // Setting bus at clock rising edge of TState 1 (it might repeat when wait states are inserted during interrupt)
        {
        case 1:
            if ( !sm_waitTState ) {
                m_addrPort->setOutStatFast( sAO );
                if ( m_lastBusOp == oMemWrite || m_lastBusOp == oIOWrite )
                    m_dataPort->setPinMode( input ); // If previous bus op. was write data then release data bus
                if ( mc_busOp == oM1 || mc_busOp == oIntAck )
                    m_m1Pin->setOutStatFast( false ); // If current bus op. is Fetch (machine cycle 1) then set M1
                if ( m_lastBusOp == oM1 || m_lastBusOp == oIntAck )
                    m_rfshPin->setOutStatFast(
                        true ); // If previous bus op. was Fetch (machine cycle 1) then signal RFSH is reset
            }
            break;
        case 2:
            if (
                sm_waitTState
                == false ) // Setting bus at clock rising edge of TState 2 (it might repeat when wait states are inserted)
            {
        ...
        case 3:
            if (
                mc_busOp == oM1
                || mc_busOp
                    == oIntAck ) // If current bus op. is Fetch (machine cycle 1) then reset M1, set RFSH and update address bus
            {
                m_m1Pin->setOutStatFast( true );
            ...
            if ( mc_busOp == oIntAck )
                m_iorqPin->setOutStatFast( true ); // If current bus op. is Fetch with interrupt then reset IORQ
            break;
            // Setting bus at clock rising edge of TState 5 in case of longer machine cycle than 4 TStates
        case 5:
            if ( mc_busOp == oM1
                 || mc_busOp
                     == oIntAck ) // If bus op. is Fetch (machine cycle 1) then address bus is restored and reset RFSH
            {
                m_addrPort->setOutStatFast( sAO );
                m_rfshPin->setOutStatFast( true );
            }
...
void Z80Core::fallingEdgeDelayed() {
    if ( sBusAck
         == false ) { // Setting bus only when bus is not requested by signal BUSRQ and acknowledged by signal BUSACK
        switch ( sm_TState ) {
            // Setting bus at clock rising edge of TState 1 (it might repeat when wait states are inserted during interrupt)
        case 1:
            if ( sm_waitTState == false ) {
                if ( mc_busOp
                     == oM1 ) { // If  current bus op. is Fetch without interrupt (machine cycle 1) then set MREQ and M1
                ...
                if ( mc_busOp == oMemWrite )
                    m_mreqPin->setOutStatFast( false ); // If  current bus op. is write memory then set MREQ
                if ( mc_busOp == oMemWrite
                     || mc_busOp == oIOWrite ) { // If current bus op. is write memory or I/O then set data bus
        ...
        case 2:
            if (
                ...
                if ( mc_busOp == oIntAck )
                    m_iorqPin->setOutStatFast( false );
                if ( mc_busOp == oMemWrite )
                    m_wrPin->setOutStatFast( false ); // If  current bus op. is write memory then set WR
            }
            break;
        case 3:
            if ( mc_busOp == oMemRead
                 || mc_busOp == oIORead ) // Sampling data bus for Memory read and Input/Output read cycle
        ...
        case 4:
            if ( mc_busOp == oM1 || mc_busOp == oIntAck )
                m_mreqPin->setOutStatFast( true ); // If current bus op. is Fetch (machine cycle 1) then reset MREQ
            /// create Halt variable, state or whatever
            if ( m_iReg == 0x76 && m_iSet == noPrefix )
                m_haltPin->setOutStatFast( false ); // If  current instruction is HALT then set HALT
            if ( sNMI || ( sInt && IFF1 ) )
                m_haltPin->setOutStatFast( true ); // If interrupt is requested and enabled then reset HALT
        }
    } /// confusion betwen sBusAck and sBusReq:
    else if ( !sBusReq )
        m_busacPin->setOutStatFast(
            true ); // If bus is not requested any more by BUSRQ signal then signal BUSACK is reset
}
...
void Z80Core::opCodeFetch() {
    ...
    case noPrefix:
        XYState = rHL;
        m_iSet = noPrefix;
        break; // no prefix - register pair HL and basic instruction set
    case prefixCB:
        m_iSet = prefixCB;
        break; // prefix CB - no change register pair and instruction set CB
    case prefixED:
        XYState = rHL;
        m_iSet = prefixED;
        break; // prefix ED - register pair HL and instruction set ED
    case prefixIX:
        XYState = rIX;
        m_iSet = noPrefix;
        break; // prefix IX - register pair IX and basic instruction set
    case prefixIY:
        XYState = rIY;
        m_iSet = noPrefix;
        break; // prefix IY - register pair IY and basic instruction set
    }

    switch ( sm_M1CycleType ) // Fetching opcode
    {
    case tOpCodeFetch:
        m_iReg = m_dataPort->getInpState(); // reading opcode from data bus
        m_PC++; // increase program counter PC
    ...
    case tInt:
        switch ( intMode ) {
        case 0:
            m_iReg = m_dataPort->getInpState();
            break; // from data bus
        case 1:
            m_iReg = 0xFF;
            break; // RST 38H
        case 2:
            m_iReg = 0x00; // NOP for IM2 has 5 machine cycles and 5 TStates
...
void Z80Core::runMCode() {
    ...
    if ( m_iSet == noPrefix ) // Execute instructions without prefix
        ...
        case 0x00:
            if ( sm_M1CycleType == tInt && intMode == 2 )
                int_im2(); // NOP
            if ( sm_M1CycleType == tNMI )
                rst( 0x66 ); // When NMI is actived the function at address 0x0066 is called
            break;
        case 0x01:
            ld_rr_imm( regPairs[rBC] );
            break; // LD BC,NN
        case 0x02:
            ld_indir_r( regPairs[rBC], regA );
            break; // LD (BC),A
        case 0x03:
            regPairs[rBC]++;
            break; // INC BC
        ...
        case 0x06:
            ...
            break; // LD B,N
        case 0x07:
            shiftOp<iRlc, false>( regA );
            break; // RLCA
        case 0x08:
            ex_rr_rr( regPairs[rAF], regAAlt, regFAlt );
            break; // EX AF,AF'
        case 0x09:
            mathOp16<iAdd>( regPairs[XYState], regPairs[rBC] );
            break; // ADD HL,BC  ADD IX,BC  ADD IY,BC
        case 0x0A:
            ld_r_indir( regA, regPairs[rBC] );
            break; // LD A,(BC)
        case 0x0B:
            regPairs[rBC]--;
            break; // DEC BC
        ...
        case 0x10: {
            ...
            break; // DJNZ N
        case 0x11:
            ld_rr_imm( regPairs[rDE] );
            break; // LD DE,NN
        case 0x12:
            ld_indir_r( regPairs[rDE], regA );
            break; // LD (DE),A
        case 0x13:
            regPairs[rDE]++;
            break; // INC DE
        ...
        case 0x16:
            ...
            break; // LD D,N
        case 0x17:
            shiftOp<iRl, false>( regA );
            break; // RLA
        case 0x18:
            jr( true );
            break; // JR N
        case 0x19:
            mathOp16<iAdd>( regPairs[XYState], regPairs[rDE] );
            break; // ADD HL,DE  ADD IX,DE  ADD IY,DE
        case 0x1A:
            ld_r_indir( regA, regPairs[rDE] );
            break; // LD A,(DE)
        case 0x1B:
            regPairs[rDE]--;
            break; // DEC DE
        ...
        case 0x1E:
            ...
            break; // LD E,N
        case 0x1F:
            shiftOp<iRr, false>( regA );
            break; // RRA
        case 0x20:
            jr( !regF.getZ() );
            break; // JR NZ,N
        case 0x21:
            ld_rr_imm( regPairs[XYState] );
            break; // LD HL,NN  LD IX,NN  LD IY,NN
        case 0x22:
            ld_mem_rr( regPairs[XYState] );
            break; // LD (NN),HL  LD (NN),IX  LD (NN),IY
        case 0x23:
            regPairs[XYState]++;
            break; // INC HL  INC IX  INC IY
        case 0x24:
            mathOp<iInc>( regs[XYState] );
            break; // INC H  INC XH  INC YH
        case 0X25:
            mathOp<iDec>( regs[XYState] );
            break; // DEC H  DEC XH  DEC YH
        case 0x26:
            ld_r_imm( regs[XYState] );
            break; // LD H,N  LD XH,N  LD YH,N
        case 0x27:
            mathOp<iDaa>( regA );
            break; // DAA
        case 0x28:
            ...
            break; // JR Z,N
        case 0x29:
            mathOp16<iAdd>( regPairs[XYState], regPairs[XYState] );
            break; // ADD HL,HL  ADD IX,IX  ADD IY,IY
        case 0x2A:
            ld_rr_mem( regPairs[XYState] );
            break; // LD HL,(NN)  LD IX,(NN)  LD IY,(NN)
        case 0x2B:
            regPairs[XYState]--;
            break; // DEC HL  DEC IX  DEC IY
        case 0x2C:
            mathOp<iInc>( regs[XYState + 1] );
            break; // INC L  INC XL  INC YL
        case 0X2D:
            mathOp<iDec>( regs[XYState + 1] );
            break; // DEC L  DEC XL  DEC YL
        case 0x2E:
            ld_r_imm( regs[XYState + 1] );
            break; // LD L,N  LD XL,N  LD YL,N
        case 0x2F:
            mathOp<iCpl>( regA );
            break; // CPL
        case 0x30:
        ...
        case 0x36:
            ld_indirXY_imm();
            break; // LD (HL),N  LD (IX+d),N  LD (IY+d),N
        case 0x37:
            regF.setC();
            flagsScfCcf();
            break; // SCF
        case 0x38:
            ...
            break; // JR C,N
        case 0x39:
            mathOp16<iAdd>( regPairs[XYState], regSP );
            break; // ADD HL,SP  ADD IX,SP  ADD IY,SP
        case 0x3A:
            ld_r_mem( regA );
            break; // LD A,(NN)
        case 0x3B:
        ...
        case 0x43:
            ...
            break; // LD B,E
        case 0x44:
            regs[rB] = regs[XYState];
            break; // LD B,H  LD B,XH  LD B,YH
        case 0x45:
            regs[rB] = regs[XYState + 1];
            break; // LD B,L  LD B,XL  LD B,YL
        case 0x46:
            ld_r_indirXY( regs[rB] );
            break; // LD B,(HL)  LD B,(IX+d)  LD B,(IY+d)
        case 0x47:
            regs[rB] = regA;
            break; // LD B,A
        case 0x48:
        ...
        case 0x4B:
            ...
            break; // LD C,E
        case 0x4C:
            regs[rC] = regs[XYState];
            break; // LD C,H  LD C,XH  LD C,YH
        case 0x4D:
            regs[rC] = regs[XYState + 1];
            break; // LD C,L  LD C,XL  LD C,YL
        case 0x4E:
            ld_r_indirXY( regs[rC] );
            break; // LD C,(HL)  LD C,(IX+d)  LD C,(IY+d)
        case 0x4F:
            regs[rC] = regA;
            break; // LD C,A
        case 0x50:
        ...
        case 0x53:
            ...
            break; // LD D,E
        case 0x54:
            regs[rD] = regs[XYState];
            break; // LD D,H  LD D,XH  LD D,YH
        case 0x55:
            regs[rD] = regs[XYState + 1];
            break; // LD D,L  LD D,XL  LD D,YL
        case 0x56:
            ld_r_indirXY( regs[rD] );
            break; // LD D,(HL)  LD D,(IX+d)  LD D,(IY+d)
        case 0x57:
            regs[rD] = regA;
            break; // LD D,A
        case 0x58:
        ...
        case 0x5B:
            break; // LD E,E
        case 0x5C:
            regs[rE] = regs[XYState];
            break; // LD E,H  LD E,XH  LD E,YH
        case 0x5D:
            regs[rE] = regs[XYState + 1];
            break; // LD E,L  LD E,XL  LD E,YL
        case 0x5E:
            ld_r_indirXY( regs[rE] );
            break; // LD E,(HL)  LD E,(IX+d)  LD E,(IY+d)
        case 0x5F:
            regs[rE] = regA;
            break; // LD E,A
        case 0x60:
            regs[XYState] = regs[rB];
            break; // LD H,B  LD XH,B  LD YH,B
        case 0x61:
            regs[XYState] = regs[rC];
            break; // LD H,C  LD XH,C  LD YH,C
        case 0x62:
            regs[XYState] = regs[rD];
            break; // LD H,D  LD XH,D  LD YH,D
        case 0x63:
            regs[XYState] = regs[rE];
            break; // LD H,E  LD XH,E  LD YH,E
        case 0x64:
            break; // LD H,H  LD XH,XH  LD XH,YH
        case 0x65:
            regs[XYState] = regs[XYState + 1];
            break; // LD H,L  LD XH,XL  LD XH,YL
        case 0x66:
            ld_r_indirXY( regs[rH] );
            break; // LD H,(HL)  LD H,(IX+d)  LD H,(IY+d)
        case 0x67:
            regs[XYState] = regA;
            break; // LD H,A  LD XH,A  LD YH,A
        case 0x68:
            regs[XYState + 1] = regs[rB];
            break; // LD L,B  LD XL,B  LD YL,B
        case 0x69:
            regs[XYState + 1] = regs[rC];
            break; // LD L,C  LD XL,B  LD YL,B
        case 0x6A:
            regs[XYState + 1] = regs[rD];
            break; // LD L,D  LD XL,B  LD YL,B
        case 0x6B:
            regs[XYState + 1] = regs[rE];
            break; // LD L,E  LD XL,B  LD YL,B
        case 0x6C:
            regs[XYState + 1] = regs[XYState];
            break; // LD L,H  LD XL,XH  LD YL,YH
        case 0x6D:
            break; // LD L,L  LD XL,XL  LD YL,YL
        case 0x6E:
            ld_r_indirXY( regs[rL] );
            break; // LD L,(HL)  LD L,(IX+d)  LD L,(IY+d)
        case 0x6F:
            regs[XYState + 1] = regA;
            break; // LD L,A  LD XL,A  LD YL,A
        case 0x70:
            ld_indirXY_r( regs[rB] );
            break; // LD (HL),B  LD (IX+d),B  LD (IY+d),B
        case 0x71:
            ld_indirXY_r( regs[rC] );
            break; // LD (HL),C  LD (IX+d),C  LD (IY+d),C
        case 0x72:
            ld_indirXY_r( regs[rD] );
            break; // LD (HL),D  LD (IX+d),D  LD (IY+d),D
        case 0x73:
            ld_indirXY_r( regs[rE] );
            break; // LD (HL),E  LD (IX+d),E  LD (IY+d),E
        case 0x74:
            ld_indirXY_r( regs[rH] );
            break; // LD (HL),H  LD (IX+d),H  LD (IY+d),H
        case 0x75:
            ld_indirXY_r( regs[rL] );
            break; // LD (HL),L  LD (IX+d),L  LD (IY+d),L
        case 0x76:
            mc_StateMachine = sHalt;
            break; // HALT
        case 0x77:
            ld_indirXY_r( regA );
            break; // LD (HL),A  LD (IX+d),A  LD (IY+d),A
        case 0x78:
            regA = regs[rB];
            break; // LD A,B
        case 0x79:
        ...
        case 0x7C:
            regA = regs[XYState];
            break; // LD A,H  LD A,XH  LD A,YH
        case 0x7D:
            regA = regs[XYState + 1];
            break; // LD A,L  LD A,XL  LD A,YL
        case 0x7E:
            ld_r_indirXY( regA );
            break; // LD A,(HL)  LD A,(IX+d)  LD A,(IY+d)
        case 0x7F:
            break; // LD A,A
        case 0x80:
            mathOp<iAdd>( regA, regs[rB] );
            break; // ADD A,B
        case 0x81:
            mathOp<iAdd>( regA, regs[rC] );
            break; // ADD A,C
        case 0x82:
            mathOp<iAdd>( regA, regs[rD] );
            break; // ADD A,D
        case 0x83:
            mathOp<iAdd>( regA, regs[rE] );
            break; // ADD A,E
        case 0x84:
            mathOp<iAdd>( regA, regs[XYState] );
            break; // ADD A,H  ADD A,XH  ADD A,YH
        case 0x85:
            mathOp<iAdd>( regA, regs[XYState + 1] );
            break; // ADD A,L  ADD A,XL  ADD A,YL
        case 0x86:
            inst_indirXY<iAdd>();
            break; // ADD A,(HL)  ADD A,(IX+d)  ADD A,(IY+d)
        case 0x87:
            mathOp<iAdd>( regA, regA );
            break; // ADD A,A
        case 0x88:
            mathOp<iAdc>( regA, regs[rB] );
            break; // ADC A,B
        case 0x89:
            mathOp<iAdc>( regA, regs[rC] );
            break; // ADC A,C
        case 0x8A:
            mathOp<iAdc>( regA, regs[rD] );
            break; // ADC A,D
        case 0x8B:
            mathOp<iAdc>( regA, regs[rE] );
            break; // ADC A,E
        case 0x8C:
            mathOp<iAdc>( regA, regs[XYState] );
            break; // ADC A,H  ADC A,XH  ADC A,YH
        case 0x8D:
            mathOp<iAdc>( regA, regs[XYState + 1] );
            break; // ADC A,L  ADC A,XL  ADC A,YL
        case 0x8E:
            inst_indirXY<iAdc>();
            break; // ADC A,(HL)  ADC A,(IX+d)  ADC A,(IY+d)
        case 0x8F:
            mathOp<iAdc>( regA, regA );
            break; // ADC A,A
        case 0x90:
            mathOp<iSub>( regA, regs[rB] );
            break; // SUB B
        case 0x91:
            mathOp<iSub>( regA, regs[rC] );
            break; // SUB C
        case 0x92:
            mathOp<iSub>( regA, regs[rD] );
            break; // SUB D
        case 0x93:
            mathOp<iSub>( regA, regs[rE] );
            break; // SUB E
        case 0x94:
            mathOp<iSub>( regA, regs[XYState] );
            break; // SUB H  SUB XH  SUB YH
        case 0x95:
            mathOp<iSub>( regA, regs[XYState + 1] );
            break; // SUB L  SUB XL  SUB YL
        case 0x96:
            inst_indirXY<iSub>();
            break; // SUB (HL)  SUB (IX+d)  SUB (IY+d)
        case 0x97:
            mathOp<iSub>( regA, regA );
            break; // SUB A
        case 0x98:
            mathOp<iSbc>( regA, regs[rB] );
            break; // SBC A,B
        case 0x99:
            mathOp<iSbc>( regA, regs[rC] );
            break; // SBC A,C
        case 0x9A:
            mathOp<iSbc>( regA, regs[rD] );
            break; // SBC A,D
        case 0x9B:
            mathOp<iSbc>( regA, regs[rE] );
            break; // SBC A,E
        case 0x9C:
            mathOp<iSbc>( regA, regs[XYState] );
            break; // SBC A,H  SBC A,XH  SBC A,YH
        case 0x9D:
            mathOp<iSbc>( regA, regs[XYState + 1] );
            break; // SBC A,L  SBC A,XL  SBC A,YL
        case 0x9E:
            inst_indirXY<iSbc>();
            break; // SBC A,(HL)  SBC A,(IX+d)  SBC A,(IY+d)
        case 0x9F:
            mathOp<iSbc>( regA, regA );
            break; // SBC A,A
        case 0xA0:
            mathOp<iAnd>( regA, regs[rB] );
            break; // AND B
        case 0xA1:
            mathOp<iAnd>( regA, regs[rC] );
            break; // AND C
        case 0xA2:
            mathOp<iAnd>( regA, regs[rD] );
            break; // AND D
        case 0xA3:
            mathOp<iAnd>( regA, regs[rE] );
            break; // AND E
        case 0xA4:
            mathOp<iAnd>( regA, regs[XYState] );
            break; // AND H  AND XH  AND YL
        case 0xA5:
            mathOp<iAnd>( regA, regs[XYState + 1] );
            break; // AND L  AND XL  AND YL
        case 0xA6:
            inst_indirXY<iAnd>();
            break; // AND (HL)  AND (IX+d)  AND (IY+d)
        case 0xA7:
            mathOp<iAnd>( regA, regA );
            break; // AND A
        case 0xA8:
            mathOp<iXor>( regA, regs[rB] );
            break; // XOR B
        case 0xA9:
            mathOp<iXor>( regA, regs[rC] );
            break; // XOR C
        case 0xAA:
            mathOp<iXor>( regA, regs[rD] );
            break; // XOR D
        case 0xAB:
            mathOp<iXor>( regA, regs[rE] );
            break; // XOR E
        case 0xAC:
            mathOp<iXor>( regA, regs[XYState] );
            break; // XOR H  XOR XH  XOR YH
        case 0xAD:
            mathOp<iXor>( regA, regs[XYState + 1] );
            break; // XOR L  XOR XL  XOR YL
        case 0xAE:
            inst_indirXY<iXor>();
            break; // XOR (HL)  XOR (IX+d)  XOR (IY+d)
        case 0xAF:
            mathOp<iXor>( regA, regA );
            break; // XOR A
        case 0xB0:
            mathOp<iOr>( regA, regs[rB] );
            break; // OR B
        case 0xB1:
            mathOp<iOr>( regA, regs[rC] );
            break; // OR C
        case 0xB2:
            mathOp<iOr>( regA, regs[rD] );
            break; // OR D
        case 0xB3:
            mathOp<iOr>( regA, regs[rE] );
            break; // OR E
        case 0xB4:
            mathOp<iOr>( regA, regs[XYState] );
            break; // OR H  OR XH  OR YL
        case 0xB5:
            mathOp<iOr>( regA, regs[XYState + 1] );
            break; // OR L  OR XL  OR YL
        case 0xB6:
            inst_indirXY<iOr>();
            break; // OR (HL)  OR (IX+d)  OR (IY+d)
        case 0xB7:
            mathOp<iOr>( regA, regA );
            break; // OR A
        case 0xB8:
            mathOp<iCp>( regA, regs[rB] );
            break; // CP B
        case 0xB9:
            mathOp<iCp>( regA, regs[rC] );
            break; // CP C
        case 0xBA:
            mathOp<iCp>( regA, regs[rD] );
            break; // CP D
        case 0xBB:
            mathOp<iCp>( regA, regs[rE] );
            break; // CP E
        case 0xBC:
            mathOp<iCp>( regA, regs[XYState] );
            break; // CP H  CP XH  CP YH
        case 0xBD:
            mathOp<iCp>( regA, regs[XYState + 1] );
            break; // CP L  CP XL  CP YL
        case 0xBE:
            inst_indirXY<iCp>();
            break; // CP (HL)  CP (IX+d)  CP (IY+d)
        case 0xBF:
            mathOp<iCp>( regA, regA );
            break; // CP A
        case 0xC0:
            ret( !regF.getZ() );
            break; // RET NZ
        ...
        case 0xD8:
            ...
            break; // RET C
        case 0xD9:
            ex_rr_rr( regPairs[rBC], regsAlt[rB], regsAlt[rC] ); // EXX
        ...
        case 0xE0:
            ...
            break; // RET PO
        case 0xE1:
            pop_rr( regPairs[XYState] );
            break; // POP HL  POP IX  POP IY
        case 0xE2:
            jp( !regF.getP() );
            break; // JP PO,NN
        case 0xE3:
            ex_SP_rr( regPairs[XYState] );
            break; // EX (SP),HL  EX (SP),IX  EX (SP),IY
        case 0xE4:
            call( !regF.getP() );
            break; // CALL PO,NN
        case 0xE5:
            push_rr( regPairs[XYState] );
            break; // PUSH HL  PUSH IX  PUSH IY
        case 0xE6:
            inst_imm<iAnd>();
            break; // AND N
        case 0xE7:
        ...
        case 0xE8:
            ...
            break; // RET PE
        case 0xE9:
            m_PC = regPairs[XYState]; // JP (HL)  JP (IX)  JP (IY)
            sAO = m_PC;
            break;
        case 0xEA:
            jp( regF.getP() );
            break; // JP PE,NN
        case 0xEB:
            ex_rr_rr( regPairs[rDE], regs[rH], regs[rL] );
            break; // EX DE,HL
        case 0xEC:
            call( regF.getP() );
            break; // CALL PE,NN
        ...
        case 0xF8:
            ...
            break; // RET M
        case 0xF9:
            regSP = regPairs[XYState];
            break; // LD SP,HL  LD SP,IX  LP SP,IY
        case 0xFA:
            jp( regF.getS() );
            break; // JP M,NN
        case 0xFB:
    ...
    if ( m_iSet == prefixED )
        switch ( m_iReg ) {
        case 0x40:
            in_r_rr( regs[rB], regPairs[rBC] );
            break; // IN B,(C)
        case 0x41:
            out_rr_r( regPairs[rBC], regs[rB] );
            break; // OUT (C),B
        case 0x42:
            mathOp16<iSbc>( regPairs[rHL], regPairs[rBC] );
            break; // SBC HL,BC
        case 0x43:
            ld_mem_rr( regPairs[rBC] );
            break; // LD (NN),BC
        case 0x44:
            mathOp<iNeg>( regA );
            break; // NEG
        ...
        case 0x47:
            ...
            break; // LD I,A
        case 0x48:
            in_r_rr( regs[rC], regPairs[rBC] );
            break; // IN C,(C)
        case 0x49:
            out_rr_r( regPairs[rBC], regs[rC] );
            break; // OUT (C),C
        case 0x4A:
            mathOp16<iAdc>( regPairs[rHL], regPairs[rBC] );
            break; // ADC HL,BC
        case 0x4B:
            ld_rr_mem( regPairs[rBC] );
            break; // LD BC,(NN)
        case 0x4C:
            mathOp<iNeg>( regA );
            break; // NEG
        ...
        case 0x4F:
            ...
            break; // LD R,A
        case 0x50:
            in_r_rr( regs[rD], regPairs[rBC] );
            break; // IN D,(C)
        case 0x51:
            out_rr_r( regPairs[rBC], regs[rD] );
            break; // OUT (C),D
        case 0x52:
            mathOp16<iSbc>( regPairs[rHL], regPairs[rDE] );
            break; // SBC HL,DE
        case 0x53:
            ld_mem_rr( regPairs[rDE] );
            break; // LD (NN),DE
        case 0x54:
            mathOp<iNeg>( regA );
            break; // NEG
        ...
        case 0x57:
            regA = regI;
            regF.copySYXzeroZ( regA ).resetHN().storeP( IFF2 );
            break; // LD A,I
        case 0x58:
            in_r_rr( regs[rE], regPairs[rBC] );
            break; // IN E,(C)
        case 0x59:
            out_rr_r( regPairs[rBC], regs[rE] );
            break; // OUT (C),E
        case 0x5A:
            mathOp16<iAdc>( regPairs[rHL], regPairs[rDE] );
            break; // ADC HL,DE
        case 0x5B:
            ld_rr_mem( regPairs[rDE] );
            break; // LD DE,(NN)
        case 0x5C:
            mathOp<iNeg>( regA );
            break; // NEG
        ...
        case 0x5F:
            ...
            break; // LD A,R
        case 0x60:
            in_r_rr( regs[rH], regPairs[rBC] );
            break; // IN H,(C)
        case 0x61:
            out_rr_r( regPairs[rBC], regs[rH] );
            break; // OUT (C),H
        case 0x62:
            mathOp16<iSbc>( regPairs[rHL], regPairs[rHL] );
            break; // SBC HL,HL
        case 0x63:
            ld_mem_rr( regPairs[rHL] );
            break; // LD (NN),HL
        case 0x64:
            mathOp<iNeg>( regA );
            break; // NEG
        ...
        case 0x67:
            ...
            break; // RRD
        case 0x68:
            in_r_rr( regs[rL], regPairs[rBC] );
            break; // IN L,(C)
        case 0x69:
            out_rr_r( regPairs[rBC], regs[rL] );
            break; // OUT (C),L
        case 0x6A:
            mathOp16<iAdc>( regPairs[rHL], regPairs[rHL] );
            break; // ADC HL,HL
        case 0x6B:
            ld_rr_mem( regPairs[rHL] );
            break; // LD HL,(NN)
        case 0x6C:
            mathOp<iNeg>( regA );
            break; // NEG
        ...
        case 0x70: {
            uint8_t none;
            in_r_rr( none, regPairs[rBC] );
        } break; // IN (C)
        case 0x71:
            out_rr_r( regPairs[rBC], ( m_cmos == false ) ? 0x00 : 0xff );
            break; // OUT (C),0  OUT(C),FF
        case 0x72:
            mathOp16<iSbc>( regPairs[rHL], regSP );
            break; // SBC HL,SP
        case 0x73:
            ld_mem_rr( regSP );
            break; // LD (NN),SP
        ...
        case 0x76:
            ...
            break; // IM 1
                // 0x77 undocumented
        case 0x78:
            in_r_rr( regA, regPairs[rBC] );
            break; // IN A,(C)
        case 0x79:
            out_rr_r( regPairs[rBC], regA );
            break; // OUT (C),A
        case 0x7A:
            mathOp16<iAdc>( regPairs[rHL], regSP );
            break; // ADC HL,SP
        case 0x7B:
            ld_rr_mem( regSP );
            break; // LD SP,(NN)
...
void Z80Core::ld_r_imm( uint8_t& reg ) //s LD r,N - load register r from byte after instruction
{
    ...
    case 2:
        reg = sDI; // store byte after instruction to register
    }
}

void Z80Core::ld_r_indir(
    uint8_t& reg,
    const Z80RegPair& rp ) //s LD A,(BC) and LD A,(DE) - load register from memory address stored in register pair
{
    switch ( sm_MCycle ) {
    case 1:
        regWZ = rp; // set memory pointer
        readMem( regWZ ); // next bus op. is read byte from memory
        break;
    case 2:
        reg = sDI; // store byte from memory to register
        regWZ++; // increase memory pointer
    }
}

void Z80Core::ld_r_indirXY(
    uint8_t&
        reg ) //s LD r,(HL), LD r,(IX+d) and LD r,(IY+d) - load resister from memory address stored in register pair
{
    switch ( sm_MCycle ) {
    ...
    case 1:
        if ( XYState != rHL ) { // test if it is used register IX or IY
            mc_StateMachine
                = sXYOffsetFetch; // set address offset fetch flag - following machine cycle is machine cycle 6
    ...
    case 6:
        regWZ = regPairs[XYState] + static_cast<signed char>( sDI ); // calculation of memory pointer
...
void Z80Core::ld_r_mem( uint8_t& reg ) // LD A,(NN) - load register from memory addres from bytes after instruction
{
    ...
    case 3:
        regWZ.setHighByte( sDI ); // high byte is stored to high byte of memory pointer
        readMem( regWZ ); // next bus op. is read byte from memory
        break;
        // Read byte from memory and store it to register
    case 4:
...
void Z80Core::ld_indir_r(
    const Z80RegPair& rp,
    const uint8_t& reg ) //s LD (BC),A and LD (DE),A - store register to memory address stored in register pair
{
    switch ( sm_MCycle ) {
    case 1:
        regWZ = rp; // set memory pointer
...
void Z80Core::ld_indirXY_r(
    const uint8_t&
        reg ) //s LD (HL),r, LD (IX+d),r and LD (IY+d),r - store resister r to memory address stored in register pair
{
    switch ( sm_MCycle ) {
        // Inicialization - if the instruction has prefix then the address offset fetch cycle is inserted
    case 1:
        if ( XYState != rHL ) { // test if it is used register IX or IY
            ...
            writeMem( regPairs[rHL], reg );
        break;
    case 2:
        break; // Write byte from register to memory
        // Address offset fetch
    case 6:
        regWZ = regPairs[XYState] + static_cast<signed char>( sDI ); // calculation of memory pointer
        noBusOp( m_PC, 5 ); // next bus op. is no operation
...
void Z80Core::ld_mem_r(
    const uint8_t& reg ) // LD (NN),A - strore register to memory addres from bytes after instruction
{
    switch ( sm_MCycle ) {
    case 1:
        readMem( m_PC++ );
...
void Z80Core::
    ld_indirXY_imm() //s LD (HL),N, LD (IX+d),N and LD (IY+d),N - store byte after instruction to memory address stored in register pair
{
    switch ( sm_MCycle ) {
        // Inicialization - if the instruction has prefix then the address offset fetch cycle is inserted
    case 1:
    ...
    case 2:
        if ( XYState != rHL )
            writeMem( regWZ, sDI ); // test if it is used register IX or IY
        else
            writeMem( regPairs[rHL], sDI ); // set address for address bus
        break;
    case 3:
        break; // Write byte after instruction to memory
        // Address offset fetch
    case 6:
        regWZ = regPairs[XYState] + static_cast<signed char>( sDI ); // calculation of memory pointer
        mc_StateMachine
            = sXYFetchFinish; // set address offset fetch finish flag - next machine cycle is machine cycle 2
        readMem( m_PC++ );
...
void Z80Core::ld_rr_imm( T& rp ) //s LD rr,NN - load register pair rr from two bytes after instruction
{
    ...
    case 2:
        rp.setLowByte( sDI ); // store byte after instruction to low byte of register pair
        readMem( m_PC++ );
        break; // next bus op. is read byte from address PC, increase program counter PC
        // Read byte after instruction and store it to high byte of register pair
    case 3:
        rp.setHighByte( sDI ); // store byte after instruction to high byte of register pair
        break;
    }
}
...
void Z80Core::ld_rr_mem( T& rp ) // LD rr,(NN) - load register pair rr from memory addres from bytes after instruction
{
    ...
    case 1:
        readMem( m_PC++ );
        break; // next bus op. is read byte from memory at address PC
        // Read low byte of address and store it to memory pointer
    case 2:
        regWZ.setLowByte( sDI ); // low byte is stored to low byte of memory pointer
        readMem( m_PC++ );
    ...
    case 4:
        rp.setLowByte( sDI ); // store byte from memory to low byte of register pair
        readMem( regWZ ); // next bus op. is read byte from memory
        break;
        // Read high byte from memory and store it to high byte of register pair
    case 5:
        rp.setHighByte( sDI ); // store byte from memory to high byte of register pair
        break;
    }
}
...
void Z80Core::ld_mem_rr( T& rp ) // LD (NN),rr - store register pair rr to memory addres from bytes after instruction
{
    ...
    case 1:
        ...
        break; // next bus op. is read byte from address PC, increase program counter PC
    case 2:
        regWZ.setLowByte( sDI ); // Read low byte of address and store it to memory pointer
        readMem( m_PC++ );
        break; // next bus op. is read byte from address PC, increase program counter PC
    case 3:
        regWZ.setHighByte( sDI ); // Read high byte of address and store it to memory pointer
        writeMem( regWZ, rp.getLowByte() ); // high byte is stored to high byte of memory pointer
        regWZ++; // increase memory pointer
        break;
    case 4:
        writeMem( regWZ, rp.getHighByte() );
        break; // Write low byte from low byte of register pair to memory
    case 5:
        break; // Write high byte from high byte of register pair to memory
    }
}
...
void Z80Core::push_rr( Z80RegPair& rp ) // PUSH rr - store register pair rr to stack
{
    switch ( sm_MCycle ) {
    case 1:
        pushStack8( rp.getHighByte() );
        break; // decrease SP // Inicialization
    case 2:
        pushStack8( rp.getLowByte() );
        break; // decrease SP // Write low byte from low byte of register pair to stack
    case 3:
        break; // Write high byte from high byte of register pair to stack
    }
}
...
void Z80Core::pop_rr( Z80RegPair& rp ) // POP rr - load register pair rr from stack
{
    ...
    case 2:
        rp.setLowByte( sDI ); // low byte is stored to low byte of register pair
        popStack8();
        break;
...
void Z80Core::ex_rr_rr( Z80RegPair& rp, uint8_t& regH,
                        uint8_t& regL ) //s EX AF,AF', EX DE,HL and EXX - exchange registers
{
    uint8_t tmp;
...
void Z80Core::ex_SP_rr(
    Z80RegPair& rp ) // EX (SP),HL, EX (SP),IX and EX (SP),IY - exchange register pair and last value in stack
{
    switch ( sm_MCycle ) {
    case 1:
        readMem( regSP ); // next bus op. is read low byte from stack
        regSP++; // increase stack pointer
        break;
        // Read low byte from stack and exchange it with low byte of register pair
    case 2:
        regWZ.setLowByte( sDI ); // low byte from stack is stored to low byte of WZ
    ...
    case 3:
        regWZ.setHighByte( sDI ); // high byte from stack is stored to high byte of WZ
    ...
    case 4:
        writeMem( regSP, rp.getLowByte() ); // next bus op. is write high byte to stack
...
template <Z80Core::eMathOp op> inline void Z80Core::inst_indirXY() {
    switch ( sm_MCycle ) {
        // Inicialization - if the instruction has prefix then the address offset fetch cycle is inserted
    case 1:
        if ( XYState != rHL ) { // test if it is used register IX or IY
    ...
    case 2:
        if ( op == iInc || op == iDec ) {
            mathOp<op>( sDI );
            if ( XYState != rHL )
                writeMem( regWZ, sDI ); // test if it is used register IX or IY set address for address bus
            else
                writeMem( regPairs[rHL], sDI ); // set address for address bus
        } else
            mathOp<op>( regA, sDI );
...
template <Z80Core::eMathOp op> inline void Z80Core::inst_imm() {
    switch ( sm_MCycle ) {
        // Inicialization
    case 1:
        readMem( m_PC++ );
        break; // next bus op. is read byte from memory at address PC, increase program counter PC
...
template <Z80Core::eMathOp op, bool changeAllFlags> inline void Z80Core::mathOp( uint8_t& regA, uint8_t reg ) {
    uint8_t carry = regA;
    uint16_t tmp;
    ...
    case iInc:
        regA++;
        carry = carry & ~regA;
        regF.copySYXzeroZ( regA ).overflowHP( carry ).resetN();
        break;
    case iDec:
        regA--;
        carry = ~carry & regA;
        regF.copySYXzeroZ( regA ).overflowHP( carry ).setN();
        break;
    case iAdc:
        regA += regF.getC();
    case iAdd:
        ...
        if ( changeAllFlags )
            regF.copySYXzeroZ( regA ).overflowHPC( carry ).resetN();
        else
            regF.copyYX( regA ).overflowHC( carry ).resetN();
        break;
    case iSbc:
        regA -= regF.getC();
    case iSub:
        regA -= reg;
        carry = ( ~carry & reg ) | ( regA & ( reg | ~carry ) );
        regF.copySYXzeroZ( regA ).overflowHPC( carry ).setN();
        break;
    case iAnd:
        regA &= reg;
        regF.copySYXzeroZparityP( regA ).setH().resetN().resetC();
        break;
    case iXor:
        regA ^= reg;
        regF.copySYXzeroZparityP( regA ).resetHN().resetC();
        break;
    case iOr:
        regA |= reg;
        regF.copySYXzeroZparityP( regA ).resetHN().resetC();
    ...
    case iCp:
        ...
        if ( changeAllFlags )
            regF.copySzeroZ( tmp ).copyYX( reg ).overflowHPC( carry ).setN();
        else
            regF.copySzeroZ( tmp ).copyYX( reg ).overflowHP( carry ).setN();
        break;
    case iCpl:
        regA = ~regA;
        regF.copyYX( regA ).setH().setN();
        break;
    case iNeg:
        regA = ~regA + 1;
        carry |= regA;
        ...
        break;
    case iDaa:
        tmp = regA; // extend accumulator to 16 bits
        if ( !regF.getN() ) { // if flag N is not set the last operation was addition
            // Decimal adjust after addition
            if (
                ( tmp & 0x000f ) > 9
                || regF.getH() ) { // if low nibble > 9 or flag H is set then the number is not valid and will be adjusted
                if ( ( tmp & 0x000f ) > 9 )
                    regF.setH(); // if low nibble > 9 it means that it overflows after addition number 6: set flag H
                else
                    regF.resetH(); // else reset flag H
                tmp += 0x0006; // add to low nibble difference between number 0x09 and 0x10 in hexadecimal
            }
            if (
                ( tmp & 0xfff0 ) > 0x0090
                || regF.getC() ) { // if high nibble> 9 or flag C is set then the number is not valid and will be adjusted
            ...
            if (
                ...
                || regF.getH() ) { // if low nibble > 9 or flag H is set then the number is not valid and will be adjusted
                if ( ( tmp & 0x000f ) > 5 )
                    regF.resetH(); // if low nibble > 5 it does not borrow after subtraction number 6: reset flag H
            ...
            if ( regA > 0x0099 || regF.getC() )
                tmp -= 0x0160; // if number > 0x99 or flag C is set then the number is not valid and will be adjusted
        ...
        if ( ( tmp & 0xff00 ) != 0 )
            regF.setC(); // logical OR overflow or borrow from extended accumulator and flag C
    }
}
...
template <Z80Core::eMathOp op, typename T> inline void Z80Core::mathOp16( Z80RegPair& rp1, T& rp2 ) {
    switch ( sm_MCycle ) {
    case 1:
        noBusOp( m_PC, 4 );
        break; // next bus op. is no operation
        // Addition of low bytes
    case 2:
        regWZ = rp1; // set memory pointer
    ...
    case 3:
        if ( op != iSbc )
            mathOp<iAdc, op != iAdd>(
                rp1.getHighByte(),
                rp2.getHighByte() ); // store value of high byte before addition for overflow calculation
        else
            mathOp<iSbc>( rp1.getHighByte(), rp2.getHighByte() );
        if ( op != iAdd )
            regF.andZ( rp1.getLowByte() == 0 );
        regWZ++; // increase memory pointer
    }
}

template <Z80Core::eShiftOp op, bool changeSZP> inline void Z80Core::shiftOp( uint8_t& reg ) {
    bool carry;
    ...
    case iRlc:
        carry = reg & 0x80;
        reg = ( reg << 1 ) | ( reg >> 7 );
        break; // rotation 1 bit left
    case iRrc:
        carry = reg & 0x01;
        reg = ( reg >> 1 ) | ( reg << 7 );
        break; // rotation 1 bit right
    case iRl:
        carry = reg & 0x80;
        reg = ( reg << 1 ) | regF.getC();
        break; // shift 1 bit left and copy flag C to bit 0
    case iRr:
        carry = reg & 0x01;
        reg = ( reg >> 1 ) | ( regF.getC() << 7 );
        break; // shift 1 bit right and copy flag C to bit 7
    case iSla:
        carry = reg & 0x80;
        reg = ( reg << 1 );
        break; // shift 1 bit left
    case iSra:
        carry = reg & 0x01;
        reg = ( reg >> 1 ) | ( reg & 0x80 );
        break; // shift 1 bit right and keep bit 7
    case iSll:
        carry = reg & 0x80;
        reg = ( reg << 1 ) | 0x01;
        break; // shift 1 bit left and set bit 0
    case iSrl:
        carry = reg & 0x01;
        reg = ( reg >> 1 );
        break; // shift 1 bit right
    default:
        carry = false; // just to prevent compiler warning
    }
    if ( changeSZP )
        regF.copySYXzeroZparityP( reg ).resetHN().storeC( carry );
    else
        regF.copyYX( reg ).resetHN().storeC( carry );
}
...
template <Z80Core::eRotDig op> void Z80Core::rlrd() {
    static uint8_t tmp;
    switch ( sm_MCycle ) {
    case 1:
        regWZ = regPairs[rHL]; // set memory pointer
        readMem( regWZ ); // next bus op. is read byte from memory
        break;
        // Read byte from memory and rotate accumulator
    case 2:
        tmp = regA; // store accumulator for next machine cycle
        if ( op == iLeft )
    ...
    case 3:
        if ( op == iLeft )
            writeMem(
                ...
                ( sDI << 4 )
                    | ( tmp
                        & 0x0f ) ); // copy low nibble of read byte to high nibble and copy low nibble from stored accumulator
        if ( op == iRight )
            writeMem(
                regWZ,
                ( sDI >> 4 )
                    | ( tmp
                        << 4 ) ); // copy high nibble of read byte to low nibble and copy low nibble from stored accumulator
...
void Z80Core::prefix_cb() {
    switch ( sm_MCycle ) {
    case 1:
        mc_prefix = prefixCB; // set prefix flag
        if ( XYState != rHL ) { // test if it is used register IX or IY
...
void Z80Core::instCB( const uint8_t& ireg, uint8_t& reg ) {
    ...
    case 1:
        if ( mc_MCycles == 1 ) { // if instruction has only one machine cycle matematical operation is with register
            instCBOp( ireg, reg );
            if ( ( ireg & 0xc0 ) == 0x40 )
                regF.copyYX( reg ); // instruction BIT
    ...
    case 3:
        reg = sDO; // copy byte from memory also to register
        break;
...
inline void Z80Core::instCBOp( const uint8_t& ireg, uint8_t& reg ) {
    ...
    case 0x00:
        ...
        break;
    case 0x40:
        regF.copySzeroZP( reg & mask( ireg ) ).setH().resetN();
        break; // test bit of register
    case 0x80:
        reg &= ~mask( ireg );
        break; // reset bit of register
    case 0xC0:
        reg |= mask( ireg );
        break; // set bit of register
    }
}
...
void Z80Core::in_r_imm( uint8_t& reg ) // IN r,(N) - load register r from I/O address from byte after instruction
{
    ...
    case 2:
        regWZ = ( reg << 8 ) | sDI; // set memory pointer
        readIO( regWZ ); // next bus op. is read byte from I/O
        break;
    case 3:
        reg = sDI; // store byte from I/O to register
        regWZ++; // increase memory pointer
    }
}

void Z80Core::in_r_rr( uint8_t& reg,
                       const Z80RegPair& rp ) // IN r,(rr) - load register r from I/O address stored in register pair
{
    switch ( sm_MCycle ) {
    case 1:
        regWZ = rp; // set memory pointer
...
void Z80Core::out_imm_r( const uint8_t& reg ) // OUT (N),r - store register r to I/O address from byte after instruction
{
    ...
    case 2:
        ...
        break;
    case 3:
        regWZ.setLowByte( regWZ.getLowByte() + 1 ); // increase memory pointer
    }
}

void Z80Core::out_rr_r( const Z80RegPair& rp,
                        const uint8_t& reg ) // OUT (rr),r - store register r to I/O address stored in register pair
{
    switch ( sm_MCycle ) {
    case 1:
        regWZ = rp; // set memory pointer
...
void Z80Core::jr(
    const bool&
        cond ) // JR N and JR cond,N - if condition cond is true then jump to relative address from byte after instruction
{
    switch ( sm_MCycle ) {
    case 1:
        readMem( m_PC++ );
    ...
    case 2:
        if ( cond )
            noBusOp( m_PC,
                     5 ); // if condition is true then jump to relative address otherwise the instruction is finished
        else
            finishInst(); // set number of machine cycles to 2 - finish instruction
        break;
        // Calculation and set program counter PC
    case 3:
        regWZ = m_PC + static_cast<signed char>( sDI ); // calculation program counter
        jumpWZ();
    }
}
...
void Z80Core::jp(
    const bool&
        cond ) // JP NN and JP cond,NN - if condition cond is true then jump to address from bytes after instruction
{
    switch ( sm_MCycle ) {
    case 1:
        readMem( m_PC++ );
...
void Z80Core::call(
    const bool&
        cond ) // CALL NN and CALL cond,NN - if condition cond is true then store program counter PC to stack and jump to address from bytes after instruction
{
    switch ( sm_MCycle ) {
    case 1:
        readMem( m_PC++ );
    ...
    case 3:
        ...
        if ( cond )
            pushStack8(
                m_PC
                >> 8 ); // if condition is true then store program counter PC to stack and jump to address otherwise the instruction is finished
        else
    ...
    case 4:
        ...
        break; // next bus op. is write byte to stack
...
void Z80Core::rst( const uint8_t& addr ) //s RST NN - store PC to stack and jump to address NN
{
    ...
    case 3:
        regWZ = addr; // set memory pointer
        jumpWZ();
    }
}

void Z80Core::ret(
    const bool& cond ) //s RET and RET cond - if condition cond is true then load program counter PC from stack
{
    switch ( sm_MCycle ) {
    case 1:
        if ( cond )
            popStack8(); // if condition is true then load program counter PC from stack otherwise the instruction is finished
        else
            finishInst(); // set number of machine cycles to 1 - finish instruction
        break;
...
void Z80Core::int_im2() {
    ...
    case 1:
        ...
        // Write high byte of program counter PC to stack
    case 2:
        pushStack8( m_PC & 0xff ); // next bus op. is write byte to stack
        break;
        // Write low byte of program counter PC to stack and calculate addrese in interrupt vector table
    case 3:
        regWZ.setHighByte( regI );
        if ( m_intVector == true )
            regWZ.setHighByte( 0xff );
    ...
    case 4:
        readMem( regWZ ); // next bus op. is read byte from address PC
        regWZ.setLowByte( sDI ); // low byte is stored to low byte of memory pointer
        break;
...
template <Z80Core::eBlock type> inline void Z80Core::ldxx() {
    switch ( sm_MCycle ) {
        // Inicialization and decrease counter of bytes to transfer
    case 1:
        regPairs[rBC]--; // decrease counter of bytes to transfer
        readMem( regPairs[rHL] ); // next bus op. is read byte from memory
        break;
        // Read byte from memory and increase/decrease register with source address
    case 2:
        if ( type == bInc || type == bIncRep ) // if the instruction is LDI or LDIR
    ...
    case 3:
        ...
        if (
            regF.getP() ) { // if counter of bytes to transfer is not zero then repeat instructions LDIR or LDDR or finish instruction LDI or LDD
            if ( type == bIncRep
                 || type == bDecRep ) { // if the instruction is LDIR or LDDR then continue exucuting instruction
                noBusOp( m_PC, 5 ); // next bus op. is no operation
    ...
    case 4:
        m_PC -= 2; // set program counter PC back to this instruction
        regF.copyYX( m_PC >> 8 );
...
// This function is a tempate with parameter bInc, bIncRep, bDec or bDecRep which selects increase or decrease of memory address and repeatibility
template <Z80Core::eBlock type> inline void Z80Core::cpxx() {
    static uint8_t ioq;
    switch ( sm_MCycle ) {
        // Inicialization and decrease counter of bytes to transfer
    case 1:
        regPairs[rBC]--; // decrease counter of bytes to compare
        readMem( regPairs[rHL] ); // next bus op. is read byte from memory
        break;
        // Read byte from memory and compare it with accumulator and increase/decrease register with memory address
    case 2:
        mathOp<iCp, false>( regA, sDI ); // compare read byte and accumulator and set flags
    ...
    case 3:
        ioq = regA - sDI - regF.getH(); // flags calculation
        ...
        if (
            ...
            && ( type == bIncRep
                 || type
                     == bDecRep ) ) { // if byte is not found and counter of bytes to compare is not zero then repeat instructions CPIR or CPDR or finish instruction CPI or CPD
            noBusOp( m_PC, 5 ); // next bus op. is no operation
        } else
            finishInst(); // if byte was found or counter of bytes to transfer is zero then finish instruction
        break;
        // Decrease program counter PC to first byte of this instrucion
    case 4:
        m_PC -= 2; // set program counter PC back to this instruction
...
// This function is a tempate with parameter bInc, bIncRep, bDec or bDecRep which selects increase or decrease of memory address and repeatibility
template <Z80Core::eBlock type> inline void Z80Core::inxx() {
    uint16_t ioq;
    switch ( sm_MCycle ) {
        // Inicialization and decrease counter of bytes to transfer
    case 1:
    ...
    case 2:
        writeMem( regPairs[rHL], sDI ); // next bus op. is write byte to memory
        break;
    ...
    case 3:
        ...
        if (
            !regF.getZ() ) { // if counter of bytes to transfer is not zero then repeat instructions INIR or INDR or finish instruction INI or IND
            if ( type == bIncRep
                 || type == bDecRep ) { // if the instruction is INIR or INDR then continue exucuting instruction
                noBusOp( m_PC, 5 ); // next bus op. is no operation
            }
        } else
            finishInst(); // if counter of bytes to transfer is zero then finish instruction
        break;

        /// Repeated below
        // Decrease program counter PC to first byte of this instrucion
    case 4:
        m_PC -= 2; // set PC back to this instruction
...
// This function is a tempate with parameter bInc, bIncRep, bDec or bDecRep which selects increase or decrease of memory address and repeatibility
template <Z80Core::eBlock type> inline void Z80Core::otxx() {
    uint16_t ioq;
    switch ( sm_MCycle ) {
        // Inicialization and decrease counter of bytes to transfer
    case 1:
    ...
    case 2:
        writeIO( regPairs[rBC], sDI ); // next bus op. is write byte to I/O
        break;
        // Write byte to I/O address, increase/decrease register with memory address and check of termination of instruction
    case 3:
        if ( type == bInc || type == bIncRep ) { // if the instruction is OUTI or OTIR
            regPairs[rHL]++; // increase memory address for transfer
            regWZ++; // increase memory pointer
        }
        ...
        regF.copySYXzeroZ( regs[rB] ).storeHC( ioq >> 8 ).parityP( ( ioq & 0x07 ) ^ regs[rB] ).copyBit7N( sDI );
        if (
            !regF.getZ() ) { // if counter of bytes to transfer is not zero then repeat instructions OTIR or OTDR or finish instruction OUTI or OUTD
            if ( type == bIncRep
                 || type == bDecRep ) { // if the instruction is OTIR or OTDR then continue exucuting instruction
                noBusOp( m_PC, 5 ); // next bus op. is no operation
            }
        } else
            finishInst(); // if counter of bytes to transfer is zero then finish instruction
...
void Z80Core::setCmos( bool cmos ) {
    m_cmos = cmos;
} // Setter for CMOS or NMOS version
void Z80Core::setIoWait( bool ioWait ) {
    m_ioWait = !ioWait;
} // Setter for single wait I/O operation
void Z80Core::setIntVector( bool intVector ) {
