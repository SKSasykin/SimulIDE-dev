...
public:
    ...
    Z80RegPair operator++( int ) {
        Z80RegPair temp = *this;
        uint16_t r16 = *this;
        r16++;
        *this = r16;
        return temp;
    ...
    Z80RegPair& operator=( const uint16_t& word ) {
        high = word >> 8;
        low = word & 0xff;
        return *this;
    }
...
class Z80Reg16 {
public:
    void setLowByte( const uint8_t& byte ) {
        reg &= 0xff00;
        reg |= byte;
    }
    void setHighByte( const uint8_t& byte ) {
        reg &= 0x00ff;
        reg |= ( byte << 8 );
    }
    uint8_t getLowByte() { return reg & 0x00ff; }
    uint8_t getHighByte() { return reg >> 8; }
    Z80Reg16 operator++( int ) {
        Z80Reg16 temp = *this;
        reg++;
        return temp;
    }
    ...
    Z80Reg16& operator=( const uint16_t& word ) {
        reg = word;
        return *this;
    }
...
public:
    ...
    Z80Flags& copyYX( const uint8_t& r ) {
        flags &= ~( fY | fX );
        flags |= r & ( fY | fX );
        changed = true;
        return *this;
    }
    Z80Flags& copyNegCtoH() {
        flags &= ~fH;
        flags |= ( ~flags << 4 ) & fH;
        changed = true;
        return *this;
    }
    Z80Flags& andZ( const bool& z ) {
        if ( !z )
            flags &= ~fZ;
        changed = true;
        return *this;
    }
    Z80Flags& orYX( const uint8_t& r ) {
        flags |= r & ( fY | fX );
        changed = true;
        return *this;
    }
    Z80Flags& storeP( const bool& p ) {
        flags &= ~fP;
        flags |= p << 2;
        changed = true;
        return *this;
    }
    Z80Flags& storeC( const bool& c ) {
        flags &= ~fC;
        flags |= c;
        changed = true;
        return *this;
    }
    Z80Flags& storeHC( const bool& hc ) {
        flags &= ~( fH | fC );
        flags |= hc | ( hc << 4 );
        changed = true;
        return *this;
    }
    Z80Flags& copySzeroZ( const uint8_t& r ) {
        flags &= ~( fS | fZ );
        flags |= r & fS;
        if ( r == 0x00 )
            flags |= fZ;
    ...
    Z80Flags& copySzeroZP( const uint8_t& r ) {
        flags &= ~( fS | fZ | fP );
        flags |= r & fS;
        if ( r == 0x00 )
            flags |= fZ | fP;
    ...
    Z80Flags& copySYXzeroZ( const uint8_t& r ) {
        flags &= ~( fS | fZ | fY | fX );
        flags |= r & ( fS | fY | fX );
        if ( r == 0x00 )
            flags |= fZ;
    ...
    Z80Flags& copySYXzeroZparityP( const uint8_t& r ) {
        flags &= ~( fS | fZ | fY | fX | fP );
        flags |= r & ( fS | fY | fX );
        if ( r == 0x00 )
            flags |= fZ;
        if ( parity( r ) == false )
    ...
    Z80Flags& overflowHP( const uint8_t& r ) {
        flags &= ~( fH | fP );
        if ( ( r & 0x08 ) != 0x00 )
            flags |= fH;
    ...
    Z80Flags& overflowHC( const uint8_t& r ) {
        flags &= ~( fH | fC );
        if ( ( r & 0x08 ) != 0x00 )
            flags |= fH;
        if ( ( r & 0x80 ) != 0x00 )
    ...
    Z80Flags& overflowHPC( const uint8_t& r ) {
        flags &= ~( fH | fP | fC );
        if ( ( r & 0x08 ) != 0x00 )
            flags |= fH;
        if ( ( r & 0xC0 ) == 0x80 || ( r & 0xC0 ) == 0x40 )
    ...
    Z80Flags& parityP( const uint8_t& r ) {
        flags &= ~fP;
        if ( parity( r ) == false )
            flags |= fP;
        changed = true;
    ...
    Z80Flags& copyBit1YX( const uint8_t& r ) {
        flags &= ~( fY | fX );
        flags |= r & fX;
        if ( ( r & 0x02 ) != 0x00 )
            flags |= fY;
    ...
    Z80Flags& copyBit7N( const uint8_t& r ) {
        flags &= ~fN;
        if ( ( r & 0x80 ) != 0x00 )
            flags |= fN;
        changed = true;
    ...
    uint8_t& operator=( const uint16_t& f ) {
        flags = f;
        return flags;
    }
