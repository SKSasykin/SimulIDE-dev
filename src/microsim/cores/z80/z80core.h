...
private:
    ...
    Z80RegPair regPairs[11] = { Z80RegPair( regs[rB], regs[rC] ),
                                Z80RegPair( regs[rB], regs[rC] ),
                                Z80RegPair( regs[rD], regs[rE] ),
                                Z80RegPair( regs[rD], regs[rE] ),
                                Z80RegPair( regs[rH], regs[rL] ),
                                Z80RegPair( regs[rH], regs[rL] ),
                                Z80RegPair( regs[rXH], regs[rXL] ),
                                Z80RegPair( regs[rXH], regs[rXL] ),
                                Z80RegPair( regs[rYH], regs[rYL] ),
                                Z80RegPair( regs[rYH], regs[rYL] ),
    ...
    enum ePrefix { noPrefix = 0, prefixCB, prefixED, prefixIX, prefixIY };
    ...
    inline void readIO( uint16_t addr ) {
        mc_busOp = oIORead;
        sAO = addr;
    }
    inline void writeIO( uint16_t addr, uint8_t data ) {
        mc_busOp = oIOWrite;
        sAO = addr;
        sDO = data;
    }
    inline void noBusOp( uint16_t addr, uint8_t tstates ) {
        mc_busOp = oNone;
        sAO = addr;
        mc_TStates = tstates;
    }
    ...
    template <typename T> inline void ld_rr_imm( T& rp );
    template <typename T> inline void ld_rr_mem( T& rp );
    template <typename T> inline void ld_mem_rr( T& rp );
    ...
    template <eMathOp op> inline void inst_indirXY();
    template <eMathOp op> inline void inst_imm();

    template <eMathOp op, bool changeAllFlags = true> inline void mathOp( uint8_t& regA, uint8_t reg = 0 );

    template <eMathOp op, typename T> inline void mathOp16( Z80RegPair& rp1, T& rp2 );

    enum eShiftOp { iRlc, iRrc, iRl, iRr, iSla, iSra, iSll, iSrl };
    template <eShiftOp op, bool changeSZP = true> inline void shiftOp( uint8_t& reg );

    enum eRotDig { iLeft, iRight };
    template <eRotDig op> inline void rlrd();
    ...
    template <eBlock type> inline void ldxx();
    template <eBlock type> inline void cpxx();
    template <eBlock type> inline void inxx();
    template <eBlock type> inline void otxx();
};
