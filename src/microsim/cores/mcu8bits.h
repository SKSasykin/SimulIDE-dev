...
protected:
    ...
    virtual uint8_t GET_RAM( uint16_t addr ) // All MCUs should use this
    {
        ...
        else if ( addr <= m_dataMemEnd )
            return m_dataMem[addr]; // Read Ram
        return 0;
    }
    ...
    uint16_t GET_REG16_LH( uint16_t addr ) { return m_dataMem[addr] + ( m_dataMem[addr + 1] << 8 ); }
