...
Mcs65Cpu::Mcs65Cpu( eMcu* mcu ) : Mcs65Interface( mcu ) {
...
void Mcs65Cpu::clkFallingEdge() {
    ...
    if ( m_state == cWRITE )
        m_rwPin->scheduleState(
            false, m_tHA ); // Write result and fetch at next cycle //m_busAddr = m_opAddr Done in instruction
    else {
        ...
        {
            if ( m_aMode <= aACCU )
                m_PC--; // If dummy Read then read the same PC
            m_debugPC = m_PC;
            readPGM(); // Read opCode
            m_RET_ADDR = m_PC;
            m_cycle = 0;
...
void Mcs65Cpu::decode() {
    ...
    case 0x20:
        m_EXEC = &Mcs65Cpu::JSR;
        m_aMode = aABSO;
        return; // JSR abs Execution controls PC
    case 0x28:
        m_EXEC = &Mcs65Cpu::PLP;
        return; // PLP
    case 0x38:
    ...
    case 5: {
        ...
        if ( group == 2 && ( Ocode == 4 || Ocode == 5 ) )
            m_aFlags = iY; // Group 2: STX zpg,Y (0x96) & LDX zpg,Y (0x86)
        else
            m_aFlags = iX;
    } break;
    case 6:
...
void Mcs65Cpu::Read() {
    ...
    case aACCU:
        m_op0 = m_Ac;
        break; /// Value = m_Acumulator
    case aIMME:
        m_op0 = readDataBus();
        break; /// Immediate: From Mem 1 byte
    case aABSO: { /// Absolute: address From Mem 2 bytes: LLHH (Little Endian)
        ...
        case 4: {
            if ( m_aFlags & iC ) { // Add carry and read
                ...
                m_op0 = readDataBus(); // Read operation finished
        } break;
        case 5:
            m_op0 = readDataBus();
        }
    ...
    case aZERO: { /// Zero Page
        ...
        case 3:
            m_op0 = readDataBus(); // Finish read operation
        }
    } break;
...
uint8_t Mcs65Cpu::readDataBus() {
    return m_dataBus->getInpState();
}
...
void Mcs65Cpu::pushStack8( uint8_t byte ) {
    m_op0 = byte;
    writeMem( 0x0100 + m_SP-- );
}

void Mcs65Cpu::popStack8() {
    readMem( 0x0100 + ( ++m_SP ) ); /*m_state = cEXEC;*/
}

void Mcs65Cpu::setNZ( uint8_t val ) {
...
void Mcs65Cpu::ADC() {
    ...
    if ( STATUS( D ) ) {
        if ( ( ( m_Ac & 0xF ) + ( m & 0xF ) + ( STATUS( C ) ? 1 : 0 ) ) > 9 )
            tmp += 6;
...
void Mcs65Cpu::BRK() // Break/Interrupt : 1 byte, 7 cycles
{
    ...
    case 4: { // Status -> Stack
        ...
        case 1:
            m_Isr = 0xFFFA;
            status &= ~BREAK;
            break; // NMI (falling edge) - B flag cleared
        case 2:
            m_Isr = 0xFFFE;
            status &= ~BREAK;
            break; // IRQ                 - B flag cleared
        default:
            m_Isr = 0xFFFE;
            status |= BREAK; // Software BRK
        }
        pushStack8( status );
        m_nextState = cEXEC;
    } break;
...
void Mcs65Cpu::CMP() {
    SET_CARRY( m_Ac >= m_op0 );
    setNZ( m_Ac - m_op0 );
}
void Mcs65Cpu::CPX() {
    SET_CARRY( m_rX >= m_op0 );
    setNZ( m_rX - m_op0 );
}
void Mcs65Cpu::CPY() {
    SET_CARRY( m_rY >= m_op0 );
    setNZ( m_rY - m_op0 );
}
...
void Mcs65Cpu::JSR() {
    ...
    case 5:
        ...
        break; // PCL -> Stack
    case 6:
        m_PC = m_opAddr; // Jump to Subroutine
    }
}
...
void Mcs65Cpu::PLP() {
    if ( m_cycle == 2 )
        ...
        *m_STATUS = readDataBus() | CONSTANT | BREAK;
    }
}
...
void Mcs65Cpu::SBC() {
    ...
    if ( STATUS( D ) ) {
        if ( ( ( m_Ac & 0x0F ) - ( STATUS( C ) ? 0 : 1 ) ) < ( m & 0x0F ) )
            tmp -= 6;
        if ( tmp > 0x99 )
            tmp -= 0x60;
    }
