...
PicAdc* PicAdc::createAdc( eMcu* mcu, QString name, int type ) {
    switch ( type ) {
    case 00:
        return new PicAdc00( mcu, name );
        break;
    case 01:
        return new PicAdc01( mcu, name );
        break;
    case 10:
        return new PicAdc10( mcu, name );
        break;
    case 11:
        return new PicAdc11( mcu, name );
        break;
    case 20:
        return new PicAdc20( mcu, name );
        break;
    default:
        return nullptr;
    }
...
PicAdc::PicAdc( eMcu* mcu, QString name ) : McuAdc( mcu, name ) { }
PicAdc::~PicAdc() { }
...
void PicAdc::setAdcClock( uint8_t prs ) {
    ...
    if ( m_adcClock )
        ...
        m_convTime = m_mcu->psInst() * 12 * m_prescList[prs];
}
...
PicAdc00::PicAdc00( eMcu* mcu, QString name ) : PicAdc( mcu, name ) { }
PicAdc00::~PicAdc00() { }
...
void PicAdc00::configureB( uint8_t newADCON1 ) // ADCON1
{
    ...
    if ( mode != m_mode ) {
        ...
        for ( uint i = 0; i < m_adcPin.size(); ++i )
            if ( m_adcPin[i] )
                m_adcPin[i]->setAnalog( analog & ( 1 << i ) );
    }
}
...
PicAdc01::PicAdc01( eMcu* mcu, QString name ) : PicAdc( mcu, name ) { }
PicAdc01::~PicAdc01() { }
...
PicAdc1::PicAdc1( eMcu* mcu, QString name ) : PicAdc( mcu, name ) { }
PicAdc1::~PicAdc1() { }
...
void PicAdc1::updtANSEL() {
    ...
    for ( uint i = 0; i < m_adcPin.size(); ++i )
        if ( m_adcPin[i] )
            m_adcPin[i]->setAnalog( ( analog >> i ) & 1 );
}

void PicAdc1::updtVref() {
    m_vRefP = ( m_mode & 1 ) ? m_pRefPin->getVoltage() : m_mcu->vdd();
    if ( m_nRefPin )
        m_vRefN = ( m_mode & 0b00000010 ) ? m_nRefPin->getVoltage() : 0;
}

//------------------------------------------------------
//-- PIC ADC Type 10 p16F88x ---------------------------

PicAdc10::PicAdc10( eMcu* mcu, QString name ) : PicAdc1( mcu, name ) { }
PicAdc10::~PicAdc10() { }
...
PicAdc11::PicAdc11( eMcu* mcu, QString name ) : PicAdc1( mcu, name ) { }
PicAdc11::~PicAdc11() { }
...
PicAdc20::PicAdc20( eMcu* mcu, QString name ) : PicAdc( mcu, name ) { }
PicAdc20::~PicAdc20() { }
...
void PicAdc20::updtVref() {
    ...
    case 2:
        m_vRefP = m_pRefPin->getVoltage();
        break; // VREF+ is connected to external VREF+ pin
    case 3:
        m_vRefP = m_fvr->getDacVref();
        break; // VREF+ is connected to internal Fixed Voltage Reference (FVR) module
    }
...
PicAdc3::PicAdc3( eMcu* mcu, QString name ) : PicAdc( mcu, name ) { }
PicAdc3::~PicAdc3() { }
