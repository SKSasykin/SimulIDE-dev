...
PicCcpUnit::PicCcpUnit( eMcu* mcu, QString name, int type ) : McuModule( mcu, name ), eElement( name ) {
...
void PicCcpUnit::ccprWriteH( uint8_t val ) {
    if ( m_ccpMode == ccpPWM )
        m_mcu->m_regOverride = *m_ccpRegH; // Read only
    else
        m_comUnit->ocrWriteH( val );
}
...
void PicCcpUnit::configureA( uint8_t CCPxCON ) //
{
    ...
    if ( CCPxM == 0 ) {
        ...
        m_ccpMode = ccpCOM;
        m_comUnit->configure( CCPxM );
    } // Compare Mode (Enhanced):
    else if ( CCPxM < 8 ) {
        m_ccpMode = ccpCAP;
        m_capUnit->configure( CCPxM );
    } // Capture Mode:
    else if ( CCPxM < 12 ) {
        m_ccpMode = ccpCOM;
        m_comUnit->configure( CCPxM );
