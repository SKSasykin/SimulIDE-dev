...
PicPort::PicPort( eMcu* mcu, QString name ) : McuPort( mcu, name ) {
    m_sleepMode = 0xFF;
}
PicPort::~PicPort() { }
...
void PicPort::outChanged( uint8_t val ) {
    ...
    if ( m_interrupt )
        m_interrupt->clearFlag(); // Clear interrupt flag
}
...
void PicPort::readPort( uint8_t ) {
    if ( m_mcu->isCpuRead() )
        ...
        m_mcu->m_regOverride = *m_outReg; // RAM Table reads actual RAM value
