...
Pic14eCore::Pic14eCore( eMcu* mcu ) : PicMrCore( mcu ) {
    m_stackSize = 16;
...
void Pic14eCore::decode( uint16_t instr ) {
    if ( ( instr & 0x3FC0 ) == 0 ) // Miscellaneous instrs
    {
        if ( ( instr & 0x0030 ) == 0 ) {
            if ( instr == 0x0001 )
                reset(); // RESET 00 0000 0000 0001
            else if ( instr == 0x000A )
                CALLW(); // CALLW 00 0000 0000 1010
            else if ( instr == 0x000B )
                BRW(); // BRW   00 0000 0000 1011
            ...
            if ( ( instr & 0x0008 ) == 0 ) {
                ...
                case 0:
                    MOVIW_iF( n );
                    return; // MOVIW ++FSRn 00 0000 0001 0n00
                case 1:
                    MOVIW_dF( n );
                    return; // MOVIW −−FSRn 00 0000 0001 0n01
                case 2:
                    MOVIW_Fi( n );
                    return; // MOVIW FSRn++ 00 0000 0001 0n10
                case 3:
                    MOVIW_Fd( n );
                    return; // MOVIW FSRn−− 00 0000 0001 0n11
                }
            } else {
                switch ( instr & 0x0003 ) {
                case 0:
                    MOVWI_iF( n );
                    return; // MOVWI ++FSRn 00 0000 0001 1n00
                case 1:
                    MOVWI_dF( n );
                    return; // MOVWI −−FSRn 00 0000 0001 1n01
                case 2:
                    MOVWI_Fi( n );
                    return; // MOVWI FSRn++ 00 0000 0001 1n10
                case 3:
                    MOVWI_Fd( n );
                    return; // MOVWI FSRn−− 00 0000 0001 1n11
                }
            }
        } else if ( ( instr & 0x0020 ) > 0 )
            MOVLB( instr & 0x1F ); // MOVLB k 00 0000 001k kkkk
        ...
        case 0x3500:
            LSLF( f, d );
            return; // LSLF   f,d 11 0101 dfff ffff
        case 0x3600:
            LSRF( f, d );
            return; // LSRF   f,d 11 0110 dfff ffff
        case 0x3700:
            ASRF( f, d );
            return; // ASRF   f,d 11 0111 dfff ffff
        case 0x3B00:
            SUBWFB( f, d );
            return; // SUBWFB f,d 11 1011 dfff ffff
        case 0x3D00:
            ADDWFC( f, d );
            return; // ADDWFC f,d 11 1101 dfff ffff
        }
        uint8_t n = instr & 1 << 6;
        // Operations with literal k
        switch ( instr & 0x3F80 ) {
        case 0x3100:
            ADDFSR( n, instr & 0x7F );
            return; // ADDFSR FSRn,k 11 0001 0nkk kkkk
        case 0x3180:
            MOVLP( instr & 0x1F );
            return; // MOVLP       k 11 0001 1kkk kkkk
        case 0x3F00:
            MOVIW( n, instr & 0x7F );
            return; // MOVIW k[FSRn] 11 1111 0nkk kkkk
        case 0x3F80:
            MOVWI( n, instr & 0x7F );
            return; // MOVWI k[FSRn] 11 1111 1nkk kkkk
        }
        if ( ( instr & 0x3C00 ) == 0x3200 ) {
            BRA( instr & 0x1FF );
            return;
