...
PicTwi::PicTwi( eMcu* mcu, QString name ) : McuTwi( mcu, name ) { }
PicTwi::~PicTwi() { }
...
void PicTwi::configureB( uint8_t newSSPCON2 ) {
    ...
    bool newStart = getRegBitsBool( newSSPCON2, m_SEN ) | getRegBitsBool( newSSPCON2, m_RSEN );
    ...
    if ( m_mode == TWI_MASTER ) {
        ...
        if ( rcen ) {
            ...
            if ( m_i2cState == I2C_IDLE )
                masterRead( m_sendACK ); // Read a byte and send ACK/NACK
            if ( m_masterACK && !m_sendACK )
                clearRegBits( m_RW );
        }
    }
}
