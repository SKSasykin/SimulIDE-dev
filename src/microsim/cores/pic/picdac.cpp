...
PicDac::PicDac( eMcu* mcu, QString name ) : McuDac( mcu, name ) { }
PicDac::~PicDac() { }
...
void PicDac::initialize() {
    ...
    if ( m_pins.size() > 0 )
        ...
        qDebug() << "PicDac::configureA: Error: NULL VREF+ Pin";
    ...
    if ( m_pins.size() > 2 )
        m_outPin = m_pins.at( 2 );
    else
        qDebug() << "PicDac::configureA: Error: NULL Out Pin";
}

void PicDac::voltChanged() {
    if ( m_usePinP && m_pRefPin )
        m_vRefP = m_pRefPin->getVoltage();
    if ( m_usePinN && m_nRefPin )
...
void PicDac::configureA( uint8_t newDACCON0 ) //
{
    ...
    if ( m_pRefPin )
        m_pRefPin->changeCallBack( this, m_usePinP );
    if ( m_nRefPin )
        m_nRefPin->changeCallBack( this, m_usePinN );
...
void PicDac::updtOutVolt() {
    ...
    if ( m_enabled )
        m_outVolt = ( m_vRefP - m_vRefN ) * (double) m_outVal / 32 + m_vRefN;
    else
        m_outVolt = m_daclps ? m_vRefP : m_vRefN;

    if ( m_outPin && m_outVoltEn )
        m_outPin->setVoltage( m_outVolt );
}
