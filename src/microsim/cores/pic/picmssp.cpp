...
PicMssp::PicMssp( eMcu* mcu, QString name, int type ) : McuModule( mcu, name ), eElement( name ) { }
PicMssp::~PicMssp() { }
...
void PicMssp::configureA( uint8_t SSPCON ) {
    ...
    if ( m_mode != mode || m_enabled != enabled ) {
        ...
        case 6:
            twiMode = TWI_SLAVE;
            break; // I2C Slave, 7-bit address
        case 7:
            twiMode = TWI_SLAVE;
            break; // I2C Slave, 10-bit address
        case 8:
            twiMode = TWI_MASTER;
            break; // I2C Master, clock = FOSC/(4 *(SSPADD+1))
        case 9:
            break; // Load Mask function
        case 10:
            break; // Reserved
        case 11:
            break; // I2C firmware controlled Master (Slave idle)
        case 12:
            break; // Reserved
        case 13:
            break; // Reserved
        case 14:
            twiMode = TWI_SLAVE;
            break; // I2C Slave,  7-bit address, Start & Stop interrupts enabled
        case 15:
            twiMode = TWI_SLAVE;
            break; // I2C Slave, 10-bit address, Start & Stop interrupts enabled
        }
        if ( enabled ) // First disable, then enable
        {
            if ( spiMode == SPI_OFF )
                m_spiUnit->setMode( spiMode );
            if ( twiMode == TWI_OFF )
                m_twiUnit->setMode( twiMode );
            if ( spiMode != SPI_OFF )
                m_spiUnit->setMode( spiMode );
            if ( twiMode != TWI_OFF )
                m_twiUnit->setMode( twiMode );
