...
{
    if ( type == 00 )
        return new PicPwmUnit00( mcu, name );
    if ( type == 01 )
        return new PicPwmUnit01( mcu, name );
    return nullptr;
}

PicOcUnit::PicOcUnit( eMcu* mcu, QString name ) : McuOcUnit( mcu, name ) {
...
void PicOcUnit::configure( uint8_t CCPxM ) // CCPxM0,CCPxM1,CCPxM2,CCPxM3
{
    ...
    switch ( CCPxM ) {
    case 2:
        setOcActs( ocTOG, ocNON );
        break; // Toggle OC Pin - by ci (works in non-enhanced real device)
    case 8:
        setOcActs( ocSET, ocCLR );
        break; // Set OC Pin
    case 9:
        setOcActs( ocCLR, ocSET );
        break; // Clear OC Pin
    case 10:
        ctrlPin = false;
        break; // Only interrupt
    case 11:
        ctrlPin = false;
        m_specEvent = true; // Special event
    }
...
PicPwmUnit::PicPwmUnit( eMcu* mcu, QString name ) : McuOcUnit( mcu, name ) {
    m_enhanced = name.contains( "+" );
}
...
PicPwmUnit00::PicPwmUnit00( eMcu* mcu, QString name ) : PicPwmUnit( mcu, name ) { }
...
void PicPwmUnit00::setup() {
    ...
    if ( m_enhanced )
        m_PxM = getRegBits( "P" + n + "M0,P" + n + "M1", m_mcu );
}

//------------------------------------------------------
//-- PIC 16f627 PWM Unit -------------------------------

PicPwmUnit01::PicPwmUnit01( eMcu* mcu, QString name ) : PicPwmUnit( mcu, name ) { }
