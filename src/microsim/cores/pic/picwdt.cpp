...
PicWdt* PicWdt::createWdt( eMcu* mcu, QString name, int type ) {
    switch ( type ) {
    case 01:
        return new PicWdt01( mcu, name );
        break;
    default:
        return new PicWdt00( mcu, name );
        break;
    }
}

PicWdt::PicWdt( eMcu* mcu, QString name ) : McuWdt( mcu, name ) {
...
PicWdt00::PicWdt00( eMcu* mcu, QString name ) : PicWdt( mcu, name ) { }
PicWdt00::~PicWdt00() { }
...
void PicWdt00::configureA( uint8_t newOPTION ) // OPTION Written
{
    ...
    if ( getRegBitsVal( newOPTION, m_PSA ) )
        ...
        prIndex = 0; // Prescaler asigned to TIMER0
...
PicWdt01::PicWdt01( eMcu* mcu, QString name ) : PicWdt( mcu, name ) { }
PicWdt01::~PicWdt01() { }
