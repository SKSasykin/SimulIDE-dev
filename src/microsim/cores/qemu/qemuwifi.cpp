...
QemuWifi::QemuWifi( QemuDevice* mcu, QString name, int n,
...
{
    m_type = "wifi";
    m_arena->wifi_rx.head = m_arena->wifi_rx.tail = 0;
    m_arena->wifi_tx.head = m_arena->wifi_tx.tail = 0;
}

QemuWifi::~QemuWifi() { delete m_backend; }
...
void QemuWifi::setHostLink( quint16 port ) {
    if( port ) m_backend->setHostLink( port );
    else       m_backend->closeLink();
}

void QemuWifi::injectHostFrame( const QByteArray& frame ) {
    if( rxFull() ) return;
...
bool QemuWifi::selfTest() {
    if( !m_backend->setHostLink( 14567 ) ) return false;
    QByteArray probe( "SIMULIDE_WIFI_SELFTEST", 21 );
