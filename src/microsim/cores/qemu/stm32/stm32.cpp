...
Stm32::Stm32( QString type, QString id, QString device ) : QemuDevice( type, id ) {
    ...
    switch ( pkg ) {
    case 0:
        m_packageFile = "stm32_dip36.package";
        break; // T
    case 1:
        m_packageFile = "stm32_dip48.package";
        break; // C
    case 2:
        m_packageFile = "stm32_dip64.package";
        break; // R
    case 3:
        m_packageFile = "stm32_dip100.package";
        break; // V
    ...
    if ( m_i2cN > 0 )
        m_i2cs[0] = new Stm32Twi( this, "I2C1", 0, &m_apb1, 0x00005400, 0x000057FF );
    if ( m_i2cN > 1 )
        m_i2cs[1] = new Stm32Twi( this, "I2C2", 1, &m_apb1, 0x00005800, 0x00005BFF );

    m_spis.resize( m_spiN );
    if ( m_spiN > 0 )
        m_spis[0] = new Stm32Spi( this, "I2C1", 0, &m_apb1, 0x00013000, 0x000133FF );
    if ( m_spiN > 1 )
        m_spis[1] = new Stm32Spi( this, "I2C2", 1, &m_apb1, 0x00003800, 0x00003BFF );
    if ( m_spiN > 2 )
        m_spis[2] = new Stm32Spi( this, "I2C3", 2, &m_apb1, 0x00003C00, 0x00003FFF );

    m_usarts.resize( m_usartN );
    if ( m_usartN > 0 )
        m_usarts[0] = new Stm32Usart( this, "Usart1", 0, &m_apb2, 0x00013800, 0x00013BFF );
    if ( m_usartN > 1 )
        m_usarts[1] = new Stm32Usart( this, "Usart2", 1, &m_apb1, 0x00004400, 0x000047FF );
    if ( m_usartN > 2 )
        m_usarts[2] = new Stm32Usart( this, "Usart3", 2, &m_apb1, 0x00004800, 0x00004BFF );
    if ( m_usartN > 3 )
        m_usarts[3] = new Stm32Usart( this, "Uart4", 3, &m_apb1, 0x00004C00, 0x00004FFF );
    if ( m_usartN > 4 )
        m_usarts[4] = new Stm32Usart( this, "Uart5", 4, &m_apb1, 0x00005000, 0x000053FF );
...
void Stm32::stamp() {
    if ( m_i2cN > 0 )
        m_i2cs[0]->setPins( m_ports[1]->getPin( 6 ), m_ports[1]->getPin( 7 ) );
    if ( m_i2cN > 1 )
        m_i2cs[1]->setPins( m_ports[1]->getPin( 10 ), m_ports[1]->getPin( 11 ) );

    if ( m_spiN > 0 )
        m_spis[0]->setPins( m_ports[0]->getPin( 7 ), m_ports[0]->getPin( 6 ), m_ports[0]->getPin( 5 ),
                            m_ports[0]->getPin( 4 ) );
    if ( m_spiN > 1 )
        m_spis[1]->setPins( m_ports[1]->getPin( 15 ), m_ports[1]->getPin( 14 ), m_ports[1]->getPin( 13 ),
                            m_ports[1]->getPin( 12 ) );
    if ( m_spiN > 2 )
        m_spis[2]->setPins( m_ports[1]->getPin( 5 ), m_ports[1]->getPin( 4 ), m_ports[1]->getPin( 3 ),
                            m_ports[0]->getPin( 15 ) );

    if ( m_usartN > 0 )
        m_usarts[0]->setPins( { m_ports[0]->getPin( 9 ), m_ports[0]->getPin( 10 ) } ); // No Remap (TX/PB6, RX/PB7)
    if ( m_usartN > 1 )
        m_usarts[1]->setPins(
            { m_ports[0]->getPin( 2 ),
              m_ports[0]->getPin(
                  3 ) } ); // No remap (CTS/PA0, RTS/PA1, TX/PA2, RX/PA3, CK/PA4), Remap (CTS/PD3, RTS/PD4, TX/PD5, RX/PD6, CK/PD7)
    if ( m_usartN > 2 )
        m_usarts[2]->setPins( { m_ports[1]->getPin( 10 ), m_ports[1]->getPin( 11 ) } );
    if ( m_usartN > 3 )
        m_usarts[3]->setPins( { m_ports[2]->getPin( 10 ), m_ports[2]->getPin( 11 ) } );
    if ( m_usartN > 4 )
        m_usarts[4]->setPins( { m_ports[2]->getPin( 12 ), m_ports[3]->getPin( 2 ) } );
...
Pin* Stm32::addPin( QString id, QString type, QString label, int n, int x, int y, int angle, int length, int space ) {
    IoPin* pin = nullptr;
...
void Stm32::timerRemap( int number, uint8_t value ) {
    ...
    case 0: {
        ...
        case 0: { //00: No remap (ETR/PA12, CH1/PA8, CH2/PA9, CH3/PA10, CH4/PA11, BKIN/PB12, CH1N/PB13, CH2N/PB14, CH3N/PB15)
            timer->setOcPins( m_ports[0]->getPin( 8 ), m_ports[0]->getPin( 9 ), m_ports[0]->getPin( 10 ),
                              m_ports[0]->getPin( 11 ) );
        } break;
        case 1: { //01: Partial remap (ETR/PA12, CH1/PA8, CH2/PA9, CH3/PA10, CH4/PA11, BKIN/PA6, CH1N/PA7, CH2N/PB0, CH3N/PB1)
            timer->setOcPins( m_ports[1]->getPin( 8 ), m_ports[1]->getPin( 9 ), m_ports[1]->getPin( 10 ),
                              m_ports[1]->getPin( 11 ) );
        } break;
        case 2:
            break; //10: not used
        case 3: { //11: Full remap (ETR/PE7, CH1/PE9, CH2/PE11, CH3/PE13, CH4/PE14, BKIN/PE15, CH1N/PE8, CH2N/PE10, CH3N/PE12)
            timer->setOcPins( m_ports[5]->getPin( 9 ), m_ports[5]->getPin( 11 ), m_ports[5]->getPin( 13 ),
                              m_ports[5]->getPin( 14 ) );
        } break;
        }
    } break;
    case 1: {
        ...
        case 0: { //00: No remap (CH1/ETR/PA0, CH2/PA1, CH3/PA2, CH4/PA3)
            timer->setOcPins( m_ports[0]->getPin( 0 ), m_ports[0]->getPin( 1 ), m_ports[0]->getPin( 2 ),
                              m_ports[0]->getPin( 3 ) );
        } break;
        case 1: { //01: Partial remap (CH1/ETR/PA15, CH2/PB3, CH3/PA2, CH4/PA3)
            timer->setOcPins( m_ports[0]->getPin( 15 ), m_ports[1]->getPin( 3 ), m_ports[0]->getPin( 2 ),
                              m_ports[0]->getPin( 3 ) );
        } break;
        case 2: { //10: Partial remap (CH1/ETR/PA0, CH2/PA1, CH3/PB10, CH4/PB11)
            timer->setOcPins( m_ports[0]->getPin( 0 ), m_ports[0]->getPin( 1 ), m_ports[1]->getPin( 10 ),
                              m_ports[1]->getPin( 11 ) );
        } break;
        case 3: { //11: Full remap (CH1/ETR/PA15, CH2/PB3, CH3/PB10, CH4/PB11)
            timer->setOcPins( m_ports[0]->getPin( 15 ), m_ports[1]->getPin( 3 ), m_ports[1]->getPin( 10 ),
                              m_ports[1]->getPin( 11 ) );
        } break;
        }
    } break;
    case 2: {
        ...
        case 0: { //00: No remap (CH1/PA6, CH2/PA7, CH3/PB0, CH4/PB1)
            timer->setOcPins( m_ports[0]->getPin( 6 ), m_ports[0]->getPin( 7 ), m_ports[1]->getPin( 0 ),
                              m_ports[1]->getPin( 1 ) );
        } break;
        case 1:
            break; //01: Not used
        case 2: { //10: Partial remap (CH1/PB4, CH2/PB5, CH3/PB0, CH4/PB1)
            timer->setOcPins( m_ports[1]->getPin( 4 ), m_ports[1]->getPin( 5 ), m_ports[1]->getPin( 0 ),
                              m_ports[1]->getPin( 1 ) );
        } break;
        case 3: { //11: Full remap (CH1/PC6, CH2/PC7, CH3/PC8, CH4/PC9)
            timer->setOcPins( m_ports[2]->getPin( 6 ), m_ports[2]->getPin( 7 ), m_ports[2]->getPin( 8 ),
                              m_ports[2]->getPin( 9 ) );
        } break;
        }
    } break;
    case 3: {
        ...
        case 0: { // No remap (CH1/PB6, CH2/PB7, CH3/PB8, CH4/PB9)
            timer->setOcPins( m_ports[1]->getPin( 6 ), m_ports[1]->getPin( 7 ), m_ports[1]->getPin( 8 ),
                              m_ports[1]->getPin( 9 ) );
        } break;
        case 1: { // Full remap (CH1/PD12, CH2/PD13, CH3/PD14, CH4/PD15)
            timer->setOcPins( m_ports[3]->getPin( 12 ), m_ports[3]->getPin( 13 ), m_ports[3]->getPin( 14 ),
                              m_ports[3]->getPin( 15 ) );
        } break;
        }
    } break;
