...
void Stm32Usart::writeRegister() {
    ...
    switch ( offset ) {
    case SR_OFFSET:
        writeSR( m_eventValue );
        break;
    case DR_OFFSET:
        writeDR( m_eventValue );
        break;
    case BRR_OFFSET:
        writeBRR( m_eventValue );
        break;
    case CR1_OFFSET:
...
void Stm32Usart::writeSR( uint16_t newSR ) {
    if ( !( newSR & 1 << SR_RXNE_BIT ) )
        m_SR &= ~( 1 << SR_RXNE_BIT ); //s->SR_RXNE = 0;
    if ( !( newSR & 1 << SR_TC_BIT ) )
        m_SR &= ~( 1 << SR_TC_BIT ); //s->SR_TC   = 0;
...
void Stm32Usart::writeDR( uint16_t newDR ) {
    ...
    m_DR = newDR;
    if ( m_SR & 1 << SR_TC_BIT )
        sendNext(); // Transmission Completed, start.
    else
        m_SR &= ~( 1 << SR_TXE_BIT ); // Mark buffer as not empty
...
void Stm32Usart::byteReceived( uint8_t data ) {
    ...
    if ( m_SR & 1 << SR_RXNE_BIT ) // Set the overflow flag if buffer not empty
    {
        ...
        m_SR |= 1 << SR_RXNE_BIT; // Mark the buffer as not empty.
...
{
    uint16_t intFlags = m_SR
        & ( 1 << SR_TC_BIT | 1 << SR_TXE_BIT | 1 << SR_RXNE_BIT | 1 << SR_ORE_BIT ); // Interupts currently raised
