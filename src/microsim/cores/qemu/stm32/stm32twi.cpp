...
static uint16_t SR1_W0_bits = BERR_bit | ARLO_bit | AF_bit | OVR_bit | PECERR_bit | TIMEOUT_bit | SMBALERT_bit;

static uint16_t errIrqMask = BERR_bit | ARLO_bit | AF_bit | OVR_bit | PECERR_bit | TIMEOUT_bit | SMBALERT_bit;

static uint16_t evtIrqMask0 = SB_bit | ADDR_bit | ADD10_bit | STOPF_bit | BTF_bit;

static uint16_t evtIrqMask1 = TxE_bit | RxNE_bit;
...
void Stm32Twi::writeRegister() {
    ...
    switch ( offset ) {
    case CR1_OFFSET:
        writeCR1( m_eventValue );
        break;
    case CR2_OFFSET:
        writeCR2( m_eventValue );
        break;
    case DR_OFFSET:
        writeDR( m_eventValue );
        break;
    case SR1_OFFSET:
        m_SR1 &= m_eventValue | ~SR1_W0_bits; // Clear write-0 bits
        break;
    case SR2_OFFSET: /*Read Only*/
        break;
...
void Stm32Twi::readRegister() {
    ...
    switch ( offset ) {
    case CR1_OFFSET:
        m_arena->regData = m_CR1;
        break;
    case CR2_OFFSET:
        m_arena->regData = m_CR2;
        break;
    case DR_OFFSET:
        if ( m_SR1 & BTF_bit ) {
            if ( m_sr1Read ) {
    ...
    if ( newCR1 & STOP_bit ) // Bit 9 STOP: Stop generation
    {
        if ( m_mode == TWI_MASTER ) {
            if ( m_SR1 & TxE_bit )
                ...
                m_needStop = 1; // Stop after current operation.
        }
...
void Stm32Twi::writeCR2( uint16_t newCR2 ) {
    ...
    if ( freqMHz < 2 )
        freqMHz = 2; // Minimum allowed frequency is 2 MHz
...
void Stm32Twi::setTwiState( twiState_t state ) {
    ...
    case TWI_MTX_ADR_NACK: // SLA+W transmitted, NACK received
        ...
        m_SR1 |= TxE_bit;
        if ( m_nextData )
            masterWrite( m_DR, false, true ); // I2C_WRITE
        else
            m_SR1 |= BTF_bit;
        break;
