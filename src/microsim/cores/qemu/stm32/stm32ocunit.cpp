...
void Stm32OcUnit::writeCCMR( uint8_t ccmr ) {
    ...
    } else // Output
    {
        ...
        if ( m_OCM != ocm ) {
            ...
            case 0:
                break; // Frozen: No output
            case 1:
                break; // OCxREF High at match
            case 2:
                break; // OCxREF Low  at match
            case 3:
                break; // OCxREF Toggle at match
            case 4:
                setPinState( false );
                break; // OCxREF is forced low
            case 5:
                setPinState( true );
                break; // OCxREF is forced high
            case 6:
                break; // PWM mode 1, Birirectional: High below math
            case 7:
                break; // PWM mode 2, Birirectional: Low below math
            default:
                break;
            }
        }
