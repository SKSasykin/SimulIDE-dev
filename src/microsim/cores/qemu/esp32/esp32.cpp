...
Esp32::Esp32( QString type, QString id, QString device ) : QemuDevice( type, id ) {
    ...
    for ( int i = 0; i < m_spiN; ++i )
        m_spis[i]->setPins( dummyP, dummyP, dummyP, dummyP );

    m_usartN = 3;
    m_usarts.resize( m_usartN );
    m_usarts[0] = new Esp32Usart( this, id + "Usart1", 0, &m_apbFreq, 0x00040000, 0x00040FFF, Esp32Uart, 34 );
    m_usarts[1] = new Esp32Usart( this, id + "Usart2", 1, &m_apbFreq, 0x00050000, 0x00050FFF, Esp32Uart, 35 );
    m_usarts[2] = new Esp32Usart( this, id + "Usart3", 2, &m_apbFreq, 0x0006E000, 0x0006EFFF, Esp32Uart, 36 );
...
bool Esp32::createArgs() {
    ...
    if ( size < 4194304 ) {
        ...
        QString base = fi.baseName() + "-" + QString::number( qHash( fi.absoluteFilePath() ), 16 );
        QString padPath = QDir( cachePath ).filePath( "simulide-esp32-" + base + "-flash.bin" );
        QFile pad( padPath );
        if ( pad.exists() )
            pad.remove();
...
Pin* Esp32::addPin( QString id, QString type, QString label, int n, int x, int y, int angle, int length, int space ) {
    IoPin* pin = nullptr;
