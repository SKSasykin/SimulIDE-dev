...
public:
    ...
    enum cpuState_t { cpu_RESET = 0, cpu_FETCH, cpu_OPERAND, cpu_EXEC, cpu_WRITE };
...
protected:
    ...
    enum memState_t { mem_IDLE = 0, mem_LAEN, mem_ADDR, mem_LADI, mem_DATA, mem_READ };
    ...
    inline uint16_t checkAddr( uint16_t addr ) {
        if ( m_upperData && ( addr > m_lowDataMemEnd ) )
            addr += m_regEnd;
        return addr;
    }
    ...
    inline void writeInd( uint16_t addr, uint8_t val ) {
        if ( addr > m_lowDataMemEnd ) {
            if ( m_upperData )
                m_dataMem[addr + m_regEnd] = val;
        } else
            m_dataMem[m_opAddr] = val;
    }
