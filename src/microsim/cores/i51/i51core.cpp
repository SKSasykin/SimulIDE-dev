...
I51Core::I51Core( eMcu* mcu ) : Mcu8bits( mcu ), eElement( mcu->getId() + "-I51Core" ) {
...
void I51Core::reset() {
    ...
    uint64_t readCycle = m_mcu->psInst() / 2; // We are doing 2 Read cycles per Machine cycle
    m_psStep = readCycle / 12; // Half clock

    m_addrSetTime = 3 * m_psStep; // Addr pins
    m_laEnEndTime = 4 * m_psStep; // LA 0
    m_readSetTime = 6 * m_psStep; // PSEN 0
    // m_writeSetTime ???
    m_readBusTime = readCycle - 10; // PSEN 1, Read data Bus
}

void I51Core::runEvent() // External Memory operations
{
    switch ( m_memState ) {
    case mem_IDLE:
        break;
...
void I51Core::runStep() // Machine cycle: 1/6 frequency
{
    m_readCycle++;
    ...
    if ( extPGM )
        m_pgmData = m_data; // Read from External Memory
    else
        m_pgmData = m_progMem[m_readPC]; // Read from Internal ROM
    ...
    {
        readOperand(); // Read next operand
    } else if ( m_cpuState == cpu_RESET )
        m_cpuState = cpu_FETCH; // First cycle used fetching first instruction
    ...
    if ( extPGM ) // Reading external Memory
    {
        m_read = true;
        m_addr = m_readPC;
        m_addrH = m_readPC >> 8;
    ...
    if ( m_readCycle & 1 )
        m_mcu->cyclesDone = 1; // 1 machine cyle = 2 read cycle, so (m_readCycle+1) should be multiple of 2
    else
        m_mcu->cyclesDone = 0;
    if ( !extPGM && m_readPC >= m_progSize )
        m_readPC -= m_progSize;
}
...
void I51Core::readOperand() {
    m_readPC++;
    if ( --m_rCycles == 1 )
        m_cpuState = cpu_EXEC; // All cycles

    if ( m_readOp.isEmpty() )
        return; // All operands ready
    ...
    if ( addrMode & aIMME ) {
        if ( addrMode & aORIG )
            ...
            if ( addrMode & a16BIT_HIGH )
                m_opAddr = (uint16_t) m_pgmData << 8; //16bit address high
            else if ( addrMode & a16BIT_LOW )
                m_opAddr |= m_pgmData; //16bit address low
            else
                m_opAddr = m_pgmData;
        }
    } else if ( addrMode & aDIRE ) {
        if ( addrMode & aORIG )
            m_op0 = GET_RAM( m_pgmData );
        else if ( addrMode & aRELA )
            m_op2 = GET_RAM( m_pgmData );
        else
            m_opAddr = m_pgmData;
    } else if ( addrMode & aRELA ) {
        m_op2 = m_pgmData;
...
void I51Core::operRgx() {
    m_op0 = m_dataMem[m_RxAddr];
} //
void I51Core::operInd() {
    m_op0 = readInd( I_RX_VAL );
} //
void I51Core::operI08() {
    m_readOp.append( aIMME | aORIG );
} // m_op0 = data
void I51Core::operDir() {
    m_readOp.append( aDIRE | aORIG );
} // m_op0 = GET_RAM( data );
void I51Core::operACC() {
    m_op0 = ACC;
} //
void I51Core::opr2I08() {
    m_readOp.append( aIMME | aRELA );
} // m_op2 = data;
void I51Core::opr2Dir() {
    m_readOp.append( aDIRE | aRELA );
} // m_op2 = GET_RAM( data );

void I51Core::addrRgx() {
    m_opAddr = m_RxAddr;
}
void I51Core::addrInd() {
    m_opAddr = checkAddr( I_RX_VAL );
} //
void I51Core::addrI08() {
    m_readOp.append( aIMME );
} // m_opAddr = data;
void I51Core::addrI16() {
    m_readOp.append( aIMME | a16BIT_HIGH );
    m_readOp.append( aIMME | a16BIT_LOW );
} // m_opAddr = data16;
void I51Core::addrDir() {
    m_readOp.append( aDIRE );
} // m_opAddr = data;
void I51Core::addrBit( bool invert ) {
    m_readOp.append( aBIT ); // m_opAddr = addr, m_op0 = bitMask
    m_invert = invert;
}
...
void I51Core::addFlags( uint8_t value1, uint8_t value2, uint8_t acc ) {
    ...
    write_S_Bit( AC,
                 ( ( value1 & 0x0F ) + ( value2 & 0x0F ) + acc )
                     & 1 << 4 ); // Auxiliary carry: overflow from 3th bit to 4th bit
    write_S_Bit( OV,
                 ( ( ( value1 & 127 ) + ( value2 & 127 ) + acc ) ^ c )
                     & 1 << 7 ); // Overflow: overflow from 6th or 7th bit, but not both
}
...
void I51Core::AJMP() {
    m_PC = ( m_PC & 0xF800 ) | m_opAddr | ( ( m_opcode & 0xE0 ) << 3 );
}
void I51Core::LJMP() {
    m_PC = m_opAddr;
}
void I51Core::JMP() {
    m_PC = GET_REG16_LH( REG_DPL ) + ACC;
}
void I51Core::SJMP() {
...
void I51Core::JB() {
    if ( GET_RAM( m_bitAddr ) & m_bitMask )
        m_PC += (int8_t) m_opAddr;
}
...
void I51Core::MOVbc() {
    ...
    if ( carry )
        value = GET_RAM( m_bitAddr ) | m_bitMask; // Set bit if Carry
...
void I51Core::CLRb() {
    SET_RAM( m_bitAddr, m_dataMem[m_bitAddr] & ~m_bitMask );
}
void I51Core::SETBb() {
    SET_RAM( m_bitAddr, m_dataMem[m_bitAddr] | m_bitMask );
}
...
void I51Core::INCd() {
    ...
    if ( !m_dataMem[REG_DPL] )
        SET_RAM( REG_DPH, m_dataMem[REG_DPH] + 1 );
}
void I51Core::INC() {
    SET_RAM( m_opAddr, m_dataMem[m_opAddr] + 1 );
}
void I51Core::DEC() {
    SET_RAM( m_opAddr, m_dataMem[m_opAddr] - 1 );
}
...
void I51Core::ORLm() {
    SET_RAM( m_opAddr, m_dataMem[m_opAddr] | m_op0 );
}
void I51Core::ANLm() {
    SET_RAM( m_opAddr, m_dataMem[m_opAddr] & m_op0 );
}
...
void I51Core::MULab() {
    ...
    //write_S_Bit( OV, GET_RAM( REG_B ) );  /// FIXME ????
    clear_S_Bit( OV );
    clear_S_Bit( Cy );
}

void I51Core::MOVr() {
    m_dataMem[m_opAddr] = m_op0;
}
void I51Core::MOVm() {
    SET_RAM( m_opAddr, m_op0 );
}
void I51Core::MOVml() {
    writeInd( m_opAddr, m_op0 );
}
...
void I51Core::MOVCd() {
    ACC = m_progMem[GET_REG16_LH( REG_DPL ) + ACC];
}
void I51Core::MOVCp() {
    ACC = m_progMem[m_PC + 1 + ACC];
}
void I51Core::MOVd() {
    SET_REG16_LH( REG_DPL, m_opAddr );
}
...
void I51Core::Decode() {
    m_rCycles = 2; // Default 1 Instruction cycle = 2 Read cycles
    m_readOp.clear();
    if ( m_opcode & 8 ) // Rx
    {
        m_RxAddr = ( m_opcode & 0x07 ) + 8 * BANK;

        switch ( m_opcode & 0xF0 ) { // b-c Inst
        case 0x00: // 1-1 INC  Rx <- R++
        case 0x10:
            addrRgx();
            break; // 1-1 DEC  Rx <- R--
        case 0x20: // 1-1 ADD  Ac += Rx
        case 0x30: // 1-1 ADDC Ac += Rx+C
        case 0x40: // 1-1 ORL  Ac |= Rx
        case 0x50: // 1-1 ANL  Ac &= Rx
        case 0x60:
            operRgx();
            break; // 1-1 XRL  Ac ^= Rx
        case 0x70:
            addrRgx();
            operI08();
            break; // 2-1 MOVr Rx <- #imm8
        case 0x80:
            addrDir();
            operRgx();
            m_rCycles = 4;
            break; // 2-2 MOVm Di <- Rx
        case 0x90:
            operRgx();
            break; // 1-1 SUBB Ac -= Rx-C
        case 0xA0:
            addrRgx();
            operDir();
            m_rCycles = 4;
            break; // 2-2 MOVr Rx <- Di
        case 0xB0:
            operRgx();
            opr2I08();
            addrDir();
            m_rCycles = 4;
            break; // 3-2 CJNE Rx == Op2 ? 0 :Jump
        case 0xC0:
            addrRgx();
            break; // 1-1 XCH  Ac <> Rx
        case 0xD0:
            ...
            addrDir();
            m_rCycles = 4;
            break; // 2-2 DJNZ Rx--      ? Jump : 0
        case 0xE0:
            operRgx();
            break; // 1-1 MOVa Ac <- Rx
        case 0xF0:
            addrRgx();
            operACC();
            break; // 1-1 MOV  Rx <- A
        ...
        case 0x01:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 AJMP addr11
        case 0x02:
            addrI16();
            m_rCycles = 4;
            break; // 3-2 LJMP addr16
        case 0x03:
            break; // 1-1 RRa Ac >> 1
        case 0x04:
            break; // 1-1 INC Ac <- Ac++  Accumulator
        case 0x05:
            addrDir();
            break; // 2-1 INC Di <- Di++  Direct
        case 0x06:
        case 0x07:
            addrInd();
            break; // 1-1 INC In <- In++  @Indirect Rx
        ...
        case 0x11:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 ACALL addr11
        case 0x12:
            addrI16();
            m_rCycles = 4;
            break; // 3-2 LCALL addr16
        case 0x13:
            break; // 1-1 RRCa C >> Ac>>1
        case 0x14:
            break; // 1-1 DEC Ac <- Ac++  Accumulator
        case 0x15:
            addrDir();
            break; // 2-1 DEC Di <- Di++  Direct
        case 0x16:
        case 0x17:
            addrInd();
            break; // 1-1 DEC In <- In++  @Indirect Rx
        ...
        case 0x24:
            operI08();
            break; // 2-1 ADD Ac += #imm8  #Imminent
        case 0x25:
            operDir();
            break; // 2-1 ADD AC += Di     Direct
        case 0x26:
        case 0x27:
            operInd();
            break; // 1-1 ADD Ac += In     @Indirect
        ...
        case 0x32:
            ...
            break; // 1-2 RETI
        case 0x33:
            break; // 1-1 RLCa C << Ac<<1
        case 0x34:
            operI08();
            break; // 2-1 ADDC Ac += C+#imm8  #Imminent
        case 0x35:
            operDir();
            break; // 2-1 ADDC AC += C+Di     Direct
        case 0x36:
        case 0x37:
            operInd();
            break; // 1-1 ADDC Ac += C+In     @Indirect

        case 0x40:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 JC C ? jump : 0
        case 0x41:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 AJMP addr11
        ...
        case 0x44:
            operI08();
            break; // 2-1 ORL Ac |= #I8   #Imminent
        case 0x45:
            operDir();
            break; // 2-1 ORL AC |= Di    Direct
        case 0x46:
        case 0x47:
            operInd();
            break; // 1-1 ORL Ac |= In    @Indirect

        case 0x50:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 JNC C ? 0 : jump
        case 0x51:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 ACALL addr11
        ...
        case 0x54:
            operI08();
            break; // 2-1 ANL Ac &= #I8 #Imminent
        case 0x55:
            operDir();
            break; // 2-1 ANL Ac &= Di Direct
        case 0x56:
        case 0x57:
            operInd();
            break; // 1-1 ANL Ac &= In    @Indirect

        case 0x60:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 JZ A == 0 ? jump : 0
        case 0x61:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 AJMP addr11
        ...
        case 0x64:
            operI08();
            break; // 2-1 XRLa Ac ^= #I8 #Imminent
        case 0x65:
            operDir();
            break; // 2-1 XRLa Ac ^= Di  Direct
        case 0x66:
        case 0x67:
            operInd();
            break; // 1-1 XRLa Ac ^= In  @Indirect

        case 0x70:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 JNZ A == 0 ? 0 : jump
        case 0x71:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 ACALL addr11
        case 0x72:
            addrBit();
            m_rCycles = 4;
            break; // 2-2 ORLc  C  <- C|b
        case 0x73:
            m_rCycles = 4;
            break; // 1-2 JMP  Dptr+A
        case 0x74:
            operI08();
            break; // 2-1 MOVa Ac <- #I8  #Imminent
        case 0x75:
            addrDir();
            operI08();
            m_rCycles = 4;
            break; // 3-2 MOVm Di <- #I8
        case 0x76:
        case 0x77:
            addrInd();
            operI08();
            break; // 2-1 MOVm In <- #I8  @Indirect

        case 0x80:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 SJMP PC += addr8
        case 0x81:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 AJMP addr11
        case 0x82:
            addrBit();
            m_rCycles = 4;
            break; // 2-2 ANLc  C  <- C&b
        case 0x83:
            m_rCycles = 4;
            break; // 1-2 MOVCp Ac += (PC)
        case 0x84:
            m_rCycles = 8;
            break; // 1-4 DIVab Ac <- Ac/B
        case 0x85:
            operDir();
            addrDir();
            m_rCycles = 4;
            break; // 3-2 MOVm  Di <- Di ( 0x85 source addr, dest addr )
        case 0x86:
        case 0x87:
            addrDir();
            operInd();
            m_rCycles = 4;
            break; // 2-2 MOVm Di <- In  @Indirect

        case 0x90:
            addrI16();
            m_rCycles = 4;
            break; // 3-2 MOVd  Dptr <- addr16;
        case 0x91:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 ACALL addr11
        ...
        case 0x93:
            m_rCycles = 4;
            break; // 1-2 MOVCd Ac += (Dptr)
        case 0x94:
            operI08();
            break; // 2-1 SUBB  Ac -= #I8 #Imminent
        case 0x95:
            operDir();
            break; // 2-1 SUBB  Ac -= Di  Direct
        case 0x96:
        case 0x97:
            operInd();
            break; // 1-1 SUBB Ac -= In  @Indirect
        ...
        case 0xb4:
            ...
            addrDir();
            m_rCycles = 4;
            break; // 3-2 CJNE Ac == #in8 ? 0 :Jump
        case 0xb5:
            operACC();
            opr2Dir();
            addrDir();
            m_rCycles = 4;
            break; // 3-2 CJNE Ac == Di   ? 0 :Jump
        case 0xb6:
        case 0xb7:
            operInd();
            opr2I08();
            addrDir();
            break; // 3-2 CJNE In == #in8 ? 0 :Jump

        case 0xc0:
            operDir();
            m_rCycles = 4;
            break; // 2-2 PUSH Stack <- Di
        case 0xc1:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 AJMP addr11
        ...
        case 0xc5:
            addrDir();
            break; // 1-1 XCH Ac <> Di  Direct
        case 0xc6:
        case 0xc7:
            addrInd();
            break; // 1-1 XCH Ac <> In @Indirect

        case 0xd0:
            addrDir();
            m_rCycles = 4;
            break; // 2-2 POP  Di <- Stack
        case 0xd1:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 ACALL addr11
        ...
        case 0xd5:
            ...
            addrDir();
            m_rCycles = 4;
            break; // 3-2 DJNZ Dir-- ? Jump : 0
        case 0xd6:
        case 0xd7:
            addrInd();
            break; // 1-1 XCHD A In @Indirect

        case 0xe0:
            addrI16();
            break; //     MOVX A <- @DPTR, unimplemented
        case 0xe1:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 AJMP addr11
        case 0xe2:
            break; //     MOVX A <- @R0, unimplemented
        case 0xe3:
            break; //     MOVX A <- @R1, unimplemented
        case 0xe4:
            break; // 1-1 CLR A
        case 0xe5:
            operDir();
            break; // 2-1 MOVa A <- Di  Direct
        case 0xe6:
        case 0xe7:
            operInd();
            break; // 1-1 MOV Ac <- In  @Indirect Rx

        case 0xf0:
            addrI16();
            break; //     MOVX @DPTR <- A, unimplemented
        case 0xf1:
            addrI08();
            m_rCycles = 4;
            break; // 2-2 ACALL addr11
        case 0xf2:
            break; //     MOVX @R0 <- A, unimplemented
        case 0xf3:
            break; //     MOVX @R1 <- A, unimplemented
        case 0xf4:
            break; // 1-1 CPL A
        case 0xf5:
            addrI08();
