...
AvrEeprom::AvrEeprom( eMcu* mcu, QString name ) : McuEeprom( mcu, name ) { }
AvrEeprom::~AvrEeprom() { }
...
void AvrEeprom::runEvent() // Write cycle end reached
{
    ...
    if ( eempe )
        clearRegBits( m_EEMPE ); // No read operation took place: clear EEMPE
    else
        clearRegBits( m_EEPE ); // Read operation took place: clear EEPE
}
...
void AvrEeprom::configureA( uint8_t newEECR ) // EECR is being written
{
    ...
    if ( !eepe ) {
        ...
        {
            m_mcu->m_regOverride = newEECR
                & ~( m_EERE.mask ); // Clear EERE: it happens after 4 cycles, but cpu is halted for these cycles
            m_mcu->cyclesDone += 4;
    ...
    if ( eempe ) {
        ...
        {
            m_mcu->m_regOverride
                = newEECR & ~( m_EEMPE.mask ); // Clear EEMPE: it happens after 4 cycles but we need to cancel it now
