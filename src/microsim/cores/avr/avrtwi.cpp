...
AvrTwi::AvrTwi( eMcu* mcu, QString name ) : McuTwi( mcu, name ) { }
AvrTwi::~AvrTwi() { }
...
void AvrTwi::configureA( uint8_t newTWCR ) // TWCR is being written
{
    ...
    if ( newStop && !oldStop ) /// Generate Stop Condition
    {
        ...
        { /// DONE if Stop and Start at same time, then Start Condition should be sheduled
            if ( !newStart && getStaus() < TWI_NO_STATE )
                masterStop(); //I2Cstop();
        } else
            setMode(
                TWI_SLAVE ); // Slave: Stop Cond restarts Slave mode (can be used to recover from an error condition)
    }
...
{
    newTWSR &= 0b00000011;
    if ( m_prIndex != newTWSR ) {
        setPrescIndex( newTWSR );
        updateFreq();
    }
...
void AvrTwi::writeTwiReg( uint8_t newTWDR ) // TWDR is being written
{
    ...
    if ( isAddr )
        write = ( ( newTWDR & 1 ) == 0 ); // Sending address for Read or Write?
    ...
    *m_statReg |= state; // Write new state
    ...
    if ( state == TWI_NO_STATE && m_i2cState == I2C_STOP ) // Stop Condition sent
    {
        ...
        if ( m_mode == TWI_MASTER ) {
            ...
            if ( state == TWI_SRX_ADR_DATA_ACK || state == TWI_SRX_ADR_DATA_NACK || state == TWI_SRX_GEN_DATA_ACK
