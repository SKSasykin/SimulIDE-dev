...
{
    if ( type == 800 )
        return new AvrTimer800( mcu, name );
    else if ( type == 801 )
        return new AvrTimer801( mcu, name );
    else if ( type == 810 )
        return new AvrTimer810( mcu, name );
    else if ( type == 820 )
        return new AvrTimer820( mcu, name );
    else if ( type == 821 )
        return new AvrTimer821( mcu, name );
    else if ( type == 822 )
        return new AvrTimer822( mcu, name );
    else if ( type == 160 )
        return new AvrTimer16bit( mcu, name );

    return nullptr;
}

AvrTimer::AvrTimer( eMcu* mcu, QString name ) : McuTimer( mcu, name ) {
    m_OCA = nullptr;
    m_OCB = nullptr;
    m_OCC = nullptr;
}
...
void AvrTimer::addOcUnit( McuOcUnit* ocUnit ) {
    ...
    if ( ocUnit->getId().endsWith( "A" ) )
        m_OCA = (AvrOcUnit*) ocUnit;
    else if ( ocUnit->getId().endsWith( "B" ) )
        m_OCB = (AvrOcUnit*) ocUnit;
    else if ( ocUnit->getId().endsWith( "C" ) )
        m_OCC = (AvrOcUnit*) ocUnit;
    else
        m_OCA = (AvrOcUnit*) ocUnit;
}
...
{
    if ( m_OCA )
        m_OCA->configure( newTCCRXA ); // Done in ocunits
    if ( m_OCB )
        m_OCB->configure( newTCCRXA );
    if ( m_OCC )
        m_OCC->configure( newTCCRXA );
...
void AvrTimer::configureOcUnits( bool wgm3 ) {
    ...
    } else if ( m_wgmMode == wgmFAST ) // Fast PWM
    {
        if ( m_OCA ) {
            ...
            if ( comActA == ocCLR )
                tovActA = ocSET;
            else if ( comActA == ocSET )
                tovActA = ocCLR;
        }
        if ( m_OCB ) {
            if ( comActB == ocTOG )
                comActB = ocNON;
            else if ( comActB == ocCLR )
                tovActB = ocSET;
            else if ( comActB == ocSET )
                tovActB = ocCLR;
        }
        if ( m_OCC ) {
            if ( comActC == ocTOG )
                comActC = ocNON;
            else if ( comActC == ocCLR )
                tovActC = ocSET;
            else if ( comActC == ocSET )
                tovActC = ocCLR;
        }
    }
    if ( m_OCA )
    ...
    if ( m_OCC )
        m_OCC->setOcActs( comActC, tovActC );

    if ( m_bidirec )
        m_ovfPeriod = m_ovfMatch;
    else
        m_ovfPeriod = m_ovfMatch + 1;
}
...
AvrTimer8bit::AvrTimer8bit( eMcu* mcu, QString name ) : AvrTimer( mcu, name ) {
    m_maxCount = 0xFF;
}
...
void AvrTimer8bit::topReg0Changed( uint8_t val ) {
    ...
    *m_topReg0L = val;
    ...
    uint16_t ovf = 0xFF;
    if ( m_wgmMode == wgmCTC || ( ( m_wgm32Val ) && ( ( m_wgmMode == wgmPHAS ) || ( m_wgmMode == wgmFAST ) ) ) ) {
        ovf = val;
    } // Top = OCRA
...
AvrTimer800::AvrTimer800( eMcu* mcu, QString name ) : AvrTimer8bit( mcu, name ) { }
...
AvrTimer801::AvrTimer801( eMcu* mcu, QString name ) : McuTimer( mcu, name ) {
    m_maxCount = 0xFF;
}
...
AvrTimer810::AvrTimer810( eMcu* mcu, QString name ) : AvrTimer( mcu, name ) { }
...
void AvrTimer810::configureA( uint8_t newTCCR1 ) // TCCR1
{
    ...
    if ( getRegBitsBool( newTCCR1, m_CTC1 ) )
        mode |= 1 << 2;
...
void AvrTimer810::updateMode() {
    if ( m_mode )
        m_ovfMatch = *m_topReg0L; // Top = OCR1C
    else
        m_ovfMatch = 0xFF;
...
AvrTimer820::AvrTimer820( eMcu* mcu, QString name ) : AvrTimer8bit( mcu, name ) { }
...
AvrTimer821::AvrTimer821( eMcu* mcu, QString name ) : AvrTimer8bit( mcu, name ) { }
...
void AvrTimer821::configureA( uint8_t newTCCRx ) {
    if ( m_OCA )
        m_OCA->configure( newTCCRx ); // COM20 COM21 Done in ocunits

    updtPrescaler( newTCCRx );

    m_wgm10Val = ( ( ( newTCCRx & 1 << 6 ) >> 6 ) | ( ( newTCCRx & 1 << 3 ) >> 2 ) ); // WGM20 WGM21
...
AvrTimer16bit::AvrTimer16bit( eMcu* mcu, QString name ) : AvrTimer( mcu, name ) { }
...
void AvrTimer16bit::runEvent() // Overflow
{
    ...
    if ( m_wgmMode == wgmFAST && m_useICR && m_ICunit )
        m_ICunit->getInterrupt()->raise();
    McuTimer::runEvent();
}
...
void AvrTimer16bit::updtWgm() {
    ...
    case 1:
        mode = wgmPHAS;
        ovf = mask = 0x00FF;
        break; // PWM, Phase Correct, 8-bit
    case 2:
        mode = wgmPHAS;
        ovf = mask = 0x01FF;
        break; // PWM, Phase Correct, 9-bit
    case 3:
        mode = wgmPHAS;
        ovf = mask = 0x03FF;
        break; // PWM, Phase Correct, 10-bit
    case 4:
        mode = wgmCTC;
        ovf = OCRXA16;
        break; // CTC
    case 5:
        mode = wgmFAST;
        ovf = mask = 0x00FF;
        break; // Fast PWM, 8-bit
    case 6:
        mode = wgmFAST;
        ovf = mask = 0x01FF;
        break; // Fast PWM, 9-bit
    case 7:
        mode = wgmFAST;
        ovf = mask = 0x03FF;
        break; // Fast PWM, 10-bit
    case 8:
        mode = wgmPHAS;
        m_useICR = true;
        break; // PWM, Phase and Frequency Correct
    case 9:
        mode = wgmPHAS;
        ovf = OCRXA16;
        break; // PWM, Phase and Frequency Correct
    ...
    case 15:
        mode = wgmFAST;
        ovf = OCRXA16;
        break; // Fast PWM OCRXA
    }
    if ( m_useICR )
        ovf = ICRX16;
    if ( m_ICunit )
        m_ICunit->enable( !m_useICR );
    ...
    if ( m_OCA )
        m_OCA->setOcrMask( mask );
    if ( m_OCB )
        m_OCB->setOcrMask( mask );
    if ( m_OCC )
        m_OCC->setOcrMask( mask );
