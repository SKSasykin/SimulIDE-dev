...
AvrCore::AvrCore( eMcu* mcu ) : Mcu8bits( mcu ) {
    if ( mcu->regExist( "EIND" ) )
...
void AvrCore::runStep() {
    ...
    case 0x8000: {
        ...
        if ( instruction & 0x0200 )
            SET_RAM( v + q, m_dataMem[d] );
        else
            SET_RAM( d, GET_RAM( v + q ) );
    ...
    case 0xf000: {
        ...
        case 0xfb00: { // BST -- Bit Store into T from bit in Register -- 1111 101d dddd 0bbb
            get_vd5_s3( instruction ) write_S_Bit( S_T, ( vd >> s ) & 1 );
        } break;
        case 0xfc00:
        case 0xfe00: { // SBRS/SBRC -- Skip if Bit in Register is Set/Clear -- 1111 11sd dddd 0bbb
            get_vd5_s3_mask( instruction ) int set = ( instruction & 0x0200 ) != 0;
