...
AvrUsart::AvrUsart( eMcu* mcu, QString name, int number ) : McuUsart( mcu, name, number ) { }
AvrUsart::~AvrUsart() { }
...
void AvrUsart::setup() {
    ...
    if ( n == "" )
        m_modeRB = getRegBits( "UMSEL", m_mcu ); // atmega8
    else
        m_modeRB = getRegBits( "UMSEL" + n + "0,UMSEL" + n + "1", m_mcu );
    ...
    if ( n == "" )
        m_UPE = getRegBits( "PE", m_mcu );
    else
        m_UPE = getRegBits( "UPE" + n, m_mcu );
}
