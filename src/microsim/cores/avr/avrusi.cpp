...
AvrUsi::AvrUsi( eMcu* mcu, QString name ) : McuModule( mcu, name ), eElement( name ) {
    m_DOpin = nullptr;
    m_DIpin = nullptr;
    m_CKpin = nullptr;
}
...
void AvrUsi::reset() {
    ...
    if ( !m_DOpin )
        qDebug() << "AvrUsi::reset: Error: null DO Pin";
    if ( !m_DIpin )
        qDebug() << "AvrUsi::reset: Error: null DI Pin";
    if ( !m_CKpin )
        qDebug() << "AvrUsi::reset: Error: null CK Pin";
}
...
void AvrUsi::voltChanged() // Clk Pin changed
{
    ...
    if ( m_mode > 1 ) // TWI start/stop detector
    {
        ...
        if ( clkState ) {
            if ( m_sdaState && !sdaState ) {
                ...
                setRegBits( m_USIPF ); // Stop condition, set USIPF flag
        }
        m_sdaState = sdaState;
    }
    ...
    if ( m_extClk ) {
        if ( !m_usiClk )
            stepCounter(); // Counter Both edges

        bool oldSRclock = m_clkEdge ? m_clkState : !m_clkState;
        bool newSRclock = m_clkEdge ? clkState : !clkState;

        if ( !oldSRclock && newSRclock )
            shiftData(); // SR Leading  Edge
        else if ( oldSRclock && !newSRclock )
            setOutput(); // SR Trailibg Edge
    }
    m_clkState = clkState;
}
...
void AvrUsi::configureA( uint8_t newUSICR ) {
    ...
    if ( m_mode != mode ) {
        ...
        case 1:
            spi = true;
            break; // 3 Wire mode: Uses DO, DI, and USCK pins.
        case 2: // 2 Wire mode: Uses SDA (DI) and SCL (USCK) pins.
        case 3:
            twi = true; // Same as 2 wire above & SCL held low at counter overflow
        }
        if ( m_spi != spi ) {
            m_spi = spi;
            if ( m_DOpin )
                m_DOpin->controlPin( spi, false );
        }
        if ( m_twi != twi ) {
            m_twi = twi;
    ...
    if ( m_clockMode != clockMode ) {
        ...
        switch ( clockMode ) {
        case 0:
            break; // Software clock strobe (USICLK)
        case 1:
            timer = true;
            break; // Timer0 Compare Match
        case 2:
            m_clkEdge = true; // External, shiftData() positive edge
        case 3:
            extClk = true; // External, shiftData() negative edge
        }
        if ( m_extClk != extClk ) { // Activate/Deactivate External Clock
            m_extClk = extClk;
            if ( m_CKpin )
                m_CKpin->changeCallBack( this, extClk );
        }
        if ( m_timer != timer ) {
            m_timer = timer;
    ...
    if ( m_mode ) {
        ...
        if ( usiTc )
            toggleClock(); // USITC always toggles Clock (PORT Register)
        ...
        if ( !m_timer ) {
            if ( m_extClk ) { // USICS1 = 1
                if ( m_usiClk && usiTc )
                    stepCounter(); // Software counter strobe (USITC)
            } else { // USICS1 = 0
                if ( m_usiClk ) {
...
void AvrUsi::configureB( uint8_t newUSISR ) {
    ...
    if ( oldUsiSR && newUsiSR )
        m_mcu->m_regOverride = newUSISR & ~m_USIPF.mask; // clear USIPF by writing a 1 to it
}
...
void AvrUsi::shiftData() {
    *m_dataReg = *m_dataReg << 1; // Shift Data Register
    ...
    if ( !m_DIpin )
        return;
    if ( m_DIpin->getInpState() )
        *m_dataReg |= 1; // Read input & store in Data Reg bit0
    else
        *m_dataReg &= ~1;
}
...
void AvrUsi::setOutput() // Set output *m_dataReg & 1<<7
{
    if ( m_mode == 1 ) {
        if ( m_DOpin )
            m_DOpin->setOutState( m_DoState );
    } // SPI
    else if ( m_mode > 1 )
        writeBitsToReg( m_DIbit, m_DoState, m_mcu ); // TWI
}
