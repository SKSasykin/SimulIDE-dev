...
void watchRegister( uint16_t addr, int write, T* inst, void ( T::*func )( uint8_t ), DataSpace* mcu,
                    uint8_t mask = 0xFF ) {
...
void watchRegNames( QString regNames, int write, T* inst, void ( T::*func )( uint8_t ), DataSpace* mcu ) {
    if ( regNames.isEmpty() )
        return;
...
void watchBitNames( QString bitNames, int write, T* inst, void ( T::*func )( uint8_t ), DataSpace* mcu ) {
    if ( bitNames.isEmpty() )
        return;
