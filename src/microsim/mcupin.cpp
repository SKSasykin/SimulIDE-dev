...
McuPin::McuPin( McuPort* port, int i, QString id, Component* mcu ) : IoPin( 0, QPoint( 0, 0 ), id, 0, mcu ) {
    m_id = id;
    m_port = port;
    m_number = i;
...
void McuPin::stamp() {
    ...
    if ( !m_outCtrl && m_outMask )
        IoPin::setOutState( true );
    update();
}
...
void McuPin::voltChanged() {
    ...
    if ( m_extInt ) {
        ...
        case pinLow:
            trigger
                = ( Simulator::self()->circTime() == 1 ); // Trigger Low level without pin change at simulation start
            raise = !newState; // Fallthrough: Trigger Low level at pin change
        case pinChange:
            trigger |= ( oldState != newState );
            break;
        case pinFalling:
            trigger = ( oldState && !newState );
            break;
...
void McuPin::setPortState( bool state ) // Port Is being witten
{
    m_portState = state;
    if ( m_outCtrl )
        return; // Port is not controlling Pin State

    m_outState = state; /// ??? Should this be controlled by IoPin?
    if ( m_isOut )
        IoPin::scheduleState( state, 0 );
}
...
void McuPin::setDirection( bool out ) {
    ...
    if ( m_isOut )
        m_portMode = m_openColl ? openCo : output; // Set Pin to Output
    else
        m_portMode = input; // Set Pin to Input

    if ( m_dirCtrl )
        return; // Is someone is controlling us, just save Pin Mode

    changeCallBack(
        this, m_changeCB || !m_isOut ); // Receive voltage change notifications only if input or always (m_changeCB)
...
void McuPin::controlPin( bool outCtrl, bool dirCtrl ) {
    ...
    {
        if ( m_pinMode > input )
            scheduleState( m_portState, 0 ); // Set Previous Pin State
        else
            m_outState = m_portState;
    }
    m_outCtrl = outCtrl;
}
...
QStringList McuPin::registerScript( asIScriptEngine* engine ) {
    ...
    engine->RegisterObjectMethod( "McuPin", "void setDirection( bool o )",
                                  asMETHODPR( McuPin, setDirection, (bool), void ), asCALL_THISCALL );

    memberList << "setPortState( bool state )";
    engine->RegisterObjectMethod( "McuPin", "void setPortState( bool s )",
                                  asMETHODPR( McuPin, setPortState, (bool), void ), asCALL_THISCALL );
    ...
    engine->RegisterObjectMethod( "McuPin", "void controlPin( bool outCtrl, bool dirCtrl )",
                                  asMETHODPR( McuPin, controlPin, (bool, bool), void ), asCALL_THISCALL );

    memberList << "setExtInt( uint mode )";
    engine->RegisterObjectMethod( "McuPin", "void setExtInt( uint mode )",
                                  asMETHODPR( McuPin, setExtInt, ( uint ), void ), asCALL_THISCALL );

    // IoPin ------------

    memberList << "setPinMode( uint mode )";
    engine->RegisterObjectMethod( "McuPin", "void setPinMode(uint m)", asMETHODPR( McuPin, setPinMode, ( uint ), void ),
                                  asCALL_THISCALL );

    memberList << "getInpState()";
    engine->RegisterObjectMethod( "McuPin", "bool getInpState()", asMETHODPR( McuPin, getInpState, (), bool ),
                                  asCALL_THISCALL );

    memberList << "void setOutState( bool state )";
    engine->RegisterObjectMethod( "McuPin", "void setOutState(bool s)", asMETHODPR( McuPin, setOutState, (bool), void ),
                                  asCALL_THISCALL );

    memberList << "getVoltage()";
    engine->RegisterObjectMethod( "McuPin", "double getVoltage()", asMETHODPR( McuPin, getVoltage, (), double ),
                                  asCALL_THISCALL );

    memberList << "setVoltage( double voltage )";
    engine->RegisterObjectMethod( "McuPin", "void setVoltage(double v)",
                                  asMETHODPR( McuPin, setVoltage, (double), void ), asCALL_THISCALL );

    memberList << "changeCallBack( eElement@ e, bool call )";
    engine->RegisterObjectMethod( "McuPin", "void changeCallBack(eElement@ p, bool s)",
                                  asMETHODPR( McuPin, changeCallBack, (eElement*, bool), void ), asCALL_THISCALL );
