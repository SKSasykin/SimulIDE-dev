...
MainWindow::MainWindow() : QMainWindow() {
    ...
    if ( m_settings->contains( "fontName" ) )
        fontName = m_settings->value( "fontName" ).toString();
    ...
    if ( QFile::exists( backPath ) ) {
        ...
        if ( msgBox.exec() == QMessageBox::Open )
            ...
            QFile::remove( backPath ); // Remove backup file
    }
}
...
void MainWindow::readSettings() {
    ...
    if ( m_settings->contains( "autoBck" ) )
        m_autoBck = m_settings->value( "autoBck" ).toInt();
    Circuit::self()->setAutoBck( m_autoBck );

    m_autoUpdt = 1;
    if ( m_settings->contains( "autoUpdt" ) )
        m_autoUpdt = m_settings->value( "autoUpdt" ).toInt();
}
...
void MainWindow::createWidgets() {
    ...
    connect( m_searchComponent, SIGNAL( editingFinished() ), this, SLOT( searchChanged() ) );
...
QSettings* MainWindow::compSettings() {
    return m_compSettings;
}
