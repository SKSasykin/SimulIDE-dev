...
LibraryItem* Potentiometer::libraryItem() {
    return new LibraryItem( tr( "Potentiometer" ), "Resistors", "potentiometer.png", "Potentiometer",
                            Potentiometer::construct );
}

Potentiometer::Potentiometer( QString type, QString id )
    : Dialed( type, id ), eElement( ( id + "-eElement" ) ), m_pinA( 180, QPoint( -16, 0 ), id + "-PinA", 0, this ),
      m_pinM( 270, QPoint( 0, 16 ), id + "-PinM", 0, this ), m_pinB( 0, QPoint( 16, 0 ), id + "-PinB", 0, this ),
      m_ePinA( id + "-ePinA", 1 ), m_ePinB( id + "-ePinB", 1 ), m_resA( id + "-resA" ), m_resB( id + "-resB" ) {
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        new DoubProp<Potentiometer>( "Resistance", tr( "Resistance" ), "Ω", this,
                                                     &Potentiometer::resistance, &Potentiometer::setResistance ),

                        new DoubProp<Potentiometer>( "Value_Ohm", tr( "Current Value" ), "Ω", this,
                                                     &Potentiometer::getValue, &Potentiometer::setValue ),
                    },
                    0 } );
...
void Potentiometer::initialize() {
    ...
    if ( !m_midEnode )
        m_midEnode = new eNode( m_id + "-mideNode" ); // Not connected: Create mid eNode
...
double Potentiometer::getValue() {
    return m_resistance * m_dialW.value() / 1000;
}
