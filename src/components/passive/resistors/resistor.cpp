...
LibraryItem* Resistor::libraryItem() {
    return new LibraryItem( tr( "Resistor" ), "Resistors", "resistor.png", "Resistor", Resistor::construct );
}

Resistor::Resistor( QString type, QString id ) : Comp2Pin( type, id ), eResistor( id ) {
    m_ePin[0] = m_pin[0];
    m_ePin[1] = m_pin[1];
    ...
    addPropGroup( { tr( "Main" ),
                    { new DoubProp<Resistor>( "Resistance", tr( "Resistance" ), "Ω", this, &Resistor::resistance,
                                              &Resistor::setResistance ) },
                    0 } );
