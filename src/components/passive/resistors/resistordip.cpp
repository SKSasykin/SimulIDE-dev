...
LibraryItem* ResistorDip::libraryItem() {
    return new LibraryItem( tr( "ResistorDip" ), "Resistors", "resistordip.png", "ResistorDip",
                            ResistorDip::construct );
}

ResistorDip::ResistorDip( QString type, QString id ) : Component( type, id ), eElement( id ) {
    m_pullUp = false;
    m_puVolt = 5;
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        new DoubProp<ResistorDip>( "Resistance", tr( "Resistance" ), "Ω", this,
                                                   &ResistorDip::resistance, &ResistorDip::setResistance ),

                        new IntProp<ResistorDip>( "Size", tr( "Size" ), "", this, &ResistorDip::size,
                                                  &ResistorDip::setSize, propNoCopy, "uint" ),

                        new BoolProp<ResistorDip>( "PullUp", tr( "Pullup" ), "", this, &ResistorDip::pullUp,
                                                   &ResistorDip::setPullUp ),

                        new DoubProp<ResistorDip>( "PuVolt", tr( "Pullup Voltage" ), "V", this, &ResistorDip::puVolt,
                                                   &ResistorDip::setPuVolt ),
                    },
                    0 } );
...
void ResistorDip::updateStep() {
    ...
    for ( eResistor* res : m_resistor )
        res->setResistance( m_resistance );
    stamp();
}
...
void ResistorDip::createResistors( int c ) {
    ...
    for ( int i = start; i < m_size; i++ ) {
        ...
        m_pin[index]
            = new Pin( 180, QPoint( -16, -32 + 8 + i * 8 ), reid + "-ePin" + QString::number( index ), 0, this );
        m_resistor[i]->setEpin( 0, m_pin[index] );

        m_pin[index + 1]
            = new Pin( 0, QPoint( 16, -32 + 8 + i * 8 ), reid + "-ePin" + QString::number( index + 1 ), 0, this );
...
void ResistorDip::setSize( int size ) {
    ...
    if ( size < m_size )
        deleteResistors( m_size - size );
    else if ( size > m_size )
        createResistors( size - m_size );
...
void ResistorDip::setPullUp( bool p ) {
    ...
    for ( int i = 0; i < m_size; i++ ) {
        ...
        if ( p )
            m_pin[index]->removeConnector();
        else
            m_pin[index]->setEnode( nullptr );
    }
    update();
}
...
void ResistorDip::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    if ( m_ansiSymbol ) {
        for ( int i = 0; i < m_size; i++ )
            Resistor::drawAnsi( p, 0, -32 + 8 + i * 8, 0.7, 0.6 );
    } else
        p->drawRect( QRect( -9, -28, 18, m_size * 8 ) );
