...
LibraryItem* RTD::libraryItem() {
    return new LibraryItem( tr( "RTD" ), "Resistive Sensors", "rtd.png", "RTD", RTD::construct );
}

RTD::RTD( QString type, QString id ) : ThermistorBase( type, id ) {
...
double RTD::sensorFunction( double temp ) {
    ...
    if ( temp < 0 )
        r_sense += m_r0 * coef_temp_c * ( temp - 100 ) * qPow( temp, 3.0 );
