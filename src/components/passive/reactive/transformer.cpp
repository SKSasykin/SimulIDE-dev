...
LibraryItem* Transformer::libraryItem() {
    return new LibraryItem( tr( "Transformer" ), "Reactive", "transformer.png", "Transformer", Transformer::construct );
}

Transformer::Transformer( QString type, QString id ) : Component( type, id ), eElement( id ) {
    m_coupCoeff = 0.99;
    m_baseInd = 1;
    m_lenght = 0;
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        new DoubProp<Transformer>( "CoupCoeff", tr( "Coupling Coefficient" ), "", this,
                                                   &Transformer::coupCoeff, &Transformer::setCoupCoeff ),

                        new DoubProp<Transformer>( "BaseInd", tr( "Base Inductance" ), "H", this, &Transformer::baseInd,
                                                   &Transformer::setBaseInd ),

                        new StrProp<Transformer>( "Primary", tr( "Primary" ), "", this, &Transformer::primary,
                                                  &Transformer::setPrimary, propNoCopy ),

                        new StrProp<Transformer>( "Secondary", tr( "Secondary" ), "", this, &Transformer::secondary,
                                                  &Transformer::setSecondary, propNoCopy ),
                    },
                    0 } );
...
void Transformer::stamp() {
    ...
    double admit[n][n];
    for ( int i = 0; i < n; i++ )
        admit[i][i] = m_coils.at( i )->inductance;
    for ( int i = 0; i < n; i++ ) {
        eCoil* coili = m_coils.at( i );
        for ( int j = 0; j < i; ++j ) {
            eCoil* coilj = m_coils.at( j );
            admit[i][j] = admit[j][i]
                = m_coupCoeff * qSqrt( coili->inductance * coilj->inductance ) * coili->sign * coilj->sign;
        }
    }
    ...
    for ( int i = 0; i < n; i++ ) {
        ...
        for ( int j = 0; j < n; j++ )
            if ( i != j )
                coili->addIductor( m_coils.at( j ), m_inva[i][j] );
    }
...
void Transformer::runEvent() {
    ...
    for ( int i = 0; i < n; i++ ) {
        double current = 0;
        for ( int j = 0; j < n; ++j )
            current += m_coils.at( j )->getVolt() * m_inva[i][j];
...
void Transformer::createCoils() {
    ...
    if ( priTopo == m_primary.topology )
        modify( &m_primary );
    else
        create( &m_primary );

    if ( secTopo == m_secondary.topology )
        modify( &m_secondary );
    else
        create( &m_secondary );
}
    ...
    for ( int i = 0; i < 2; ++i ) {
        ...
        for ( int coilN = 0; coilN < coils.size(); ++coilN ) {
            ...
            if ( coil->sign >= 0 )
                ...
                p->drawLine( QPointF( x - 0.01, y + 20 ), QPointF( x + 0.01, y + 20 ) );

            for ( int n = 0; n < coil->size; ++n ) {
                if ( i )
                    p->drawArc( QRectF( 6, y, 8, 8 ), 90 * 16, 180 * 16 );
                else
                    p->drawArc( QRectF( -14, y, 8, 8 ), 90 * 16, -180 * 16 );
                y += 8;
            }
        }
    }
