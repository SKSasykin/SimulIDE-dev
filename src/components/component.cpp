...
Component::Component( QString type, QString id ) : CompBase( type, id ), QGraphicsItem() {
    m_help = "";
    m_Hflip = 1;
    m_Vflip = 1;
    ...
    addPropGroup(
        { "CompBase",
          { new StrProp<Component>( "itemtype", "", "", this, &Component::itemType, &Component::setItemType ),
            ...
            new BoolProp<Component>( "mainComp", "", "", this, &Component::isMainComp,
                                     &Component::setMainComp ), // Related to Subcircuit:
...
bool Component::setPropStr( QString prop, QString val ) {
    ...
    if ( prop == m_showProperty ) {
        QString text = "";
        if ( !m_showProperty.isEmpty() )
            text = getPropStr( m_showProperty );
...
void Component::setup() {
    ...
    if ( !showP.isEmpty() )
        setValLabelText( getPropStr( showP ) );
}
...
void Component::mouseMoveEvent( QGraphicsSceneMouseEvent* event ) {
    ...
    if ( !m_moving ) // Get lists of elements to move and save Undo state
    {
        ...
        for ( QGraphicsItem* item : itemList ) {
            if ( item->type() == UserType + 2 ) // ConnectorLine selected
            {
                ...
                if ( !m_conMoveList.contains( con ) )
                    m_conMoveList.append( con ); // Connectors selected
            } else if ( item->type() == UserType + 1 ) // Component selected
            {
                Component* comp = qgraphicsitem_cast<Component*>( item );
                if ( !comp->freeMove( ctrlMod ) )
                    m_moveFree = false;

                m_compMoveList.append( comp );

                std::vector<Pin*> pins = comp->getPins();
                for ( Pin* pin : pins ) { // Connectors attached to selected Component
                    if ( !pin )
                        continue;
        ...
        if ( !m_moveFree && delta == QPointF( 0, 0 ) )
            return;
    ...
    for ( ConnectorLine* line : m_lineMoveList )
        line->moveSimple( delta ); // Move ConnectorLine
    for ( Component* comp : m_compMoveList )
        comp->move( delta ); // Move Components selected
    for ( Connector* con : m_conMoveList )
        con->isMoved(); // Update Connectors
}
...
void Component::slotProperties() {
    if ( !m_propDialog ) {
        if ( m_help == "" ) {
            ...
            if ( m_type == "Subcircuit" || m_type == "MCU" )
                findHelp();
            else
...
void Component::slotH_flip() {
    if ( !m_hidden )
        Circuit::self()->saveCompChange( m_id, "hflip", getPropStr( "hflip" ) );
    m_Hflip = -m_Hflip;
    setflip();
}
...
void Component::slotV_flip() {
    if ( !m_hidden )
        Circuit::self()->saveCompChange( m_id, "vflip", getPropStr( "vflip" ) );
    m_Vflip = -m_Vflip;
    setflip();
}
...
void Component::setflip() {
    ...
    for ( Pin* pin : m_signalPin )
        pin->flip( m_Hflip, m_Vflip );

    if ( !m_hidden )
        moveSignal(); // Used by sockets
}
...
void Component::rotateAngle( double a ) {
    if ( !m_hidden )
        Circuit::self()->saveCompChange( m_id, "rotation", getPropStr( "rotation" ) );
...
QString Component::idLabel() {
    return m_idLabel->toPlainText();
}
void Component::setIdLabel( QString id ) {
    m_idLabel->setPlainText( id );
    m_idLabel->updtLabelPos();
}

QPointF Component::getIdPos() {
    return m_idLabel->getLabelPos();
}
void Component::setIdPos( QPointF p ) {
    m_idLabel->setLabelPos( p );
}

int Component::getIdRot() {
    return m_idLabel->getAngle();
}
void Component::setIdRot( int r ) {
    m_idLabel->setAngle( r );
}
...
QPointF Component::getValPos() {
    return m_valLabel->getLabelPos();
}
void Component::setValPos( QPointF p ) {
    m_valLabel->setLabelPos( p );
}

int Component::getValRot() {
    return m_valLabel->getAngle();
}
void Component::setValRot( int r ) {
    m_valLabel->setAngle( r );
}
...
void Component::updtValLabelPos() {
    m_valLabel->updtLabelPos();
}
...
QString Component::getValLabelText() {
    return m_valLabel->toPlainText();
}

QString Component::showProp() {
    if ( m_showVal )
        return m_showProperty;
    else
        return "";
}
...
void Component::setBackground( QString bck ) {
    ...
    if ( bck.startsWith( "color" ) ) {
        ...
        if ( !QFile::exists( pixmapPath ) )
            pixmapPath = ":/" + bck; // Image not in simulide data folder, use hardcoded image
