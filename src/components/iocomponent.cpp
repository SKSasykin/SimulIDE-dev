...
IoComponent::IoComponent( QString type, QString id ) : Component( type, id ), LogicFamily() {
    m_outValue = 0;
    ...
    addPropGroup( { "IoHidden",
                    {
                        new StrProp<IoComponent>( "invertPins", "", "", this, &IoComponent::invertedPins,
                                                  &IoComponent::setInvertPins, propHidden ),
                    },
                    groupHidden } );
}
...
QList<ComProperty*> IoComponent::familyProps() {
    QList<ComProperty*> props = {
        new StrProp<IoComponent>( "Family", tr( "Logic Family" ), m_families.keys().join( "," ), this,
                                  &IoComponent::family, &IoComponent::setFamily, 0, "enum" ),

        new DoubProp<IoComponent>( "SupplyV", tr( "Supply Voltage" ), "V", this, &IoComponent::supplyV,
                                   &IoComponent::setSupplyV ),
    };
    return props;
}
...
QList<ComProperty*> IoComponent::inputProps() {
    QList<ComProperty*> props
        = { new DoubProp<IoComponent>( "Input_High_V", tr( "Low to High Threshold" ), "V", this, &IoComponent::inpHighV,
                                       &IoComponent::setInpHighV ),

            new DoubProp<IoComponent>( "Input_Low_V", tr( "High to Low Threshold" ), "V", this, &IoComponent::inpLowV,
                                       &IoComponent::setInpLowV ),

            new DoubProp<IoComponent>( "Input_Imped", tr( "Input Impedance" ), "MΩ", this, &IoComponent::inputImp,
                                       &IoComponent::setInputImp ),

            new DoubProp<IoComponent>( "InPullups", tr( "Input pullups" ), "Ω", this, &IoComponent::inpPullups,
                                       &IoComponent::setInpPullups, propNoCopy ) };
    return props;
}

QList<ComProperty*> IoComponent::outputProps() {
    QList<ComProperty*> props = { //new ComProperty("", tr("Outputs:"),"","",0),

                                  new DoubProp<IoComponent>( "Out_High_V", tr( "Output High Voltage" ), "V", this,
                                                             &IoComponent::outHighV, &IoComponent::setOutHighV ),

                                  new DoubProp<IoComponent>( "Out_Low_V", tr( "Output Low Voltage" ), "V", this,
                                                             &IoComponent::outLowV, &IoComponent::setOutLowV ),

                                  new DoubProp<IoComponent>( "Out_Imped", tr( "Output Impedance" ), "Ω", this,
                                                             &IoComponent::outImp, &IoComponent::setOutImp ),

                                  new DoubProp<IoComponent>( "Oen_Imped", tr( "Output Z Impedance" ), "Ω", this,
                                                             &IoComponent::openImp, &IoComponent::setOpenImp ) };
    return props;
}

QList<ComProperty*> IoComponent::outputType() {
    return { new DoubProp<IoComponent>( "Pullups", tr( "Ouput pullups" ), "Ω", this, &IoComponent::outPullups,
                                        &IoComponent::setOutPullups, propNoCopy ),

             new BoolProp<IoComponent>( "Inverted", tr( "Invert Outputs" ), "", this, &IoComponent::invertOuts,
                                        &IoComponent::setInvertOuts, propNoCopy ),

             new BoolProp<IoComponent>( "Open_Collector", tr( "Open Drain" ), "", this, &IoComponent::openCol,
                                        &IoComponent::setOpenCol, propNoCopy ) };
}

QList<ComProperty*> IoComponent::edgeProps() {
    return { new DoubProp<IoComponent>( "pd_n", tr( "Delay Multiplier" ), "", this, &IoComponent::propSize,
                                        &IoComponent::setPropSize ),

             new DoubProp<IoComponent>( "Tpd_ps", tr( "Family Delay" ), "ns", this, &IoComponent::propDelay,
                                        &IoComponent::setPropDelay ),

             new DoubProp<IoComponent>( "Tr_ps", tr( "Rise Time" ), "ns", this, &IoComponent::riseTime,
                                        &IoComponent::setRiseTime ),

             new DoubProp<IoComponent>( "Tf_ps", tr( "Fall Time" ), "ns", this, &IoComponent::fallTime,
                                        &IoComponent::setFallTime ) };
}
...
void IoComponent::scheduleOutPuts( eElement* el ) {
    ...
    {
        if ( m_nextOutVal == m_outQueue.back() )
            return;
...
void IoComponent::setInpHighV( double volt ) {
    ...
    for ( IoPin* pin : m_inpPin )
        pin->setInputHighV( volt );
    for ( IoPin* pin : m_outPin )
        pin->setInputHighV( volt );
    for ( IoPin* pin : m_otherPin )
        pin->setInputHighV( volt );
...
void IoComponent::init( QStringList pins ) // Example: pin = "IL02Name" => input, left, number 2, label = "Name"
{
    ...
    for ( QString pin : pins ) {
        if ( pin.startsWith( "I" ) )
            inputs.append( pin );
        else if ( pin.startsWith( "O" ) )
            outputs.append( pin );
        else
            qDebug() << " LogicComponent::init: pin name error " << pin;
    }
    int i = m_inpPin.size();
...
void IoComponent::setupPin( IoPin* pin, QString data ) // Example data = "L02" => left side, position 2
{
    ...
    } else if ( pos == "R" ) // Right
    {
        ...
        y = m_area.y() + num * 8;
        if ( label.length() == 1 || ( label.length() == 2 && label.startsWith( "!" ) ) )
            label.prepend( " " );
    }
...
void IoComponent::setNumPins( std::vector<IoPin*>* pinList, uint pins, QString label, int bit0, bool out, int id0 ) {
    ...
    if ( pins < oldSize )
        deletePins( pinList, oldSize - pins );
    else
        pinList->resize( pins );

    if ( m_outPin.size() > m_inpPin.size() )
        m_height = m_outPin.size();
    else
        m_height = m_inpPin.size();

    int halfH;
    if ( label.isEmpty() )
        halfH = m_height * 8 / 2; // Gates
    else {
        m_height += 1;
        halfH = ( m_height / 2 ) * 8;
    }
    ...
    for ( uint i = 0; i < pins; ++i ) {
        ...
        if ( i < oldSize )
            ...
            pinList->at( i ) = pin;
            initPin( pin );
            if ( mode == output && m_invOutputs )
                pin->setInverted( true );
            if ( mode == input && m_invInputs )
                pin->setInverted( true );

            if ( !label.isEmpty() )
                pin->setLabelText( label + num );
            pin->setLabelColor( QColor( 0, 0, 0 ) );
        }
        if ( bit0 >= 0 ) {
            id0++;
...
std::vector<Pin*> IoComponent::getPins() {
    ...
    for ( Pin* pin : m_inpPin )
        pins.emplace_back( pin );
    for ( Pin* pin : m_outPin )
        pins.emplace_back( pin );
    for ( Pin* pin : m_otherPin )
        pins.emplace_back( pin );
    return pins;
}
