...
Node::Node( QString type, QString id ) : Component( type, id ) {
    setZValue( 101 );
...
void Node::paint( QPainter* p, const QStyleOptionGraphicsItem* option, QWidget* widget ) {
    ...
    if ( m_isBus )
        p->drawEllipse( QPointF( 0, 0 ), 1.8, 1.8 );
    else
