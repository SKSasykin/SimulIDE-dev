...
ClockBase::ClockBase( QString type, QString id ) : FixedVolt( type, id ) {
...
void ClockBase::setHidden( bool hide, bool hidArea, bool hidLabel ) {
    ...
    if ( hidArea )
        m_area = QRectF( 0, 0, -4, -4 ); // Totally hidden
    else if ( hide )
        m_area = QRectF( -30, -6, 12, 12 ); // In Board
    else
        m_area = QRect( -14, -8, 22, 16 ); // Normal
