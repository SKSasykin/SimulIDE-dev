...
LibraryItem* Battery::libraryItem() {
    return new LibraryItem( tr( "Battery" ), "Sources", "battery.png", "Battery", Battery::construct );
}

Battery::Battery( QString type, QString id ) : Comp2Pin( type, id ), eResistor( id ) {
    ...
    addPropGroup(
        { tr( "Main" ),
          {
              new DoubProp<Battery>( "Voltage", tr( "Voltage" ), "V", this, &Battery::voltage, &Battery::setVoltage ),

              new DoubProp<Battery>( "Resistance", tr( "Resistance" ), "mΩ", this, &Battery::resistance,
                                     &Battery::setResistance ),
          },
          0 } );
}
Battery::~Battery() { }
