...
LibraryItem* WaveGen::libraryItem() {
    return new LibraryItem( tr( "Wave Generator" ), "Sources", "wavegen.png", "WaveGen", WaveGen::construct );
}

WaveGen::WaveGen( QString type, QString id ) : ClockBase( type, id ) {
    m_bipolar = false;
    m_floating = false;
    m_voltBase = -2.5;
    m_phaseShift = 0;
    ...
    QString waves = "Sine,Saw,Triangle,Square,Random,Wav;" + tr( "Sine" ) + "," + tr( "Saw" ) + "," + tr( "Triangle" )
        + "," + tr( "Square" ) + "," + tr( "Random" ) + ",Wav";

    addPropGroup(
        { tr( "Main" ),
          { new StrProp<WaveGen>( "Wave_Type", tr( "Wave Type" ), waves, this, &WaveGen::waveType,
                                  &WaveGen::setWaveType, 0, "enum" ),

            new DoubProp<WaveGen>( "Freq", tr( "Frequency" ), "kHz", this, &WaveGen::freq, &WaveGen::setFreq ),

            new DoubProp<WaveGen>( "Phase", tr( "Phase shift" ), "_º", this, &WaveGen::phaseShift,
                                   &WaveGen::setPhaseShift ),

            new IntProp<WaveGen>( "Steps", tr( "Minimum Steps" ), "", this, &WaveGen::minSteps, &WaveGen::setMinSteps ),
            ...
            new BoolProp<WaveGen>( "Always_On", tr( "Always On" ), "", this, &WaveGen::alwaysOn,
                                   &WaveGen::setAlwaysOn ) },
          0 } );

    addPropGroup( { tr( "Electric" ),
                    { new BoolProp<WaveGen>( "Bipolar", tr( "Bipolar" ), "", this, &WaveGen::bipolar,
                                             &WaveGen::setBipolar, propNoCopy ),

                      new BoolProp<WaveGen>( "Floating", tr( "Floating" ), "", this, &WaveGen::floating,
                                             &WaveGen::setFloating, propNoCopy ),

                      new DoubProp<WaveGen>( "Semi_Ampli", tr( "Semi Amplitude" ), "V", this, &WaveGen::semiAmpli,
                                             &WaveGen::setSemiAmpli ),

                      new DoubProp<WaveGen>( "Mid_Volt", tr( "Middle Voltage" ), "V", this, &WaveGen::midVolt,
                                             &WaveGen::setMidVolt ) },
                    0 } );
...
bool WaveGen::propNotFound( QString prop, QString val ) {
    if ( prop == "Volt_Base" )
        m_voltBase = val.toDouble(); //  Old: TODELETE
    else
        return ClockBase::propNotFound( prop, val );
    return true;
}
...
void WaveGen::initialize() {
    ...
    if ( m_waveType == Wav )
        m_eventTime = m_psPerCycleInt;
    else if ( m_waveType == Square )
        m_eventTime = m_psPerCycleDbl - m_halfW;
    else if ( m_waveType == Random )
        m_eventTime = m_psPerCycleInt / 3;
    else {
        m_eventTime = 0;
        if ( m_isRunning )
            AnalogClock::self()->addClkElement( this );
    }
    if ( m_isRunning && m_eventTime )
        Simulator::self()->addEvent( m_eventTime, this );
}
...
void WaveGen::runEvent() {
    ...
    if ( m_vOut != m_lastVout ) {
        ...
        if ( m_bipolar ) {
            ...
            m_outpin->setVoltage( m_voltBase + m_voltage * m_vOut );
    }
...
void WaveGen::genTriangle() {
    if ( m_time >= m_halfW )
        m_vOut = 1 - ( m_time - m_halfW ) / ( m_psPerCycleDbl - m_halfW );
    else
        m_vOut = m_time / m_halfW;
}
...
void WaveGen::updtProperties() {
    ...
    if ( m_waveType == Triangle )
        showDuty = true;
    else if ( m_waveType == Square )
        showDuty = true;
    else if ( m_waveType == Wav )
        showFile = true;
...
void WaveGen::slotLoad() {
    ...
    if ( fil.isEmpty() )
        fil = Circuit::self()->getFilePath();

    const QString dir = fil;

    QString fileName
        = QFileDialog::getOpenFileName( 0l, tr( "Load Image" ), dir, tr( "Wav files (*.wav);;All files (*.*)" ) );
...
void WaveGen::setFile( QString fileName ) {
    ...
    if ( dataSize ) // Read samples
    {
        ...
        {
            qDebug() << "WaveGen::setFile Audio format: PCM" << m_bitsPerSample << "bits" << m_numChannels
                     << "Channels";
            if ( bytes == 1 ) {
                m_minValue = 0;
                m_maxValue = 255;
                ...
                qDebug() << "WaveGen::setFile Error: PCM format" << bytes << "bytes";
                WAVFile.close();
                return;
            }
        ...
        {
            if ( bytes == 4 || bytes == 8 ) {
                qDebug() << "WaveGen::setFile Audio format: IEEE_FLOAT" << m_bitsPerSample << "bits" << m_numChannels
                         << "Channels";
                m_minValue = -1;
                m_maxValue = 1;
            } else {
                qDebug() << "WaveGen::setFile Error: IEEE_FLOAT format" << bytes << "bytes";
                WAVFile.close();
                return;
            }
        } else {
            qDebug() << "WaveGen::setFile Error: Audio format not supported:";
            WAVFile.close();
            return;
        }
        ...
        qDebug() << "WaveGen::setFile Error reading wav file:\n" << fileNameAbs;
    qDebug() << "\n";
