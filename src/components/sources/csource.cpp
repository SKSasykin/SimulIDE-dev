...
LibraryItem* Csource::libraryItem() {
    return new LibraryItem( tr( "Controlled Source" ), "Sources", "csource.png", "Csource", Csource::construct );
}

Csource::Csource( QString type, QString id ) : Component( type, id ), eResistor( id ) {
    ...
    addPropGroup( { tr( "Main" ),
                    { new BoolProp<Csource>( "Control_Pins", tr( "Use Control Pins" ), "", this, &Csource::controlPins,
                                             &Csource::setControlPins, propNoCopy ),

                      new BoolProp<Csource>( "CurrSource", tr( "Current Source" ), "", this, &Csource::currSource,
                                             &Csource::setCurrSource ),

                      new BoolProp<Csource>( "CurrControl", tr( "Current Controlled" ), "", this, &Csource::currControl,
                                             &Csource::setCurrControl, propNoCopy ),
                      ...
                      new DoubProp<Csource>( "Current", tr( "Current" ), "A", this, &Csource::outCurrent,
                                             &Csource::setOutCurrent ) },
                    0 } );
}
Csource::~Csource() { }
...
void Csource::setVoltage( double v ) {
    ...
    if ( m_currSource )
        curr = -curr; // Current source
    else if ( curr != 0 )
        curr /= low_imp; // Voltage source
    if ( m_currControl )
        curr *= m_admit; // Current controlled
...
void Csource::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    if ( !m_controlPins && !m_linkedTo ) {
        ...
        points << QPointF( -8, 0 ) << QPointF( 0, -13 ) << QPointF( 8, 0 ) << QPointF( 0, 13 );
