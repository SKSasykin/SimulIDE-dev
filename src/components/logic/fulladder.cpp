...
LibraryItem* FullAdder::libraryItem() {
    return new LibraryItem( tr( "Full Adder" ), "Arithmetic", "2to2.png", "FullAdder", FullAdder::construct );
}

FullAdder::FullAdder( QString type, QString id ) : IoComponent( type, id ), eElement( id ) {
    m_width = 3;
    m_height = 4;
    m_bits = 0;
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        new IntProp<FullAdder>( "Bits", tr( "Size" ), "_bits", this, &FullAdder::bits,
                                                &FullAdder::setBits, propNoCopy, "uint" ),
                    },
                    groupNoCopy } );
...
void FullAdder::voltChanged() {
    ...
    for ( int i = 0; i < m_bits; ++i ) {
        if ( m_inpPin[1 + i]->getInpState() )
            A |= 1 << i;
        if ( m_inpPin[1 + i + m_bits]->getInpState() )
            B |= 1 << i;
    }
    m_nextOutVal = A + B + Ci;
