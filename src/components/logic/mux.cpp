...
LibraryItem* Mux::libraryItem() {
    return new LibraryItem( tr( "Mux" ), "Converters", "mux.png", "Mux", Mux::construct );
}

Mux::Mux( QString type, QString id ) : LogicComponent( type, id ) {
    m_width = 4;
    m_height = 10;
    m_tristate = true;
    ...
    addPropGroup( { tr( "Main" ),
                    { new IntProp<Mux>( "Address_Bits", tr( "Address Size" ), "_bits", this, &Mux::addrBits,
                                        &Mux::setAddrBits, propNoCopy ) },
                    groupNoCopy } );
    ...
    addPropGroup(
        { tr( "Inputs" ),
          IoComponent::inputProps()
              + QList<ComProperty*>( { new BoolProp<Mux>( "Invert_Inputs", tr( "Invert Inputs" ), "", this,
    ...
    addPropGroup(
        { tr( "Outputs" ),
          IoComponent::outputProps()
              + QList<ComProperty*>( {
                  new BoolProp<Mux>( "Tristate", tr( "Tristate" ), "", this, &Mux::tristate, &Mux::setTristate ),
              } ),
          0 } );
...
void Mux::setAddrBits( int bits ) {
    ...
    for ( int i = 0; i < 3; ++i ) {
        ...
        if ( i < 2 )
            m_outPin[i]->setY( -h + i * 8 + 16 ); // Outputs
    }
    for ( int i = 0; i < 8; ++i ) {
        Pin* pin = m_inpPin[i];
        if ( i < channels ) {
...
QPainterPath Mux::shape() const {
    ...
    points << QPointF( -w, -h - 6 ) << QPointF( -w, h + 6 ) << QPointF( w, h - 2 ) << QPointF( w, -h + 2 );
...
void Mux::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* wi ) {
    ...
    QPointF points[4] = { QPointF( -w, -h - 6 ), QPointF( -w, h + 6 ), QPointF( w, h - 2 ), QPointF( w, -h + 2 ) };
