...
LibraryItem* BcdTo7S::libraryItem() {
    return new LibraryItem( tr( "Bcd To 7S." ), "Converters", "2to3g.png", "BcdTo7S", BcdTo7S::construct );
}

BcdTo7S::BcdTo7S( QString type, QString id ) : BcdBase( type, id ) {
    m_width = 4;
    m_height = 8;
    ...
    init( { // Inputs:
            "IL03S0", "IL04S1", "IL05S2", "IL06S3",
            // Outputs:
            "OR01a", "OR02b", "OR03c", "OR04d", "OR05e", "OR06f", "OR07g" } );
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        new StrProp<BcdTo7S>( "Custom_Segments", tr( "Custom Characters CSV" ), "", this,
                                              &BcdTo7S::customChars, &BcdTo7S::setCustomChars, propNoCopy ),

                        new BoolProp<BcdTo7S>( "UseRS", tr( "Reset Pin" ), "", this, &BcdTo7S::pinReset,
                                               &BcdTo7S::setPinReset, propNoCopy ),
                    },
                    0 } );
    ...
    addPropGroup( { tr( "Inputs" ),
                    IoComponent::inputProps()
                        + QList<ComProperty*>(
                            { new BoolProp<BcdTo7S>( "Invert_Inputs", tr( "Invert Inputs" ), "", this,
    ...
    addPropGroup( { tr( "Outputs" ),
                    IoComponent::outputProps() + IoComponent::outputType()
                        + QList<ComProperty*>( {
                            new BoolProp<BcdTo7S>( "Tristate", tr( "Tristate" ), "", this, &BcdTo7S::tristate,
                                                   &BcdTo7S::setTristate ),
                        } ),
                    0 } );
...
void BcdTo7S::voltChanged() {
    ...
    BcdBase::voltChanged();
    if ( m_useReset && m_resetPin->getInpState() )
        m_nextOutVal = 0;
    else
