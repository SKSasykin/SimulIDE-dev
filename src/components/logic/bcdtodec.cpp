...
LibraryItem* BcdToDec::libraryItem() {
    return new LibraryItem( tr( "Decoder(4 to 10/16)" ), "Converters", "2to3g.png", "BcdToDec", BcdToDec::construct );
}

BcdToDec::BcdToDec( QString type, QString id ) : LogicComponent( type, id ) {
    m_width = 4;
    m_height = 11;
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        new BoolProp<BcdToDec>( "_16_Bits", tr( "16 Bits" ), "", this, &BcdToDec::is16Bits,
                                                &BcdToDec::set_16bits, propNoCopy ),
                    },
                    groupNoCopy } );
    ...
    addPropGroup( { tr( "Inputs" ),
                    IoComponent::inputProps()
                        + QList<ComProperty*>(
                            { new BoolProp<BcdToDec>( "Invert_Inputs", tr( "Invert Inputs" ), "", this,
                                                      &BcdToDec::invertInps, &BcdToDec::setInvertInps, propNoCopy ) } ),
                    0 } );

    addPropGroup( { tr( "Outputs" ),
                    IoComponent::outputProps() + IoComponent::outputType()
                        + QList<ComProperty*>( {
                            new BoolProp<BcdToDec>( "Tristate", tr( "Tristate" ), "", this, &BcdToDec::tristate,
                                                    &BcdToDec::setTristate ),
                        } ),
                    0 } );
