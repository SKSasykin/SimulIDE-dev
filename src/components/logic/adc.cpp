...
LibraryItem* ADC::libraryItem() {
    return new LibraryItem( tr( "ADC" ), "Other Logic", "1to3.png", "ADC", ADC::construct );
}

ADC::ADC( QString type, QString id ) : LogicComponent( type, id ) {
    m_width = 4;
    m_height = 9;
    ...
    addPropGroup(
        { tr( "Main" ),
          {
              new IntProp<ADC>( "Num_Bits", tr( "Size" ), "_bits", this, &ADC::numOuts, &ADC::setNumOutputs, propNoCopy,
                                "uint" ),
...
void ADC::voltChanged() {
    ...
    if ( m_nextOutVal > m_maxValue )
        m_nextOutVal = m_maxValue;
