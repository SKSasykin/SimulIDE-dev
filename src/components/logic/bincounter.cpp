...
LibraryItem* BinCounter::libraryItem() {
    return new LibraryItem( tr( "Binary Counter" ), "Arithmetic", "2to3g.png", "BinCounter", BinCounter::construct );
}

BinCounter::BinCounter( QString type, QString id ) : LogicComponent( type, id ) {
    m_topValue = 15;
    m_width = 4;
    m_height = 6;
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        ...
                        new IntProp<BinCounter>( "Bits", tr( "Size" ), "_bits", this, &BinCounter::bits,
                                                 &BinCounter::setBits, propNoCopy, "uint" ),

                        new IntProp<BinCounter>( "Max_Value", tr( "Top Value" ), "", this, &BinCounter::maxVal,
                                                 &BinCounter::setMaxVal, 0, "uint" ),

                        new BoolProp<BinCounter>( "Parallel_input", tr( "Parallel Input" ), "", this,
                                                  &BinCounter::parallelIn, &BinCounter::setParallelIn, propNoCopy ),

                        new BoolProp<BinCounter>( "RCO", tr( "Use RCO & RBO" ), "", this, &BinCounter::useRCO,
                                                  &BinCounter::setUseRCO, propNoCopy ),

                        new BoolProp<BinCounter>( "Bidir", tr( "Bidirectional" ), "", this, &BinCounter::bidirectional,
                                                  &BinCounter::setBidirectional, propNoCopy ),
    ...
    addPropGroup( { tr( "Inputs" ),
                    IoComponent::inputProps()
                        + QList<ComProperty*>( { new BoolProp<BinCounter>( "Invert_Inputs", tr( "Invert Inputs" ), "",
                                                                           this, &BinCounter::invertInps,
                                                                           &BinCounter::setInvertInps, propNoCopy ) } ),
                    0 } );
    ...
    } else if ( clkRising ) // Clock edge
    {
        if ( m_dirPin->getInpState() ) // Count up
        {
            ...
            if ( m_counter == m_topValue )
                m_rcoPin->scheduleState( true, m_delayBase * m_delayMult );
            ...
            m_counter--;
            if ( m_counter == 0 )
                m_rboPin->scheduleState( true, m_delayBase * m_delayMult );
            else if ( m_counter < 0 ) {
