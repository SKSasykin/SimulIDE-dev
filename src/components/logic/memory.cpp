...
LibraryItem* Memory::libraryItem() {
    return new LibraryItem( tr( "Ram/Rom" ), "Memory", "2to3g.png", "Memory", Memory::construct );
}

Memory::Memory( QString type, QString id ) : IoComponent( type, id ), eElement( id ), MemData() {
    m_width = 4;
    m_height = 11;
    ...
    addPropGroup(
        { tr( "Main" ),
          { new IntProp<Memory>( "Address_Bits", tr( "Address Size" ), "_bits", this, &Memory::addrBits,
                                 &Memory::setAddrBits, propNoCopy, "uint" ),

            new IntProp<Memory>( "Data_Bits", tr( "Data Size" ), "_bits", this, &Memory::dataBits, &Memory::setDataBits,
                                 propNoCopy, "uint" ),

            new BoolProp<Memory>( "Persistent", tr( "Persistent" ), "", this, &Memory::persistent,
                                  &Memory::setPersistent ),
    ...
    addPropGroup( { tr( "Inputs" ),
                    IoComponent::inputProps()
                        + QList<ComProperty*>(
                            { new BoolProp<Memory>( "Invert_Inputs", tr( "Invert Inputs" ), "", this,
                                                    &Memory::invertInps, &Memory::setInvertInps, propNoCopy ) } ),
                    0 } );
    ...
    addPropGroup(
        { "Hidden", { new StrProp<Memory>( "Mem", "", "", this, &Memory::getMem, &Memory::setMem ) }, groupHidden } );
}
Memory::~Memory() { }
...
void Memory::updateStep() {
    if ( m_changed ) {
        for ( IoPin* pin : m_inpPin )
            pin->changeCallBack( this, m_asynchro && m_cs );
        for ( IoPin* pin : m_outPin )
            pin->changeCallBack( this, m_asynchro && m_cs && m_we );
        m_changed = false;
    }
    if ( m_memTable )
...
void Memory::slotShowTable() {
    ...
    if ( m_persistent )
        m_memTable->setWindowTitle( "ROM: " + idLabel() );
    else
        m_memTable->setWindowTitle( "RAM: " + idLabel() );
    m_memTable->setData( &m_ram, m_dataBytes );
}
