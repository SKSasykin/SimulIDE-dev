...
LibraryItem* SevenSegmentBCD::libraryItem() {
    return new LibraryItem( tr( "7 Seg BCD" ), "Other Logic", "7segbcd.png", "7 Seg BCD,7-Seg BCD",
                            SevenSegmentBCD::construct );
}

SevenSegmentBCD::SevenSegmentBCD( QString type, QString id ) : BcdBase( type, id ) {
    m_graphical = true;
    ...
    pinList // Inputs:
        << "ID041" << "ID032" << "ID024" << "ID018" << "IU04E" << "IU01.";
    init( pinList );
    ...
    addPropGroup( { tr( "Main" ),
                    { new BoolProp<SevenSegmentBCD>( "Show_Point_Pin", tr( "Show Point Pin" ), "", this,
                                                     &SevenSegmentBCD::isShowDotPin, &SevenSegmentBCD::setShowDotPin,
                                                     propNoCopy ),

                      new BoolProp<SevenSegmentBCD>( "Show_Enable_Pin", tr( "Show Enable Pin" ), "", this,
                                                     &SevenSegmentBCD::isShowEnablePin,
                                                     &SevenSegmentBCD::setShowEnablePin, propNoCopy ) },
                    groupNoCopy } );
...
void SevenSegmentBCD::setLinkedValue( double v, int i ) {
    ...
    vInt &= 0xFF;
    if ( i == 0 )
        m_digit = m_segments[vInt]; // Display value
    else
        m_digit = vInt; // 1 bit for each segment
    m_changed = true;
}
...
void SevenSegmentBCD::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    if ( m_digit & 1 << 0 )
        p->drawLine( x1 + tk + ds, y1, x2 - tk + ds, y1 );
    if ( m_digit & 1 << 1 )
        p->drawLine( x2 + ds, y1 + tk, x2, -tk );
    if ( m_digit & 1 << 2 )
        p->drawLine( x2, tk, x2 - ds, y2 - tk );
    if ( m_digit & 1 << 3 )
        p->drawLine( x2 - tk - ds, y2, x1 + tk - ds, y2 );
    if ( m_digit & 1 << 4 )
        p->drawLine( x1 - ds, y2 - tk, x1, tk );
    if ( m_digit & 1 << 5 )
        p->drawLine( x1, -tk, x1 + ds, y1 + tk );
    if ( m_digit & 1 << 6 )
        p->drawLine( x1 + tk, 0, x2 - tk, 0 );
    if ( m_digit & 1 << 7 ) { // Point
        p->setPen( Qt::NoPen );
