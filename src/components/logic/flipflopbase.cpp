...
FlipFlopBase::FlipFlopBase( QString type, QString id ) : LogicComponent( type, id ) {
    m_dataPins = 0;
    m_useRS = true;
    //m_srInv = false;
    ...
    addPropGroup(
        { tr( "Main" ),
          {
              new BoolProp<FlipFlopBase>( "UseRS", tr( "Use Set/Reset Pins" ), "", this, &FlipFlopBase::pinsRS,
                                          &FlipFlopBase::usePinsRS, propNoCopy ),
              ...
              new StrProp<FlipFlopBase>( "Trigger", tr( "Trigger Type" ), LogicComponent::m_triggerList, this,
...
bool FlipFlopBase::sPinState() {
    if ( m_useRS )
        return m_setPin->getInpState();
    return false;
}

bool FlipFlopBase::rPinState() {
    if ( m_useRS )
        return m_rstPin->getInpState();
    return false;
}
