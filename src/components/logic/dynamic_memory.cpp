...
LibraryItem* DynamicMemory::libraryItem() {
    return new LibraryItem( tr( "Dynamic Ram" ), "Memory", "2to3g.png", "DynamicMemory", DynamicMemory::construct );
}

DynamicMemory::DynamicMemory( QString type, QString id ) : LogicComponent( type, id ), MemData() {
    m_width = 4;
    m_height = 11;
    ...
    addPropGroup(
        { tr( "Main" ),
          { new IntProp<DynamicMemory>( "Row_Bits", tr( "Row Address Size" ), "_bits", this,
                                        &DynamicMemory::rowAddrBits, &DynamicMemory::setRowAddrBits, propNoCopy,
                                        "uint" ),

            new IntProp<DynamicMemory>( "Column_Bits", tr( "Column Address Size" ), "_bits", this,
                                        &DynamicMemory::colAddrBits, &DynamicMemory::setColAddrBits, propNoCopy,
                                        "uint" ),

            new IntProp<DynamicMemory>( "Data_Bits", tr( "Data Size" ), "_bits", this, &DynamicMemory::dataBits,
                                        &DynamicMemory::setDataBits, propNoCopy, "uint" ),

            new DoubProp<DynamicMemory>( "Refresh", tr( "Refresh period" ), "ps", this, &DynamicMemory::refreshPeriod,
                                         &DynamicMemory::setRefreshPeriod ) },
          groupNoCopy } );
    ...
    addPropGroup( { tr( "Inputs" ),
                    IoComponent::inputProps()
                        + QList<ComProperty*>( { new BoolProp<DynamicMemory>(
                            "Invert_Inputs", tr( "Invert Inputs" ), "", this, &DynamicMemory::invertInps,
                            &DynamicMemory::setInvertInps, propNoCopy ) } ),
                    0 } );
...
void DynamicMemory::stamp() // Called at Simulation Start
{
    ...
    for ( int& data : m_ram )
        data = rand() % (int) ( pow( 2, m_dataBits ) );
...
void DynamicMemory::voltChanged() // Some Pin Changed State, Manage it
{
    ...
    if ( m_oe != OE ) {
        m_oe = OE;
        for ( IoPin* pin : m_outPin )
            pin->setPinMode( OE ? output : input ); // Enable/Disable output buffers
    }
        ...
        if ( RAS ) // falling edge at /RAS
        {
            ...
            if ( !m_cas ) { // RAS before CAS - row refresh from address bus
                ...
                if ( m_intRefresh >= pow( 2, m_rowAddrBits ) )
                    m_intRefresh = 0; //qDebug() << "Internal refresh row" << m_rowAddress;
            }
            if ( Simulator::self()->circTime() > ( m_rowLastRefresh[refreshRow] + m_refreshPeriod ) ) {
                m_refreshError
                    = true; //qDebug() << "Row" << refreshRow << "of dynamic RAM " + idLabel() + " has not been refreshed for" << ( Simulator::self()->circTime() - m_rowLastRefresh[refreshRow] ) / 1e9 << "ms";
            }
...
void DynamicMemory::setRowAddrBits( int rowAddrBits ) {
    ...
    if ( rowAddrBits == m_rowAddrBits )
        return;
    if ( rowAddrBits == 0 )
        rowAddrBits = 8;
    if ( ( rowAddrBits + m_colAddrBits ) > 24 )
        rowAddrBits = 24 - m_colAddrBits;
    ...
    m_ram.resize( pow(
        2,
        m_rowAddrBits
            + m_colAddrBits ) ); // this is the second resize, because there is the first resize in function setAddrBits
...
void DynamicMemory::setColAddrBits( int colAddrBits ) {
    ...
    if ( colAddrBits == m_colAddrBits )
        return;
    if ( colAddrBits == 0 )
        colAddrBits = 8;
    if ( ( m_rowAddrBits + colAddrBits ) > 24 )
        colAddrBits = 24 - m_rowAddrBits;
...
void DynamicMemory::setAddrBits( int bits ) {
    ...
    if ( bits < m_addrBits )
        deleteAddrBits( m_addrBits - bits );
    else if ( bits > m_addrBits )
        createAddrBits( bits - m_addrBits );
    m_addrBits = bits;
...
void DynamicMemory::setDataBits( int bits ) {
    ...
    if ( bits < m_dataBits )
        deleteDataBits( m_dataBits - bits );
    else if ( bits > m_dataBits )
        createDataBits( bits - m_dataBits );
