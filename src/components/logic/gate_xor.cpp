...
LibraryItem* XorGate::libraryItem() {
    return new LibraryItem( tr( "Xor Gate" ), "Gates", "xorgate.png", "Xor Gate", XorGate::construct );
}

XorGate::XorGate( QString type, QString id ) : Gate( type, id, 2 ) {
    XorGate::updatePath();
    ...
    addPropGroup( { tr( "Inputs" ),
                    IoComponent::inputProps()
                        + QList<ComProperty*>(
                            { new BoolProp<XorGate>( "Invert_Inputs", tr( "Invert Inputs" ), "", this,
                                                     &XorGate::invertInps, &XorGate::setInvertInps, propNoCopy ) } ),
                    0 } );
