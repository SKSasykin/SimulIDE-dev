...
LibraryItem* ShiftReg::libraryItem() {
    return new LibraryItem( tr( "Shift Register" ), "Arithmetic", "1to3.png", "ShiftReg", ShiftReg::construct );
}

ShiftReg::ShiftReg( QString type, QString id ) : LogicComponent( type, id ) {
    m_width = 4;
    m_height = 9;
    m_tristate = true;
    m_parallelIn = false;
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        new IntProp<ShiftReg>( "Bits", tr( "Size" ), "_bits", this, &ShiftReg::bits, &ShiftReg::setBits,
                                               propNoCopy, "uint" ),

                        new BoolProp<ShiftReg>( "Parallel_input", tr( "Parallel Input" ), "", this,
                                                &ShiftReg::parallelIn, &ShiftReg::setParallelIn, propNoCopy ),

                        new BoolProp<ShiftReg>( "Bidirectional", tr( "Bidirectional" ), "", this,
                                                &ShiftReg::bidirectional, &ShiftReg::setBidirectional, propNoCopy ),
    ...
    addPropGroup( { tr( "Outputs" ),
                    IoComponent::outputProps() + IoComponent::outputType()
                        + QList<ComProperty*>( {
                            new BoolProp<ShiftReg>( "Tristate", tr( "Tristate" ), "", this, &ShiftReg::tristate,
                                                    &ShiftReg::setTristate ),
                        } ),
                    0 } );
...
void ShiftReg::voltChanged() {
    ...
    if ( reset )
        m_nextOutVal = 0; // Reset shift register
