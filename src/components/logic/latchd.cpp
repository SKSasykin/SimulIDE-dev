...
LibraryItem* LatchD::libraryItem() {
    return new LibraryItem( tr( "Latch" ), "Memory", "subc.png", "LatchD", LatchD::construct );
}

LatchD::LatchD( QString type, QString id ) : LogicComponent( type, id ) {
    m_width = 4;
    m_height = 10;
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        new IntProp<LatchD>( "Channels", tr( "Size" ), "_bits", this, &LatchD::channels,
                                             &LatchD::setChannels, propNoCopy, "uint" ),

                        new StrProp<LatchD>( "Trigger", tr( "Trigger Type" ), LogicComponent::m_triggerList, this,
                                             &LatchD::triggerStr, &LatchD::setTriggerStr, propNoCopy, "enum" ),

                        new BoolProp<LatchD>( "UseRS", tr( "Reset Pin" ), "", this, &LatchD::pinReset,
                                              &LatchD::setPinReset, propNoCopy ),
                    },
                    groupNoCopy } );
    ...
    addPropGroup( { tr( "Outputs" ),
                    IoComponent::outputProps() + IoComponent::outputType()
                        + QList<ComProperty*>( {
                            new BoolProp<LatchD>( "Tristate", tr( "Tristate" ), "", this, &LatchD::tristate,
                                                  &LatchD::setTristate ),
                        } ),
                    0 } );
