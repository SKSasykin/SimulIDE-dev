...
LibraryItem* Lm555::libraryItem() {
    return new LibraryItem( "LM555", "Other Logic", "ic2.png", "Lm555", Lm555::construct );
}

Lm555::Lm555( QString type, QString id )
    : Component( type, id ), eElement( id ), m_ePinA( ( id + "-ePinA" ), 0 ), m_ePinB( ( id + "-ePinB" ), 0 ),
      m_ePinC( ( id + "-ePinC" ), 0 ), m_ePinD( ( id + "-ePinD" ), 0 ), m_resA( ( id + "-resA" ) ),
      m_resB( ( id + "-resB" ) ), m_resD( ( id + "-resD" ) ) {
...
void Lm555::stamp() {
    ...
    for ( int i = 0; i < 8; ++i ) {
        if ( ( i == 2 ) || ( i == 6 ) )
            continue; // Output or Discharge
        if ( m_pin[i]->isConnected() )
            m_pin[i]->changeCallBack( this ); //  ->getEnode()->addToNoLinList(this);
    }
}

void Lm555::voltChanged() {
    if ( !m_Gnd->isConnected() || !m_Vcc->isConnected() )
        return;
