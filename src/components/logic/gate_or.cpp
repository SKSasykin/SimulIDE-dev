...
LibraryItem* OrGate::libraryItem() {
    return new LibraryItem( tr( "Or Gate" ), "Gates", "orgate.png", "Or Gate", OrGate::construct );
}

OrGate::OrGate( QString type, QString id ) : Gate( type, id, 2 ) {
    OrGate::updatePath();
    ...
    addPropGroup( { tr( "Inputs" ),
                    IoComponent::inputProps()
                        + QList<ComProperty*>( {
                            new IntProp<OrGate>( "Num_Inputs", tr( "Input Size" ), "_Inputs", this, &OrGate::numInps,
                                                 &OrGate::setNumInputs, propNoCopy, "uint" ),

                            new BoolProp<OrGate>( "Invert_Inputs", tr( "Invert Inputs" ), "", this, &OrGate::invertInps,
                                                  &OrGate::setInvertInps, propNoCopy ),
                        } ),
                    0 } );
