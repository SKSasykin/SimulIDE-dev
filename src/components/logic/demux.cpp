...
LibraryItem* Demux::libraryItem() {
    return new LibraryItem( tr( "Demux" ), "Converters", "demux.png", "Demux", Demux::construct );
}

Demux::Demux( QString type, QString id ) : LogicComponent( type, id ) {
    m_width = 4;
    m_height = 10;
    m_tristate = true;

    init( { // Inputs:
            "ID03S0", "ID02S1", "ID01S2", "IL05DI" } );
    ...
    addPropGroup( { tr( "Main" ),
                    { new IntProp<Demux>( "Address_Bits", tr( "Address Size" ), "_bits", this, &Demux::addrBits,
                                          &Demux::setAddrBits, propNoCopy, "uint" ) },
                    groupNoCopy } );
    ...
    addPropGroup(
        { tr( "Outputs" ),
          IoComponent::outputProps() + IoComponent::outputType()
              + QList<ComProperty*>( {
                  new BoolProp<Demux>( "Tristate", tr( "Tristate" ), "", this, &Demux::tristate, &Demux::setTristate ),
              } ),
          0 } );
...
QPainterPath Demux::shape() const {
    ...
    points << QPointF( -w, -h + 2 ) << QPointF( -w, h - 2 ) << QPointF( w, h + 6 ) << QPointF( w, -h - 6 );
...
void Demux::paint( QPainter* p, const QStyleOptionGraphicsItem* option, QWidget* widget ) {
    ...
    QPointF points[4] = { QPointF( -w, -h + 2 ), QPointF( -w, h - 2 ), QPointF( w, h + 6 ), QPointF( w, -h - 6 ) };
