...
LibraryItem* DAC::libraryItem() {
    return new LibraryItem( tr( "DAC" ), "Other Logic", "3to1.png", "DAC", DAC::construct );
}

DAC::DAC( QString type, QString id ) : LogicComponent( type, id ) {
    m_width = 4;
    m_height = 9;
    ...
    addPropGroup(
        { tr( "Main" ),
          { new IntProp<DAC>( "Num_Bits", tr( "Size" ), "_bits", this, &DAC::numInps, &DAC::setNumInputs, propNoCopy,
                              "uint" ),
    ...
    addPropGroup( { tr( "Inputs" ),
                    IoComponent::inputProps()
                        + QList<ComProperty*>( { new BoolProp<IoComponent>(
                            "Invert_Inputs", tr( "Invert Inputs" ), "", this, &IoComponent::invertInps,
                            &IoComponent::setInvertInps, propNoCopy ) } ),
                    0 } );
