...
LibraryItem* SerialPort::libraryItem() {
    return new LibraryItem( tr( "Serial Port" ), "Peripherals", "serialport.png", "SerialPort", SerialPort::construct );
}

SerialPort::SerialPort( QString type, QString id )
    : Component( type, id ), UsartModule( nullptr, id + "-Uart" ), eElement( ( id + "-eElement" ) ) {
    ...
    addPropGroup(
        { tr( "Main" ),
          {
              new BoolProp<SerialPort>( "Auto", tr( "Auto Open" ), "", this, &SerialPort::autoOpen,
                                        &SerialPort::setAutoOpen ),
    ...
    addPropGroup( { tr( "Config" ),
                    {
                        new IntProp<SerialPort>( "Baudrate", tr( "Baudrate" ), "_Bd", this, &SerialPort::baudRate,
                                                 &SerialPort::setBaudRate, 0, "uint" ),

                        new IntProp<SerialPort>( "DataBits", tr( "Data Bits" ), "_bits", this, &SerialPort::dataBits,
                                                 &SerialPort::setDataBits, 0, "uint" ),

                        new IntProp<SerialPort>( "StopBits", tr( "Stop Bits" ), "_bits", this, &SerialPort::stopBits,
                                                 &SerialPort::setStopBits, 0, "uint" ),
                    },
                    0 } );

    addPropGroup( { "Hidden",
                    {
                        new BoolProp<SerialPort>( "SerialMon", "", "", this, &SerialPort::monitorOpen,
                                                  &SerialPort::setSerialMon ),
                    },
                    groupHidden } );
}
...
void SerialPort::setflip() {
    Component::setflip();
    m_proxy->setPos(
        QPoint( -4 + ( m_Hflip > 0 ? 0 : m_button->width() ), -10 + ( m_Vflip > 0 ? 0 : m_button->height() ) ) );
