...
LibraryItem* SerialTerm::libraryItem() {
    return new LibraryItem( tr( "Serial Terminal" ), "Peripherals", "serialterm.png", "SerialTerm",
...
SerialTerm::SerialTerm( QString type, QString id )
    ...
    addPropGroup( { tr( "Config" ),
                    {
                        new IntProp<SerialTerm>( "Baudrate", tr( "Baudrate" ), "_Bd", this, &SerialTerm::baudRate,
                                                 &SerialTerm::setBaudRate, 0, "uint" ),

                        new IntProp<SerialTerm>( "DataBits", tr( "Data Bits" ), "_bits", this, &SerialTerm::dataBits,
                                                 &SerialTerm::setDataBits, 0, "uint" ),

                        new IntProp<SerialTerm>( "StopBits", tr( "Stop Bits" ), "_bits", this, &SerialTerm::stopBits,
                                                 &SerialTerm::setStopBits, 0, "uint" ),
                    },
                    0 } );

    addPropGroup( { "Hidden",
                    {
                        new BoolProp<SerialTerm>( "SerialMon", "", "", this, &SerialTerm::monitorOpen,
                                                  &SerialTerm::setSerialMon ),
                    },
                    groupHidden } );
}
...
void SerialTerm::setflip() {
    Component::setflip();
    m_proxy->setPos(
        QPoint( 8 + ( m_Hflip > 0 ? 0 : m_button->width() ), -10 + ( m_Vflip > 0 ? 0 : m_button->height() ) ) );
