...
LibraryItem* Ds18b20::libraryItem() {
    return new LibraryItem( "DS18B20", "Sensors", "ic2_comp.png", "DS18B20", Ds18b20::construct );
}

Ds18b20::Ds18b20( QString type, QString id )
    : Component( type, id ), OneWire( id ), m_vdd( 270, QPoint( 8, 28 ), id + "-PinVdd", 0, this ),
        ...
        { tr( "Main" ),
          {
              ...
              new DoubProp<Ds18b20>( "TempInc", tr( "Temp. increment" ), "°C", this, &Ds18b20::tempInc,
                                     &Ds18b20::setTempInc ),
          },
          0 } );
}
...
void Ds18b20::convertTemp() // Code 44h, temperature already in the Scratchpad, nothing to do
{
    ...
    if ( m_temp < 0 )
        temp = ~temp + 1; // Temp under 0? Make 2nd complement
    ...
    case 9:
        m_busyTime = 93750 * 1e6;
        break; // 9-bits  93.75 ms
    case 10:
        m_busyTime = 187500 * 1e6;
        break; // 10-bit  187.5 ms
    case 11:
        m_busyTime = 375000 * 1e6;
        break; // 11-bit  375   ms
    case 12:
        m_busyTime = 750000 * 1e6;
        break; // 12-bit  750   ms
    }
...
void Ds18b20::readScratchpad() // Code BEh : send Scratchpad LSB
{
    m_txBuff.clear();
    for ( int i = 8; i >= 0; i-- )
        m_txBuff.push_back( m_scratchpad[i] );
