...
LibraryItem* KY040::libraryItem() {
    return new LibraryItem( tr( "Rotary Encoder (relative)" ), "Peripherals", "ky-040.png", "KY040", KY040::construct );
}

KY040::KY040( QString type, QString id ) : Component( type, id ), eElement( id ) {
    m_graphical = true;
    m_changed = false;
    ...
    addPropGroup( { tr( "Main" ),
                    { new IntProp<KY040>( "Steps", tr( "Steps per Rotation" ), "", this, &KY040::steps,
                                          &KY040::setSteps, 0, "uint" ) },
                    groupNoCopy } );
}
KY040::~KY040() { }
...
void KY040::updateStep() {
    ...
    if ( m_prevDialVal != val ) {
        ...
        if ( m_prevDialVal > top && val < bot )
            m_prevDialVal -= max;
        else if ( m_prevDialVal < bot && val > top )
            m_prevDialVal += max;
