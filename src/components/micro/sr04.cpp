...
LibraryItem* SR04::libraryItem() {
    return new LibraryItem( "HC-SR04", "Sensors", "sr04_ico.png", "SR04", SR04::construct );
}

SR04::SR04( QString type, QString id ) : Component( type, id ), eElement( id ) {
    m_graphical = true;
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        new BoolProp<SR04>( "Slider", tr( "Use slider" ), "", this, &SR04::slider, &SR04::setSlider ),
                    },
                    0 } );
}
SR04::~SR04() { }
...
void SR04::voltChanged() // Called when Trigger Pin changes
{
    ...
    } else if ( m_lastTrig && !trigState ) // Triggered
    {
        ...
        if ( time >= 10 * 1e6 ) // >=10 uS Trigger pulse
        {
            ...
            if ( m_echouS < 116 )
                m_echouS = 116; // Min range 2 cm = 116 us pulse
            else if ( m_echouS > 38000 )
                m_echouS = 38000; // Timeout 38 ms
