...
LibraryItem* DS1307::libraryItem() {
    return new LibraryItem( "DS1307", "Peripherals", "dsxxx_ico.png", "DS1307", DS1307::construct );
}

DS1307::DS1307( QString type, QString id ) : Component( type, id ), TwiModule( id ), m_clock( id + "_clock" ) {
    m_graphical = true;
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        new BoolProp<DS1307>( "Time_Updtd", tr( "Set current time at start" ), "", this,
                                              &DS1307::timeUpdtd, &DS1307::setTimeUpdtd ),
                    },
                    0 } );
}
DS1307::~DS1307() { }
...
void DS1307::writeByte() // Read from RAM
{
    if ( m_addrPtr < 7 ) {
        ...
        case 0: {
            ...
            m_txReg = decToBcd( m_clock.m_time.second() ) | disabled;
            break;
        } break;
        case 1:
            m_txReg = decToBcd( m_clock.m_time.minute() );
            break;
        case 2: {
        ...
        case 3:
            m_txReg = decToBcd( m_clock.m_date.dayOfWeek() );
            break;
        case 4:
            m_txReg = decToBcd( m_clock.m_date.day() );
            break;
        case 5:
            m_txReg = decToBcd( m_clock.m_date.month() );
            break;
        case 6:
            m_txReg = decToBcd( m_clock.m_date.year() - 2000 );
            break;
        }
    } else
...
void DS1307::updtTime() {
    ...
    case 0: {
        ...
        m_clock.m_time.setHMS( m_clock.m_time.hour(), m_clock.m_time.minute(), bcdToDec( m_rxReg & ~( 1 << 7 ) ) );
        break;
    }
    case 1:
        m_clock.m_time.setHMS( m_clock.m_time.hour(), bcdToDec( m_rxReg ), m_clock.m_time.second() );
        break;
    case 2: {
        int hour;
        if ( m_rxReg >= 0x40 ) // 12 hour
        {
            ...
            hour = bcdToDec( m_rxReg & 0b00111111 ); // 24 hout
...
void DS1307::updtDate() {
    switch ( m_addrPtr ) {
    case 4:
        m_clock.m_date.setDate( m_clock.m_date.year(), m_clock.m_date.month(), bcdToDec( m_rxReg ) );
        break;
    case 5:
        m_clock.m_date.setDate( m_clock.m_date.year(), bcdToDec( m_rxReg ), m_clock.m_date.day() );
        break;
    case 6:
        m_clock.m_date.setDate( 2000 + bcdToDec( m_rxReg ), m_clock.m_date.month(), m_clock.m_date.day() );
    }
}
