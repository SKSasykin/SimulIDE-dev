...
LibraryItem* SdCard::libraryItem() {
    return new LibraryItem( tr( "SD Card Reader" ), "Peripherals", "sdcard.png", "SdCard", SdCard::construct );
}

SdCard::SdCard( QString type, QString id )
    : Component( type, id ), SpiModule( id ), m_pinCS( 180, QPoint( -32, -8 ), id + "-PinCS", 0, this, input ),
    ...
    for ( int i = 0; i < 4; i++ )
        m_pin[i]->setLabelColor( Qt::black );
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        new StrProp<SdCard>( "File", tr( "File" ), "", this, &SdCard::getFile, &SdCard::setFile ),
                    },
                    0 } );
}
...
void SdCard::endTransaction() {
    ...
    case 6: // crc
    {
        ...
        if ( ( m_R1 & 1 ) && m_command != 0 && m_command != 8 && m_command != 41 && m_command != 55 && m_command != 58
             && m_command != 59 ) {
...
void SdCard::command() {
    ...
    case 0:
        resetCard();
        break; // GO_IDLE_STATE - init card in spi mode if CS low
    case 1:
        break; // SEND_OP_COND
        /// TODO: Initialization should take hundreds of ms
    case 8:
        m_repply = d_COND; // SEND_IF_COND - verify SD Memory Card interface operating condition.
        m_repply[4] = m_arg & 0xFF; // check patern
        break;
    case 9:
        m_repply = d_CSD;
        break; // SEND_CSD - Read CSD register
    case 10:
        m_repply = d_CID;
        break; // SEND_CID - Read CID register
    case 12:
        m_multi = 0;
        break; // STOP_TRANSMISSION - end multiple block sequence
    case 13: // SEND_STATUS - read the card status register
        m_repply = d_Packet;
        m_repply[1] = 0x00; // R2
        break;
    case 16:
        break; // SET_BLOCKLEN - Set block length (in bytes) for all following block commands
        /// TODO: set block size
    ...
    case 18: // READ_MULTIPLE_BLOCK until a STOP_TRANSMISSION
        ...
        if ( !m_diskImage->seek( m_arg ) ) {
            qDebug() << "SdCard::command: File seek failed!";
            return;
        }
        readDataBlock();
        break;
    ...
    case 32:
        m_start = m_arg / 512;
        break; // ERASE_WR_BLK_START - sets the address of the first block to be erased
    case 33:
        m_end = m_arg / 512;
        break; // ERASE_WR_BLK_END - sets the address of the last block of the continuous range to be erased
    case 38:
        eraseBlocks();
        break; // ERASE    - previously selected blocks
    case 55:
        m_appCom = 1;
        break; // APP_CMD  - Set app command
    case 58:
        m_repply = d_OCR;
        break; // READ_OCR - Read OCR register
    case 59:
        m_doCRC = m_arg;
        break; // CRC_OFF  - Set CRC checking
    default:
...
void SdCard::appCommand() {
    ...
    case 13:
        m_repply = d_STATUS;
        break; // SD_STATUS - Send the SD Status.
    case 23:
        break; // SET_WR_BLK_ERASE_COUNT - Set the number of write blocks to be pre-erased before writing
    case 41:
        m_R1 = 0;
        break; // SD_SEND_OP_COMD - Sends host capacity support information and activates the card's initialization process
    ...
    case 51:
        m_repply = d_SCR;
        break; // SEND_SCR - Reads the SD Configuration Register (SCR).
    default:
        //    m_reply[0] |= 0x04;  // R1 |0|ParameterE|AddressE|EraseE|CRCE|IllegalC|EraseR|idle|
...
void SdCard::eraseBlocks() {
    ...
    if ( !m_diskImage->seek( m_arg ) ) {
        qDebug() << "SdCard::eraseBlocks: File seek failed";
        return;
    }
    for ( uint32_t i = m_start; i < m_end; i++ )
        m_diskImage->write( reinterpret_cast<const char*>( m_buffer ), 512 );
}
...
void SdCard::LoadFile() {
    ...
    QString fileName
        = QFileDialog::getOpenFileName( 0l, tr( "Load Disk Image" ), dir, tr( "img files (*.img);;All files (*.*)" ) );
...
void SdCard::setFile( QString fileName ) {
    ...
    if ( m_diskKb >= 4194304L )
        d_SCR[1] = 0xC1; // Busy:1 CCS:1 resrved:5  S18A:1
    else
        d_SCR[1] = 0x81; // Busy:1 CCS:1 resrved:5  S18A:1
