...
LibraryItem* Esp01::libraryItem() {
    return new LibraryItem( "Esp01 (TCP)", "Peripherals", "esp01_ico.png", "Esp01", Esp01::construct );
}

Esp01::Esp01( QString type, QString id ) : Component( type, id ), UsartModule( nullptr, id + "-Uart" ), eElement( id ) {
    m_graphical = true;
    ...
    addPropGroup(
        { tr( "Main" ),
          {
              new IntProp<Esp01>( "Baudrate", tr( "Baudrate" ), "_Bd", this, &Esp01::baudRate, &Esp01::setBaudRate, 0,
                                  "uint" ),
    ...
    addPropGroup( { "Hidden",
                    {
                        new BoolProp<Esp01>( "SerialMon", "", "", this, &Esp01::monitorOpen, &Esp01::setSerialMon ),
                    },
                    groupHidden } );
}
Esp01::~Esp01() { }
    ...
    case tcpSend: {
        ...
        if ( tcpSocket ) {
            if ( tcpSocket->state() == QAbstractSocket::ConnectedState ) {
                if ( m_debug )
                    qDebug() << "Esp01 - Sending data to link:" << m_link << "\n" << m_tcpData;
                int bytes = tcpSocket->write( m_tcpData );
                ///tcpSocket->flush();                         // Send data immediately

                if ( m_debug )
                    qDebug() << "Esp01 -" << bytes << "Bytes Written to link" << m_link;
                m_uartReply = "\r\nSEND OK\r\n";
                Simulator::self()->addEvent( 1, this ); // Send Reply
            } else
                qDebug() << "Esp01 - Error Sending data: link" << m_link << "not connected\n";
        }
    } break;
    case tcpClose: {
        QTcpSocket* tcpSocket = m_tcpSockets.value( m_link );
        if ( tcpSocket && tcpSocket->state() == QAbstractSocket::ConnectedState ) {
            if ( m_debug )
                qDebug() << "Esp01 - Disconnecting link" << m_link << "from" << m_host << m_port;
            tcpSocket->disconnectFromHost();
        } else if ( m_debug )
            qDebug() << "Esp01 - Error Disconnecting link" << m_link << "from" << m_host << m_port
...
void Esp01::reset() {
    ...
    for ( QTcpSocket* tcpSocket : m_tcpSockets )
        tcpSocket->disconnectFromHost();
    ;
}

void Esp01::runEvent() {
    if ( m_uartReply.isEmpty() )
        return;
    if ( m_debug )
        qDebug() << "Esp01 - Reply:" << m_uartReply;
...
void Esp01::byteReceived( uint8_t byte ) {
    ...
    printIn( byte );
    if ( m_dataLenght ) {
        if ( m_tcpData.size() < m_dataLenght - 2 )
            m_tcpData.append( byte );
...
void Esp01::command() {
    ...
    } else if ( command.startsWith( "AT+CWMODE=" ) ) //
    {
        ...
        if ( m_mode == 1 || m_mode == 0 )
            ...
            m_mode = 0;
    } else if ( command.startsWith( "AT+CWJAP=" ) || command.startsWith( "AT+CWJEAP=" ) ) // Connect to an AP
    {
...
void Esp01::tcpConnected( int link ) {
    if ( m_debug )
        qDebug() << "Esp01 - link" << link << "Connected to" << m_host << m_port;
    connectReply( "CONNECT", link );
}
void Esp01::tcpDisconnected( int link ) {
    if ( m_debug )
        qDebug() << "Esp01 - link" << link << "Disconnected from" << m_host << m_port;
...
void Esp01::tcpReadyRead( int link ) {
    ...
    if ( m_debug )
        qDebug() << "Esp01 - link" << link << "Received from Host:" << tcpSocket->peerName() << tcpSocket->peerPort()
                 << "\n"
                 << m_uartReply;
