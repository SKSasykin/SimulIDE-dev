...
LibraryItem* Dial::libraryItem() {
    return new LibraryItem( tr( "Dial" ), "Other", "dial.png", "Dial", Dial::construct );
}

Dial::Dial( QString type, QString id ) : Dialed( type, id ) {
    ...
    addPropGroup(
        { tr( "Body" ),
          {
              new IntProp<Dial>( "width", tr( "Width" ), "_px", this, &Dial::width, &Dial::setWidth, 0, "uint" ),

              new IntProp<Dial>( "height", tr( "Height" ), "_px", this, &Dial::height, &Dial::setHeight, 0, "uint" ),

              new IntProp<Dial>( "Border", tr( "Border" ), "_px", this, &Dial::border, &Dial::setBorder ),

              new StrProp<Dial>( "Background", tr( "Background" ), "", this, &Dial::background, &Dial::setBackground ),
          },
          0 } );
    ...
    addPropGroup( { "Hidden",
                    {
                        ...
                        new IntProp<Dial>( "Value", "", "", this, &Dial::value, &Dial::setValue ),
                    },
                    groupHidden } );
}
Dial::~Dial() { }
...
void Dial::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    if ( m_backPixmap )
        p->drawPixmap( QRect( m_area.x(), m_area.y(), m_width, m_height ), *m_backPixmap );
    else {
