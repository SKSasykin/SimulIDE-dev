...
LibraryItem* SubPackage::libraryItem() {
    return new LibraryItem( tr( "Package" ), "Other", "resistordip.png", "Package", SubPackage::construct );
}

SubPackage::SubPackage( QString type, QString id ) : Chip( type, id ) {
    ...
    QString enums = "None,Logic,Board,Shield,Module;" + tr( "None" ) + "," + tr( "Logic" ) + "," + tr( "Board" ) + ","
        + tr( "Shield" ) + "," + tr( "Module" );

    addPropGroup(
        { tr( "Main" ),
          {
              new StrProp<SubPackage>( "SubcType", tr( "Type" ), enums, this, &SubPackage::subcTypeStr,
                                       &SubPackage::setSubcTypeStr, 0, "enum" ),

              new IntProp<SubPackage>( "Width", tr( "Width" ), "", this, &SubPackage::width, &SubPackage::setWidth, 0,
                                       "uint" ),

              new IntProp<SubPackage>( "Height", tr( "Height" ), "", this, &SubPackage::height, &SubPackage::setHeight,
                                       0, "uint" ),

              new StrProp<SubPackage>( "Name", tr( "Name" ), "", this, &SubPackage::name, &SubPackage::setName ),

              new StrProp<SubPackage>( "Package_File", tr( "Package File" ), "", this, &SubPackage::packageFile,
                                       &SubPackage::setPackageFile, propHidden ),

              new BoolProp<SubPackage>( "Logic_Symbol", tr( "Logic Symbol" ), "", this, &SubPackage::logicSymbol,
                                        &SubPackage::setLogicSymbol ),

              new BoolProp<SubPackage>( "Custom_Color", tr( "Custom Color" ), "", this, &SubPackage::customColor,
                                        &SubPackage::setCustomColor ),

              new StrProp<SubPackage>( "BckGndColor", tr( "Background Color" ), "", this, &SubPackage::pkgColorStr,
                                       &SubPackage::setPkgColorStr, 0, "color" ),

              new StrProp<SubPackage>( "Background", tr( "Background image" ), "", this, &SubPackage::background,
                                       &SubPackage::setBackground, 0, "file" ),

              new BoolProp<SubPackage>( "Border", tr( "Border" ), "", this, &SubPackage::border,
                                        &SubPackage::setBorder ),

              new StrProp<SubPackage>( "BckGndData", "", "", this, &SubPackage::bckGndData, &SubPackage::setBckGndData,
                                       propHidden ),

              new StrProp<SubPackage>( "Pins", "", "", this, &SubPackage::packagePins, &SubPackage::setPackagePins,
                                       propHidden ),
          },
          0 } );
}
...
void SubPackage::setSubcTypeStr( QString s ) {
    ...
    if ( s == "Board" || s == "Shield" || s == "Module" ) {
        ...
        m_isBoard = true;
    } else if ( currentBoard == this )
        Circuit::self()->setBoard( nullptr );
...
void SubPackage::hoverMoveEvent( QGraphicsSceneHoverEvent* event ) {
    if ( ( event->modifiers() & Qt::ShiftModifier ) && !( event->modifiers() & Qt::ControlModifier ) ) {
        ...
        int xPos = snapToGrid4( (int) event->pos().x() );
        int yPos = snapToGrid4( (int) event->pos().y() );
...
void SubPackage::mousePressEvent( QGraphicsSceneMouseEvent* event ) {
    if ( m_selecComp )
        return; // Used when linking or creating Boards to set this as main component
...
void SubPackage::compSelected( Component* comp ) {
    if ( comp )
        comp->setMainComp( !comp->isMainComp() );
    else
...
void SubPackage::setBoardMode( bool mode ) {
    ...
    for ( Node* nod : *Circuit::self()->nodeList() )
        nod->setHidden( mode );
...
void SubPackage::addNewPin( QString id, QString type, QString label, int pos, int xpos, int ypos, int angle, int length,
                            int space ) {
    PackagePin* pin
        = new PackagePin( angle, QPoint( xpos, ypos ), m_id + "-" + id, pos - 1, this ); // pos in package starts at 1
...
void SubPackage::setPointPin( bool point ) {
    ...
    if ( m_angle == 180 )
        m_eventPin->moveBy( -deltaL, 0 ); // Left
    else if ( m_angle == 0 )
        m_eventPin->moveBy( deltaL, 0 ); // Right
    else if ( m_angle == 90 )
        m_eventPin->moveBy( 0, -deltaL ); // Top
    else if ( m_angle == 270 )
        m_eventPin->moveBy( 0, deltaL ); // Bottom
...
void SubPackage::embeedBackground( QString pixmapPath ) {
...
QString SubPackage::packagePins() {
    if ( m_pkgePins.isEmpty() )
        return " "; // Force to save Pins property even if empty

    QString pins;
    int pP = 1;
    for ( Pin* pin : m_pkgePins ) {
        pins += pinStrEntry( pin );
        pP++;
    }
    return pins;
}
...
QString SubPackage::pinStrEntry( Pin* pin ) {
    ...
    if ( pin->inverted() )
        ...
        type = "bus";
    else if ( pin->pinType() == Pin::pinNull )
        type = "nul";
    else if ( pin->pinType() == Pin::pinRst )
        type = "rst";
...
void SubPackage::savePackage( QString fileName ) {
    ...
    out << "<packageB name=\"" + m_name + "\" width=\"" + QString::number( m_width ) + "\" height=\""
            + QString::number( m_height ) + "\" background=\"" + m_background + "\" custom_color=\""
            + ( m_customColor ? "true" : "false" ) + "\" bckgndcolor=\"" + m_pkgColor.name() + "\" border=\""
            + ( m_border ? "true" : "false" ) + "\" type=\"" + subcTypeStr() + "\" logic_symbol=\""
            + ( m_isLS ? "true" : "false" ) + "\" >\n\n";
...
QString SubPackage::pinEntry( Pin* pin ) {
    ...
    QString label = "label=\"" + pin->getLabelText().replace( "<", "&lt;" )
    ...
    return "    <pin " + adjustSize( type, 11 ) + adjustSize( xpos, 12 ) + adjustSize( ypos, 12 )
        + adjustSize( angle, 12 ) + adjustSize( length, 12 ) + adjustSize( space, 12 ) + adjustSize( id, 10 )
        + adjustSize( label, 12 ) + "/>\n";
}

void SubPackage::setCustomColor( bool c ) {
    m_customColor = c;
    updateColor();
    updtProperties();
}

void SubPackage::slotProperties() {
    Component::slotProperties();
    updtProperties();
}

void SubPackage::updtProperties() {
    if ( !m_propDialog )
        return;
...
EditDialog::EditDialog( SubPackage* pack, Pin* eventPin, QWidget* parent )
    ...
    QObject::connect( m_idLineEdit, &QLineEdit::textEdited, [=]( const QString& s ) { m_package->setPinId( s ); } );
    ...
    QObject::connect( m_invertCheckBox, &QCheckBox::toggled, [=]( bool t ) { m_package->invertPin( t ); } );

    QObject::connect( m_unuseCheckBox, &QCheckBox::toggled, [=]( bool t ) { m_package->unusePin( t ); } );

    QObject::connect( m_pointCheckBox, &QCheckBox::toggled, [=]( bool t ) { this->setPointPin( t ); } );

    QObject::connect( m_busCheckBox, &QCheckBox::toggled, [=]( bool t ) { this->setBusPin( t ); } );
}

void EditDialog::setPointPin( bool p ) {
    m_package->setPointPin( p );
    if ( m_invertCheckBox->isChecked() )
        m_invertCheckBox->setChecked( !p );
}

void EditDialog::setBusPin( bool b ) {
    m_package->setBusPin( b );
    if ( m_invertCheckBox->isChecked() )
        m_invertCheckBox->setChecked( !b );
}
