...
const QString LogicComponent::m_triggerList
    = "None,Clock,Enable;" + tr( "None" ) + "," + tr( "Clock" ) + "," + tr( "Enable" );

LogicComponent::LogicComponent( QString type, QString id ) : IoComponent( type, id ), eClockedDevice( id ) {
    m_oePin = nullptr;
    m_tristate = false;
    m_outEnable = true;
}
...
std::vector<Pin*> LogicComponent::getPins() {
    ...
    if ( m_oePin )
        pins.emplace_back( m_oePin );
    if ( m_clkPin )
        pins.emplace_back( m_clkPin );
    return pins;
}
