...
Linker::Linker() : m_linkCursor( QPixmap( ":/link.png" ), 10, 10 ) { }
Linker::~Linker() {
    if ( m_selecComp == this )
        Linker::compSelected( nullptr ); // Cancel link to components
...
{
    if ( m_selecComp )
        m_selecComp->compSelected( nullptr ); // Finish previous linking
    m_selecComp = this;
