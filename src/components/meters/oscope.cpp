...
LibraryItem* Oscope::libraryItem() {
    return new LibraryItem( tr( "Oscope" ), "Meters", "oscope.png", "Oscope", Oscope::construct );
}

Oscope::Oscope( QString type, QString id ) : PlotBase( type, id ) {
    m_numChannels = 4;
    m_trigger = 4;
    m_auto = 4;
    ...
    for ( int i = 0; i < 4; i++ ) {
        m_pin[i] = m_inPin[i] = new IoPin( 180, QPoint( -80 - 8, -48 + 32 * i ), id + "-Pin" + QString::number( i ), 0,
                                           this, undef_mode );
...
void Oscope::updateStep() {
    ...
    if ( !Simulator::self()->isPaused() ) {
        if ( m_trigger < 4 )
            period = m_channel[m_trigger]->m_period; // We want a trigger
        ...
        if ( period > 10 ) // We have a Trigger
        {
            ...
            simTime = Simulator::self()->circTime(); // free running
...
void Oscope::setTrigger( int ch ) {
    ...
    for ( int i = 0; i < 4; ++i ) {
        if ( ch == i )
            m_channel[i]->m_trigger = true;
        else
            m_channel[i]->m_trigger = false;
    }
}
...
QString Oscope::timPos() {
    QString list;
    for ( int i = 0; i < 4; ++i )
        list.append( QString::number( m_timePos[i] ) ).append( "," );
    return list;
}
...
QString Oscope::volDiv() {
    QString list;
    for ( int i = 0; i < 4; ++i )
        list.append( QString::number( m_voltDiv[i] ) ).append( "," );
    return list;
}
...
QString Oscope::volPos() {
    QString list;
    for ( int i = 0; i < 4; ++i )
        list.append( QString::number( m_voltPos[i] ) ).append( "," );
    return list;
}
...
void Oscope::moveTimePos( int64_t delta ) {
    if ( m_trigger < 4 ) {
        ...
        if ( period > 10 )
            return; // We have a Trigger: Don't Zoom at mouse pos.
    }
    for ( int i = 0; i < 4; ++i ) {
