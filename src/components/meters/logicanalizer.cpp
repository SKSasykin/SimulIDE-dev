...
LibraryItem* LAnalizer::libraryItem() {
    return new LibraryItem( tr( "Logic Analyzer" ), "Meters", "lanalizer.png", "LAnalizer", LAnalizer::construct );
}

LAnalizer::LAnalizer( QString type, QString id ) : PlotBase( type, id ) {
    m_numChannels = 8;
    m_bufferSize = 100000;
    ...
    m_inPin.resize( 8 );
    for ( int i = 0; i < 8; ++i ) {
        m_pin[i] = m_inPin[i] = new IoPin( 180, QPoint( -80 - 8, -64 + 16 * i ), id + "-Pin" + QString::number( i ), 0,
                                           this, undef_mode );
    ...
    addPropGroup( { tr( "Export" ),
                    {
                        new IntProp<LAnalizer>( "TimeStep", tr( "Base Time Step" ), "_ps", this, &LAnalizer::timeStep,
                                                &LAnalizer::setTimeStep, 0, "uint" ),

                        new BoolProp<LAnalizer>( "AutoExport", tr( "Export at pause" ), "", this,
                                                 &LAnalizer::autoExport, &LAnalizer::setAutoExport ),
                    },
                    0 } );
    ...
    if ( !Simulator::self()->isPaused() ) {
        ...
        if ( m_trigger < 9 ) {
            ...
            if ( m_trigger == 8 )
                ...
                risEdge = m_channel[m_trigger]->m_risEdge;
...
void LAnalizer::setVoltDiv( double ) {
    ...
    for ( int i = 0; i < 8; i++ )
        m_display->setVTick( i, m_voltDiv );
}
...
void LAnalizer::setTrigger( int ch ) {
    ...
    for ( int i = 0; i < 8; i++ )
        m_channel[i]->m_pauseOnCond = pauseOnCond;
}
...
void LAnalizer::dumpData( QString fn ) {
    ...
    if ( (int64_t) startTime + m_timePos >= 0 )
        startTime += m_timePos;
    else
        startTime = 1;
    ...
    for ( uint ch = 0; ch < 8; ++ch ) {
        ...
        for ( int i = 0; i < m_bufferSize; ++i ) {
            ...
            if ( index >= m_bufferSize )
                index -= m_bufferSize; // It's a circular buffer

            double val = m_channel[ch]->m_buffer[index];
            uint64_t time = m_channel[ch]->m_time[index];

            if ( pTime == time )
                continue; // Avoid repeated times
            pTime = time;

            if ( time == 0 )
                continue; // Empty samples: Simulation times start at 1 ps
            if ( time <= startTime ) {
                initVal = val;
                continue;
            } // Previous value from first valid sample
            if ( time > endTime ) {
                time = endTime;
                val = pVal;
