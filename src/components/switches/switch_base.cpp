...
SwitchBase::SwitchBase( QString type, QString id ) : MechContact( type, id ) {
    m_graphical = true;
    m_changed = true;
    m_key = "";
...
void SwitchBase::setHidden( bool hide, bool hidArea, bool hidLabel ) {
    ...
    if ( hidArea )
        ...
        m_area = QRectF( -6, 6, 12, 12 );
    else
        m_area = QRectF( -13, -16 * m_numPoles, 26, 16 * m_numPoles );
...
void SwitchBase::setKey( QString key ) {
    if ( key.startsWith( "&#x3" ) )
        key.replace( "&#x3D", "=" ).replace( "&#x3C", "<" ).replace( "&#x3E", ">" );
    if ( key.size() > 1 )
        key = key.left( 1 );
    m_key = key;
