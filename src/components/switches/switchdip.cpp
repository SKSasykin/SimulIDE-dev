...
LibraryItem* SwitchDip::libraryItem() {
    return new LibraryItem( tr( "Switch Dip" ), "Switches", "switchdip.png", "SwitchDip", SwitchDip::construct );
}

SwitchDip::SwitchDip( QString type, QString id ) : Component( type, id ), eElement( id ) {
    m_graphical = true;
    m_changed = true;
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        new IntProp<SwitchDip>( "Size", tr( "Size" ), "", this, &SwitchDip::size, &SwitchDip::setSize,
                                                propNoCopy, "uint" ),

                        new BoolProp<SwitchDip>( "Exclusive", tr( "Exclusive" ), "", this, &SwitchDip::exclusive,
                                                 &SwitchDip::setExclusive ),

                        new BoolProp<SwitchDip>( "CommonPin", tr( "Common Pin" ), "", this, &SwitchDip::commonPin,
                                                 &SwitchDip::setCommonPin ),
                    },
                    groupNoCopy } );
...
void SwitchDip::stamp() {
    ...
    for ( int i = 0; i < m_size; i++ ) {
        ...
        if ( m_commonPin )
            m_pin[pin1]->setEnode( node0 );
        else
            node0 = m_pin[pin1]->getEnode();
...
void SwitchDip::setSize( int size ) {
    ...
    if ( size < m_size )
        deleteSwitches( m_size - size );
    else if ( size > m_size )
        createSwitches( size - m_size );
...
void SwitchDip::setHidden( bool hid, bool hidArea, bool hidLabel ) {
    ...
    {
        for ( int i = 2; i < m_size * 2; i += 2 )
            m_pin[i]->setVisible( !m_commonPin && !hid );
    }
}
