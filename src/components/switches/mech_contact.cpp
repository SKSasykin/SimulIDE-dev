...
MechContact::MechContact( QString type, QString id ) : Component( type, id ), eElement( id ) {
    m_numthrows = 0;
    m_numPoles = 0;
    m_pin0 = 0;
...
void MechContact::setSwitch( bool closed ) {
    ...
    for ( int i = 0; i < m_numPoles; i++ ) {
        ...
        if ( closed )
            m_switches[switchN]->setAdmit( 1e3 );
        else
            m_switches[switchN]->setAdmit( 0 );
...
void MechContact::SetupSwitches( int poles, int throws ) {
    ...
    if ( m_pin0 == 0 )
        m_area = QRectF( -12, 8 - 16 * poles, 24, 16 * poles - 4 ); // Switches
    else
        m_area = QRectF( -12, -8 - 16 * poles - 4, 24, 8 + 16 * poles + 8 + 4 ); // Relays
    int start = m_pin0 / 2;
