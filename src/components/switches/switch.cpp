...
LibraryItem* Switch::libraryItem() {
    return new LibraryItem( tr( "Switch (all)" ), "Switches", "switch.png", "Switch", Switch::construct );
}

Switch::Switch( QString type, QString id ) : SwitchBase( type, id ) {
    ...
    addPropGroup(
        { tr( "Main" ),
          {
              new BoolProp<Switch>( "Norm_Close", tr( "Normally Closed" ), "", this, &Switch::nClose,
                                    &Switch::setNClose ),

              new BoolProp<Switch>( "DT", tr( "Double Throw" ), "", this, &Switch::dt, &Switch::setDt, propNoCopy ),

              new IntProp<Switch>( "Poles", tr( "Poles" ), "", this, &Switch::poles, &Switch::setPoles, propNoCopy,
                                   "uint" ),

              new StrProp<Switch>( "Key", tr( "Key" ), "", this, &Switch::key, &Switch::setKey ),

              new BoolProp<Switch>( "Checked", "", "", this, &Switch::checked, &Switch::setChecked, propHidden ),
          },
          0 } );
}
Switch::~Switch() { }
...
void Switch::setLinkedValue( double v, int i ) {
    bool newState = ( v != 0 );
    if ( m_button->isChecked() == newState )
        return;
