...
LibraryItem* Push::libraryItem() {
    return new LibraryItem( tr( "Push" ), "Switches", "push.png", "Push", Push::construct );
}

Push::Push( QString type, QString id ) : PushBase( type, id ) {
...
void Push::paint( QPainter* p, const QStyleOptionGraphicsItem* option, QWidget* widget ) {
    ...
    for ( int i = 0; i < m_numPoles; i++ ) // Draw Switches
    {
        int offset = 16 * i;
        if ( m_closed )
            p->drawLine( -9, -2 - offset, 9, -2 - offset );
        else
            p->drawLine( -9, -8 - offset, 9, -8 - offset );
    }
    if ( m_numPoles > 1 ) {
