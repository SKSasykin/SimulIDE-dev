...
LibraryItem* KeyPad::libraryItem() {
    return new LibraryItem( tr( "KeyPad" ), "Switches", "keypad.png", "KeyPad", KeyPad::construct );
}

KeyPad::KeyPad( QString type, QString id ) : Component( type, id ), eElement( id ) {
    m_graphical = true;
    ...
    addPropGroup(
        { tr( "Main" ),
          { new BoolProp<KeyPad>( "Diodes", tr( "Diodes" ), "", this, &KeyPad::hasDiodes, &KeyPad::setHasDiodes ),

            new BoolProp<KeyPad>( "Dir", tr( "Diodes Direction" ), "", this, &KeyPad::direction,
                                  &KeyPad::setDirection ),

            new IntProp<KeyPad>( "Rows", tr( "Rows" ), "", this, &KeyPad::rows, &KeyPad::setRows, propNoCopy, "uint" ),

            new IntProp<KeyPad>( "Cols", tr( "Columns" ), "", this, &KeyPad::cols, &KeyPad::setCols, propNoCopy,
                                 "uint" ),

            new StrProp<KeyPad>( "Key_Labels", tr( "Key Labels" ), "", this, &KeyPad::keyLabels,
                                 &KeyPad::setKeyLabels ) },
          0 } );
}
KeyPad::~KeyPad() { }
...
void KeyPad::setupButtons( int newRows, int newCols ) {
    ...
    for ( int i = newRows; i < m_rows; ++i )
        deletePin( m_rowPins.at( i ) );
    for ( int i = newCols; i < m_cols; ++i )
        deletePin( m_colPins.at( i ) );
    ...
    for ( int row = 0; row < newRows; row++ ) {
        ...
        for ( int col = 0; col < newCols; col++ ) {
            ...
            QString buttonLabel = "";
            if ( pos <= labelMax )
                buttonLabel = m_keyLabels.mid( pos, 1 );
...
void KeyPad::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    if ( m_hasDiodes ) {
        if ( m_direction ) {
            ...
            for ( int col = 0; col < m_cols; col++ ) {
                QPointF points[3]
                    = { QPointF( col * 16 - 2, y_pos ), QPointF( col * 16, y_pos2 ), QPointF( col * 16 + 2, y_pos ) };
            ...
            for ( int row = 0; row < m_rows; row++ ) {
                QPointF points[3] = { QPointF( x_pos, 8 + row * 16 - 2 ), QPointF( x_pos2, 8 + row * 16 ),
