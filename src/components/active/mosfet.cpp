...
LibraryItem* Mosfet::libraryItem() {
    return new LibraryItem( tr( "Mosfet" ), "Transistors", "mosfet.png", "Mosfet", Mosfet::construct );
}

Mosfet::Mosfet( QString type, QString id ) : Component( type, id ), eMosfet( id ) {
        ...
        { tr( "Main" ),
          {
              ...
              new BoolProp<Mosfet>( "Depletion", tr( "Depletion" ), "", this, &Mosfet::depletion,
                                    &Mosfet::setDepletion ),
          },
          0 } );
    ...
    addPropGroup( { tr( "Electric" ),
                    { new DoubProp<Mosfet>( "RDSon", tr( "RDSon" ), "Ω", this, &Mosfet::rdson, &Mosfet::setRDSon ),
                      ...
                      new DoubProp<Mosfet>( "Threshold", tr( "Threshold" ), "V", this, &Mosfet::threshold,
                                            &Mosfet::setThreshold ) },
                    0 } );
}
Mosfet::~Mosfet() { }
...
void Mosfet::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    if ( Circuit::self()->animateCurr() && m_gateV > 0 )
    ...
    p->setBrush( Qt::black );
    if ( m_Pchannel ) {
        QPointF points[3] = { QPointF( 7, 0 ), QPointF( 3, -2 ), QPointF( 3, 2 ) };
        p->drawPolygon( points, 3 );
    } else {
        QPointF points[3] = { QPointF( 1, 0 ), QPointF( 5, -2 ), QPointF( 5, 2 ) };
        p->drawPolygon( points, 3 );
    }
    if ( m_depletion ) {
