...
LibraryItem* MuxAnalog::libraryItem() {
    return new LibraryItem( tr( "Analog Mux" ), "Other Active", "1to3-c.png", "MuxAnalog", MuxAnalog::construct );
}

MuxAnalog::MuxAnalog( QString type, QString id ) : Component( type, id ), eElement( id ) {
    setLabelPos( -16, -16, 0 );
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        new IntProp<MuxAnalog>( "Address_Bits", tr( "Address Size" ), "_bits", this,
                                                &MuxAnalog::addrBits, &MuxAnalog::setAddrBits, propNoCopy, "uint" ),

                        new DoubProp<MuxAnalog>( "Impedance", tr( "Impedance" ), "mΩ", this, &MuxAnalog::impedance,
                                                 &MuxAnalog::setImpedance ),
                    },
                    0 } );
}
MuxAnalog::~MuxAnalog() { }
...
void MuxAnalog::runEvent() {
    ...
    for ( int i = 0; i < m_channels; ++i ) {
        if ( i == m_address ) {
            if ( m_resistor[i]->admit() == low_imp )
                m_resistor[i]->setAdmit( m_admit );
        } else if ( m_resistor[i]->admit() != low_imp )
            m_resistor[i]->setAdmit( low_imp );
    }
}
...
void MuxAnalog::setAddrBits( int bits ) {
    ...
    if ( channels < m_channels )
        deleteResistors( m_channels - channels );
    else
        createResistors( channels - m_channels );
...
void MuxAnalog::createAddrBits( int c ) {
    ...
    for ( int i = start; i < m_addrBits; i++ ) {
        m_addrPin[i]
            = new Pin( 180, QPoint( -3 * 8, 3 * 8 + i * 8 ), m_id + "-pinAddr" + QString::number( i ), 0, this );
...
void MuxAnalog::deleteAddrBits( int d ) {
    for ( int i = m_addrBits - d; i < m_addrBits; i++ )
        deletePin( m_addrPin[i] );
