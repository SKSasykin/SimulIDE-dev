...
LibraryItem* Diac::libraryItem() {
    return new LibraryItem( tr( "Diac" ), "Rectifiers", "diac.png", "Diac", Diac::construct );
}

Diac::Diac( QString type, QString id ) : Component( type, id ), eElement( id ) {
...
void Diac::stamp() {
    ...
    if ( node0 )
        node0->addToNoLinList( this );
    if ( node1 )
        node1->addToNoLinList( this );
...
void Diac::voltChanged() {
    ...
    if ( qFabs( current ) < m_holdCurr )
        state = false; /// Mingw needs qFabs
    if ( qFabs( voltage ) > m_brkVolt )
        state = true;
...
void Diac::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    static const QPointF points1[3] = { QPointF( 7, -8 ), QPointF( -8, -15 ), QPointF( -8, 0 ) };
    p->drawPolygon( points1, 3 );

    static const QPointF points2[3] = { QPointF( -7, 8 ), QPointF( 8, 0 ), QPointF( 8, 15 ) };
