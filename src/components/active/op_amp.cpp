...
LibraryItem* OpAmp::libraryItem() {
    return new LibraryItem( tr( "OpAmp" ), "Other Active", "opamp.png", "opAmp", OpAmp::construct );
}

OpAmp::OpAmp( QString type, QString id ) : Component( type, id ), eElement( id ) {
    ...
    m_pin[3]->setFontSize( 9 );
    m_pin[3]->setSpace( 1.7 );
    m_pin[3]->setLabelText( "+" );
    m_pin[3]->setLabelColor( Qt::red );

    m_pin[4] = new Pin( 270, QPoint( 0, 16 ), id + "-powerNeg", 4, this );
    m_pin[4]->setFontSize( 9 );
    m_pin[4]->setSpace( 1.7 );
    m_pin[4]->setLabelText( "–" ); // U+2013
    ...
    addPropGroup( { tr( "Main" ),
                    { new DoubProp<OpAmp>( "Gain", tr( "Gain" ), "", this, &OpAmp::gain, &OpAmp::setGain ),
                      ...
                      new DoubProp<OpAmp>( "Out_Imped", tr( "Output Impedance" ), "Ω", this, &OpAmp::outImp,
                                           &OpAmp::setOutImp ) },
                    0 } );

    addPropGroup( { tr( "Supply" ),
                    {
                        new BoolProp<OpAmp>( "Power_Pins", tr( "Use Supply Pins" ), "", this, &OpAmp::powerPins,
                                             &OpAmp::setPowerPins, propNoCopy ),

                        new BoolProp<OpAmp>( "Switch_Pins", tr( "Switch Supply Pins" ), "", this, &OpAmp::switchPins,
                                             &OpAmp::setSwitchPins, propNoCopy ),
...
void OpAmp::stamp() {
    if ( m_inputP->isConnected() )
        m_inputP->getEnode()->addToNoLinList( this );
    if ( m_inputN->isConnected() )
        m_inputN->getEnode()->addToNoLinList( this );
...
void OpAmp::voltChanged() // Called when any pin node change volt
{
    ...
    if ( out > m_voltPos )
        out = m_voltPos;
    else if ( out < m_voltNeg )
        out = m_voltNeg;

    if ( ( m_step == 0 ) && ( qFabs( m_lastIn - vd ) < m_accuracy ) && ( qFabs( out - m_lastOut ) < m_accuracy ) ) {
        m_step = 0;
        return;
    } // Converged
...
QPainterPath OpAmp::shape() const {
    ...
    points << QPointF( -16, -16 ) << QPointF( -16, 16 ) << QPointF( 16, 1 ) << QPointF( 16, -1 );
...
void OpAmp::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    static const QPointF points[4] = { QPointF( -16, -16 ), QPointF( -16, 16 ), QPointF( 16, 1 ), QPointF( 16, -1 ) };
