...
LibraryItem* Triac::libraryItem() {
    return new LibraryItem( tr( "Triac" ), "Rectifiers", "triac.png", "Triac", Triac::construct );
}

Triac::Triac( QString type, QString id ) : Component( type, id ), eElement( id ) {
    ...
    addPropGroup(
        { tr( "Main" ),
          { new DoubProp<Triac>( "GateRes", tr( "Gate Resistance" ), "Ω", this, &Triac::gateRes, &Triac::setGateRes ),
            ...
            new DoubProp<Triac>( "TrigCurr", tr( "Trigger Current" ), "mA", this, &Triac::trigCurr,
                                 &Triac::setTrigCurr ),

            new DoubProp<Triac>( "HoldCurr", tr( "Holding Current" ), "mA", this, &Triac::holdCurr,
                                 &Triac::setHoldCurr ) },
          0 } );
}
Triac::~Triac() {
...
void Triac::stamp() {
    ...
    if ( nodeT2 )
        nodeT2->addToNoLinList( this );
    if ( nodeT1 )
        nodeT1->addToNoLinList( this );
    if ( nodeG )
        nodeG->addToNoLinList( this );
...
void Triac::voltChanged() {
    ...
    if ( qFabs( current1 ) < m_holdCurr )
        state = false; /// Mingw needs qFabs
    if ( qFabs( currentG ) > m_trigCurr )
        state = true;
...
void Triac::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    static const QPointF points1[3] = { QPointF( -7, -8 ), QPointF( 8, 0 ), QPointF( 8, -15 ) };
    p->drawPolygon( points1, 3 );

    static const QPointF points2[3] = { QPointF( 7, 8 ), QPointF( -8, 15 ), QPointF( -8, 0 ) };
