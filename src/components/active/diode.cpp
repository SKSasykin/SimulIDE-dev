...
LibraryItem* Diode::libraryItem() {
    return new LibraryItem( tr( "Diode" ), "Rectifiers", "diode.png", "Diode", Diode::construct );
}

Diode::Diode( QString type, QString id, bool zener ) : LinkerComponent( type, id ), eDiode( id ) {
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        new StrProp<Diode>( "Model", tr( "Model" ), m_diodes.keys().join( "," ), this, &Diode::model,
                                            &Diode::setModel, 0, "enum" ),
                    },
                    0 } );

    addPropGroup( { tr( "Electric" ),
                    {
                        new DoubProp<Diode>( "Threshold", tr( "Forward Voltage" ), "V", this, &Diode::threshold,
                                             &Diode::setThreshold ),

                        new DoubProp<Diode>( "MaxCurrent", tr( "Max Current" ), "A", this, &Diode::maxCurrent,
                                             &Diode::setMaxCurrent ),

                        new DoubProp<Diode>( "Resistance", tr( "Resistance" ), "Ω", this, &Diode::resistance,
                                             &Diode::setResSafe ),
                    },
                    0 } );

    addPropGroup( { tr( "Advanced" ),
                    { new DoubProp<Diode>( "BrkDownV", tr( "Breakdown Voltage" ), "V", this, &Diode::brkDownV,
                                           &Diode::setBrkDownV ),

                      new DoubProp<Diode>( "SatCurrent", tr( "Saturation Current" ), "nA", this, &Diode::satCur,
                                           &Diode::setSatCur ),

                      new DoubProp<Diode>( "EmCoef", tr( "Emission Coefficient" ), "", this, &Diode::emCoef,
                                           &Diode::setEmCoef ) },
                    0 } );
}
Diode::~Diode() {
...
void Diode::updateStep() {
    ...
    if ( m_changed )
        voltChanged(); // m_changed cleared at eDiode::voltChanged
}
...
void Diode::paint( QPainter* p, const QStyleOptionGraphicsItem* option, QWidget* widget ) {
    ...
    static const QPointF points[3] = { QPointF( 7, 0 ), QPointF( -8, -7 ), QPointF( -8, 7 ) };
