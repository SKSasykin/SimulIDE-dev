...
LibraryItem* SCR::libraryItem() {
    return new LibraryItem( tr( "SCR" ), "Rectifiers", "scr.png", "SCR", SCR::construct );
}

SCR::SCR( QString type, QString id ) : Component( type, id ), eElement( id ) {
    ...
    addPropGroup(
        { tr( "Main" ),
          { new DoubProp<SCR>( "GateRes", tr( "Gate Resistance" ), "Ω", this, &SCR::gateRes, &SCR::setGateRes ),
            ...
            new DoubProp<SCR>( "TrigCurr", tr( "Trigger Current" ), "mA", this, &SCR::trigCurr, &SCR::setTrigCurr ),

            new DoubProp<SCR>( "HoldCurr", tr( "Holding Current" ), "mA", this, &SCR::holdCurr, &SCR::setHoldCurr ) },
          0 } );
}
SCR::~SCR() {
    delete m_diode;
...
void SCR::stamp() {
    ...
    if ( nodeA )
        nodeA->addToNoLinList( this );
    if ( nodeC )
        nodeC->addToNoLinList( this );
    if ( nodeG )
...
void SCR::voltChanged() {
    ...
    if ( qFabs( voltAC ) < .01 && qFabs( voltGC ) < .01 )
        return; // Converged
