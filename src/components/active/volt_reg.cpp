...
LibraryItem* VoltReg::libraryItem() {
    return new LibraryItem( tr( "Volt. Regulator" ), "Other Active", "voltreg.png", "VoltReg", VoltReg::construct );
}

VoltReg::VoltReg( QString type, QString id ) : Component( type, id ), eResistor( id ) {
    ...
    m_ePin[0] = m_pin[0] = new Pin( 180, QPoint( -16, 0 ), id + "-input", 0, this, 5 );
    m_pin[0]->setLabelText( "I" );
    m_pin[0]->setLabelColor( QColor( 0, 0, 0 ) );

    m_ePin[1] = m_pin[1] = new Pin( 0, QPoint( 16, 0 ), id + "-output", 1, this, 5 );
    ...
    addPropGroup( { tr( "Main" ),
                    { new DoubProp<VoltReg>( "Voltage", tr( "Output Voltage" ), "V", this, &VoltReg::outVolt,
                                             &VoltReg::setOutVolt ) },
                    0 } );
...
void VoltReg::stamp() {
    m_connected = m_ePin[0]->isConnected() && m_ePin[1]->isConnected() && m_ePin[2]->isConnected();
...
void VoltReg::voltChanged() {
    ...
    if ( qFabs( m_lastCurrent - current ) < 1e-3 )
        return;
