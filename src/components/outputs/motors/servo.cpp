...
LibraryItem* Servo::libraryItem() {
    return new LibraryItem( tr( "Servo Motor" ), "Motors", "servo.png", "Servo", Servo::construct );
}

Servo::Servo( QString type, QString id ) : LogicComponent( type, id ) {
    m_graphical = true;
    m_width = 10;
    m_height = 6;
    m_pos = 90;
    ...
    addPropGroup( { tr( "Main" ),
                    { new DoubProp<Servo>( "Speed", tr( "Speed " ), "_s/60º", this, &Servo::speed, &Servo::setSpeed ),

                      new DoubProp<Servo>( "MinPulse", tr( "Min. Pulse Width" ), "_µs", this, &Servo::minPulse,
                                           &Servo::setMinPulse ),

                      new DoubProp<Servo>( "MaxPulse", tr( "Max. Pulse Width" ), "_µs", this, &Servo::maxPulse,
                                           &Servo::setMaxPulse ) },
                    0 } );
...
void Servo::stamp() {
    ...
    if ( m_inpPin[0]->isConnected() && m_inpPin[1]->isConnected() && m_inpPin[2]->isConnected() ) {
...
QPainterPath Servo::shape() const {
    ...
    points << QPointF( -40, -24 ) << QPointF( -40, 24 ) << QPointF( 6, 24 ) << QPointF( 6, 40 ) << QPointF( 33, 40 )
           << QPointF( 57, 16 ) << QPointF( 57, -16 ) << QPointF( 33, -40 ) << QPointF( 6, -40 ) << QPointF( 6, -24 )
           << QPointF( -40, -24 );
