...
LibraryItem* Stepper::libraryItem() {
    return new LibraryItem( tr( "Stepper" ), "Motors", "steeper.png", "Stepper", Stepper::construct );
}

Stepper::Stepper( QString type, QString id )
    : LinkerComponent( type, id ), eElement( ( id + "-eElement" ) ), m_resA1( ( id + "-eEresistorA1" ) ),
      m_resA2( ( id + "-eEresistorA2" ) ), m_resB1( ( id + "-eEresistorB1" ) ), m_resB2( ( id + "-eEresistorB2" ) ),
      ...
      m_pinB2( 180, QPoint( -72, 32 ), id + "-PinB2", 0, this ), m_ePinA1Co( ( id + "-ePinA1Co" ), 0 ),
      m_ePinA2Co( ( id + "-ePinA2Co" ), 0 ), m_ePinB1Co( ( id + "-ePinB1Co" ), 0 ),
    ...
    addPropGroup(
        { tr( "Main" ),
          { new BoolProp<Stepper>( "Bipolar", tr( "Bipolar" ), "", this, &Stepper::bipolar, &Stepper::setBipolar ),
            ...
            new IntProp<Stepper>( "Steps", tr( "Steps per Rotation" ), "", this, &Stepper::steps, &Stepper::setSteps ),

            new DoubProp<Stepper>( "Resistance", tr( "Resistance" ), "Ω", this, &Stepper::resistance,
                                   &Stepper::setResistance ) },
          0 } );
}
Stepper::~Stepper() { }
...
void Stepper::updateStep() {
    if ( m_linkedComp.size() ) {
        ...
        for ( Component* comp : m_linkedComp )
            comp->setLinkedValue( val ); // 0-1000
    }
    if ( m_changed ) {
        m_changed = false;
...
void Stepper::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    if ( !m_hidden )
        p->drawRoundedRect( -64, -40, 25, 80, 1, 1 );

    p->setBrush( QColor( 50, 70, 100 ) );
    p->drawRoundedRect( -48, -48, 96, 96, 1, 1 );
