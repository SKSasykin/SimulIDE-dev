...
LibraryItem* AudioOut::libraryItem() {
    return new LibraryItem( tr( "Audio Out" ), "Other Outputs", "audio_out.png", "AudioOut", AudioOut::construct );
}

AudioOut::AudioOut( QString type, QString id ) : Component( type, id ), eResistor( id ) {
        ...
        { tr( "Main" ),
          {
              ...
              new DoubProp<AudioOut>( "Impedance", tr( "Impedance" ), "Ω", this, &AudioOut::impedance,
                                      &AudioOut::setImpedance ),

              new DoubProp<AudioOut>( "Frequency", tr( "Frequency" ), "Hz", this, &AudioOut::frequency,
                                      &AudioOut::setFrequency ),
          },
          0 } );
...
QPainterPath AudioOut::shape() const {
    ...
    points << QPointF( -10, -12 ) << QPointF( -10, 4 ) << QPointF( 0, 4 ) << QPointF( 10, 16 ) << QPointF( 10, -24 )
           << QPointF( 0, -12 ) << QPointF( -10, -12 );
...
void AudioOut::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    if ( m_buzzer ) {
        ...
        static const QPointF points[7]
            = { QPointF( -10, -12 ), QPointF( -10, 4 ), QPointF( 0, 4 ),    QPointF( 10, 16 ),
                QPointF( 10, -24 ),  QPointF( 0, -12 ), QPointF( -10, -12 ) };
