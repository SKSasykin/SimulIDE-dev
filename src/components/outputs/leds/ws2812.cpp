...
LibraryItem* WS2812::libraryItem() {
    return new LibraryItem( tr( "WS2812 Led" ), "Leds", "ws2812.png", "WS2812", WS2812::construct );
}

WS2812::WS2812( QString type, QString id ) : Component( type, id ), eClockedDevice( id ) {
    m_graphical = true;
    ...
    addPropGroup(
        { tr( "Main" ),
          {
              new IntProp<WS2812>( "Rows", tr( "Rows" ), "", this, &WS2812::rows, &WS2812::setRows ),

              new IntProp<WS2812>( "Cols", tr( "Columns" ), "", this, &WS2812::cols, &WS2812::setCols, 0, "uint" ),
          },
          groupNoCopy } );

    addPropGroup( { tr( "Timing" ),
                    {
                        new IntProp<WS2812>( "RstTime", tr( "Minimum Reset pulse" ), "_µs", this, &WS2812::resetPulse,
                                             &WS2812::setResetPulse ),

                        new IntProp<WS2812>( "T0H", tr( "0, High time" ), "_ns", this, &WS2812::t0H, &WS2812::setT0H ),

                        new IntProp<WS2812>( "T0L", tr( "0, Low time" ), "_ns", this, &WS2812::t0L, &WS2812::setT0L ),

                        new IntProp<WS2812>( "T1H", tr( "1, High time" ), "_ns", this, &WS2812::t1H, &WS2812::setT1H ),

                        new IntProp<WS2812>( "T1L", tr( "1, Low time" ), "_ns", this, &WS2812::t1L, &WS2812::setT1L ),
                    },
                    groupNoCopy } );
}
WS2812::~WS2812() { }
    ...
    if ( m_clkState == Clock_Rising ) // Input Rising edge
    {
        ...
        } else if ( ( time > ( m_T1L - 150 ) * 1e3 ) && ( time < ( m_T0L + 150 ) * 1e3 ) ) // Valid L State Time
        {
            if ( m_word >= m_leds )
                ...
                if ( m_lastHstate ) {
                    if ( time < ( m_T1L + 150 ) * 1e3 )
                        saveBit( 1 );
                } // Valid bit = 1
                else if ( time > ( m_T0L - 150 ) * 1e3 )
                    saveBit( 0 ); // Valid bit = 0
            }
        }
        m_newWord = true;
...
void WS2812::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    if ( !m_hidden ) {
        QPointF points[3] = { QPointF( -8, -2 ), QPointF( -8 + 3, 0 ), QPointF( -8, 2 ) };
