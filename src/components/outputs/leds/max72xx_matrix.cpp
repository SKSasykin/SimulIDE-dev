...
LibraryItem* Max72xx_matrix::libraryItem() {
    return new LibraryItem( tr( "Max72xx matrix" ), "Leds", "max72xx.png", "Max72xx_matrix",
                            Max72xx_matrix::construct );
}

Max72xx_matrix::Max72xx_matrix( QString type, QString id ) : LogicComponent( type, id ) {
    m_graphical = true;
    m_numDisplays = 4;
    ...
    addPropGroup(
        { tr( "Main" ),
          {
              new StrProp<Max72xx_matrix>( "Color", tr( "Color" ), LedBase::getColorList(), this,
                                           &Max72xx_matrix::colorStr, &Max72xx_matrix::setColorStr, 0, "enum" ),
              new IntProp<Max72xx_matrix>( "NumDisplays", tr( "Size" ), "", this, &Max72xx_matrix::numDisplays,
                                           &Max72xx_matrix::setNumDisplays, 0, "uint" ),
          },
          groupNoCopy } );
}
...
void Max72xx_matrix::setHidden( bool hid, bool hidArea, bool hidLabel ) {
    ...
    if ( hid )
        m_area = QRectF( -32, -40, 64 * m_numDisplays, 64 );
    else
        m_area = QRectF( -36, -44, 4 + 64 * m_numDisplays + 4, 88 );
}
...
void Max72xx_matrix::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    for ( int display = 0; display < m_numDisplays; display++ ) {
        ...
        color = QColor( 25 + color.red() * factor / 16, 25 + color.green() * factor / 16,
                        25 + color.blue() * factor / 16 );
