...
LibraryItem* SevenSegment::libraryItem() {
    return new LibraryItem( tr( "7 Segment" ), "Leds", "seven_segment.png", "Seven Segment", SevenSegment::construct );
}

SevenSegment::SevenSegment( QString type, QString id ) : Component( type, id ), eElement( id ) {
    m_graphical = true;
    ...
    addPropGroup(
        { tr( "Main" ),
          { new StrProp<SevenSegment>( "Color", tr( "Color" ), LedBase::getColorList(), this, &SevenSegment::colorStr,
                                       &SevenSegment::setColorStr, 0, "enum" ),

            new IntProp<SevenSegment>( "NumDisplays", tr( "Size" ), "", this, &SevenSegment::numDisplays,
                                       &SevenSegment::setNumDisplays, propNoCopy ),

            new BoolProp<SevenSegment>( "Vertical_Pins", tr( "Vertical Pins" ), "", this, &SevenSegment::verticalPins,
                                        &SevenSegment::setVerticalPins, propNoCopy ) },
          0 } );

    addPropGroup(
        { tr( "Electric" ),
          {
              new BoolProp<SevenSegment>( "CommonCathode", tr( "Common Cathode" ), "", this,
                                          &SevenSegment::isComCathode, &SevenSegment::setComCathode, propNoCopy ),

              new DoubProp<SevenSegment>( "Threshold", tr( "Forward Voltage" ), "V", this, &SevenSegment::threshold,
                                          &SevenSegment::setThreshold ),

              new DoubProp<SevenSegment>( "MaxCurrent", tr( "Max Current" ), "A", this, &SevenSegment::maxCurrent,
                                          &SevenSegment::setMaxCurrent ),

              new DoubProp<SevenSegment>( "Resistance", tr( "Resistance" ), "Ω", this, &SevenSegment::resistance,
                                          &SevenSegment::setResistance ),
          },
          0 } );
}
...
void SevenSegment::stamp() {
    for ( int i = 0; i < 8; ++i )
        m_enode[i] = m_ePin[i]->getEnode(); // Get eNode of pin i
...
void SevenSegment::setNumDisplays( int displays ) {
    ...
    if ( displays > m_numDisplays ) {
        resizeData( displays );
        for ( int i = m_numDisplays; i < displays; ++i )
            createDisplay( i );
    } else {
        for ( int i = displays; i < m_numDisplays; ++i )
            deleteDisplay( i );
        resizeData( displays );
    }
...
void SevenSegment::createDisplay( int n ) {
    ...
    for ( int i = 0; i < 8; ++i ) // Create segments
    {
        ...
        LedSmd* lsmd;
        if ( i < 7 )
            lsmd = new LedSmd( "LEDSMD", pinid, QRectF( 0, 0, 13.5, 1.5 ) ); // Segment
        else
            lsmd = new LedSmd( "LEDSMD", pinid, QRectF( 0, 0, 1.5, 1.5 ) ); // Point
...
void SevenSegment::setHidden( bool hid, bool hidArea, bool hidLabel ) {
    ...
    if ( hid )
        m_area = QRect( -16, -24, 32 * m_numDisplays, 48 );
    else
        m_area = QRect( -18, -24 - 4, 32 * m_numDisplays + 4, 48 + 8 );
}
