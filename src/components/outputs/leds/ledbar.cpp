...
LibraryItem* LedBar::libraryItem() {
    return new LibraryItem( tr( "Led Bar" ), "Leds", "ledbar.png", "LedBar", LedBar::construct );
}

LedBar::LedBar( QString type, QString id ) : Component( type, id ) {
    ...
    addPropGroup(
        { tr( "Main" ),
          {
              new StrProp<LedBar>( "Color", tr( "Color" ), LedBase::getColorList(), this, &LedBar::colorStr,
                                   &LedBar::setColorStr, 0, "enum" ),
    ...
    addPropGroup( { tr( "Electric" ),
                    {
                        new DoubProp<LedBar>( "Threshold", tr( "Forward Voltage" ), "V", this, &LedBar::threshold,
                                              &LedBar::setThreshold ),

                        new DoubProp<LedBar>( "MaxCurrent", tr( "Max Current" ), "mA", this, &LedBar::maxCurrent,
                                              &LedBar::setMaxCurrent ),

                        new DoubProp<LedBar>( "Resistance", tr( "Resistance" ), "Ω", this, &LedBar::resistance,
                                              &LedBar::setResistance ),
                    },
                    0 } );
}
LedBar::~LedBar() { }
...
QString LedBar::colorStr() {
    if ( m_led[0] )
        return m_led[0]->colorStr();
    else
        return "Yellow";
}

void LedBar::setColorStr( QString color ) {
    for ( LedSmd* led : m_led )
        led->setColorStr( color );
...
void LedBar::setSize( int size ) {
    ...
    if ( size < 1 )
        size = 1;
    if ( size < m_size )
        deleteLeds( m_size - size );
    else if ( size > m_size )
        createLeds( size - m_size );
...
void LedBar::setMaxCurrent( double current ) {
    for ( int i = 0; i < m_size; i++ )
        m_led[i]->setMaxCurrent( current );
}

double LedBar::resistance() {
    return m_led[0]->resistance();
}

void LedBar::setResistance( double resist ) {
    for ( int i = 0; i < m_size; i++ )
        m_led[i]->setResistance( resist );
}
