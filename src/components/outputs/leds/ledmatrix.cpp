...
LibraryItem* LedMatrix::libraryItem() {
    return new LibraryItem( tr( "LedMatrix" ), "Leds", "ledmatrix.png", "LedMatrix", LedMatrix::construct );
}

LedMatrix::LedMatrix( QString type, QString id ) : Component( type, id ), eElement( id ) {
    m_graphical = true;
    ...
    addPropGroup( { tr( "Main" ),
                    {
                        new StrProp<LedMatrix>( "Color", tr( "Color" ), LedBase::getColorList(), this,
                                                &LedMatrix::colorStr, &LedMatrix::setColorStr, 0, "enum" ),

                        new IntProp<LedMatrix>( "Rows", tr( "Rows" ), "", this, &LedMatrix::rows, &LedMatrix::setRows,
                                                propNoCopy, "uint" ),

                        new IntProp<LedMatrix>( "Cols", tr( "Columns" ), "", this, &LedMatrix::cols,
                                                &LedMatrix::setCols, propNoCopy, "uint" ),
    ...
    addPropGroup( { tr( "Electric" ),
                    {
                        new DoubProp<LedMatrix>( "Threshold", tr( "Forward Voltage" ), "V", this, &LedMatrix::threshold,
                                                 &LedMatrix::setThreshold ),

                        new DoubProp<LedMatrix>( "MaxCurrent", tr( "Max Current" ), "mA", this, &LedMatrix::maxCurrent,
                                                 &LedMatrix::setMaxCurrent ),

                        new DoubProp<LedMatrix>( "Resistance", tr( "Resistance" ), "Ω", this, &LedMatrix::resistance,
                                                 &LedMatrix::setResistance ),
                    },
                    0 } );
}
LedMatrix::~LedMatrix() { }
...
void LedMatrix::createMatrix() {
    if ( m_verticalPins )
        m_area = QRect( -4, -8, m_cols * 8, m_rows * 8 + 8 );
    else
...
void LedMatrix::deleteMatrix() {
    for ( int row = 0; row < m_rows; ++row ) {
        for ( int col = 0; col < m_cols; ++col )
            delete m_led[row][col];
        deletePin( m_rowPin[row] );
    }
    for ( int col = 0; col < m_cols; col++ )
        deletePin( m_colPin[col] );
...
void LedMatrix::setHidden( bool hid, bool hidArea, bool hidLabel ) {
    ...
    if ( hid )
        m_area = QRect( -4, -4, m_cols * 8, m_rows * 8 );
    else {
        if ( m_verticalPins )
            m_area = QRect( -4, -8, m_cols * 8, m_rows * 8 + 8 );
        else
