...
LibraryItem* Led::libraryItem() {
    return new LibraryItem( tr( "Led" ), "Leds", "led.png", "Led", Led::construct );
}

Led::Led( QString type, QString id ) : LedBase( type, id ) {
    ...
    addPropGroup(
        { "Hidden", { new StrProp<Led>( "Links", "Links", "", this, &Led::getLinks, &Led::setLinks ) }, groupHidden } );
}
Led::~Led() { }
...
void Led::drawForeground( QPainter* p ) {
    static const QPointF points[3] = { QPointF( 8, 0 ), QPointF( -3, -6 ), QPointF( -3, 6 ) };
