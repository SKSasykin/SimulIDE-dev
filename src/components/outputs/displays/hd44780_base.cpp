...
Hd44780_Base::Hd44780_Base( QString type, QString id ) : Component( type, id ), m_fontImg( ":font2.png" ) {
    m_graphical = true;
...
void Hd44780_Base::proccessCommand( int command ) {
    ...
    if ( command == 0 )
        return;
    if ( command & 128 ) {
        setDDaddr( command - 128 );
        return;
    } //1....... Set DDRAM address    Sets the DDRAM address. DDRAM data are sent and received after this setting 37 μs
    if ( command & 64 ) {
        setCGaddr( command - 64 );
        return;
    } //01...... Set CGRAM address    Sets the CGRAM address. CGRAM data are sent and received after this setting 37 μs
    if ( command & 32 ) {
        functionSet( command );
        return;
    } //001..... Function set         Sets interface data length(DL), nº of display line(N), and character font(F) 37 μs
    if ( command & 16 ) {
        C_D_Shift( command );
        return;
    } //0001.... Cursor/display shift Sets cursor-move or display-shift(S/C), shift direction(R/L). DDRAM unchanged 37 μs
    if ( command & 8 ) {
        dispControl( command );
        return;
    } //00001... Display on/off       Sets on/off of all display(D), cursor on/off(C), blink of cursor position character(B) 37 μs
    if ( command & 4 ) {
        entryMode( command );
        return;
    } //000001.. Entry mode set       Sets cursor move direction(I/D); Shift the display(S). Performed during data read/write. 37 μs
    if ( command & 2 ) {
        cursorHome();
        return;
    } //0000001. Cursor home          Cursor to home position. Returns display being shifted to original position. DDRAM unchanged. 1.52 ms
    if ( command & 1 ) {
        clearLcd();
        return;
    } //00000001 Clear display        Clears display and returns cursor to the home position (address 0). 1.52 ms
}

void Hd44780_Base::functionSet( int data ) {
    if ( data & 16 )
        m_dataLength = 8; // Data Length
    else
        m_dataLength = 4;

    if ( data & 8 )
        m_lineLength = 40; // Display Lines
    else
        m_lineLength = 80;
...
void Hd44780_Base::dispControl( int data ) {
    if ( data & 4 )
        m_dispOn = 1; // Display On/Off
    else
        m_dispOn = 0;

    if ( data & 2 )
        m_cursorOn = 1; // Cursor On/Off
    else
        m_cursorOn = 0;

    if ( data & 1 )
        m_cursorBlink = 1; // Cursor Blink
    else
        m_cursorBlink = 0;
}
    ...
    if ( m_dispOn != 0 ) {
        ...
        for ( int row = 0; row < m_rows; row++ ) {
            for ( int col = 0; col < m_cols; col++ ) {
                ...
                mem_pos += m_shiftPos;
                if ( mem_pos > lineEnd )
                    mem_pos -= m_lineLength;
                if ( mem_pos < lineStart )
                    mem_pos += m_lineLength;
