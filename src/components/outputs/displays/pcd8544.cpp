...
LibraryItem* Pcd8544::libraryItem() {
    return new LibraryItem( "Pcd8544", "Displays", "pcd8544.png", "Pcd8544", Pcd8544::construct );
}

Pcd8544::Pcd8544( QString type, QString id )
    : Component( type, id ), eElement( id + "-eElement" ), m_pRst( 270, QPoint( -32, 40 ), id + "-PinRst", 0, this ),
      m_pCs( 270, QPoint( -16, 40 ), id + "-PinCs", 0, this ), m_pDc( 270, QPoint( 0, 40 ), id + "-PinDc", 0, this ),
      m_pSi( 270, QPoint( 16, 40 ), id + "-PinSi", 0, this ), m_pScl( 270, QPoint( 32, 40 ), id + "-PinScl", 0, this ) {
    m_graphical = true;
...
void Pcd8544::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    if ( m_bPD )
        p->fillRect( -42, -42, 84, 48, QColor( 200, 215, 180 ) ); // Power-Down mode
    else if ( !m_bD && !m_bE )
        p->fillRect( -42, -42, 84, 48, QColor( 200, 215, 180 ) ); // Blank Display mode, blank the visuals
    else if ( !m_bD && m_bE )
        p->fillRect( -42, -42, 84, 48, Qt::black ); // All segments on
    else {
