...
LibraryItem* Ks0108::libraryItem() {
    return new LibraryItem( "KS0108", "Displays", "ks0108.png", "Ks0108", Ks0108::construct );
}

Ks0108::Ks0108( QString type, QString id )
    : Component( type, id ), eElement( ( id + "-eElement" ) ),
    ...
    addPropGroup( { tr( "Main" ),
                    { new BoolProp<Ks0108>( "CS_Active_Low", tr( "CS Active Low" ), "", this, &Ks0108::csActLow,
                                            &Ks0108::setCsActLow ) },
                    groupNoCopy } );
}
Ks0108::~Ks0108() { }
...
{
    if ( m_pinRst.getVoltage() < 2.5 )
        reset(); // Reset Pin is Low
    else
        m_reset = false;
    ...
    if ( m_Write != Write ) // Set Read or Write Impedances
    {
        ...
        for ( int i = 0; i < 8; i++ ) {
            if ( Write )
                m_dataPin[i]->setPinMode( input );
            else
                m_dataPin[i]->setPinMode( output );
        }
    }
    ...
    if ( Scl && !m_lastScl ) { // This is a clock Rising Edge
        m_lastScl = true;
        if ( Write )
            return; // Only Read in Rising Edge
    } else if ( !Scl && m_lastScl ) { // This is a clock Falling edge
        m_lastScl = false;
        if ( !Write )
            return; // Only Write in Falling Edge
    } else {
        m_lastScl = Scl;
        return;
    }
    ...
    if ( Write ) {
        ...
        {
            if ( m_dataPin[pin]->getVoltage() > 2.5 )
                m_input += pow( 2, pin );
        }
    }
    ...
    {
        if ( m_reset )
            return; // Can't write data while Reset
        if ( Write )
            writeData( m_input ); // Write Data
        else
            ReadData(); // Read Data
    } else { // Command
        if ( Write )
            proccessCommand( m_input ); // Write Command
        else
...
void Ks0108::ReadData() {
    int data = 0;
    if ( m_Cs1 )
        data = m_aDispRam[m_addrX1][m_addrY1];
    if ( m_Cs2 )
        data = m_aDispRam[m_addrX2][m_addrY2 + 64];
...
void Ks0108::proccessCommand( int command ) {
    ...
    if ( command < 64 ) {
        dispOn( command & 1 );
        return;
    } //0011111.  // Display On/Off internal status &Ram not affected
    if ( command < 128 ) {
        setYaddr( command & 63 );
        return;
    } //01......  // Set Y address
...
void Ks0108::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    if ( !m_dispOn )
        p->fillRect( -64, -42, 128, 64, QColor( 200, 215, 180 ) );
    else {
