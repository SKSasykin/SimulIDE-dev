...
LibraryItem* Lamp::libraryItem() {
    return new LibraryItem( tr( "Incandescent lamp" ), "Other Outputs", "lamp.png", "Lamp", Lamp::construct );
}

Lamp::Lamp( QString type, QString id ) : Comp2Pin( type, id ), eResistor( id ) {
    ...
    addPropGroup(
        { tr( "Main" ),
          {
              new DoubProp<Lamp>( "Resistance", tr( "Resistance" ), "Ω", this, &Lamp::resistance, &Lamp::setResSafe ),

              new DoubProp<Lamp>( "MaxCurrent", tr( "Max Current" ), "A", this, &Lamp::maxCurrent,
                                  &Lamp::setMaxCurrent ),
          },
          0 } );
}
...
void Lamp::updateVI() {
    ...
    if ( m_lastCurrent > 0 )
        m_totalCurrent += m_lastCurrent * period;
...
void Lamp::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
