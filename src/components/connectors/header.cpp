...
LibraryItem* Header::libraryItem() {
    return new LibraryItem( tr( "Header" ), "Connectors", "header.png", "Header", Header::construct );
}

Header::Header( QString type, QString id ) : ConnBase( type, id ) {
    this->setZValue( -1 );
...
void Header::updatePixmap() {
    if ( m_pointDown )
        m_pinPixmap.load( ":/header_D.png" );
    else
        m_pinPixmap.load( ":/header_U.png" );
