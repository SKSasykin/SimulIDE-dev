...
LibraryItem* Socket::libraryItem() {
    return new LibraryItem( tr( "Socket" ), "Connectors", "socket.png", "Socket", Socket::construct );
}

Socket::Socket( QString type, QString id ) : ConnBase( type, id ) {
    setZValue( -2 );
