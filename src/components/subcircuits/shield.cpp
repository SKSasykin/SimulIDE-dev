...
ShieldSubc::ShieldSubc( QString type, QString id, QString device ) : BoardSubc( type, id, device ) {
    m_subcType = "Shield";
    m_boardId = "";
    setZValue( 1 );
...
void ShieldSubc::attachToBoard() {
    ...
    this->setRotation( 0 );
    for ( Tunnel* tunnel : m_subcTunnels )
        tunnel->setName( m_boardId + "-" + tunnel->tunnelUid() );
}
...
void ShieldSubc::renameTunnels() {
    for ( Tunnel* tunnel : m_subcTunnels )
        tunnel->setName( m_id + "-" + tunnel->tunnelUid() );
}
