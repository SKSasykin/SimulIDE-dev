...
ModuleSubc::ModuleSubc( QString type, QString id, QString device ) : ShieldSubc( type, id, device ) {
    m_subcType = "Module";
    setZValue( 2 );
    ...
    for ( QGraphicsItem* it : list ) {
        ...
        if ( subcType == "Shield" || subcType == "Module" ) {
            ...
            if ( m_shields.contains( shield ) )
                continue; // Is my child
        }
        boardList.append( board );
    }
    for ( BoardSubc* board : boardList ) {
        if ( board->zValue() > myZ )
            continue; // Is above me
...
void ModuleSubc::renameTunnels() {
    for ( Tunnel* tunnel : getPinTunnels() )
