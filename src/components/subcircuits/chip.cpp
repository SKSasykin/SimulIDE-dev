...
Chip::Chip( QString type, QString id, QString device ) : Component( type, id ), eElement( id ), m_label( this ) {
    m_id = id;
    m_device = device;

    QStringList list = id.split( "-" );
    if ( list.size() > 1 )
        m_name = list.at( list.size() - 2 ); // for example: "atmega328-1" to: "atmega328"
...
bool Chip::propNotFound( QString prop, QString val ) {
    if ( prop == "Logic_Symbol" ) {
        if ( m_packageList.isEmpty() )
            ...
            if ( pkgDIP.isEmpty() )
                pkgDIP = m_packageList.first();
            if ( pkgLS.isEmpty() )
                pkgLS = m_packageList.first();
...
QMap<QString, QString> Chip::getPackages( QString compFile ) // Static
{
    ...
    for ( QString line : docLines ) {
        ...
        for ( propStr_t prop : properties ) {
            ...
            if ( propName == "SubcType" ) {
                if ( propValue != "None" )
                    s_subcType = propValue;
            ...
            if ( propName == "Pins" ) {
                ...
                if ( propValue.contains( "&" ) )
                    propValue = cleanPinName( propValue );
                pkgStr += "\n" + propValue;
...
void Chip::setPackage( QString package ) {
    if ( !m_packageList.contains( package ) )
        package = m_packageList.keys().first();
...
void Chip::initPackage( QString pkgStr ) {
    ...
    m_customColor = false;
    m_tempPins.clear();
    for ( Pin* pin : m_ncPins )
        m_tempPins.append( pin );
    for ( Pin* pin : m_pin )
        m_tempPins.append( pin );
    m_ncPins.clear();
    m_ePin.clear();
    ...
    for ( QString line : docLines ) {
        ...
        if ( item == "Package" ) {
            ...
            for ( propStr_t property : properties ) {
                ...
                if ( name == "width" )
                    m_width = val.split( " " ).first().toInt();
                else if ( name == "height" )
                    m_height = val.split( " " ).first().toInt();
                else if ( name == "name" )
                    embedName = val;
                else if ( name == "border" )
                    m_border = ( val == "true" );
                else if ( name == "custom_color" )
                    m_customColor = ( val == "true" );
                else if ( name == "background" )
                    background = val;
                else if ( name == "bckgnddata" )
                    bckgndData = val;
                else if ( name == "bckgndcolor" )
                    setPkgColorStr( val );
                else if ( name == "logic_symbol" )
                    m_isLS = ( val == "true" );
            }
...
QString Chip::getDevice( QString id ) // Static
{
    ...
    {
        if ( device.contains( "@" ) )
            list = device.split( "@" ); // Nested subcircuit not supported for versions < 1916
    ...
    if ( !pin ) {
        if ( type == "nc" ) {
            if ( m_pinMap.contains( id ) ) {
                ...
                pin = new Pin( angle, QPoint( xpos, ypos ), m_id + "-" + id, pos - 1,
                               this ); // pos in package starts at 1
            ...
            pin = addPin( id, type, label, pos, xpos, ypos, angle, length, space );

        if ( !m_pinMap.contains( id ) )
            m_pinMap.insert( id, pin );
    }
    if ( !pin )
        return;
...
void Chip::setBackground( QString bck ) {
    ...
    if ( bck.startsWith( "color" ) ) {
        ...
        if ( QFile::exists( bck ) ) // full path to image
        {
            ...
            pixmapPath = MainWindow::self()->getCircFilePath( bck ); // Image in circuit/data folder
...
void Chip::findHelp() {
    ...
    if ( QFileInfo::exists( helpFile ) )
        m_help = fileToString( helpFile, "Chip::findHelp" );
    else
        m_help = MainWindow::self()->getHelp( m_name, false );
}
...
void Chip::setMargins( QString margins ) {
    ...
    mList.removeAll( "" );
    if ( margins.size() )
        m_topMargin = mList.takeFirst().toInt();
    if ( margins.size() )
        m_bottomMargin = mList.takeFirst().toInt();
    if ( margins.size() )
        m_rightMargin = mList.takeFirst().toInt();
    if ( margins.size() )
        m_leftMargin = mList.takeFirst().toInt();
    update();
}
...
void Chip::paint( QPainter* p, const QStyleOptionGraphicsItem* o, QWidget* w ) {
    ...
    QRect imgArea
        = QRect( m_area.x() + m_leftMargin, m_area.y() + m_topMargin, m_area.width() - m_leftMargin - m_rightMargin,
    ...
    //else
    {
        ...
        if ( m_backPixmap )
            p->drawPixmap( imgArea, *m_backPixmap );
