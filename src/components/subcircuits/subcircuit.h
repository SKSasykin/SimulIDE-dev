...
protected:
    ...
    virtual Pin* addPin( QString id, QString type, QString label, int pos, int xpos, int ypos, int angle,
                         int length = 8, int space = 0 ) override;

    virtual Pin* updatePin( QString id, QString type, QString label, int xpos, int ypos, int angle, int length = 8,
    ...
    static QMap<QString, subcData_t>
        s_localDevices; // Data for Subcircuits in Circuit folder (cleared at Circuit close)
};
