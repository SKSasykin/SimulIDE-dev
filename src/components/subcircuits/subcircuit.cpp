...
QMap<QString, SubCircuit::subcData_t>
    SubCircuit::s_localDevices; // Data for Subcircuits in Circuit folder (deleted at Circuit close)
...
Component* SubCircuit::construct( QString type, QString id ) {
    ...
    if ( subcFile.isEmpty() )
        subcFile
            = MainWindow::self()->getCircFilePath( device + ".sim1" ); // Search sim1 in circuit or circuit/data folder
    if ( subcFile.isEmpty() ) // Search sim1 in circuit/name or circuit/data/name folder
    ...
    } else // Get Files from list
    {
        ...
        if ( !s_subcDir.isEmpty() )
            subcFile = s_subcDir + "/" + device + ".sim2";
        if ( !QFileInfo::exists( subcFile ) )
            subcFile = s_subcDir + "/" + device + ".sim1";
    ...
    if ( circuit.isEmpty() ) // We need to load data from files
    {
        ...
        if ( isLocal )
            s_localDevices.insert( device, deviceData );
        else
            s_globalDevices.insert( device, deviceData );
    }
    ...
    if ( subcTyp == "Logic" )
        subcircuit = new LogicSubc( type, id, device );
    else if ( subcTyp == "Board" )
        subcircuit = new BoardSubc( type, id, device );
    else if ( subcTyp == "Shield" )
        subcircuit = new ShieldSubc( type, id, device );
    else if ( subcTyp == "Module" )
        subcircuit = new ModuleSubc( type, id, device );
    else
        subcircuit = new SubCircuit( type, id, device );
    ...
    if ( m_error != 0 ) {
        ...
        if ( packageList.size() > 1 ) // Add package list Property if there is more than 1 to choose
            subcircuit->addProperty( tr( "Main" ),
                                     new StrProp<SubCircuit>( "Package", tr( "Package" ), pkges.join( "," ), subcircuit,
                                                              &SubCircuit::package, &SubCircuit::setPackage, 0,
                                                              "enum" ) );
...
LibraryItem* SubCircuit::libraryItem() {
    return new LibraryItem( tr( "Subcircuit" ),
                            "", // Category Not dispalyed
                            "", "Subcircuit", SubCircuit::construct );
}

SubCircuit::SubCircuit( QString type, QString id, QString device ) : Chip( type, id, device ) {
    addPropGroup( { tr( "Main" ), {}, 0 } );

    if ( s_graphProps.isEmpty() )
        loadGraphProps();
}
        ...
        if ( type == "Connector" ) {
            ...
            for ( propStr_t prop : properties ) {
                if ( prop.name == "startpinid" )
                    startPinId = numId + "@" + prop.value;
                else if ( prop.name == "endpinid" )
                    endPinId = numId + "@" + prop.value;
            ...
            if ( startPin && endPin ) // Create Connection
            {
                ...
                if ( startPin->isBus() )
                    endPin->setIsBus( true );
                if ( endPin->isBus() )
                    startPin->setIsBus( true );
            } else // Start or End pin not found
            {
                if ( !startPin )
                    qDebug() << "\n   ERROR!!  SubCircuit::loadSubCircuit: " << m_name << m_id + " null startPin in "
                             << type << startPinId;
                if ( !endPin )
                    qDebug() << "\n   ERROR!!  SubCircuit::loadSubCircuit: " << m_name << m_id + " null endPin in "
                             << type << endPinId;
            }
        } else {
            ...
            if ( type == "Node" )
                comp = new Node( type, newUid );
            else
                comp = circ->createItem( type, newUid, false );
            ...
            for ( propStr_t prop : properties ) {
                ...
                if ( !s_graphProps.contains( propName ) )
                    comp->setPropStr( propName, prop.value );
            }
            if ( mcu )
                mcu->m_subcFolder = "";
            ...
            if ( this->isBoard() && comp->isGraphical() ) {
                ...
                {
                    comp->setBoardPos(
                        QPointF( -1e6, -1e6 ) ); // Used in setLogicSymbol to identify Components not visible
                    comp->moveTo( QPointF( 0, 0 ) );
                    comp->setVisible( false );
                }
                if ( m_isLS && m_packageList.size() > 1 )
                    comp->setVisible( false ); // Don't show any component if Logic Symbol
            } else {
...
Pin* SubCircuit::addPin( QString id, QString type, QString label, int, int xpos, int ypos, int angle, int length,
    ...
    if ( m_initialized && m_pinTunnels.contains( m_id + "-" + id ) ) {
        ...
        if ( angle == 180 )
            tunnel->setRotation( 0 );
        else if ( angle == 90 )
            tunnel->setRotation( -90 ); // QGraphicsItem 0º i at right side
        else
...
void SubCircuit::loadGraphProps() {
    ...
    {
        if ( ( pg.name != "CompGraphic" ) )
            continue;
        for ( ComProperty* prop : pg.propList )
            s_graphProps.append( prop->name() );
        break;
    }
}
