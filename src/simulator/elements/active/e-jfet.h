...
protected:
    ...
    double
        m_Idss; // Idss = Ids when gate "shorted" to source (i.e., nearly maxCurrent). Idss is specified in datasheets.
    double m_Vp; // Pinch off voltage or Vgs(off), specified in datasheets
    double
        m_LambdaInv; // Not specified in datasheets but (nearly) identical to lambda in LTSpice accounting for channel length
};
