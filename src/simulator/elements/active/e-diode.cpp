...
eDiode::eDiode( QString id ) : eResistor( id ) {
    m_vt = 0.025865;
    m_vzCoef = 1 / m_vt;
    m_maxCur = 1;
...
void eDiode::voltChanged() {
    ...
    if ( m_changed )
        m_changed = false;
    else if ( qFabs( voltPN - m_voltPN ) < .01 ) {
        m_step = 0;
        m_converged = true;
        return;
    } // Converged
    ...
    if ( voltPN > m_vCriti
         && qFabs( voltPN - m_voltPN ) > m_vScale * 2 ) // check new voltage; has current changed by factor of e^2?
    {
...
inline double eDiode::limitStep( double vnew, double vold, double scale, double vc ) {
    if ( vold > 0 ) {
        ...
        if ( arg > 0 )
            vnew = vold + scale * qLn( arg );
        else
            vnew = vc;
    } else
        vnew = scale * qLn( vnew / scale );
        ...
        if ( itemset.tagName() == "itemset" ) {
            ...
            while ( !nodei.isNull() ) {
                ...
                if ( item.tagName() == "item" ) {
                    ...
                    if ( type == "diode" )
                        m_diodes.insert( name, data );
                    else if ( type == "zener" )
                        m_zeners.insert( name, data );
                    else if ( type == "led" )
                        m_leds.insert( name, data );
                }
...
void eDiode::setModel( QString model ) {
    ...
    if ( model == "Custom" )
        return;
    if ( m_diodes.contains( model ) )
        setModelData( m_diodes.value( model ) );
    else if ( m_zeners.contains( model ) )
        setModelData( m_zeners.value( model ) );
    else if ( m_leds.contains( model ) )
        setModelData( m_leds.value( model ) );
}
