...
void ePin::addSingAdm( eNode* node, double admit ) {
    if ( m_enode )
        m_enode->addSingAdm( this, node, admit );
}

void ePin::stampSingAdm( double admit ) {
    if ( m_enode )
        m_enode->stampSingAdm( this, admit );
}

void ePin::createCurrent() {
    if ( m_enode )
        m_enode->createCurrent( this );
}
...
void ePin::changeCallBack( eElement* el, bool cb ) {
    if ( !m_enode )
        return;
    if ( cb )
        m_enode->voltChangedCallback( el );
    else
        m_enode->remFromChangedCallback( el );
}
