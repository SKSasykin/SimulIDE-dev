...
void eNode::addConnection( ePin* epin, eNode* node ) {
    ...
    while ( first ) {
        if ( first->epin == epin )
            return; // Connection already in the list
        first = first->next;
    }
    ...
    if ( !m_nodeList.contains( conn->nodeNum ) )
        m_nodeList.append( conn->nodeNum ); // Used by CircMatrix
}
...
void eNode::stampMatrix() {
    ...
    if ( m_admitChanged ) {
        ...
        if ( m_single ) {
            Connection* conn = m_firstAdmit;
            while ( conn ) {
                m_totalAdmit += conn->value;
                conn = conn->next;
            } // Calculate total admitance
        } else {
            ...
            while ( na ) { // Stamp non diagonal
                ...
                if ( enode >= 0 )
                    CircMatrix::self()->stampMatrix( m_nodeNum, enode, -admit );
                na = na->next;
            }
        }
        m_admitChanged = false;
    ...
    if ( m_currChanged ) {
        ...
        Connection* conn = m_firstCurrent;
        while ( conn ) {
            m_totalCurr += conn->value;
            conn = conn->next;
        } // Calculate total current

        if ( !m_single )
            CircMatrix::self()->stampCoef( m_nodeGroup, m_nodeNum, m_totalCurr );
        m_currChanged = false;
    }
    if ( m_single )
        solveSingle();
...
void eNode::addNodeComp( Node* n ) {
    if ( !m_nodeCompList.contains( n ) )
        m_nodeCompList.append( n );
}
