...
### Поддержка UART для ESP

Модели UART для ESP32, ESP32-S3 и ESP32-C3 предоставляют 128-байтные FIFO TX/RX,
побитовую передачу с учётом времени и поведение регистров `INT_RAW`, `INT_ST`,
`INT_ENA`, `INT_CLR`, `STATUS`, `CONF0` и `CONF1`, используемых ESP-IDF и
Arduino. Реализованы различающаяся между чипами раскладка порога
`TXFIFO_EMPTY`, условие `TX_DONE`, заполнение и переполнение RX, сброс FIFO и
правильный источник матрицы прерываний для каждого UART.

Переходы уровня UART IRQ передаются через границу SimulIDE-QEMU по отдельному
кольцу событий в разделяемой памяти. Они доставляются независимо от ответов на
чтение MMIO, поэтому передатчик, ожидающий свободного места в FIFO, не теряет
пробуждение при одновременном опросе регистра и возникновении IRQ. Видимый
прошивке TX FIFO остаётся ограниченным 128 байтами; уже принятые синхронным
мостом записи сохраняются по порядку до освобождения места, а активный байт
сдвигового регистра учитывается отдельно. Это позволяет непрерывно передавать
объём данных существенно больше FIFO без изменения порядка байтов и временных
параметров линии.

Для ESP8266 сохраняется опрашиваемый режим UART. В его текущем QEMU-мосте нет
подключения UART к матрице прерываний, поэтому доставка UART IRQ для этого
...
| --- | --- | --- | --- |
| ESP32 | SLC DMA NIC с DHCP/NAT через libslirp | Да | HCI-транспорт для разработки; детерминированные BLE-соединения |
| ESP32-S3 | SLC DMA NIC с DHCP/NAT через libslirp | Да | HCI-транспорт для разработки; детерминированные BLE-соединения |
| ESP32-C3 | SLC DMA NIC с DHCP/NAT через libslirp | Да | HCI-транспорт для разработки; детерминированные BLE-соединения |
...
BLE GATT между симулируемыми ESP доступен через встроенные примеры
**BLE GATT Demo** для ESP32, ESP32-S3 и ESP32-C3. В каждом примере совместно
работают peripheral и central: central сканирует, соединяется, подписывается,
записывает `0x19`, проверяет notification и readback, затем постоянно включает
индикатор успеха на GPIO4. Встроенные прошивки собраны на ESP-IDF 5.5.5;
воспроизводимые исходники и данные сборки находятся в
`resources/data/bin/esp/examples/ble-gatt/`.

Со стороны хоста доступен **BLE Chat** через контекстное меню ESP
(`Open BLE Chat...`): он сканирует детерминированную среду, соединяется с
симулируемым peripheral, находит сервис/характеристику и даёт чат в стиле
Read/Write плюс notifications с переключателем String/HEX. В режиме String
печатный ASCII показывается буквально, а остальные байты как `\xNN`; в режиме
HEX байты показываются заглавными через пробел без префикса `\x`.

Все три семейства имеют DMA/H4-транспорт, а контроллер
...
Bluetooth-адаптера хоста не реализованы. Bluetooth Classic не поддерживается,
но `WiFi Link Port`, `BT Link Port` и `Host Forward Port` доступны в панели
свойств; встроенные BLE-примеры работают со значениями по умолчанию и не
требуют изменения портов.
...
так что добавлять или коммитить их вручную не нужно. Отслеживаемые Git ROM
загрузки ESP8266 и передачи управления пользовательской прошивке находятся в
том же каталоге с расширением `.rom`.
...
### Внутренние подтяжки GPIO для ESP

Настройки pull-up и pull-down передаются из гостевой прошивки в электрический
решатель SimulIDE. Подтяжки моделируются как аналоговые проводимости с
номинальным сопротивлением 45 кОм, а не как принудительные логические уровни,
поэтому внешняя схема может их переопределить.

| Устройство | GPIO с подтяжками | Исключения |
| --- | --- | --- |
| ESP8266EX | Pull-up на GPIO0-15; wake pull-down на GPIO0-15; `INPUT_PULLDOWN_16` на GPIO16 | У GPIO16 нет внутреннего pull-up; обычный Arduino `INPUT_PULLDOWN` недоступен на GPIO0-15 |
| ESP32 | GPIO0-19, GPIO21-23, GPIO25-27, GPIO32-33 | GPIO34-39 не имеют встроенных подтяжек |
| ESP32-S3 | GPIO0-21, GPIO26-48 | Часть пинов может быть занята flash, PSRAM, USB или strapping |
| ESP32-C3 | GPIO0-21 | Часть пинов может быть занята flash, USB или strapping |

RTC-пины классического ESP32 используют поля RUE/RDE блока RTC_IO; остальные
пины семейства ESP32 используют `FUN_PU`/`FUN_PD` блока IO_MUX. GPIO16 у
ESP8266 управляется отдельным RTC-регистром. Электрические подтяжки очищаются
