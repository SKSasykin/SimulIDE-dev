...
#define SUCCESS_LED_GPIO GPIO_NUM_4
...
static void finish(bool ok)
{
    ...
    s_done = true;
    if (ok) {
        gpio_set_level(SUCCESS_LED_GPIO, 1);
        ESP_LOGI(TAG, "Success: write, notification and readback matched");
        ESP_LOGI(TAG, "GPIO4 success LED is latched ON");
        ESP_LOGI(TAG, "BLE_GATT_E2E_PASS byte=0x%02x", kTestByte);
    } else {
        ESP_LOGE(TAG, "BLE_GATT_E2E_FAIL");
    }
}
                      ...
                      void *arg)
{
    ...
    if (error->status != 0) {
        ESP_LOGE(TAG, "attribute operation %d failed: %d", step, error->status);
        finish(false);
        return 0;
    }
    if (step == 1) {
        ESP_LOGI(TAG, "Notifications enabled; writing value 0x%02x", kTestByte);
        int rc = ble_gattc_write_flat(conn_handle, s_value_handle,
                                      &kTestByte, 1, on_attr_op,
                                      (void *)(intptr_t)2);
        if (rc != 0) finish(false);
    } else if (step == 2) {
        ESP_LOGI(TAG, "Write completed; reading the characteristic back");
        ...
        if (attr && attr->om && OS_MBUF_PKTLEN(attr->om) == 1) {
            ...
            if (ble_hs_mbuf_to_flat(attr->om, &value, 1, NULL) == 0 &&
                ...
                ESP_LOGI(TAG, "Readback received: 0x%02x", value);
                finish(true);
                return 0;
            }
        }
        ESP_LOGE(TAG, "Readback or notification did not match");
        finish(false);
    }
    return 0;
}
                             ...
                             void *arg)
{
    ...
    if (error->status == BLE_HS_EDONE) {
        ...
        ESP_LOGI(TAG, "Demo characteristic found; enabling notifications");
                             ...
                             void *arg)
{
    ...
    if (error->status == BLE_HS_EDONE) {
        if (s_value_handle == 0) {
            ESP_LOGE(TAG, "demo characteristic not found");
            finish(false);
            return 0;
        }
                             ...
                             void *arg)
{
    ...
    if (error->status == BLE_HS_EDONE) {
        if (s_svc_start == 0) {
            ESP_LOGE(TAG, "demo service not found");
            finish(false);
            return 0;
        }
...
static int gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISC: {
        if (event->disc.event_type != BLE_HCI_ADV_RPT_EVTYPE_ADV_IND &&
            ...
            return 0;
        ESP_LOGI(TAG, "Peripheral found; connecting");
        ble_gap_disc_cancel();
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        ESP_LOGI(TAG, "Connected; discovering the demo service");
        {
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ESP_LOGE(TAG, "Disconnected: %d", event->disconnect.reason);
        finish(false);
        return 0;
    case BLE_GAP_EVENT_NOTIFY_RX: {
        if (event->notify_rx.attr_handle == s_value_handle &&
            ...
            if (ble_hs_mbuf_to_flat(event->notify_rx.om, &value, 1, NULL) == 0 &&
                value == kTestByte) {
                s_notified = true;
                ESP_LOGI(TAG, "Notification received: 0x%02x", value);
            }
        }
        return 0;
    }
...
static void on_sync(void)
{
    ...
    rc = ble_gap_disc(BLE_OWN_ADDR_PUBLIC, BLE_HS_FOREVER, &params,
                      gap_event, NULL);
    assert(rc == 0);
    ESP_LOGI(TAG, "BLE GATT central started; scanning for the SimulIDE peripheral");
...
void app_main(void)
{
    ESP_ERROR_CHECK(gpio_reset_pin(SUCCESS_LED_GPIO));
    ESP_ERROR_CHECK(gpio_set_direction(SUCCESS_LED_GPIO, GPIO_MODE_OUTPUT));
    ESP_ERROR_CHECK(gpio_set_level(SUCCESS_LED_GPIO, 0));
